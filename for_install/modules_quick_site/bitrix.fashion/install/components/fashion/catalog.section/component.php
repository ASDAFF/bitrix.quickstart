...
if($this->StartResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation)))
{
	...
	// filter
	if(isset($arParams['BRANDS_IBLOCK_ID'])&&isset($arParams["BRAND_CODE"])){
		$res = CIBlockElement::GetList(Array(), Array("IBLOCK_ID"=>$arParams['BRANDS_IBLOCK_ID'], "ACTIVE"=>"Y", "CODE"=>$arParams["BRAND_CODE"]), false, false, Array("ID", "CODE", "NAME", "IBLOCK_ID", "PROPERTY_xmlcode"));
		if($arElement = $res->GetNext()){
			...
			if($product = $rsProduct->GetNext()){
				...
				$arResult['BRAND']['name'] = $product["VALUE"];
				$arResult['BRAND']['code'] = $arElement['CODE'];
			}
		}elseif($product = CIBlockPropertyEnum::GetByID($arParams["BRAND_CODE"])){
			$arrFilter["PROPERTY_fil_models_brand"] = $arResult["VARIABLES"]["BRAND_CODE"];
			
			$arResult['BRAND']['name'] = $product['VALUE'];
			$arResult['BRAND']['code'] = $product['ID'];
		}else{
			// error - brand didn't found
...
if($arParams["SET_TITLE"])
	$APPLICATION->SetTitle($arResult["NAME"].(isset($arParams['FROMBRANDS'])&&$arParams['FROMBRANDS']=='Y'?' '.$arResult['BRAND']['name']:''), $arTitleOptions);
...
{
	if(isset($arParams['FROMBRANDS'])&&$arParams['FROMBRANDS']=='Y'){
		$APPLICATION->AddChainItem($arResult['BRAND']['name'], $arParams['BRAND_FOLDER'].$arResult['BRAND']['code']."/");
	}
