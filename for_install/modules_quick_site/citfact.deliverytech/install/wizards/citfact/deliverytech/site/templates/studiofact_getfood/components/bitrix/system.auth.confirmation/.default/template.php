...
<?if($arResult["SHOW_FORM"]):?>
	<form method="post" action="<?echo $arResult["FORM_ACTION"]?>">
		<table class="data-table bx-auth-table bx-confirm-table">
			<tr>
				<td class="bx-auth-label">
					<?echo GetMessage("CT_BSAC_LOGIN")?>:
				</td>
				<td>
					<input type="text" class="bx-auth-input" name="<?echo $arParams["LOGIN"]?>" maxlength="50" value="<?echo (strlen($arResult["LOGIN"]) > 0? $arResult["LOGIN"]: $arResult["USER"]["LOGIN"])?>" size="17" />
				</td>
			</tr>
			<tr>
				<td class="bx-auth-label">
				...
				<td>
					<input type="text" class="bx-auth-input" name="<?echo $arParams["CONFIRM_CODE"]?>" maxlength="50" value="<?echo $arResult["CONFIRM_CODE"]?>" size="17" />
				</td>
			</tr>
			<tr>
				<td colspan="2" align="center"><input type="submit" value="<?echo GetMessage("CT_BSAC_CONFIRM")?>" /></td>
			</tr>
		</table>
