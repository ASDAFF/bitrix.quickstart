...
class CSaleOrderLoaderMHT extends CSaleOrderLoader
{
...
function elementHandler($path, $attr)
	...
	function nodeHandler(CDataXML $value)
	{
		...
		//	arraytofile($arOrder ,  $_SERVER["DOCUMENT_ROOT"]."/upload/import_orders_log.txt", "arOrderInputdata");
				...
				if(strlen($arOrder["ID"]) > 0) // exists site order
				{
					...
					if($orderInfo = $dbOrder->Fetch())
					{
						...
						//Пока апдейтим всегда
							...
							if ($arOrder['TRAITS']['Stage']==GetMessage("GOTOV_K_OTGRUZKE")  || $arOrder['TRAITS']['Stage']=='Готов к обеспечению' || $arOrder['TRAITS']['Stage']=='Ожидается отгрузка'){
								CSaleOrder::StatusOrder($orderId, 'P');
							}
							if ($arOrder['TRAITS']['Stage']=='Ожидается оплата (после отгрузки)' || $arOrder['TRAITS']['Stage']=='Готов к закрытию'){
								CSaleOrder::StatusOrder($orderId, 'H');
							}
							if ($arOrder['TRAITS']['AllArrived']=='Да'){
								CSaleOrder::StatusOrder($orderId, 'W');
								CSaleOrder::DeductOrder($orderId, "Y");
							}
							//	if ($arOrder['TRAITS']['Stage']=='Закрыт' && $arOrder['TRAITS']['AllArrived']=='Да'){
							//		CSaleOrder::StatusOrder($orderId, 'F');
							// }
							//	if ($arOrder['TRAITS']['Stage']=='Закрыт' && $arOrder['TRAITS']['AllArrived']=='Нет'){
							//		CSaleOrder::StatusOrder($orderId, 'C');
							//	}
							
							if (trim($arOrder['TRAITS']['Stage'])==GetMessage("ZAKRYT")){
								//arraytofile($arOrder,  $_SERVER["DOCUMENT_ROOT"]."/upload/import_orders_log.txt", "arOrder");
								CSaleOrder::StatusOrder($orderId, 'F');
							}
							
							
							AddOrderProperty(24,$arOrder['TRAITS']["СпособДоставки"],$orderId);
							AddOrderProperty(25,$arOrder['TRAITS']["СпособДоставки"],$orderId);
							
							AddOrderProperty(26,$arOrder['TRAITS']["АдресДоставки"],$orderId);
							AddOrderProperty(27,$arOrder['TRAITS']["АдресДоставки"],$orderId);
							
							if (trim($arOrder['TRAITS']['Отменен'])=='да'){
								//arraytofile($arOrder,  $_SERVER["DOCUMENT_ROOT"]."/upload/import_orders_log.txt", "arOrder");
								CSaleOrder::CancelOrder($orderId, 'Y','1C');
							}
						...
						else
						{
							...
							if(strlen($arOrder["TRAITS"][GetMessage("CC_BSC1_1C_DELIVERY_DATE")])>1)
							{
								...
								$arAditFields['DELIVERY_DOC_DATE']=$arOrder["TRAITS"][GetMessage("CC_BSC1_1C_DELIVERY_DATE")];
							}
						}
