...
class ItmitCatalogSmartFilter extends CBitrixNewsBase
*/

class ItmitCatalogSmartFilter extends CBitrixComponent
{
    public $IBLOCK_ID = 0;
    ...
    */
    ...
    {
        $error = "";
        $lookupValue = rawurldecode(htmlspecialchars_decode($lookupValue));
        $searchValue = \Bitrix\Main\Text\Encoding::convertEncoding($lookupValue, LANG_CHARSET, "utf-8", $error);
        if (!$error) {
            $encodedValue = rawurlencode($searchValue);
    ...
    /**
     * Транслитирует русские сиволы в английские.
     *
     * @param string $string
     * @return string
     */
