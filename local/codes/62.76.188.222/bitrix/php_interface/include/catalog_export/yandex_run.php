...
if (empty($arRunErrors))
{
	...
	@fwrite($fp, "<name>".htmlspecialcharsbx($APPLICATION->ConvertCharset(COption::GetOptionString("main", "site_name", ""), LANG_CHARSET, 'windows-1251'))."</name>\n");

	@fwrite($fp, "<company>".htmlspecialcharsbx($APPLICATION->ConvertCharset(COption::GetOptionString("main", "site_name", ""), LANG_CHARSET, 'windows-1251'))."</company>\n");
	...
	else
	{
		...
		$filter = Array("IBLOCK_TYPE"=>'catalog', "ACTIVE"=>"Y", "IBLOCK_ACTIVE"=>"Y", "GLOBAL_ACTIVE"=>"Y");
	        $db_res = CIBlockSection::GetList(array("left_margin"=>"asc"), $filter);
		while ($ar_res = $db_res->Fetch())
		{
			$arAvailGroups[$ar_res["ID"]] = array(
	...
	if ('D' == $arCatalog['CATALOG_TYPE'] || 'O' == $arCatalog['CATALOG_TYPE'])
	{
		...
		$filter = Array("IBLOCK_TYPE"=>'catalog', "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y");
		if (!$bAllSections)
		{
			...
			if (intval($arAcc["DETAIL_PICTURE"])>0 || intval($arAcc["PREVIEW_PICTURE"])>0)
			{
				...
				if ($ar_file = CFile::GetFileArray($pictNo))
				{
					...
					elseif(preg_match("/^(http|https):\\/\\/(.*?)\\/(.*)\$/", $ar_file["SRC"], $match))
						...
						$strFile = $ar_file["SRC"];
					$strTmpOff.="gggg<picture>".$strFile."</picture>\n";
				}
			}
