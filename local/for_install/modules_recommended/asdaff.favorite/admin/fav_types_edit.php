...
)
{
	...
	if (!strlen($error))
	{
		...
		if (strlen($save))
			...
			LocalRedirect('asdaff_fav_types_edit.php?CODE='.(strlen($NEW_CODE) ? $NEW_CODE : $CODE).'&lang='.LANG);
	}
	else
	{
...
if (strlen($CODE)>0)
	$APPLICATION->SetTitle(GetMessage('asdaff_mod_title_add'));
else
	$APPLICATION->SetTitle(GetMessage('asdaff_mod_title_edit'));

require($_SERVER['DOCUMENT_ROOT'].'/bitrix/modules/main/include/prolog_admin_after.php');

$aContext = array();
$aContext[] = array(
			'TEXT'	=> GetMessage('asdaff_mod_but_list'),
			'TITLE'	=> GetMessage('asdaff_mod_but_list_title'),
			'LINK'	=> 'asdaff_fav_types_list.php?lang='.LANG,
			'ICON'	=> 'btn_list',
			);
if (strlen($CODE) > 0)
{
...
$tabControl->Buttons(
	array(
		'disabled' => false,
		'back_url' => 'asdaff_fav_types_list.php?lang='.LANG,
	)
);
$tabControl->End();
?>
