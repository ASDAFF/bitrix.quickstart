...
use Indi\Main\Util;
$ahref = Option::get("indi.main", "upload_file");
preg_match_all("/\/(.*)\/(.*)\./", $ahref, $nameFile, PREG_PATTERN_ORDER);
?>
<tr id="<?=$result["SERVICE"]?>">
    ...
    <td colspan="2">
        <table border="0" cellspacing="0" cellpadding="5">
            <tr>
                <td colspan="2">
                    <input type='file' name="inputFile" value="Файл" id="<?=$result["SERVICE"]?>_work_start" class="js-work-file" data-service="<?=$result["SERVICE"]?>" />
                    <input type=button value="Старт" disabled id="<?=$result["SERVICE"]?>_work_start" class="js-work-start" data-service="<?=$result["SERVICE"]?>" />
