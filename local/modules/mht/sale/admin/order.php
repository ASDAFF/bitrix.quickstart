		...
		while ($arProps = $dbProps->GetNext())
		{
			if (array_key_exists($arProps["ORDER_PROPS_ID"], $arOrderProps) || array_key_exists($arProps["CODE"], $arOrderPropsCode))
			{
				...
				elseif($arProps["TYPE"] == "LOCATION")
				{
					...
					{
						$path = Helper::getLocationStringByCode($arProps["VALUE"]);
						if(!strlen($path))
							$path = $arProps["VALUE"];
						...
						if(strlen($arProps["CODE"]) > 0)
							...
							$row->AddField("PROP_".$arProps["ORDER_PROPS_ID"], $path);
					}
					else
					{
...
if ($saleModulePermissions == "W")
{
	...
	$rcmFilter['>BASKET_RECOMMENDATION'] = 0;

	$dbRecommendationSum = CSaleOrder::GetList(
		array("BASKET_CURRENCY" => "ASC"),
		$rcmFilter,
		array("BASKET_CURRENCY", "SUM" => "BASKET_PRICE_TOTAL"),
		false, //array("nPageSize"=>CAdminResult::GetNavSize($sTableID)),
		array("BASKET_CURRENCY", "SUM" => "BASKET_PRICE_TOTAL")
	);

	while ($arOrderList = $dbRecommendationSum->Fetch())
	{
		$rcmCount += $arOrderList['CNT'];
...
elseif (($saleModulePermissions < "W") && (COption::GetOptionString("sale", "show_order_sum", "N")=="Y"))
{
	...
	$dbOrderList = CSaleOrder::GetList(
		...
		$arGroupByTmp,
		false, //array("nPageSize"=>CAdminResult::GetNavSize($sTableID)),
		array('ID', 'CURRENCY', 'PRICE', 'BASKET_RECOMMENDATION', 'BASKET_PRICE_TOTAL', 'BASKET_CURRENCY')
	);
...
?>
	...
	<div class="adm-c-bigdatabar-container">
		<div class="adm-c-bigdatabar-summ"><?=GetMessage('SALE_BIGDATA_SUM')?>
			<?if(!empty($arResult['RECOMMENDATION_ORDERS_VALUE'])):?>
				<strong><?=$arResult['RECOMMENDATION_ORDERS_VALUE']?></strong>
			<? else: ?>
				<?=GetMessage('SALE_BIGDATA_SALES_NODATA')?>
			<? endif; ?>
		</div>
		<div class="adm-c-bigdatabar-content">
			...
			<div class="adm-c-bigdatabar-line">
				...
				<? if ($installed): ?>
					<span class="adm-c-bigdatabar-line-task"><?=GetMessage('SALE_BIGDATA_WIDGET_ENABLED')?></span>
				<? else: ?>
					<span class="adm-c-bigdatabar-line-task bx-not-available"><?=GetMessage('SALE_BIGDATA_WIDGET_DISABLED')?></span>
				<? endif; ?>

				<? $available = \Bitrix\Main\Analytics\Catalog::isOn(); ?>
				<? if ($available): ?>
					<span class="adm-c-bigdatabar-line-task"><?=GetMessage('SALE_BIGDATA_IS_ON')?></span>
				<? else: ?>
					<span class="adm-c-bigdatabar-line-task bx-not-available"><?=GetMessage('SALE_BIGDATA_IS_OFF')?></span>
				<? endif; ?>

				<a href="sale_personalization.php?lang=<?=LANGUAGE_ID?>" class="adm-c-bigdatabar-line-task-link"><?=GetMessage('SALE_BIGDATA_ABOUT')?></a>
			</div>
		</div>
		<div class="clb"></div>
	</div>
...
.adm-c-bigdatabar{
	...
	padding: 11px;
	border-radius: 3px;
	box-shadow: 0 1px 1px 0 #cdd4d6;
	background: #fff;
	font-family: "Helvetica Neue", Arial, Helvetica, sans-serif;
}
.adm-c-bigdatabar-container{
	background: #f0f5f5 url(data:image/png;base64,) no-repeat 10px center;
	padding: 8px 11px 8px 45px;
	border-radius: 3px;
	box-shadow: inset 0 1px 1px 0 #d3dee0;
}
.adm-c-bigdatabar-summ{
	float: right;
	padding: 7px 0;
	font-size: 18px;
}
.adm-c-bigdatabar-summ strong{
	font-size: 23px;
	color: #303232;
}
.adm-c-bigdatabar-content{
	font-size: 13px;
}
.adm-c-bigdatabar-line{
	padding: 3px 0;
}
.adm-c-bigdatabar-line-task{
	font-size: 12px;
	display: inline-block;
	padding-right: 15px;
	padding-left: 17px;
	background: url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAsAAAAKCAQAAADI+WwIAAAAZklEQVQIHQXBIQ6CAAAF0AdTNwJBR/ACHMRB4AQEm5XgZnEGHcFiZRT/bXkPAAAKAAA4e2sAAE5+ogMAal8xAg2g8hE3BVysWhw8xaQEBrFo3cXDDoCr+IuXCgBKk5jVAAB7vSMAbOyfD1EzC4U6AAAAAElFTkSuQmCC) no-repeat left center;
}
.adm-c-bigdatabar-line-task.bx-not-available{
	color: #e40a00;
	background: url(data:image/png;base64,) no-repeat left center;
}
.adm-c-bigdatabar-line-task-link{
	border-bottom: 1px dashed;
	text-decoration: none;
}
.adm-c-bigdatabar-line-task-link:hover{
	border-bottom: none;
	text-decoration: none;
}
.clb{clear: both;}
