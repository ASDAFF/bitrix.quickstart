...
<?
 ...
 <div class="b-cart__image js-hover__detail" data-id="#item-<?=$arBasketItems['ID']?>">
<?if($arBasketItems["PREVIEW_PICTURE"]):?>
           <img src="<?=CFile::GetPath($arBasketItems["PREVIEW_PICTURE"]);?>"   alt="<?=$arBasketItems["NAME"]?>" title="<?=$arBasketItems["NAME"]?>" />
            
<?else:?>
<img border="0" src="/images/img-element__image.png"  alt="<?=$arBasketItems["NAME"]?>" title="<?=$arBasketItems["NAME"]?>" />
<?endif?>
</div>
<div class="b-cart__text">
                ...
                ?>
                <div class="b-cart__link  js-hover__detail" data-id="#item-<?=$arBasketItems['ID']?>"><?=$props["PROPERTY_TYPE_VALUE"]?> 
        ...
        <div class="b-cart__price" price='<?=$arBasketItems["PRICE"]?>' final_price='<?=$arBasketItems["PRICE"]*$arBasketItems["QUANTITY"]?>'>
            <div class="clearfix">
        ...
        <div class="b-cart__total">
            <div class="clearfix">
                <div class="b-cart__total_left">
                    <div class="total_right b-slider__price m-price__16px"><?=number_format( $arBasketItems["PRICE"]*$arBasketItems["QUANTITY"], 2, '.', ' ' )?>.-</div>
                   <!-- <div class="b-slider__price_clearing">Безнал<br /><b>19 540.8.–</b></div>   -->                         
                </div>
	...
	<div id="item-<?=$arBasketItems['ID']?>" class="b-item-detail">
<div class="b-item-detail__image">

           <img src="<?=($arBasketItems["PREVIEW_PICTURE"]?CFile::GetPath($arBasketItems["PREVIEW_PICTURE"]):"/images/img-element__image.png");?>"   alt="<?=$arBasketItems["NAME"]?>" title="<?=$arBasketItems["NAME"]?>" />          

</div>
		<div class="b-slider__price m-item-detail__center"><?=$arBasketItems["PRICE_FORMATED"]?></div>
		<div class="b-catalog-list__link"><?=$props["PROPERTY_TYPE_VALUE"]?> 
                    <?if (strlen($arBasketItems["DETAIL_PAGE_URL"])>0):
                        ?><a href="<?=$arBasketItems["DETAIL_PAGE_URL"] ?>"><?
                                endif;?>
                        <?=$arBasketItems["NAME"] ?> <?=$props["PROPERTY_MODEL_VALUE"]?> (<?=$props["PROPERTY_ARTICLE_VALUE"]?>)
                    <?if (strlen($arBasketItems["DETAIL_PAGE_URL"])>0):?></a><?endif;?></div>
		<div class="b-catalog-list__info"><?=$arBasketItems["PREVIEW_TEXT"]?></div>
	</div>
    </div>
    <?
        $i++;
...
<div class="b-cart__btn clearfix">
    <div class="b-cart__btn_left m-left">
        <a class="b-button__fast" href="<?=$_SESSION['LAST_CATALOG_URL']?$_SESSION['LAST_CATALOG_URL']:'/catalogue/';?>">Вернуться<br />к покупкам</a>
