...
$arResult["DISPLAY_ACTIVE_FROM"] = CIBlockFormatProperties::DateFormat("d F Y", MakeTimeStamp($arResult["ACTIVE_FROM"], CSite::GetDateFormat()));

if(!empty($arResult["PROPERTIES"]["PHOTO_TOP"]["VALUE"]))
	foreach($arResult["PROPERTIES"]["PHOTO_TOP"]["VALUE"] as $key=>$arItem)
	{
		$arResult["PROPERTIES"]["PHOTO_TOP"]["ITEMS"][$key]["PREVIEW"] = CFile::ResizeImageGet(
			$arItem,
			array("width" => 800, "height"=>800),
			BX_RESIZE_IMAGE_PROPORTIONAL,
			true
		);
		$arResult["PROPERTIES"]["PHOTO_TOP"]["ITEMS"][$key]["PREVIEW"]["SRC"] = $arResult["PROPERTIES"]["PHOTO_TOP"]["ITEMS"][$key]["PREVIEW"]["src"];
		$arResult["PROPERTIES"]["PHOTO_TOP"]["ITEMS"][$key]["DETAIL"] = CFile::GetFileArray($arItem);		
		$arResult["PROPERTIES"]["PHOTO_TOP"]["ITEMS"][$key]["DESCRIPTION"] = $arResult["PROPERTIES"]["PHOTO_TOP_DESCRIPTION"]["VALUE"][$key];
	}
