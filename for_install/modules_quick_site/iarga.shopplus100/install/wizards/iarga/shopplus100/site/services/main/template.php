...
if(!$wizrdTemplateId) $wizrdTemplateId = "iarga.shopplus100.main"; 
COption::SetOptionString("main", "wizard_template_id", $wizrdTemplateId, false, WIZARD_SITE_ID);

function replaceceInJs($dir,$arr){
	$d = opendir($dir);
	while($el = readdir($d)){
		if(preg_match("#.js#",$el)){
			$link = $dir.$el;
			...
			file_put_contents($link, $cont);
		}
	}
	closedir($d);
}

replaceceInJs($_SERVER["DOCUMENT_ROOT"]."/bitrix/templates/".WIZARD_TEMPLATE_ID."/js/", Array("SITE_DIR" => WIZARD_SITE_DIR));

WizardServices::ReplaceMacrosRecursive($_SERVER["DOCUMENT_ROOT"]."/bitrix/templates/".WIZARD_TEMPLATE_ID."/components/bitrix/", Array("SITE_DIR" => WIZARD_SITE_DIR));
WizardServices::ReplaceMacrosRecursive($_SERVER["DOCUMENT_ROOT"]."/bitrix/templates/".WIZARD_TEMPLATE_ID."/inc/parts/", Array("SITE_DIR" => WIZARD_SITE_DIR));
WizardServices::ReplaceMacrosRecursive($_SERVER["DOCUMENT_ROOT"]."/bitrix/templates/".WIZARD_TEMPLATE_ID."/header.php", Array("SITE_DIR" => WIZARD_SITE_DIR));
WizardServices::ReplaceMacrosRecursive($_SERVER["DOCUMENT_ROOT"]."/bitrix/templates/".WIZARD_TEMPLATE_ID."/footer.php", Array("SITE_DIR" => WIZARD_SITE_DIR));
?>
