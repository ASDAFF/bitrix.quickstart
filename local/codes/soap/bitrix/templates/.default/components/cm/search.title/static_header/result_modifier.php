    ...
    foreach($arResult["CATEGORIES"] as $category_id => $arCategory):
        foreach($arCategory["ITEMS"] as $i => $arItem):
            ...
            if($ob = $res->GetNextElement())
            {
                ...
                $file = CFile::ResizeImageGet($arFields['PREVIEW_PICTURE'], array('width'=>150, 'height'=>150), BX_RESIZE_IMAGE_EXACT, true);                
                //$img = '<img src="'.$file['src'].'" width="'.$file['width'].'" height="'.$file['height'].'" />';
                $arCategory["ITEMS"][$i]["PREV"]["SRC"] = $file['src'];
                $arCategory["ITEMS"][$i]["PREV"]["WIDTH"] = $file['width'];
                $arCategory["ITEMS"][$i]["PREV"]["HEIGHT"] = $file['height'];
            }
            endforeach;
        endforeach;
?>