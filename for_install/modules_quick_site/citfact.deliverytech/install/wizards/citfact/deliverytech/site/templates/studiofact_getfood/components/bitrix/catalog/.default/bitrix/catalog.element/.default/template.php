...
<div class="bx_item_detail box padding good_box" id="<?=$this->GetEditAreaId($arResult["ID"]);?>" itemscope itemtype="http://schema.org/Product">
	<div class="row">
								...
								<div class="flexslider slider" id="slider_<?=$arOffer["ID"];?>">
									<ul class="slides">
										<? foreach ($arOffer["PHOTO_SLIDER"] as $key => $value) {
											?><li>
												<a href="<?=$value["ORIGINAL_SRC"];?>" title="<?=$strTitle;?>" class="fancybox" rel="gallery_<?=$arOffer["ID"];?>" itemprop="image"><img src="<?=(strlen($value["SRC"]) > 1 ? $value["SRC"] : $value["ORIGINAL_SRC"]);?>" /></a>
											</li><?
										} ?>
									</ul>
								</div>
						...
						<div class="flexslider slider" id="slider_<?=$arResult["ID"];?>">
							<ul class="slides">
								<? foreach ($arResult["PHOTO_SLIDER"] as $key => $value) {
									?><li>
										<a href="<?=$value["ORIGINAL_SRC"];?>" title="<?=$strTitle;?>" class="fancybox" rel="gallery_<?=$arResult["ID"];?>" itemprop="image"><img src="<?=(strlen($value["SRC"]) > 1 ? $value["SRC"] : $value["ORIGINAL_SRC"]);?>" /></a>
									</li><?
								} ?>
							</ul>
						</div>
		...
		<? if ('Y' == $arParams['BRAND_USE'] || 'Y' == $arParams['USE_VOTE_RATING']) { ?>
		...
		<? if ('Y' == $arParams['BRAND_USE'] || 'Y' == $arParams['USE_VOTE_RATING']) {
			?><div class="col-lg-2 col-md-2 col-sm-4 col-xs-4"><div class="bx_optionblock"><?
	...
	<div class="tabs_header">
		...
		<? if ("Y" == $arParams["USE_COMMENTS"]) { ?><div class="tabs_head"><a href=".dt4" title="<?=GetMessage("COMMENTARY");?>" onClick="showComment('0'); $(this).removeAttr('onClick');"><span class="icon_comments icons_head"></span><span class="text"><?=GetMessage("COMMENTARY");?></span></a></div><? } ?>
	</div>
				...
				<? if (count($arResult["OFFERS"]) > 0) {
					foreach ($arResult["OFFERS"] as $arOffer) {
						if (strlen($arOffer["DETAIL_TEXT"]) > 0) { ?>
							<div class="main_detail_text_<?=$arOffer["ID"];?> offers_hide" itemprop="description" style="display: none;"><?=$arOffer["DETAIL_TEXT"];?></div>
						<? }
					}
				} ?>
			</div>
