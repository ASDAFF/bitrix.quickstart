...
class CAllSaleDiscount
{
				...
				if ($applyFlag && self::__Unpack($arOrder, $discount['UNPACK']))
				{
					...
					if (!empty($discountResult['DELIVERY']) || !empty($discountResult['BASKET']))
					{
						...
						$resultDiscountList[$resultDiscountIndex] = array(
							...
							'COUPON' => $discount['DISCOUNT_COUPON']
						);
	...
	public function CheckFields($ACTION, &$arFields)
	{
		...
		if ($useConditions)
		{
			...
			{
				$arFields['UNPACK'] = '';
				if (!self::prepareDiscountConditions($arFields['CONDITIONS'], $arFields['UNPACK'], $conditionData, self::PREPARE_CONDITIONS))
				{
					return false;
				}
			}
		...
		if ($useActions)
		{
			...
			else
			{
				...
				if (!self::prepareDiscountConditions($arFields['ACTIONS'], $arFields['APPLICATION'], $actionData, self::PREPARE_ACTIONS))
				{
					return false;
				}
			}
		...
		if ($updateData)
		{
			if (!$useConditions)
			{
				...
				{
					$discountInfo['UNPACK'] = '';
					if (!self::prepareDiscountConditions($discountInfo['CONDITIONS'], $discountInfo['UNPACK'], $conditionData, self::PREPARE_CONDITIONS))
					{
						return false;
					}
				}
			...
			if (!$useActions)
			{
				...
				if ($discountInfo = $rsDiscounts->Fetch())
				{
					...
					if (!self::prepareDiscountConditions($discountInfo['ACTIONS'], $discountInfo['APPLICATION'], $actionData, self::PREPARE_ACTIONS))
					{
						return false;
					}
				}
