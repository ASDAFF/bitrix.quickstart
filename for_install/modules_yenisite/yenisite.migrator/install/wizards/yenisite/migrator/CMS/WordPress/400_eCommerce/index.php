...
/* количество записей */
$query = "SELECT COUNT(*) as CNT  FROM {$arResult["prefix"]}term_taxonomy, {$arResult["prefix"]}terms WHERE {$arResult["prefix"]}term_taxonomy.taxonomy='wpsc_product_category' AND {$arResult["prefix"]}term_taxonomy.term_id={$arResult["prefix"]}terms.term_id ORDER BY {$arResult["prefix"]}terms.term_id ASC";	
...
$iblock = new CIBlock;
$res = CIBlock::GetList(array(), array("CODE" => "wp_catalog"))->GetNext();
if($res)
	$id = $res["ID"];
else
...
if(!CIBlockProperty::GetList(array(), array("IBLOCK_ID" => $id, "CODE" => 'HEIGHT'))->GetNext())
{
