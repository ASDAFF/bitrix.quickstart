	...
	if (!empty($arSource))
	{
		...
		foreach($arSource as $arProperties)
		{
			?>
				...
				{
					?>
					<p><input type="text" maxlength="250" class="input_text_style" size="<?=$arProperties["SIZE1"]?>" value="<?=$arProperties["VALUE"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>">
					</p>
					<?
				}
...
{
	?>
	<p><?=GetMessage("SOA_TEMPL_PROP_PROFILE")?></p>
	...
	<p>
	<span class="description"><?=GetMessage("SOA_TEMPL_PROP_CHOOSE_DESCR")?></span></p>
	<?
}
?>
...
<textarea class="order-des" name="ORDER_DESCRIPTION"><?=$arResult["USER_VALS"]["ORDER_DESCRIPTION"]?></textarea>
