...
if ($arParams['ENABLE_HIDDEN_ANTISPAM_FIELDS'] == 'Y')
{
	...
	{
		echo '<div id="'.ToLower($antiSpamFieldName).'" class="field importantField">
...
// подписка
if(count($arResult['SIGN']) > 0)
{
	foreach ($arResult['SIGN'] as $signHTML)
	{
		echo    '<div class="field sign">	
					'.$signHTML['HTML'].'
					<span class="label">'.$signHTML['LABEL'].'</span>
				</div>';
	}
}
