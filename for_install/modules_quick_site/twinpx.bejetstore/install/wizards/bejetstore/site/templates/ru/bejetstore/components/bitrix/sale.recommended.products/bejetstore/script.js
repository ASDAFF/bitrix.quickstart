...
window.JCCatalogSectionSRec.prototype.ChangeInfo = function()
{
	...
	if (-1 < index)
	{
		...
		if (!!this.obPrice)
		{
			strPrice = '<div class="text-large text-info">'+this.offers[index].PRICE.PRINT_DISCOUNT_VALUE+'</div>';
			if (this.showOldPrice && (this.offers[index].PRICE.DISCOUNT_VALUE !== this.offers[index].PRICE.VALUE))
			{
				strPrice += ' <div class="text-small"><s>'+this.offers[index].PRICE.PRINT_VALUE+'</s></div>';
			}
