<?if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true)die();?>
				<div class="b-nav-category m-menu">
					<a href="#" class="b-nav-category__link" id="b-slide__aside">Поиск<br>по категориям</a>
					<ul class="b-nav-menu">
<?
$CURRENT_DEPTH=$arResult["SECTION"]["DEPTH_LEVEL"]+1;
$last_el = end($arResult["SECTIONS"]);
$last_parent_id = $last_el['IBLOCK_SECTION_ID'];
...
<?if(count($db_list)==1 && $arSection["DEPTH_LEVEL"]==1):?>
	<li class="b-nav-menu__item" ><a href="<?=$arSection["SECTION_PAGE_URL"]?>" class="b-nav-menu__link"><?=$arSection["NAME"]?></a></li>
<?elseif(count($db_list)>1 && $arSection["DEPTH_LEVEL"]==1):?>
						<li class="b-nav-menu__item m-menu__child <?=($last_parent_id==$arSection['ID']?"m-menu__last":'')?>">
							<a href="<?=$arSection["SECTION_PAGE_URL"]?>" class="b-nav-menu__link"><?=$arSection["NAME"]?></a>
<?else:?>
	<li class="b-nav-sub_menu__item"><a href="<?=$arSection["SECTION_PAGE_URL"]?>" class="b-nav-sub_menu__link"><?=$arSection["NAME"]?></a></li>
<?endif;?>
<?
