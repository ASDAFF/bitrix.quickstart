...
class SelectTemplateStep extends CSelectTemplateWizardStep
{
	...
	function OnPostForm()
	{
		...
		if ($wizard->IsNextButtonClick())
		{
			...
			if (in_array($templateID,  array("kidstore")))
			{
				$wizard->SetCurrentStep("site_settings");
				$wizard->SetVar("templateID", "kidstore");
			}
			else
				$wizard->SetVar("templateID", "kidstore");
		}
	...
	function ShowStep()
	{
		...
		if (in_array("kidstore", array_keys($arTemplates)))
		{
			$arTemplateOrder[] = "kidstore";
		}

		$defaultTemplateID = COption::GetOptionString("main", "wizard_template_id", "kidstore", $wizard->GetVar("siteID"));
		if (!in_array($defaultTemplateID, array("kidstore"))) $defaultTemplateID = "kidstore";
		...
		$arTemplateInfo = array(
			"kidstore" => array(
				...
				"DESCRIPTION" => "",
				"PREVIEW" => $wizard->GetPath()."/site/templates/kidstore/images/".LANGUAGE_ID."/preview.gif",
				"SCREENSHOT" => $wizard->GetPath()."/site/templates/kidstore/images/".LANGUAGE_ID."/screen.gif",
			)
		);
