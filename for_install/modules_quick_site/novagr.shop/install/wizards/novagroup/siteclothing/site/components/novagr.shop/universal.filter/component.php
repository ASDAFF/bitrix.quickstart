...
if(CModule::IncludeModule("iblock"))
{
    ...
    $arResult['CURRENT_PARENT_SECTION'] = array();
    $arResult['SUBSECTIONS'] = array();
    if($arSection['IBLOCK_SECTION_ID'] > 0 ){
        ...
        while ($section = $resSection -> Fetch()) {

            if ($section["CODE"] == $arResult['CURRENT_SECTION_CODE']) $section["ACTIVE"] = 1;
            else $section["ACTIVE"] = 0;

            $arResult['SUBSECTIONS'][] = $section;
        }

        $arFilterParent = array(
            'IBLOCK_ID'				=> $arParams['CATALOG_IBLOCK_ID'],
            'GLOBAL_ACTIVE'			=> "Y",
        ...
        if ($section = $resSection -> Fetch()) {
            ...
            $arResult['CURRENT_PARENT_SECTION'] = $section;
        }
    }
    ...
    if($arParams['SHOW_PRICE_SLIDER'] == "Y")
    {
        ...
        // max price
        $rsElement = CIBlockElement::GetList(
            array($arParams['CATALOG_PRICE'] => "DESC"),
            array(
                'IBLOCK_ID' => $arParams['TRADEOF_IBLOCK_ID'],
            ...
            false,
            array('nTopCount' => 1),
            array('IBLOCK_ID', 'ID', $arParams['CATALOG_GROUP'])
        );
        if($arElement = $rsElement -> Fetch())
            $maxPrice = (int)$arElement[ $arParams['CATALOG_PRICE'] ];
        // min price
        $rsElement = CIBlockElement::GetList(
            array($arParams['CATALOG_PRICE'] => "ASC"),
            array(
                'IBLOCK_ID' => $arParams['TRADEOF_IBLOCK_ID'],
                'PROPERTY_CML2_LINK' => $arID,
                ">".$arParams['CATALOG_PRICE']	=> 0
        ...
        if($arElement = $rsElement -> Fetch())
            $price = (int)$arElement[ $arParams['CATALOG_PRICE'] ];
