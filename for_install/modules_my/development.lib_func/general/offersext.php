...
class RSLibFuncOffersExtension
{
	...
	public static function GetJSONElement($arElement,$arProps=array(),$arPrices=array(),$params=array())
	{
		...
		if(is_array($arOffers) && count($arOffers)>0)
		{
			...
			foreach($arOffers as $key1 => $arOffer)
			{
				...
				//RSLibFunc::GetDataForProductItem($arrNewOffers,$params);
			}
			/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			$iTime = ConvertTimeStamp(time(),'FULL');
			// add quickbuy
			if(CModule::IncludeModule('development.quickbuy'))
			{
				$arFilter = array(
					'DATE_FROM' => $iTime,
					'DATE_TO' => $iTime,
			...
			// /add quickbuy
			// add da2
			if(CModule::IncludeModule('development.daysarticle2'))
			{
				$arFilter = array(
					'DATE_FROM' => $iTime,
					'DATE_TO' => $iTime,
	...
	public static function getPictures($arSizes=array(),$arElement=array(),$params=array())
	{
		...
		{
			$arrReturn[] = RSLibFunc::GetNoPhoto(array('MAX_WIDTH'=>$arSizes['WIDTH'],'MAX_HEIGHT'=>$arSizes['HEIGHT']));
		}
		return $arrReturn;
	}
	...
	public static function _PriceToProperty($ELEM_ID,$TMP_ID=0)
	{
		...
		{
			$propcode_cml2link = COption::GetOptionString('development.lib_func', 'propcode_cml2link', 'CML2_LINK');
			$propcode_fakeprice = COption::GetOptionString('development.lib_func', 'propcode_fakeprice', 'PROD_PRICE_FALSE');
	...
	public static function CheckIssetProperty($IBLOCK_ID,$PROP_CODE='PROD_PRICE_FALSE')
	{
		...
		if(IntVal($IBLOCK_ID)>0)
		{
			...
			if($arProp = $res->GetNext())
			{
				...
				// Add property
				$arFields = Array(
					'NAME' => GetMessage('RSLIB_FUNC_PRICE_FALSE_NAME'),
					'ACTIVE' => 'Y',
					'SORT' => '100000',
					'CODE' => $PROP_CODE,
					'PROPERTY_TYPE' => 'N',
