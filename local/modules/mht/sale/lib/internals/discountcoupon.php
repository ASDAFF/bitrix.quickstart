...
class DiscountCouponTable extends Main\Entity\DataManager
{
	...
	public static function updateUseCoupons()
	{
		...
		Type\Collection::normalizeArrayValuesByInt(self::$discountCheckList);
	...
	public static function deleteByDiscount($discount)
	{
		...
		if (!empty($couponsList))
		{
			...
			$event = new Event('sale', 'onAfterDeleteDoscountCoupons', array($discount, $couponsList));
			$event->send();
		}
	}
	 ...
	 * @param Type\DateTime $currentTime	Current datetime.
	 * @return array|bool
	 */
	public static function saveApplied($coupons, $userId, Type\DateTime $currentTime)
	{
		...
		if (empty($coupons))
			return false;
		Type\Collection::normalizeArrayValuesByInt($coupons);
		if (empty($coupons))
			return false;
		...
		while ($existCoupon = $couponIterator->fetch())
		{
			if ($existCoupon['DISCOUNT_ACTIVE'] != 'Y')
				continue;
			if (
				($existCoupon['DISCOUNT_ACTIVE_FROM'] instanceof Type\DateTime && $existCoupon['DISCOUNT_ACTIVE_FROM']->getTimestamp() > $currentTimestamp)
				||
				($existCoupon['DISCOUNT_ACTIVE_TO'] instanceof Type\DateTime && $existCoupon['DISCOUNT_ACTIVE_TO']->getTimestamp() < $currentTimestamp)
			)
				continue;
			...
			if ($existCoupon['USER_ID'] > 0 && $existCoupon['USER_ID'] != $userId)
				continue;
			if (
				($existCoupon['ACTIVE_FROM'] instanceof Type\DateTime && $existCoupon['ACTIVE_FROM']->getTimestamp() > $currentTimestamp)
				||
				($existCoupon['ACTIVE_TO'] instanceof Type\DateTime && $existCoupon['ACTIVE_TO']->getTimestamp() < $currentTimestamp)
			)
				continue;
			if (
	...
	{
		$check = ($check === true);
		$event = new Event('sale', self::EVENT_ON_GENERATE_COUPON, array('CHECK' => $check));
		$event->send();
	...
	public static function prepareCouponData(&$fields)
	{
		...
		if (!empty($fields) && is_array($fields))
		{
			if (isset($fields['ACTIVE_FROM']) && is_string($fields['ACTIVE_FROM']))
			{
				...
				$fields['ACTIVE_FROM'] = ($fields['ACTIVE_FROM'] !== '' ? new Type\DateTime($fields['ACTIVE_FROM']) : null);
				}
				catch (Main\ObjectException $e)
				{
					$fields['ACTIVE_FROM'] = new Type\Date($fields['ACTIVE_FROM']);
				}
			}
			if (isset($fields['ACTIVE_TO']) && is_string($fields['ACTIVE_TO']))
			{
				try
				{
					...
					$fields['ACTIVE_TO'] = ($fields['ACTIVE_TO'] !== '' ? new Type\DateTime($fields['ACTIVE_TO']) : null);
				}
				catch(Main\ObjectException $e)
				{
					$fields['ACTIVE_TO'] = new Type\Date($fields['ACTIVE_TO']);
				}
			}
		}
		return $result;
	...
	protected static function setTimestamp(&$result, $data, $keys)
	{
		foreach ($keys as &$oneKey)
		{
			...
			if ($setField)
				$result[$oneKey] = new Type\DateTime();
		}
		unset($oneKey);
	}
}