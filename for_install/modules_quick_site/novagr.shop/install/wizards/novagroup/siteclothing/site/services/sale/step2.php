...
if(COption::GetOptionString("eshop", "wizard_installed", "N", WIZARD_SITE_ID) != "Y" || WIZARD_INSTALL_DEMO_DATA)
{
	...
	if(!$dbDelivery->Fetch())
	{
        ...
        if($bRus)
        {
            ...
            //if exists, set action
            if ($arResult = $dbResult->GetNext())
            {
                //set fields
                $arFields = array();
                $arFields["ACTIVE"] = ($delivery["cpcr"] != "Y") ? "N" : "Y";
                CSaleDeliveryHandler::Set("cpcr", $arFields);
            }
            else
            {
                //if not exists and cpcr=Y
                if($delivery["cpcr"] == "Y")
                {
            ...
            //get delivery rus_post
            $dbResult = CSaleDeliveryHandler::GetBySID("rus_post");
            //if exists, set action
            if ($arResult = $dbResult->GetNext())
            {
                //set fields
                $arFields = array();
                $arFields["ACTIVE"] = ($delivery["rus_post"] != "Y") ? "N" : "Y";
                CSaleDeliveryHandler::Set("rus_post", $arFields);
            }

            //get delivery rus_post_first
            $dbResult = CSaleDeliveryHandler::GetBySID("rus_post_first");
            //if exists, set action
            if ($arResult = $dbResult->GetNext())
            {
                //set fields
                $arFields = array();
                $arFields["ACTIVE"] = ($delivery["rus_post_first"] != "Y") ? "N" : "Y";
                CSaleDeliveryHandler::Set("rus_post_first", $arFields);
            }

            //get delivery russianpost
            $dbResult = CSaleDeliveryHandler::GetBySID("russianpost");
            //if exists, set action
            if ($arResult = $dbResult->GetNext())
            {
                //set fields
                $arFields = array();
                $arFields["ACTIVE"] = ($delivery["russianpost"] != "Y") ? "N" : "Y";
        ...
        //get delivery ups
        $dbResult = CSaleDeliveryHandler::GetBySID("ups");
        //if exists, set action
        if ($arResult = $dbResult->GetNext())
        {
            //set fields
            $arFields = array();
            $arFields["ACTIVE"] = ($delivery["ups"] != "Y") ? "N" : "Y";
        ...
        //get delivery dhlusa
        $dbResult = CSaleDeliveryHandler::GetBySID("dhlusa");
        //if exists, set action
        if ($arResult = $dbResult->GetNext())
        {
            //set fields
            $arFields = array();
            $arFields["ACTIVE"] = ($delivery["dhlusa"] != "Y") ? "N" : "Y";
            CSaleDeliveryHandler::Set("dhlusa", $arFields);
        }
        else
        {
            //if not exists and cpcr=Y
            if($delivery["dhlusa"] == "Y")
            {
                $arFields = Array(
                    "LID" => "",
                    "ACTIVE" => "Y",
