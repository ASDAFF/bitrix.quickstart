...
//Ukraine
	...
	if(CModule::IncludeModule("currency"))
	{
		...
		{	
			if($lang == "ru")
            {
                CCurrencyLang::Update($arCur["CURRENCY"], $lang, Array("DECIMALS" => 0,"THOUSANDS_VARIANT"=>"N"));
            }
	...
	$dbResultList = CCatalogGroup::GetList(Array(), Array("CODE" => GetMessage("CATALOG_BASE_GROUP_NAME")));
	...
	function __MakeOrder($prdCnt=1, $arData = Array())
	{
		...
		if(!empty($arPrd))
		{
			for($i=0; $i<$prdCnt;$i++)
			{
                ...
                $PRODUCT_ID = $ID = $prdID["ID"]; 
                
                $arInfo = CCatalogSKU::GetInfoByProductIBlock($prdID["IBLOCK_ID"]); 
                if (is_array($arInfo)) 
                { 
                    $rsOffers = CIBlockElement::GetList(array(),array('IBLOCK_ID' => $arInfo['IBLOCK_ID'], 'PROPERTY_'.$arInfo['SKU_PROPERTY_ID'] => $prdID["ID"])); 
                    while ($arOffer = $rsOffers->GetNext()) 
                    { // тут ведем обработку 
                        $PRODUCT_ID = $arOffer['ID'];break;
                    } 
                }
