		...
		if($valueList)
		{
			...
			{										
					...
					foreach($valueList as $val=>$text)
					{
						...
						if ($key==1)
							$res .= '<ul>';
						$prop_input_id = htmlspecialchars($FILTER_NAME."_pf_".$arProp["CODE"]."_".$count);
						$res .= '<li><input type="checkbox" name="'.$name.'[]" value="'.htmlspecialchars($val).'" id="'.$prop_input_id.'"'.(in_array($val, $arValue) ? ' checked="checked"' : '').' />';
						$res .= '<label for="'.$prop_input_id.'">'.htmlspecialchars($text).'</label></li>';
						if ($key==3 || $count==count($valueList))
						{
							$res .= '</ul>';
							$key=0;
						}
