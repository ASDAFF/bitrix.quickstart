...
var nicEditorConfig = bkClass.extend({
    buttons: {
        ...
        'left': {
            name: WDR2_NicEdit['left'],
            command: 'justifyleft',
            noActive: true
        },
        'center': {
            name: WDR2_NicEdit['center'],
            command: 'justifycenter',
            noActive: true
        },
        'right': {
            name: WDR2_NicEdit['right'],
            command: 'justifyright',
            noActive: true
        },
        'justify': {
            name: WDR2_NicEdit['justify'],
            command: 'justifyfull',
            noActive: true
        },
        'ol': {
            name: WDR2_NicEdit['ol'],
            command: 'insertorderedlist',
            tags: ['OL']
        },
        'ul': {
            name: WDR2_NicEdit['ul'],
            command: 'insertunorderedlist',
            tags: ['UL']
        },
        'subscript': {
            name: WDR2_NicEdit['subscript'],
            command: 'subscript',
            tags: ['SUB']
        },
        'superscript': {
            name: WDR2_NicEdit['superscript'],
            command: 'superscript',
            tags: ['SUP']
        },
        'strikethrough': {
            name: WDR2_NicEdit['strikethrough'],
        ...
        'removeformat': {
            name: WDR2_NicEdit['removeformat'],
            command: 'removeformat',
            noActive: true
        },
        'indent': {
            name: WDR2_NicEdit['indent'],
            command: 'indent',
            noActive: true
        },
        'outdent': {
            name: WDR2_NicEdit['outdent'],
            command: 'outdent',
            noActive: true
        },
        'hr': {
            name: WDR2_NicEdit['hr'],
            command: 'insertHorizontalRule',
            noActive: true
        }
    },
    iconsPath: '/bitrix/images/webdebug.reviews/nicEditorIcons.gif',
    buttonList: ['save', 'bold', 'italic', 'underline', 'left', 'center', 'right', 'justify', 'ol', 'ul', 'fontSize', 'fontFamily', 'fontFormat', 'indent', 'outdent', 'upload', 'link', 'unlink', 'forecolor', 'bgcolor'],
    iconList: {
        "bgcolor": 1,
        "forecolor": 2,
        "bold": 3,
...
var nicEditorInstance = bkClass.extend({
    ...
    construct: function(G, D, C) {
        ...
        if (H || this.options.hasPanel) {
            ...
            this.editorContain = new bkElement("DIV").setStyle(E).addClass('editorContent').appendBefore(G);
            var A = new bkElement("DIV").setStyle({ /*width:(newX-8)+"px",*/
                margin: "4px",
                minHeight: newY + "px",
                outline: '0',
    ...
    init: function() {
        ...
        this.elm.setAttribute("id", this.ne.options.id);
        if (this.getContent() == "") {
            this.setContent("<br />")
        }
...
var nicEditorPanel = bkClass.extend({
    construct: function(E, B, A) {
        ...
        this.reorder();
        E.noSelect().setStyle('width', 'auto')
    },
...
var nicEditorPane = bkClass.extend({
    construct: function(D, C, B, A) {
        ...
        this.elm = D;
        this.pos = D.pos();
        var ThisEditorContain;
				if (A) {
					ThisEditorContain = A.ne.nicInstances[0].editorContain;
				}
        this.contain = new bkElement("div").setStyle({
            zIndex: "39",
            overflow: "hidden",
        ...
        if (A && !A.options.noClose) {
            this.close = new bkElement("div").setStyle({
				...
				if (ThisEditorContain) {
					ThisEditorContain.setStyle({position:'relative'});
					this.contain.noSelect().appendTo(ThisEditorContain);
				}
        this.init()
    },
    init: function() {},
                ...
                switch (A) {
                    case "text":
                        this.inputs[itm] = new bkElement("input").setAttributes({
                            ...
                            overflow: "hidden",
														lineHeight: "100%"
                        }).setStyle(D.style).setAttributes({autocomplete:'off'}).appendTo(B);
                        break;
                    case "select":
...
var nicSelectOptions = {
    buttons: {
        'fontSize': {
            name: WDR2_NicEdit['fontSize'],
            type: 'nicEditorFontSizeSelect',
            command: 'fontsize'
        },
        'fontFamily': {
            name: WDR2_NicEdit['fontFamily'],
...
var nicLinkOptions = {
    buttons: {
        'link': {
            name: WDR2_NicEdit['link'],
            type: 'nicLinkButton',
            tags: ['A']
        },
        'unlink': {
            name: WDR2_NicEdit['unlink'],
            command: 'unlink',
            noActive: true
        }
    }
...
var nicLinkButton = nicEditorAdvancedButton.extend({
    ...
    submit: function(C) {
        ...
        if (A == "http://" || A == "") {
            alert(WDR2_NicEdit['errorUrlEmpty']);
            return false
        }
        this.removePane();
        if (!this.ln) {
        ...
        if (this.ln) {
            this.ln.setAttributes({
                href: this.inputs.href.value
						...
						if (this.inputs.title.value.length>0) {
							this.ln.setAttributes({
...
var nicColorOptions = {
    buttons: {
        'forecolor': {
            name: WDR2_NicEdit['forecolor'],
            type: 'nicEditorColorButton',
            noClose: true
        },
        'bgcolor': {
            name: WDR2_NicEdit['bgcolor'],
