			...
			$arResult["POST"]["NEW_LOGIN"] = $arResult["POST"]["~NEW_EMAIL"];
                ...
                if (strlen($arResult["ERROR_MESSAGE"]) <= 0)
                {
					...
					if ($USER->IsAuthorized())
                        {
					$fields = Array(
					"PERSON_TYPE" => $arResult["POST"]["PERSON_TYPE"],
					"PERSONAL_PHONE" => $arResult["POST"]["PERSONAL_PHONE"]
					  );
    ...
    if ($USER->IsAuthorized())
    {
        ...
        elseif ($arResult["CurrentStep"] == 4)
        {
            ...
            while ($arProperties = $dbProperties->GetNext())
            {
                ...
                //echo "<pre>", print_r($arProperties,1), "</pre>";
                if($arProperties["GROUP_NAME"]=="Для платежа"){
                    $arResult["UR"][$arProperties["ID"]] = $arProperties;
                    unset($arProperties);
                }
                else{
                    if($arProperties["USER_PROPS"]=="Y")
