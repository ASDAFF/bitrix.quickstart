...
function getProp($id, $arResult){ 
    
    if($arResult["PRINT_PROPS_FORM"]["USER_PROPS_N"][$id])
        return $arResult["PRINT_PROPS_FORM"]["USER_PROPS_N"][$id];
    elseif($arResult["PRINT_PROPS_FORM"]["USER_PROPS_Y"][$id])
        return $arResult["PRINT_PROPS_FORM"]["USER_PROPS_Y"][$id];
...
<div id="addr">
 <?
                                    switch ($arResult["PERSON_TYPE"]) {
                                        case 1: // Физическое лицо
                                            
  $field = getProp(1, $arResult);                                          
                                        
?>                   
		<label class="b-cart-field__label city_label_">Город <span class="b-star">*</span></label>
                <select class="b-cart-field__select" id="" name="ORDER_PROP_<?=$field['ID']?>">
                    <?foreach($field["VARIANTS"] as $variant){?>
                        <option <?if ($USER->IsAuthorized()){$city = GetUserField ("USER", $USER->GetID(), "UF_LOCATION"); if ($city == $variant["ID"]) echo "selected='selected'";}?> value="<?=$variant["ID"]?>"><?=$variant["NAME"]?></option>
                    <?}?>
                </select>
 
<?
  $field = getProp(8, $arResult); 
 
?>
                  <div class="metro_"><label class="b-cart-field__label">Станция метро </label>
                <select class="b-cart-field__select" id="" name="ORDER_PROP_<?=$field['ID']?>">
                    <?foreach($field["VARIANTS"] as $variant){?>
                        <option <?if ($USER->IsAuthorized()){$metro = GetUserField ("USER", $USER->GetID(), "UF_METRO"); if ($metro == $variant["VALUE"]) echo "selected='selected'";}?> value="<?=$variant["ID"]?>"><?=$variant["NAME"]?></option>
                    <?}?>
                </select></div>
<?
  $field = getProp(4, $arResult);   
?>               
                <label class="b-cart-field__label">Адрес <span class="b-star">*</span></label>
                <textarea class="b-cart-field__input" rows="3" id="" name="ORDER_PROP_<?=$field['ID']?>"><?if ($USER->IsAuthorized()){$adress = GetUserField ("USER", $USER->GetID(), "UF_ADRES"); echo $adress;}?></textarea>
               
<?
  $field = getProp(6, $arResult);  
?>                
                <label class="b-cart-field__label">Комментарии </label>
                <textarea class="b-cart-field__input" rows="3" id="" name="ORDER_PROP_<?=$field['ID']?>"><?if ($USER->IsAuthorized()){$comment = GetUserField ("USER", $USER->GetID(), "UF_COMMENTS"); echo $comment;}?></textarea>
                                            ...
                                            break;
                                        case 2: // Юридическое лицо
                                            
  $field = getProp(3, $arResult);                                          
...
<?
  $field = getProp(9, $arResult);                                          
  ?>

                <div class="metro_">
                <label class="b-cart-field__label">Станция метро </label>
                <select class="b-cart-field__select" id="" name="ORDER_PROP_<?=$field['ID']?>">
                    <?foreach($field["VARIANTS"] as $variant){?>
                        <option <?if ($USER->IsAuthorized()){$metro = GetUserField ("USER", $USER->GetID(), "UF_METRO"); if ($metro == $variant["VALUE"]) echo "selected='selected'";}?> value="<?=$variant["ID"]?>"><?=$variant["NAME"]?></option>
                    <?}?>
...
<?
  $field = getProp(5, $arResult);   
?>               
                <label class="b-cart-field__label">Адрес <span class="b-star">*</span></label>
                <textarea class="b-cart-field__input" rows="3" id="" name="ORDER_PROP_<?=$field['ID']?>"><?if ($USER->IsAuthorized()){$adress = GetUserField ("USER", $USER->GetID(), "UF_ADRES"); echo $adress;}?></textarea>
               
<?
  $field = getProp(7, $arResult);  
?>                
                <label class="b-cart-field__label">Комментарии </label>
                <textarea class="b-cart-field__input" rows="3" id="" name="ORDER_PROP_<?=$field['ID']?>"><?if ($USER->IsAuthorized()){$comment = GetUserField ("USER", $USER->GetID(), "UF_COMMENTS"); echo $comment;}?></textarea>
 ...
 ?>                                    
             ...
             <div id="store" style="display:none;">
                 ...
                 switch ($arResult["PERSON_TYPE"]) {
                     case 1: // Физическое лицо
                         ...
                         $field = getProp(11, $arResult);
                         ?>               
            
                         <? foreach ($arResult['STORES'] as $store) { ?>      
                             <label class="b-cart-field__label"><input type="radio" value="<?= $store["TITLE"] ?>" name="ORDER_PROP_<?= $field['ID'] ?>"> <?= $store["TITLE"] ?></label>
                             <p class="store_desc"><?= $store["ADDRESS"]; ?></p>
                         <?
                         }
                         break;
                     case 2:

                         $field = getProp(12, $arResult);
                         ?>               
        ...
        <div class="b-cart-field">
            <?foreach($arResult["DELIVERY"] as $delivery){?> 
                <label class="b-cart-field__label"><input name="DELIVERY_ID" value="<?=$delivery['ID'];?>" type="radio" name="radio_1"> <?=$delivery['NAME'];?> <b><?=$delivery['PRICE_FORMATED'];?></b></label>
            <?}?>
        </div>
</div>
...
<div class="b-cart__list">
            ...
            <input type="submit" id="seond_step_btn" name="contButton" value="Продолжить оформление" class="b-button m-orange">
             </div>
            </div>
    </div>
