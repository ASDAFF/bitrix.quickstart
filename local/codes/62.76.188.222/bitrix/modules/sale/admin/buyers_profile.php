...
if(isset($_REQUEST["reorder"]) && IntVal($_REQUEST["reorder"]) > 0)
{
	...
	while ($arBasket = $dbBasketList->Fetch())
		$urlProduct .= "&product[]=".$arBasket["PRODUCT_ID"];
...
if (isset($_REQUEST['action']))
{
	...
	echo "<script type=\"text/javascript\" src=\"/bitrix/js/main/core/core_popup.js\"></script>";
	echo "<script type=\"text/javascript\" src=\"/bitrix/js/main/popup_menu.js\"></script>";
}
*/
...
if(!empty($arUser))
{
	...
	{
		for ($i = 0; $i < count($filter_order_status); $i++)
		{
	...
	while ($arOrder = $dbOrderList->Fetch())
	{
		...
		$id = "<a title=\"".GetMessage('BUYERS_ORDER_DETAIL_PAGE')."\" href=\"/bitrix/admin/sale_order_detail.php?ID=".$arOrder["ID"]."&lang=".LANG."\">".GetMessage('BUYERS_PREF').$arOrder["ID"]."</a>";
		$id .= "<input type=\"hidden\" name=\"table_id\" value=\"".$sTableID_tab3."\">";
		$row->AddField("STATUS_ID", $id);
		...
		$arActions[] = array("ICON"=>"edit", "TEXT"=>GetMessage("BUYER_PD_REORDER"),"ACTION"=>$lAdmin_tab3->ActionRedirect("/bitrix/admin/sale_buyers_profile.php?ID=".$ID."&lang=".LANG."&reorder=".$arOrder["ID"]."&lid=".$arOrder["LID"]));
	...
	while ($arBasket = $dbBasketList->GetNext())
	{
		...
		$name = "[".$arBasket["PRODUCT_ID"]."] <a href=\"".$arBasket["DETAIL_PAGE_URL"]."\">".$arBasket["NAME"]."</a>
	...
	$arFilter = array();
	$arFuserItems = CSaleUser::GetList(array("USER_ID" => $ID));//TODO: вынести в начало и использовать также в action
	...
	{
		$dateFrom = MkDateTime(FmtDate($filter_date_visit_from,"D.M.Y"),"d.m.Y");

		if ($dateFrom)
			$arFilter["DATE_FROM"] = $filter_date_visit_from;
	}
	if(strlen(trim($filter_date_visit_to))>0)
	{
		$dateFrom = MkDateTime(FmtDate($filter_date_visit_to,"D.M.Y"),"d.m.Y");
	...
	while ($arViews = $dbViewsList->Fetch())
	{
		...
		if (count($arResult["SKU_ELEMENTS"]) > 0):
			...
			$linkOrder = 'BX.adminPanel.Redirect([], \'/bitrix/admin/sale_order_new.php?user_id='.$ID.'&lang='.LANG.'&LID='.$arViews["LID"].'&product[]='.$arViews["PRODUCT_ID"].'\', event);';
			...
			<td colspan="2">
				<form name="find_form3" method="GET" action="<?echo $APPLICATION->GetCurPage()?>?">
					...
					<tr>
						...
						<td>
							<span><?=GetMessage('BUYER_F_PRICE_FROM');?></span>&nbsp;<input type="text" size="7" maxlength="10" name="filter_summa_from" value="<?=htmlspecialcharsbx($filter_summa_from)?>">&nbsp;
		...
		<tr>
			<td colspan="2">
				...
				<script>
						...
						{
							t_stat_list_tab5.GetAdminList('/bitrix/admin/sale_buyers_profile.php?ID=<?=$ID?>&lang=<?=LANGUAGE_ID?>&action=viewed_apply&viewed_id='+product_id+'&viewed_lid='+lid);
							t_stat_list_tab4.GetAdminList('/bitrix/admin/sale_buyers_profile.php?ID=<?=$ID?>&lang=<?=LANGUAGE_ID?>');
						}
								...
								if (arSKU[i]["ID"] == selectedSkuId)
								{
									...
									if (arSKU[i]["CAN_BUY"] == "Y")
									{
										...
										else
										{
											...
											BX('viewed_url_action').value = '/bitrix/admin/sale_order_new.php?user_id='+arSKU[i]["USER_ID"]+'&lang=<?=LANG?>&LID='+arSKU[i]["LID"]+'&product[]='+arSKU[i]["ID"];
