	...
	<div id="myTabContent-i02" class="tab-content">
		...
		foreach($arResult["ORDERS"] as $key => $val) {
			...
			if ($val["ORDER"]["CANCELED"] == "Y") {
				...
				// Paid, is formed to be sent
				$tableClass = ' formed';
				
			} elseif ($val["ORDER"]["STATUS_ID"] == "N") {
				// Adopted, is expected to pay
				$tableClass = ' expected';
						...
						foreach($val["BASKET_ITEMS"] as $vvval)
						{
							...
							<li>
                                ...
                                if ($vvval["QUANTITY"] > 0) {
                                    ...
                                    $propStr .= $vvval["QUANTITY"].GetMessage("STPOL_SHT");

                                }
                                if (!empty($arResult["PRODUCTS"][$vvval['PRODUCT_ID']]["COLOR"])) {
                                    if ($propStr != "")  $propStr .= ", ";
                                    $propStr .= GetMessage("COLOR_LABEL")." <span style='text-transform:lowercase'>".$arResult["PRODUCTS"][$vvval['PRODUCT_ID']]["COLOR"]."</span>";
                                }
                                if (!empty($arResult["PRODUCTS"][$vvval['PRODUCT_ID']]["STD_SIZE"])) {
                                    if ($propStr != "")  $propStr .= ", ";
                                    $propStr .= GetMessage("SIZE_LABEL")." ".$arResult["PRODUCTS"][$vvval['PRODUCT_ID']]["STD_SIZE"];
                                }
...
<div class="individ">
<?
	// code for payment buttons through the handler
	$arFilter = Array(
		"USER_ID" 	=> $USER -> GetID(),
		"ID" 		=> $orderId,
	);
