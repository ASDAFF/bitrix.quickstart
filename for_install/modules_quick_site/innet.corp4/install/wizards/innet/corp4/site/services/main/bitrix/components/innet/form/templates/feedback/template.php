...
<div id="feedback-contacts">
    <div class="title5 fs24"><?=GetMessage("INNET_FORM_TEMPLATE_HEADER_FEEDBACK")?></div>
    <form action="<?=POST_FORM_ACTION_URI?>" method="POST">
        ...
        <div class="cols5">
            <div class="col1">
                <input type="text" name="user_name" value="<?=$arResult["AUTHOR_NAME"]?>" placeholder="<?=GetMessage("INNET_FORM_TEMPLATE_NAME")?>" class="input-name">
                <input type="text" name="user_email" value="<?=(!empty($arResult["AUTHOR_EMAIL"])) ? $arResult["AUTHOR_EMAIL"] : htmlspecialcharsbx($USER->GetEmail())?>" placeholder="<?=GetMessage("INNET_FORM_TEMPLATE_EMAIL")?>" class="input-email">
                <input type="text" name="user_phone" value="<?=$arResult["AUTHOR_PHONE"]?>" placeholder="<?=GetMessage("INNET_FORM_TEMPLATE_PHONE")?>" class="input-phone">
            </div>
            ...
            <div class="align-right md-align-center" style=" float: right; margin: 41px 0 0; "><input type="submit" name="submit" value="<?=GetMessage('INNET_FORM_TEMPLATE_SUBMIT')?>" class="btn"></div>
        </div>
    </form>
</div>
