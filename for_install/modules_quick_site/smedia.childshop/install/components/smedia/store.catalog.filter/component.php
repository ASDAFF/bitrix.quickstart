...
if(!$arParams["PRICE_SORT"])
	$arParams["PRICE_SORT"] = 400;
	...
	*************************************************************************/
	...
	if (strlen($_REQUEST["set_filter"])>0)
	{
		...
		$temp_cf=array();
		foreach($_REQUEST[$FILTER_NAME."_cf"] as $key=>$value){
		    if(!is_array($value)){
		        $t=explode("-",$value);
				$_REQUEST[$FILTER_NAME."_cf"][$key]=array("LEFT" => $t[0], "RIGHT" => $t[1]);
		    }
		}
...
if($obCache->StartDataCache($arParams["CACHE_TIME"], $this->GetCacheID(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups())), "/".SITE_ID.$this->GetRelativePath()))
{
	...
	while ($arProp = $rsProp->Fetch())
	{
		...
		{			
			...
			$arResult["arrProp"][$arProp["ID"]]["SORT"] = $arProp["SORT"];
			...
			elseif($arProp["PROPERTY_TYPE"]=="S" && in_array($arProp["CODE"],$arParams["MAKE_LIST"])) 
			{
				$arResult["arrProp"][$arProp["ID"]]["PROPERTY_TYPE"]="L";
				$arProduser=array();				
				$arGroupBy=array("PROPERTY_".$arProp["CODE"]);
				$propFilter=array("IBLOCK_ID"=>$arParams["IBLOCK_ID"]);
				if($arParams["SECTION_CODE"])
					$propFilter["SECTION_CODE"]=$arParams["SECTION_CODE"];
				elseif($arParams["SECTION_ID"])
					$propFilter["SECTION_ID"]=$arParams["SECTION_ID"];
				$propFilter["INCLUDE_SUBSECTIONS"]=$arParams["INCLUDE_SUBSECTIONS"];
				$resPropVal = CIBlockElement::GetList(Array('PROPERTY_'.$arProp["CODE"]=>"ASC"), $propFilter, $arGroupBy, false, false);
				while($arPropVal = $resPropVal->GetNext())
				{
					if($arPropVal['PROPERTY_'.$arProp["CODE"].'_VALUE'])
						$arProduser[$arPropVal['PROPERTY_'.$arProp["CODE"].'_VALUE']]=$arPropVal['PROPERTY_'.$arProp["CODE"].'_VALUE'];
				}							
				$arResult["arrProp"][$arProp["ID"]]["VALUE_LIST"]=$arProduser;
			}
		}
	}
	// offer properties
...
foreach($arParams["FIELD_CODE"] as $field_code)
{
	...
	switch ($field_code)
	{
		...
		case "SEARCHABLE_CONTENT":
			$controlType = "textbox";
			//$field_res = '<input type="text" name="'.$name.'" size="'.$arParams["TEXT_WIDTH"].'" value="'.($value ? htmlspecialchars($value) : htmlspecialchars(GetMessage("IBLOCK_FIELD_".$field_code))).'" onblur="javascript:if(this.value == \'\') { this.value = \''.htmlspecialchars(GetMessage("IBLOCK_FIELD_".$field_code)).'\';}" onfocus="javascript:if(this.value == \''.htmlspecialchars(GetMessage("IBLOCK_FIELD_".$field_code)).'\') this.value = \'\';" />';
            $field_res = '<input type="text" name="'.$name.'" id="filter_'.$name.'"  size="'.$arParams["TEXT_WIDTH"].'" value="'.htmlspecialchars($value).'" />'; 
		...
		case "L":
			...
			/*if ($arProp['MULTIPLE'] == 'Y' || count($arProp["VALUE_LIST"]) == 1)
			{
				foreach($arProp["VALUE_LIST"] as $key=>$val)
				{
					...
					$res .= '<label for="'.$prop_input_id.'">'.htmlspecialchars($arProp["NAME"])." ".htmlspecialchars($val).'</label><br />';
				}
			}
			else
			{*/
				...
				/*if ($arProp["MULTIPLE"]=="Y")
					...
					$res .= '<select name="'.$name.'" size="'.$arParams["NUMBER_WIDTH"].'">';
...
foreach($arResult["arrPrice"] as $price_code => $arPrice)
{
	...
	if($arParams["USE_PRICES_RANGE"]=="Y"){
	
	    $name = $FILTER_NAME."_cf[".$arPrice["ID"]."]";
		
		$value_l = $arrCFV[$arPrice["ID"]]["LEFT"];
		if (strlen($value_l)>0)
	    {
		if(CModule::IncludeModule("catalog"))
			${$FILTER_NAME}[">=CATALOG_PRICE_".$arPrice["ID"]] = $value_l;
		else
			${$FILTER_NAME}[">=PROPERTY_".$arPrice["ID"]] = $value_l;
	    }
		
		$value_r = $arrCFV[$arPrice["ID"]]["RIGHT"];
		if (strlen($value_r)>0)
	    {
		if(CModule::IncludeModule("catalog"))
			${$FILTER_NAME}["<=CATALOG_PRICE_".$arPrice["ID"]] = $value_r;
		else
			${$FILTER_NAME}["<=PROPERTY_".$arPrice["ID"]] = $value_r;
	    }
		
		$value = $arrCFV[$arPrice["ID"]]["LEFT"].'-'.$arrCFV[$arPrice["ID"]]["RIGHT"];
		
	    $res_price .= '<select name="'.$name.'" size="'.$arParams["NUMBER_WIDTH"].'">';
		$res_price .= '<option value="">'.htmlspecialchars(count($arResult["arrPrice"]) > 1 ? $arPrice["TITLE"] : GetMessage('CC_BCF_PRICE')).'</option>';
		for($i=1; $i<=count($arParams["PRICES_RANGE"]); $i++){
		    $res_price .= '<option';
			if(intval($arParams["PRICES_RANGE"][$i-1]).'-'.intval($arParams["PRICES_RANGE"][$i])==$value)
			 {
			   $res_price .= ' selected';
			   $userVal=intval($arParams["PRICES_RANGE"][$i-1]).'-'.intval($arParams["PRICES_RANGE"][$i]);
			  }
			$res_price .= ' value="'.intval($arParams["PRICES_RANGE"][$i-1]).'-'.intval($arParams["PRICES_RANGE"][$i]).'">'.intval($arParams["PRICES_RANGE"][$i-1]).'-'.intval($arParams["PRICES_RANGE"][$i]).'</option>';	
		}
		$res_price .= '</select>';
		if(!$value || $value=="-")
			$userVal=htmlspecialchars(count($arResult["arrPrice"]) > 1 ? $arPrice["TITLE"] : GetMessage('CC_BCF_PRICE'));
	    $arResult["ITEMS"][] = array(
...
if(is_array($arResult["arrPrice"]) && count($arResult["arrPrice"])>0 && $arParams["PRICE_SORT"])
{
	usort($arResult["ITEMS"], 'sortBySort');
}
