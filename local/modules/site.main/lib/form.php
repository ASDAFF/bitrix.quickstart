...
class Form
{
	...
	**/
	...
	{
		if( \COption::GetOptionString('site.main', 'log_events_send') == 'Y' ){
			\CEventLog::Add(array(
				"SEVERITY" => "SECURITY",
				"AUDIT_TYPE_ID" => 'Письмо отправлено',
				"MODULE_ID" => "main",
				"ITEM_ID" => $arTemplate['ID'],
				"DESCRIPTION" => "Адресат: " . $arFields['EMAIL'] . ", Событие: " . $arTemplate['EVENT_NAME']
			));
		}
	}
