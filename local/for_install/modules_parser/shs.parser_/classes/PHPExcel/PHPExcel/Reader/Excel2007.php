...
class PHPExcel_Reader_Excel2007 extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
	...
	public function canRead($pFilename)
	{
		...
		if (!class_exists('ZipArchive',FALSE)) {
			throw new PHPExcel_Reader_Exception(GetMessage("KDA_IE_ZIP_ARCHIVE_NOT_ENABLED"));
		}
			...
			switch ($rel["Type"]) {
				...
				case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
					...
					if (!$this->_readDataOnly && $xmlStyles) {
						if(is_callable(array('PHPExcel_Style_Color', 'setIndexedColor')) && $xmlStyles->colors->indexedColors && $xmlStyles->colors->indexedColors->rgbColor){
							$indexedColors = array();
							$ind = -7;
							...
							PHPExcel_Style_Color::setIndexedColor($indexedColors);
						}
							...
							if ($xmlSheet && $xmlSheet->sheetData && $xmlSheet->sheetData->row) {
								...
								foreach ($xmlSheet->sheetData->row as $row) {
									...
									foreach ($row->c as $c) {
										...
										if(!$cellDataType && (string)$c["s"]==="0" && self::_castToString($c)==null) continue;
