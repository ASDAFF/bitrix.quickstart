...
$arSort = CIBlockParameters::GetElementSortFields(
	array('SHOWS', 'SHOW_COUNTER', 'SORT', 'TIMESTAMP_X', 'NAME', 'ID', 'ACTIVE_FROM', 'ACTIVE_TO'),
...
//custom
if($arDopSortFields = GetMessage('SORT_FIELDS'))
	$arSort = array_merge($arSort, $arDopSortFields);
...
//custom
$arTemplateInfo = CComponentUtil::GetTemplatesList('bitrix:catalog.section');
if(empty($arTemplateInfo))
{
	$arComponentParameters['PARAMETERS']['CATALOG_TEMPLATE'] = Array(
		'PARENT'  => 'BASE',
		'NAME'    => GetMessage('CATALOG_TEMPLATE'),
		'TYPE'    => 'STRING',
		'DEFAULT' => '',
	);
}
else
{
	...
	$arSiteTemplateList = array(
		'.default' => GetMessage('CATALOG_TEMPLATE_SITE_DEFAULT'),
	);
	$arTemplateID       = array();
	...
	foreach($arTemplateInfo as &$template)
	{
		if(isset($arHiddenTemplates[ $template['NAME'] ]))
			continue;
		$strDescr                            = $template['TITLE'] . ' (' . ('' != $template['TEMPLATE'] && '' != $arSiteTemplateList[ $template['TEMPLATE'] ] ? $arSiteTemplateList[ $template['TEMPLATE'] ] : GetMessage('CATALOG_TEMPLATE_SYSTEM')) . ')';
...
{
	$arComponentParameters['PARAMETERS']['INCLUDE_CSS']       = array(
		'PARENT'  => 'SEARCH',
		'NAME'    => GetMessage('INCLUDE_CSS'),
		'TYPE'    => 'CHECKBOX',
		'DEFAULT' => 'Y',
	);
	$arComponentParameters['PARAMETERS']['INPUT_PLACEHOLDER'] = array(
		'PARENT'  => 'SEARCH',
	...
	$arComponentParameters['PARAMETERS']['RESULT_NOT_FOUND']  = array(
		'PARENT'  => 'SEARCH',
