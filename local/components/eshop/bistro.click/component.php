...
$arResult['SHOW_CAPTCHA'] = 0;
if($arParams['SHOW_CAPTCHA']==0) {
	$arResult['SHOW_CAPTCHA'] = 0;
}elseif($arParams['SHOW_CAPTCHA']==1) {
	$arResult['SHOW_CAPTCHA'] = 1;
}elseif($arParams['SHOW_CAPTCHA']==2 && !$USER->IsAuthorized()) {
	$arResult['SHOW_CAPTCHA'] = 1;
}
...
if(intval($_REQUEST["name_bk"])==1){
	...
	if(!check_bitrix_sessid('bistrclick_sessid')) {
		$error['bistrclick_sessid'] = GetMessage('CAT_BK_FIELD_ERROR_SESS');
	}

	foreach($arResult['SEND_REQ'] as $key=>$value) {
		if($value==1 && !trim($_REQUEST[$key])) {
			if(isset($arResult['LABEL'][$key])) {
				$error[$key] = $arResult['LABEL'][$key].' '.GetMessage("CAT_BK_FIELD_ERROR_REQ");
			}else{
				$error[$key] = GetMessage("CAT_BK_FIELD_ERROR_".strtoupper($key));
			}
		}
	}

	if ($arResult['SHOW_CAPTCHA']==1 && !$GLOBALS["APPLICATION"]->CaptchaCheckCode($_REQUEST['cap'], $_REQUEST['captcha_sid'])) {
		$error['cap'] = GetMessage("CAT_BK_FIELD_ERROR_CAPCHA");
	}

	foreach($arParams['FIELD_SHOW'] as $key=>$value) {
		if($value=='phone') {
			if(CByClick::getname($_REQUEST[$value])) {
				$send[$value] = CByClick::getname($_REQUEST[$value]);
				unset($error[$value]);
				if(CModule::IncludeModule("mlife.smsservices")) {
					$obSmsServ = new CSmsServices();
					$phoneCheck = $obSmsServ->checkPhoneNumber($send[$value]);
					$send[$value] = $phoneCheck[$value];
					if(!$phoneCheck['check']) {
						$error[$value] = GetMessage("CAT_BK_FIELD_ERROR_PHONE2");
					}
				}
			}
		}elseif($value=='email') {
			...
			if(CByClick::getname($_REQUEST[$value]) && !preg_match($requlEmail,CByClick::getname($_REQUEST[$value]))) {
				$error[$value] = GetMessage("CAT_BK_FIELD_ERROR_EMAIL2");
				$send[$value] = CByClick::getname($_REQUEST[$value]);
			}elseif(CByClick::getname($_REQUEST[$value])) {
				$send[$value] = CByClick::getname($_REQUEST[$value]);
				unset($error[$value]);
			}
		}else {
	...
	$send['delivery'] = CByClick::getname($_REQUEST['delivery']);
	$send['paysystem'] = CByClick::getname($_REQUEST['paysystem']);
...
if($showform) {
	...
	$ref1 = CByClick::getref($date_ref,$key,$arResult['REF_START'],1);
	$ref2 = CByClick::getref($date_ref,$key,$arResult['REF_START'],2);

	$arResult['CHECK_SPAM'] = CByClick::checkspam($referer_start, $ref1, $ref2);
...
if(count($arResult['ERROR'])==0 && intval($_REQUEST["name_bk"])==1) {
	...
	//mprint($offer_code);
	$arProduct = CByClick::formatTovar($product_id,$arResult,$arProduct,$offer_code);
	$arEventUser = array();
	...
	if($arParams['CREATE_ORDER']=='Y'){
		...
		if($arParams['CHECK_USER']=='Y' && !$newuser && $arResult['SEND']['email']){
			$newuser = CByClick::getuserforEmail($arResult);
		}
		...
		$arFields = CByClick::fieldarr(SITE_ID,$arParams['PERSON_TYPE'],$price,$arParams['CURRENCY_ID'],$newuser,$arResult['SEND'],GetMessage("CAT_BK_MESS_ORDER"));
		...
		if ($ORDER_ID>0){
			...
			if(isset($arProduct["ID"]) && $arProduct["ID"]>0) {
				$arFields = CByClick::getFieldsOrder($arProduct,$newuser,$ORDER_ID,$arParams['OFFERS_PROPERTY_CODE']);
	...
	if($arParams['NOTICE_ADMIN']=='Y' && $arParams['NOTICE_ADMIN_MAIL']=='Y' && $arParams['NOTICE_ADMIN_MAIL_EMAIL']) {
		...
		CEvent::Send("BISTROKLICK", SITE_ID, $arEventFields, 'Y', $arParams['NOTICE_ADMIN_MAIL_EVENT_MESSAGE_ID']);
	}
	...
	if($arParams['NOTICE_ADMIN']=='Y' && $arParams['NOTICE_USER_MAIL']=='Y') {
		...
		if(isset($arEventFields['USER_PASS'])) {
			CEvent::Send("BISTROKLICK", SITE_ID, $arEventFields, 'Y', $arParams['NOTICE_USER_MAIL_EVENT_MESSAGE_ID2']);
		}else{
			CEvent::Send("BISTROKLICK", SITE_ID, $arEventFields, 'Y', $arParams['NOTICE_USER_MAIL_EVENT_MESSAGE_ID']);
		}
	}
	...
	if($arParams['NOTICE_ADMIN']=='Y' && $arParams['NOTICE_ADMIN_SMS']=='Y' && $arParams['NOTICE_ADMIN_SMS_PHONE']) {
		...
		|| (CModule::IncludeModule("asd.smsswitcher") && $arParams['NOTICE_SMS_MODULE']=='smsswitcher')) {
			$arEventFields['SEND_PHONE'] = $arParams['NOTICE_ADMIN_SMS_PHONE'];
				...
				$arEM = $rsEM->Fetch();
				$shab = $arEM['MESSAGE'];
				$mess = CByClick::сlick_macros_replace($arEventFields,$shab);
				if(CModule::IncludeModule("mlife.smsservices") && $arParams['NOTICE_SMS_MODULE']=='smsservices') {
					$obSmsServ = new CSmsServices();
