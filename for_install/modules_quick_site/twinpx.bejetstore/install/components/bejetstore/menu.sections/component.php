	...
	else
	{
		...
		while($arSection = $rsSections->GetNext())
		{
			...
			if('Y' == $arParams['HIDE_NOT_AVAILABLE']){
				$arSectionSelect = Array("ID", "NAME", "IBLOCK_ID");
				$arSectionFilter = Array("IBLOCK_ID"=>$arParams["IBLOCK_ID"],/* ">QUANTITY"=>0, "CATALOG_AVAILABLE"=>'Y',*/ "ACTIVE"=>"Y", "!CATALOG_PRICE_BASE"=>"", "SECTION_ID"=>$arSection['ID'], "INCLUDE_SUBSECTIONS" => "Y");
				if('Y' == $arParams['HIDE_NOT_AVAILABLE']){
					$arSectionFilter[">QUANTITY"] = 0;
					$arSectionFilter["CATALOG_AVAILABLE"] = 'Y';
					$arSectionFilter[">QUANTITY"] = 0;
				}
				$res = CIBlockElement::GetList(Array(), $arSectionFilter, false, Array("nTopCount"=>1), $arSectionSelect);
				$arSection["ELEMENTS_CNT"] = $res->SelectedRowsCount();
				if($arSection["ELEMENTS_CNT"]){
					$arResult["SECTIONS"][] = array(
						"ID" => $arSection["ID"],
						"DEPTH_LEVEL" => $arSection["DEPTH_LEVEL"],
						"~NAME" => $arSection["~NAME"],
						"~SECTION_PAGE_URL" => $arSection["~SECTION_PAGE_URL"],
						"ELEMENTS_CNT" => $arSection["ELEMENTS_CNT"]
					);
