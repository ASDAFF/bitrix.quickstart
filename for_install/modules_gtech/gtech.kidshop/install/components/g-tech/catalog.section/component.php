...
if($this->StartResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation)))
{
	...
	$arFilter = array(
        ...
        if($_GET[set_filter]&&!$_GET[del_filter]){
		$arFilter[] = array("NAME" => "%".$_GET[arrFilter_ff][NAME]."%");
		$arFilter[] = array(
			">CATALOG_PRICE_1" => $_GET[arrFilter_cf][1][LEFT],
			"<CATALOG_PRICE_1" => $_GET[arrFilter_cf][1][RIGHT],);
		$arFilter[] = array("PROPERTY_MANUFACTURER" => $_GET[arrFilter_pf][MANUFACTURER]);
	}
