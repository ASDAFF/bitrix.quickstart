...
/* количество записей */
$query = "SELECT COUNT(*) AS CNT FROM {$arResult['prefix']}tt_news";
...
{
	$query = "SELECT * FROM {$arResult['prefix']}tt_news LIMIT {$left}, 10";
	...
	$ib = CIBlock::GetList(array(), array("CODE"=>"typo3_news"))->GetNext();
	...
	$arLoadProductArray = Array(		  			
			...
			"NAME"           => $arItem['title'],
			"ACTIVE"         => "Y",            // активен		  
			"DETAIL_TEXT"    => $arItem['bodytext'],		
			"DETAIL_TEXT_TYPE" => 'html',
			"PREVIEW_TEXT"    => $arItem['short'],		
	...
	$query = "SELECT * FROM {$arResult['prefix']}tt_news_cat_mm WHERE {$arResult['prefix']}uid_local={$arItem['uid']}";
