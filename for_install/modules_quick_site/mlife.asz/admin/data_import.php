...
$arAszCatalogFields = array(
	"ASZ_kol" => array(
		"field" => "ASZ_kol",
		"important" => "N",
		"name" => GetMessage("MLIFE_ASZ_DATA_IMP_1"),
	),
	"ASZ_zak" => array(
		"field" => "ASZ_zak",
		"important" => "N",
		"name" => GetMessage("MLIFE_ASZ_DATA_IMP_2"),
	),
);
...
$curency = \Mlife\Asz\CurencyTable::getList(
	array(
		'select' => array('CODE'),
		'filter' => array(array("LOGIC"=>"OR",array("=SITEID"=>$siteId),array("=SITEID"=>false)),array("BASE"=>"Y")),
		'group' => array("CODE"),
	)
);
$arCurency = array();
if($arCurencydb = $curency->Fetch()){
	$arCurency[] = $arCurencydb["CODE"];
}
if(count($arCurency)==0){
	$arCurency[] = "RUB";
}

//получаем ид системного свойства
$asz_system_id = 0;
$properties = CIBlockProperty::GetList(Array("sort"=>"asc", "name"=>"asc"), Array("IBLOCK_ID"=>$IBLOCK_ID, "CODE"=>"ASZ_SYSTEM"));
if($prop_fields = $properties->GetNext()){
	$asz_system_id = $prop_fields["ID"];
}

foreach($arPrice as $price){
	$arAszCatalogFields["ASZ_cod".$price["CODE"]] = array(
		"field" => "ASZ_cod".$price["CODE"],
		"important" => "N",
		"name" => GetMessage("MLIFE_ASZ_DATA_IMP_3").": ".$price["NAME"],
	);
	$arAszCatalogFields["ASZ_cur".$price["CODE"]] = array(
		"field" => "ASZ_cur".$price["CODE"],
		"important" => "N",
		"name" => GetMessage("MLIFE_ASZ_DATA_IMP_3").": ".$price["NAME"]. " - ".GetMessage("MLIFE_ASZ_DATA_IMP_4"),
	);
}
...
class CAssocData extends CCSVData
{
	...
	function makePropAszSystemData($data,$arPrice,$curencyBase){
		
		$strAsz = "";
		
		if(isset($data["kol"])){
			$strAsz .= "kol:::".intval($data["kol"]);
		}else{
			$strAsz .= "kol:::0";
		}
		
		if(isset($data["zak"])){
			$strAsz .= ":::".intval($data["zak"]);
		}else{
			$strAsz .= ":::0";
		}
		...
		foreach($arPrice as $price) {
			if(isset($data['cod'.$price["CODE"]])){
				$priceVal = str_replace(",", ".", $data['cod'.$price["CODE"]]);
				$priceVal = preg_replace("/[^0-9\.]/", "", $priceVal);
				$priceVal = round($priceVal,2);
			}else{
				$priceVal = "0.00";
			}
			if(isset($data['cur'.$price["CODE"]])){
				$priceCur = $data['cur'.$price["CODE"]];
			}else{
				$priceCur = $curencyBase;
			}
			$strAsz .= "cod".$price["CODE"].":::".$priceVal.":::".$priceCur."+++";
		}
	...
	if ($STEP > 3)
	{
		...
		if (strlen($strError) <= 0)
		{
			...
			while ($arRes = $csvFile->FetchAssoc())
			{
				...
				//ASZ магазин
				$arLoadProductASZArray = array();
				foreach ($arAszCatalogFields as $key => $arField)
				{
					if (array_key_exists($key, $arRes))
						$arLoadProductASZArray[str_replace("ASZ_","",$arField["field"])] = $arRes[$key];
				}
				
				$strAsz = $csvFile->makePropAszSystemData($arLoadProductASZArray,$arPrice,$arCurency[0]);
				...
				if($asz_system_id>0){
					$arLoadProductArray["PROPERTY_VALUES"][$asz_system_id] = $strAsz;
				}
...
?>
	...
	foreach ($arAszCatalogFields as $arField)
	{
		$arAvailFields[] = array(
			"value" => $arField["field"],
			"name" => $arField["name"],
