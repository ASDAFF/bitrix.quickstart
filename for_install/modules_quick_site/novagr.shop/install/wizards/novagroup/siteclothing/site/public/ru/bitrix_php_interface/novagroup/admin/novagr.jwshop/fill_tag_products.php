...
if (intval($countElems)>0) {
    ...
    while ($data = $rsSubElement -> GetNextElement())
    {
        ...
        $arParams["OFFERS_PROPERTY_CODE"] = array("COLOR_STONE","STD_SIZE");
        ...
        $arResultPrices = CIBlockPriceTools::GetCatalogPrices((int)$_REQUEST['NOVAGROUP_FILL_IBLOCK_ID'], $arParams["PRICE_CODE"]);
        ...
        foreach($arOffers as $arOffer)
        {
            ...
            $arColors[] = $arOffer["DISPLAY_PROPERTIES"]["COLOR_STONE"]["VALUE"];
        ...
        if(is_array($arProps["SAMPLES"]["VALUE"]))
        ...
        if (!empty($arFields["CATALOG_PRICE_1"])) {
            ...
            if (Novagroup_Classes_General_Main::endsWith($arFields["CATALOG_PRICE_1"], '.00')) {
    ...
    if (is_array($products)) {
        ...
        foreach ($products as $key => $value) {
            ...
            foreach ($value["COLORS"] as $item) {
                //$elem["COLORS"]
                if (!empty($arResult['mixData'][$item]["NAME"])) $tagsContent[] = $arResult['mixData'][$item]["NAME"];
            }
            ...
            if(is_array($value["CML2_BAR_CODE"]))
            foreach ($value["CML2_BAR_CODE"] as $item) {
                $tagsContent[] = ($item);
            }
            if(is_array($value["CML2_ARTICLE"]))
