...
if($REQUEST_METHOD=="POST" && strlen($Update.$Apply)>0 && check_bitrix_sessid())
{
  foreach($arAllOptions as $arOption)
  {
    $name=$arOption[0];
    $val=$_REQUEST[$name];
    COption::SetOptionString("refreshlab.err404", $name, $val, $arOption[1], $arOption[4]);
  }
...
<?$tabControl->BeginNextTab();?>
	<?
	foreach($arAllOptions as $arOption):
		$val = COption::GetOptionString("refreshlab.err404", $arOption[0], $arOption[2], $arOption[4]);
		$type = $arOption[3];
	?>
	<tr>
