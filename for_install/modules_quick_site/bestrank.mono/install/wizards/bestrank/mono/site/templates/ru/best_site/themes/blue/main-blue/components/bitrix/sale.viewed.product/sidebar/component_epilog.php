...
if (count($arResult['IDS']) > 0 && CModule::IncludeModule('sale'))
{
	...
	if (count($arPageItems) > 0 || count($arPageItemsDelay) > 0)
	{
		...
		{
			echo "disableAddToCart('viewed_catalog_add2cart_link_".$id."', 'list', '".GetMessage("CATALOG_IN_CART")."');\r\n";
		}
		foreach ($arPageItemsDelay as $id) 
		{
			echo "disableAddToCart('viewed_catalog_add2cart_link_".$id."', 'list', '".GetMessage("CATALOG_IN_CART_DELAY")."');\r\n";
		}
		echo '})</script>';
	}
	...
	/*if (count($arItemsInCompare) > 0)
	{
		...
		{
			echo "disableAddToCompare('catalog_add2compare_link_".$id."', 'list', '".GetMessage("CATALOG_IN_COMPARE")."');\r\n";
		}
		echo '})</script>';
	}      */
}
