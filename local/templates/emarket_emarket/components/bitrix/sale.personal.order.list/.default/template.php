...
<form method="GET" action="<?=SITE_DIR?>personal/" name="bfilter">
<input type="hidden" name="page" value="orders" />
<table class="sale-personal-order-list-filter data-table">
	<tr>
		<th colspan="2" class="head"><?echo GetMessage("SPOL_T_F_FILTER")?></th>
	</tr>
	<tr>
	...
	<tr>
		<th style="text-align: right;" colspan="2">
			<input type="submit" class="em_button" style="float: right; margin-left: 10px;" name="filter" value="<?=GetMessage("SPOL_T_F_SUBMIT")?>">&nbsp;&nbsp;
			<input type="submit" class="em_button" name="del_filter" style="float: right;" value="<?=GetMessage("SPOL_T_F_DEL")?>">
		</th>
	</tr>
</table>
</form>
	...
	<?foreach($arResult["ORDERS"] as $val):?>
		<tr>
			...
			<td><a title="<?=GetMessage("SPOL_T_DETAIL_DESCR")?>" href="<?=$val["ORDER"]["URL_TO_DETAIL"]?>"><?=GetMessage("SPOL_T_DETAIL")?></a><br />
				<?if(false):?>
                <a title="<?=GetMessage("SPOL_T_COPY_ORDER_DESCR")?>" href="<?=$val["ORDER"]["URL_TO_COPY"]?>&page=orders"><?=GetMessage("SPOL_T_COPY_ORDER")?></a><br />
                <?endif;?>
				<?if(false && $val["ORDER"]["CAN_CANCEL"] == "Y"):?>
					<a title="<?=GetMessage("SPOL_T_DELETE_DESCR")?>" href="<?=$val["ORDER"]["URL_TO_CANCEL"]?>&page=orders"><?=GetMessage("SPOL_T_DELETE")?></a>
				<?endif;?>
			</td>
		</tr>
	<?endforeach;?>
