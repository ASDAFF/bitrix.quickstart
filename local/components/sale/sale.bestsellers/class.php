...
class CSaleBestsellersComponent extends CCustomCatalogViewedProductsComponent
{
	/**
	 * @param $params
	 * @override
...
//									">=DATE_UPDATE" => $date,
	...
	protected function getProductIds()
	{
		...
		if (!empty($ordersfilter) && $this->arParams["PAGE_ELEMENT_COUNT"] > 0)
		{
			$productIds = array();
            ...
            $res = CIBlockElement::GetList(
                ["CREATED" => "DESC"],
                ['IBLOCK_ID' => 2, 'ID' => $products, 'CATALOG_AVAILABLE' => "Y", ">CATALOG_QUANTITY" => 0],
                false,
			...
			$res->NavStart(2);
			$this->arResult["NavPageNomer"] = $res->NavPageNomer;
			$this->arResult["NavPageCount"] = $res->NavPageCount;
			$this->arResult["ShowBtnMore"] = $res->NavPageNomer < (int)$res->NavPageCount;
			return $productIds;
		}
		$this->arResult["ShowBtnMore"] = true;
		return array();
	}
