...
if (!isset($arParams['ASD_INCLUDE_SCRIPTS']))
...
$arResult['ASD_TEXT'] = __urlencode($arParams['ASD_TEXT']);
...
if ($arParams['SCRIPT_IN_HEAD'] == 'Y') {
	if (strlen($arParams['ASD_PICTURE_REL'])) {
		$APPLICATION->AddHeadString('<meta property="og:image" content="' . $arParams['ASD_PICTURE_REL'] . '"/> ');
		$APPLICATION->AddHeadString('<link rel="image_src" href="' . $arParams['ASD_PICTURE_REL'] . '"/>');
	}
