BX.ready(function () {
    ...
    function work_onload(result)
    {
        try
        {
	        ...
	        var error = CurrentStatus[4];
	        if(error) {
		        throw new ReadError(error, e);
	        }
            ...
            if (strCurrentAction != 'null' && strNextRequest)
            {
	...
	BX.bindDelegate(document.body, 'change', {className: 'js-work-file' },
		function () {
			var service = this.getAttribute("data-service");
			...
			formData.append('service_file', service + '_file');
			$.ajax({
				url: self.location,
				type: 'POST',
				data: formData,
				...
				success: function( data ){
					$('.js-work-start').attr('disabled',false);
				}
			});
		}
	);
