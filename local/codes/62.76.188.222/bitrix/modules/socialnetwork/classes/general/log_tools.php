...
class CSocNetLogTools
{
	...
	function FormatEvent_CreateAvatar($arFields, $arParams, $source = "CREATED_BY_")
	{
		...
		if (intval($arFields[$source."PERSONAL_PHOTO"]) > 0)
		{
			...
			if (empty($cachedAvatars[$arFields[$source."PERSONAL_PHOTO"]]))
			{
				...
				if ($imageFile !== false)
				{
					...
					$cachedAvatars[$arFields[$source."PERSONAL_PHOTO"]] = $AvatarPath = $arFileTmp["src"];
				}
			}
			else
				$AvatarPath = $cachedAvatars[$arFields[$source."PERSONAL_PHOTO"]];
		}
	...
	function FormatEvent_SystemFriends($arFields, $arParams, $bMail = false)
	{
		...
		if (intval($arFields["MESSAGE"]) > 0)
		{
			...
			if ($bMail)
			{
				...
				{
					$arFields["URL"] = str_replace(array("#user_id#", "#USER_ID#"), $arFields["ENTITY_ID"], $friends_page);
		...
		if ($arElement)
		{
			...
			if(intval($TOPIC_ID) > 0)
			{
				if (COption::GetOptionString("forum", "FILTER", "Y") == "Y")
					$arFields["TEXT_MESSAGE"] = CFilterUnquotableWords::Filter($arFields["TEXT_MESSAGE"]);
	...
	function FormatDestinationFromRights($arRights, $arParams, &$iMoreCount = false)
	{
		...
		foreach($arRights as $right_tmp)
		{
			...
			elseif (preg_match('/^D(\d+)$/', $right_tmp, $matches) && CModule::IncludeModule("iblock"))
			{
				$cnt++;
				if (!$bJustCount)
				{
			...
			)
			{
				...
				{
					// already cached
					$arSonetGroup = CSocNetGroup::GetByID($matches[1]);
					if ($arSonetGroup)
					{
						$arDestination[] = array(
							"STYLE" => "sonetgroups",
...
class logTextParser extends CTextParser
{
	...
	function convert($text, $arImages = array(), $allow = array("HTML" => "N", "ANCHOR" => "Y", "BIU" => "Y", "IMG" => "Y", "QUOTE" => "Y", "CODE" => "Y", "FONT" => "Y", "LIST" => "Y", "SMILES" => "Y", "NL2BR" => "N", "VIDEO" => "Y", "TABLE" => "Y", "CUT_ANCHOR" => "N"), $arParams = Array())
	{
		$this->allow = array(
