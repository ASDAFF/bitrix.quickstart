...
$intisAuthClass = new CIntisTwoFactorAuthentificationLite();
$arResult['INTIS_CONFIRM'] = "N";

// register user
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_REQUEST["register_submit_button"]) && !$USER->IsAuthorized() && !$_POST['CONFIRM_CODE'] && !$_POST['CONFIRM_ID'] && !$_POST['CONFIRM_CODE'])
{
	...
	else // if there;s no any errors - create user
	{
        ...
        $intisPass = $intisAuthClass->GenConfirmCode($intisAuthClass->GetRegTemplateSymbol());
		$arResult['VALUES']["UF_INTIS_CONFIRM"] = md5($intisPass);
		...
		if (intval($ID) > 0)
		{
            ...
            $intisAuthClass->Send(GetMessage("AUTH_CONFIRM_ID").$ID." ".GetMessage("AUTH_CONFIRM_PASS").$intisPass, $arUser[$intisAuthClass->GetUserPhoneField()], $intisAuthClass->GetTokenField(), false);

            $arResult['INTIS_CONFIRM'] = "Y";
            ...
            LocalRedirect($arParams['CONFIRM_URL']."?wait_confirm=Y");
		}
		else
		{
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_REQUEST["register_submit_button"]) && !$USER->IsAuthorized() && $_POST['CONFIRM_CODE'] && $_POST['CONFIRM_ID'])
{
    $userIsFound = "N";
    $filterGroup = Array
    (
        "ID" => htmlspecialchars($_POST['CONFIRM_ID']),
        "ACTIVE" => "N"
    );
    $arParams["SELECT"] = array("UF_INTIS_CONFIRM");

    $rsUsers = CUser::GetList($by, $order, $filterGroup, $arParams);

    while ($arUSR = $rsUsers->Fetch())
    {
        if (($arUSR['ID']==htmlspecialchars($_POST['CONFIRM_ID'])) && ($arUSR['UF_INTIS_CONFIRM']==md5(htmlspecialchars($_POST['CONFIRM_CODE']))))
        {
            $user = new CUser;
            ...
            $msg = str_replace("#LOGIN#", $arUSR['LOGIN'], $intisAuthClass->HelloMessage());

            $intisAuthClass->Send($msg, $arUSR[$intisAuthClass->GetUserPhoneField()], $intisAuthClass->GetTokenField(), false);
    ...
    $arResult['USER_NOT_FOUND'] = "Y";

    LocalRedirect($arParams['CONFIRM_URL']."?wait_confirm=Y&wrong=Y");
}
