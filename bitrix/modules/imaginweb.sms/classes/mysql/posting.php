<?
require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/imaginweb.sms/classes/general/posting.php");

class SMSCPosting extends SMSCPostingGeneral
{
		...
		if (is_array($arFilter))
		{
			foreach($arFilter as $key=>$val)
			{
				...
				case "TO":
					...
					if(strlen($r) > 0)
						$arSqlSearch[] = "EXISTS (SELECT * FROM iwebsms_posting_sms PE WHERE PE.POSTING_ID=P.ID AND PE.STATUS='N' AND ".$r.")";
					break;
				case "BODY_TYPE":
				...
				case "RUB_ID":
					if(is_array($val) && count($val) > 0)
					{
						...
						if(count($rub_id))
							$arSqlSearch[] = "EXISTS (SELECT * from iwebsms_posting_rubric PR WHERE PR.POSTING_ID = P.ID AND PR.LIST_RUBRIC_ID in (".implode(", ", $rub_id)."))";
					}
					break;
				case "BODY":
