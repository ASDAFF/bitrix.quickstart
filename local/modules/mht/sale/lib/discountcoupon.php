...
class DiscountCouponsManager
{
	 ...
	 *
	 * @param string $coupon	Add coupon.
	 * @return bool
	 */
		...
		if (self::isSuccess())
		{
			...
			if (!isset(self::$coupons[$coupon]))
			{
				...
				if (!isset(self::$coupons[$couponData['COUPON']]))
				{
					$couponData['SORT'] = self::$couponIndex;
					self::createApplyFields($couponData);
					self::$coupons[$coupon] = $couponData;
					self::$couponIndex++;
					self::saveToStorage();
	 ...
	 *
	 * @param string $coupon	Delete coupon.
	 * @return bool
	 */
	...
	protected static function checkBaseData(&$data, $checkCode = self::COUPON_CHECK_OK)
	{
		...
		else
		{
			...
			if (!isset(self::$couponTypes[$data['TYPE']]))
			{
				$data['TYPE'] = Internals\DiscountCouponTable::TYPE_UNKNOWN;
				$checkCode |= self::COUPON_CHECK_CORRUPT_DATA;
			}
		...
		if (isset($data['ACTIVE_FROM']) && !($data['ACTIVE_FROM'] instanceof Type\DateTime))
		{
			$data['ACTIVE_FROM'] = null;
			$checkCode |= self::COUPON_CHECK_CORRUPT_DATA;
		}
		if (isset($data['ACTIVE_TO']) && !($data['ACTIVE_TO'] instanceof Type\DateTime))
		{
			$data['ACTIVE_TO'] = null;
			$checkCode |= self::COUPON_CHECK_CORRUPT_DATA;
		}
		...
		if (isset($data['DISCOUNT_ACTIVE_FROM']) && !($data['DISCOUNT_ACTIVE_FROM'] instanceof Type\DateTime))
		{
			$data['DISCOUNT_ACTIVE_FROM'] = null;
			$checkCode |= self::COUPON_CHECK_CORRUPT_DATA;
		}
		if (isset($data['DISCOUNT_ACTIVE_TO']) && !($data['DISCOUNT_ACTIVE_TO'] instanceof Type\DateTime))
		{
	...
	protected static function checkFullData(&$data, $mode = self::COUPON_MODE_FULL, $checkCode = self::COUPON_CHECK_OK, $currentTimestamp)
	{
		...
		if (self::$checkActivity)
		{
			...
			if ($data['DISCOUNT_ACTIVE_FROM'] instanceof Type\DateTime && $data['DISCOUNT_ACTIVE_FROM']->getTimestamp() > $currentTimestamp)
				$checkCode |= self::COUPON_CHECK_RANGE_ACTIVE_FROM_DISCOUNT;
			if ($data['DISCOUNT_ACTIVE_TO'] instanceof Type\DateTime && $data['DISCOUNT_ACTIVE_TO']->getTimestamp() < $currentTimestamp)
		...
		if ($mode == self::COUPON_MODE_FULL)
		{
			if (self::$checkActivity)
			{
				if ($data['ACTIVE_FROM'] instanceof Type\DateTime && $data['ACTIVE_FROM']->getTimestamp() > $currentTimestamp)
					$checkCode |= self::COUPON_CHECK_RANGE_ACTIVE_FROM;
				if ($data['ACTIVE_TO'] instanceof Type\DateTime && $data['ACTIVE_TO']->getTimestamp() < $currentTimestamp)
	...
	protected static function saveToStorage()
	{
		if (self::isSuccess())
		{
			...
			if (!empty(self::$coupons))
				$couponsList = array_keys(self::$coupons);
			...
			if (!self::$onlySaleDiscount)
			{
				$eventData = array(
					...
					'COUPON_TYPES' => array(
						...
						Internals\DiscountCouponTable::TYPE_ONE_ORDER,
