...
?> <?=$arResult['NavRecordCount'];?> <?=pluralForm($arResult['NavRecordCount'], GetMessage("MODEL_ONE"), GetMessage("MODEL_MANY"), GetMessage("MODEL_OF"))?></span>
        <?
        if ( $arResult['NavRecordCount'] > 16)
        {
            ?>
			<a class="npagesize <? if($arParams['nPageSize'] == 16){echo'active';}else{echo'incative';}?>" data-value="16"><?=GetMessage("FORMAT_LABEL")?> 16</a><a class="npagesize <? if($arParams['nPageSize'] == 160){echo'active';}else{echo'incative';}?>" data-value="160"><?=GetMessage("FORMAT_LABEL")?> 160</a>
        <?
        }
        ?>
	</div>
...
<div class="product-count-bottom bot-p">
    <span><?=GetMessage("FOUND_LABEL")?> <?=$arResult['NavRecordCount'];?> <?=pluralForm($arResult['NavRecordCount'], GetMessage("MODEL_ONE"), GetMessage("MODEL_MANY"), GetMessage("MODEL_OF"))?></span>
    <?
    if( $arResult['NavRecordCount'] > 16)
    {
        ?>
