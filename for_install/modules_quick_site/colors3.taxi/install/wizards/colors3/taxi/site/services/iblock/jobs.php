...
if(COption::GetOptionString("taxi", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "jobs".WIZARD_SITE_ID; 
		$iblockType = "jobs";
		...
		{
			CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/jobs/index.php", array("JOBS_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}
...
WizardServices::IncludeServiceLang("jobs.php", $lang);

CUserOptions::SetOption("form", "form_element_".$iblockID, array ( 'tabs' => 'edit1--#--'.GetMessage("WZD_OPTION_JOBS_5").'--,--ACTIVE--#--'.GetMessage("WZD_OPTION_JOBS_1").'--,--NAME--#--'.GetMessage("WZD_OPTION_JOBS_2").'--,--DETAIL_TEXT--#--'.GetMessage("WZD_OPTION_JOBS_3").'--,--SORT--#--'.GetMessage("WZD_OPTION_JOBS_4").'--;--', ));
