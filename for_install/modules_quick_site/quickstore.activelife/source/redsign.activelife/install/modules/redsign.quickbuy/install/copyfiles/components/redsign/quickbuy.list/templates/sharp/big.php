...
<?if(count($arResult["ITEMS"])>0):?>
	...
	<? if (isset($arItem['OFFERS']) && !empty($arItem['OFFERS'])) { 
	 ...
	 if (isset($arItem['OFFERS'][0]['QUICKBUY']) && !empty($arItem['OFFERS'][0]['QUICKBUY'])) { 
		...
		<!-- first view -->
		<div class="daysarticle sharp_wrap view1" id="<?=$arItem['ID']?>">
			<div class="left_part">
			...
			<div class="right_part">
				<div class="titles">
					...
					<div class="main">
						...
						<span class="title"><?=GetMessage("QUICK_END_TITLE")?></span>
						<div class="progress_bar" style="width: 150px;">
							<div class="progress" style="width: <? echo round($timer['TIMER']['TIME_LIMIT']/(strtotime($timer['DATE_TO']) - strtotime($timer['DATE_FROM']))*100); ?>%;"><? echo round($timer['TIMER']['TIME_LIMIT']/(strtotime($timer['DATE_TO']) - strtotime($timer['DATE_FROM']))*100); ?>%</div>
						</div>
					</div>
				</div>
