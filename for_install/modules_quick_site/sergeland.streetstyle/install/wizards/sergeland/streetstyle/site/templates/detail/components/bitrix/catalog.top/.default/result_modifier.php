...
if($arParams["~IBLOCK_ID"] > 1 && strlen($arParams["~IBLOCK_TYPE"]) > 1)
{	
	$arFilterLink = Array("IBLOCK_ID" => $arParams["~IBLOCK_ID"], "IBLOCK_TYPE" => $arParams["~IBLOCK_TYPE"], "ACTIVE" => "Y");	
	$arSelectLink = Array(
		"ID",
		"IBLOCK_ID",
		"CODE",
	...
	while($obRes = $res->GetNextElement())
	{
		...
		$arResult["~ITEMS"][$arFields["ID"]]["PREVIEW_PICTURE"] = (0 < $arFields["PREVIEW_PICTURE"] ? CFile::GetFileArray($arFields["PREVIEW_PICTURE"]) : false);
		$arResult["~ITEMS"][$arFields["ID"]]["DETAIL_PICTURE"]  = (0 < $arFields["DETAIL_PICTURE"]  ? CFile::GetFileArray($arFields["DETAIL_PICTURE"])  : false);
	}
}
...
{		
	...
	if(array_key_exists($arItem["ID"], $arResult["~OFFERS"]) && count($arResult["~ITEMS"]) > 0)
	{
		$arItem = $arResult["~ITEMS"][$arItem["PROPERTIES"][$arParams["LINK_PROPERTY_SID"]]["VALUE"]];
		...
		foreach($arResult["~OFFERS_ID"][$arItem["ID"]] as $OFFERS_ID)
		{
			...
			foreach($arParams["PRICE_CODE"] as $PRICE_CODE)
			{
				if(!empty($arPrice[$PRICE_CODE]) && $arPrice[$PRICE_CODE]["DISCOUNT_VALUE"] > 0)
				{
					...
					$arItem["~SELECT"]["SIZE"][$arFields["ID"]]["SELECT_QUANTITY_VALUE"]    	  = $arQuantity["QUANTITY"];
					$arItem["~SELECT"]["SIZE"][$arFields["ID"]]["SELECT_QUANTITY_RESERVED_VALUE"] = $arQuantity["QUANTITY_RESERVED"];
					$arItem["QUANTITY_ALL"] += $arQuantity["QUANTITY"];
					...
					$arItem["~QUANTITY"][$PRICE_CODE]["$index"] = $arQuantity["QUANTITY"];				
					...
					$arItem["~DISCOUNT_PRICE_PERCENT"][$PRICE_CODE]["$index"] = 100*($arPrice[$PRICE_CODE]["VALUE"] - $arPrice[$PRICE_CODE]["DISCOUNT_VALUE"])/$arPrice[$PRICE_CODE]["VALUE"];
					$arItem["~DISCOUNT_PRICE_PERCENT_FORMATED"][$PRICE_CODE]["$index"] = "-".roundEx($arItem["~DISCOUNT_PRICE_PERCENT"][$PRICE_CODE]["$index"], SALE_VALUE_PRECISION)."%";
	...
	foreach($arParams["PRICE_CODE"] as $PRICE_CODE)
	{
		...
		$arQuantity = CCatalogProduct::GetByID($arItem["ID"]);	
		if(!is_array($arQuantity)) $arQuantity = array();
		...
		{		
			if($arItem["PRICES"][$PRICE_CODE]["DISCOUNT_VALUE"] > 0 && count($arItem["~PRICES_DISCOUNT"][$PRICE_CODE]) < 1)
			{
				...
				$arItem["~DISCOUNT_PRICE_PERCENT"][$PRICE_CODE]["$index"] = 100*($arItem["PRICES"][$PRICE_CODE]["VALUE"] - $arItem["PRICES"][$PRICE_CODE]["DISCOUNT_VALUE"])/$arItem["PRICES"][$PRICE_CODE]["VALUE"];
				$arItem["~DISCOUNT_PRICE_PERCENT_FORMATED"][$PRICE_CODE]["$index"] = "-".roundEx($arItem["~DISCOUNT_PRICE_PERCENT"][$PRICE_CODE]["$index"], SALE_VALUE_PRECISION)."%";				
		...
		switch(count($arItem["~PRICES_DISCOUNT"][$PRICE_CODE]))
		{
			case 0:
						...
						$arItem["~MINIMUM_PRICE"][$PRICE_CODE]["DISCOUNT_PRICE_PERCENT_FORMATED"] = $arItem["~DISCOUNT_PRICE_PERCENT_FORMATED"][$PRICE_CODE][0];
						break;
			...
			case 1:
						...
						$arItem["~MINIMUM_PRICE"][$PRICE_CODE]["DISCOUNT_PRICE_PERCENT_FORMATED"] = $arItem["~DISCOUNT_PRICE_PERCENT_FORMATED"][$PRICE_CODE][$arItem["~PRICES_DISCOUNT"][$PRICE_CODE][0]];
						break;
		...
		if($arItem["~SKU_PRICES_COUNT"][$PRICE_CODE] < 1)
		{
			...
			$arItem["~MINIMUM_PRICE"][$PRICE_CODE]["QUANTITY"]  = $arQuantity["QUANTITY"];			
		...
		else 
		{
			...
			$arItem["~MINIMUM_PRICE"][$PRICE_CODE]["QUANTITY"]  = $arItem["~QUANTITY"][$PRICE_CODE][$arItem["~PRICES_DISCOUNT"][$PRICE_CODE][0]];							
	...
	$arResult["~COUNTDOWN"][$arItem["ID"]]["ID"] =$arItem["ID"];
	$arResult["~COUNTDOWN"][$arItem["ID"]]["PROPERTIES"]["COUNTDOWN_SALE_FROM"]["VALUE"] = $arItem["PROPERTIES"]["COUNTDOWN_SALE_FROM"]["VALUE"];
	$arResult["~COUNTDOWN"][$arItem["ID"]]["PROPERTIES"]["COUNTDOWN_SALE_TO"]["VALUE"] = $arItem["PROPERTIES"]["COUNTDOWN_SALE_TO"]["VALUE"];	
}

$cp = $this->__component; 
if(is_object($cp)) 
   $cp->SetResultCacheKeys(array("~COUNTDOWN"));
?>