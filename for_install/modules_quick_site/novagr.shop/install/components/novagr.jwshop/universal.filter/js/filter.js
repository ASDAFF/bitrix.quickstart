	...
	*/
	getElements	: function(iNumPage, changeURL) {
		...
		$('.arFilter.selected[offer="N"]').each(function(i){
			var tmp = {};
			tmp[$(this).attr('key')+""] = $(this).attr('value');
			arFilter[i] = tmp;
			addParams+="&arFilter["+(i++)+"]["+$(this).attr('key')+"]="+$(this).attr('value');
		});
		//var arFilterCtr = 0;
		...
		$('.arFilter.selected[offer="Y"]').each(function(i){
			...
			tmp[$(this).attr('key')+""] = $(this).attr('value');
			arOffer[i] = tmp;
			addParams+="&arOffer["+(i++)+"]["+$(this).attr('key')+"]="+$(this).attr('value');
		});
	...
	init		: function(arScale, arElementsSearch, CURRENCY_FORMAT, CURRENT_SECTION_ID, CURRENT_SECTION_CODE, PATH, CATALOG_IBLOCK_ID, CATALOG_IBLOCK_CODE, TRADEOF_IBLOCK_ID, TRADEOF_IBLOCK_CODE, nPageSize, secid, QUERY, ROOT_PATH, BRAND_ROOT, FASHION_ROOT, FASHION_MODE, SESS_INCLUDE_AREAS) {
		...
		$('.npagesize').live('click', function(){
			...
			if( $('.npagesize.active').attr('value') == undefined)
				objFilter.nPageSize = 16;
			else objFilter.nPageSize = $('.npagesize.active').attr('value');
			objFilter.changeSlider = objFilter.checkPriceSliderState();
...
-					$( "#maxAmount-" + key ).text( val.DATA[ obj.value[1] ] );
		...
		$('.navig li a').live('click', function(){
			...
			objFilter.FILTER_RESET = 0;
			objFilter.changeSlider = objFilter.checkPriceSliderState();
			objFilter.getElements($(this).attr('inumpage'), true);
			return false;
		});
		// обработчик фильтра
