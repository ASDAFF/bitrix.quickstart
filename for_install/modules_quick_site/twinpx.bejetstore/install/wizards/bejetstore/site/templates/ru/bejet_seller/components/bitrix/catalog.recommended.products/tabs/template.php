...
foreach ($arResult['ITEMS'] as $key => $arItem)
{
	...
	?>
	<?if($i % 4 == 0):$bOpen = true;?><div class="row"><?endif;?>
		
	<div class="col-sm-3">
	<div class="<? echo ($arItem['SECOND_PICT'] ? 'bx_catalog_item double' : 'bx_catalog_item'); ?>" style="min-height:503px;"><div class="bx_catalog_item_container" id="<? echo $strMainID; ?>">
...
?>
	...
	<div class="row bj-product-card__cart">
		...
		<?//echo "<pre>";print_r($arItem['MIN_PRICE']);echo "</pre>";?>
		...
		<?if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS']))
		{
			if ($arItem['CAN_BUY'])
			{?>
			<div id="<? echo $arItemIDs['PRICE']; ?>" class="col-xs-6">
		...
		else
		{
			...
			{
			?><div id="<? echo $arItemIDs['PRICE']; ?>" class="bx_catalog_item_controls touch">
...
var <? echo $strObName; ?> = new JCCatalogSection(<? echo CUtil::PhpToJSObject($arJSParams, false, true); ?>);
	...
	<?if($i % 4 == 3):$bOpen = false;?></div><?if($count != $i + 1):?><hr><?endif;?><?endif;?>
	<?$i++;
}
?><?if($bOpen):?></div><?endif;?>
