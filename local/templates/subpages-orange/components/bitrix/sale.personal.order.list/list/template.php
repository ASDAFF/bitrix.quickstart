...
<div class="order-list">
	<div class="inline-filter order-filter">
		...
		<?if($page != "canceled"):?><a href="<?=$arResult["CURRENT_PAGE"]?>?filter_canceled=Y&filter_history=Y"><?else:?><b><?endif;?><?=GetMessage("STPOL_F_CANCELED")?><?if($page != "canceled"):?></a><?else:?></b><?endif;?>
			...
			if ($bNoOrder)
			{
				if($page == "active"): echo ShowNote(GetMessage("STPOL_NO_ORDERS_ACTIVE")); endif;
				if($page == "all"): echo ShowNote(GetMessage("STPOL_NO_ORDERS_ALL")); endif;
				if($page == "completed"): echo ShowNote(GetMessage("STPOL_NO_ORDERS_COMPLETED")); endif;
				if($page == "canceled"): echo ShowNote(GetMessage("STPOL_NO_ORDERS_CANCELED")); endif;
			}?>
</div>
