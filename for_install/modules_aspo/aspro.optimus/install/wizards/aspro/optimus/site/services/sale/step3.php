...
$rsMess = CEventMessage::GetList($by="site_id", $order="desc", array("TYPE_ID"=>"SALE_STATUS_CHANGED_N"));
if($arMess = $rsMess->Fetch()){
	if(strpos($arMess["MESSAGE"], "personal/order/")!==false){
		$em = new CEventMessage;
		$res = $em->Update($arMess["ID"], array("MESSAGE"=>str_replace('personal/order/', 'personal/history-of-orders/order_detail.php?ID=', $arMess["MESSAGE"])));
	}
}
$rsMess = CEventMessage::GetList($by="site_id", $order="desc", array("TYPE_ID"=>"SALE_STATUS_CHANGED_F"));
if($arMess = $rsMess->Fetch()){
	if(strpos($arMess["MESSAGE"], "personal/order/")!==false){
		$em = new CEventMessage;
		$res = $em->Update($arMess["ID"], array("MESSAGE"=>str_replace('personal/order/', 'personal/history-of-orders/order_detail.php?ID=', $arMess["MESSAGE"])));
...
$rsMess = CEventMessage::GetList($by="site_id", $order="desc", array("TYPE_ID"=>"SALE_ORDER_TRACKING_NUMBER"));
if($arMess = $rsMess->Fetch()){
	if(strpos($arMess["MESSAGE"], "personal/order/detail/")!==false){
		$em = new CEventMessage;
		$res = $em->Update($arMess["ID"], array("MESSAGE"=>str_replace('personal/order/detail/', 'personal/history-of-orders/order_detail.php?ID=', $arMess["MESSAGE"])));
	}
}
$rsMess = CEventMessage::GetList($by="site_id", $order="desc", array("TYPE_ID"=>"SALE_SUBSCRIBE_PRODUCT"));
if($arMess = $rsMess->Fetch()){
	if(strpos($arMess["MESSAGE"], "personal/cart/")!==false){
		$em = new CEventMessage;
		$res = $em->Update($arMess["ID"], array("MESSAGE"=>str_replace('personal/cart/', 'basket/', $arMess["MESSAGE"])));
	}
}
$rsMess = CEventMessage::GetList($by="site_id", $order="desc", array("TYPE_ID"=>"SALE_ORDER_PAID"));
if($arMess = $rsMess->Fetch()){
	if(strpos($arMess["MESSAGE"], "personal/order/")!==false){
		$em = new CEventMessage;
		$res = $em->Update($arMess["ID"], array("MESSAGE"=>str_replace('personal/order/', 'personal/history-of-orders/order_detail.php?ID=', $arMess["MESSAGE"])));
...
$rsMess = CEventMessage::GetList($by="site_id", $order="desc", array("TYPE_ID"=>"SALE_ORDER_DELIVERY"));
if($arMess = $rsMess->Fetch()){
	if(strpos($arMess["MESSAGE"], "personal/order/")!==false){
		$em = new CEventMessage;
		$res = $em->Update($arMess["ID"], array("MESSAGE"=>str_replace('personal/order/', 'personal/history-of-orders/order_detail.php?ID=', $arMess["MESSAGE"])));
...
$rsMess = CEventMessage::GetList($by="site_id", $order="desc", array("TYPE_ID"=>"SALE_ORDER_CANCEL"));
if($arMess = $rsMess->Fetch()){
	if(strpos($arMess["MESSAGE"], "personal/order/")!==false){
		$em = new CEventMessage;
		$res = $em->Update($arMess["ID"], array("MESSAGE"=>str_replace('personal/order/', 'personal/history-of-orders/order_detail.php?ID=', $arMess["MESSAGE"])));
...
$rsMess = CEventMessage::GetList($by="site_id", $order="desc", array("TYPE_ID"=>"USER_INFO"));
if($arMess = $rsMess->Fetch()){
	if(strpos($arMess["MESSAGE"], "auth/change-password/")===false){
		$em = new CEventMessage;
		$res = $em->Update($arMess["ID"], array("MESSAGE"=>str_replace('auth/', 'auth/change-password/', $arMess["MESSAGE"])));
	}
}
$rsMess = CEventMessage::GetList($by="site_id", $order="desc", array("TYPE_ID"=>"USER_PASS_REQUEST"));
if($arMess = $rsMess->Fetch()){
	if(strpos($arMess["MESSAGE"], "auth/change-password/")===false){
		$em = new CEventMessage;
		$res = $em->Update($arMess["ID"], array("MESSAGE"=>str_replace('auth/', 'auth/change-password/', $arMess["MESSAGE"])));
...
$rsMess = CEventMessage::GetList($by="site_id", $order="desc", array("TYPE_ID"=>"NEW_USER_CONFIRM"));
if($arMess = $rsMess->Fetch()){
	if(strpos($arMess["MESSAGE"], "auth/confirmation/")===false){
		$em = new CEventMessage;
		$res = $em->Update($arMess["ID"], array("MESSAGE"=>str_replace('auth/', 'auth/confirmation/', $arMess["MESSAGE"])));
	}
}
?>