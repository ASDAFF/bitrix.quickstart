...
<div class="cart-items" id="id-na-list" style="display:none;">
	<div class="catalog-section-list">
		<ul  class="cart_sections">
			...
			<li><a href="javascript:void(0)" class="active"><?=GetMessage("SHOES_PRD_IN_BASKET_NOTA")?> (<?=$countItemsNotAvailable?>)</a></li>
		</ul>
	</div>
		...
		<table class="shopping_cart shopping_cart_row_height">
			<thead>
				<tr>
					...
					<?if (in_array("TYPE", $arParams["COLUMNS_LIST"])):?>
						<th align="center"><?echo GetMessage("SHOES_PRICE_TYPE")?></th>
					<?endif;?>
					<?if (in_array("WEIGHT", $arParams["COLUMNS_LIST"])):?>
						<th align="center"><?echo GetMessage("SHOES_WEIGHT")?></th>
					<?endif;?>
					<?if (in_array("QUANTITY", $arParams["COLUMNS_LIST"])):?>
						<th align="center"><?echo GetMessage("SHOES_QUANTITY")?></th>
					<?endif;?>
					<?if (in_array("PRICE", $arParams["COLUMNS_LIST"])):?>
						<th align="center"><?echo GetMessage("SHOES_PRICE")?></th>
					<?endif;?>
			...
			foreach($arResult["ITEMS"]["nAnCanBuy"] as $arBasketItems)
			{
				...
				<tr>
					...
					<?endif;?>
					<?if (in_array("WEIGHT", $arParams["COLUMNS_LIST"])):?>
						<td align="right"><p class="cart_tovar_qty"><?=$arBasketItems["WEIGHT_FORMATED"] ?></p></td>
					<?endif;?>
					<?if (in_array("QUANTITY", $arParams["COLUMNS_LIST"])):?>
						<td align="center"><p class="cart_tovar_qty"><?=$arBasketItems["QUANTITY"]?></p></td>
					<?endif;?>
					<?if (in_array("PRICE", $arParams["COLUMNS_LIST"])):?>
