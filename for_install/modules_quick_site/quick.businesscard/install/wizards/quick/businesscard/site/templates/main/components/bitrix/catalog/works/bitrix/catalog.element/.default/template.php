	...
	<div class="row">
		...
		<div class="col-md-<?if(!empty($arResult["PROPERTIES"]["SHOW_CALLBACK_FORM"]["VALUE"])):?>7<?else:?>12<?endif?>">
			<ul class="nav nav-pills white" role="tablist">
				<li <?if(!empty($arResult["PROPERTIES"]["PHOTO_TOP"]["ITEMS"]) || (empty($arResult["PROPERTIES"]["PHOTO_TOP"]["ITEMS"]) && empty($arResult["PROPERTIES"]["HREF_VIDEO"]["VALUE"]))):?>class="active"<?endif?>><a href="#images" role="tab" data-toggle="tab" title="images"><i class="fa fa-camera pr-5"></i> <?=GetMessage("QUICK_BUSINESSCARD_TAB_PHOTO")?></a></li>
				<li <?if(!empty($arResult["PROPERTIES"]["HREF_VIDEO"]["VALUE"]) && empty($arResult["PROPERTIES"]["PHOTO_TOP"]["ITEMS"])):?>class="active"<?endif?>><a href="#video" role="tab" data-toggle="tab" title="video"><i class="fa fa-video-camera pr-5"></i> <?=GetMessage("QUICK_BUSINESSCARD_TAB_VIDEO")?></a></li>
			</ul>
		...
		<div class="col-md-5">
			<p class="info mt-45"><?=GetMessage("QUICK_BUSINESSCARD_WORKS_INFO")?></p>
			<div id="results-callback">
				<div class="alert alert-danger" id="beforesend-callback">
					<?=GetMessage("QUICK_BUSINESSCARD_WORKS_BEFORESEND")?>
				</div>
				<div class="alert alert-danger" id="error-callback">
					<?=GetMessage("QUICK_BUSINESSCARD_WORKS_ERROR")?>
				</div> 
				<div class="alert alert-success" id="success-callback">
					<?=GetMessage("QUICK_BUSINESSCARD_WORKS_SUCCESS")?>
				</div>
			</div>
			...
			<form name="CALLBACK" action="<?=SITE_DIR?>include/" method="POST" role="form">
				...
				<div class="form-group has-feedback">
					<input type="text" name="CALLBACK[NAME]" placeholder="<?=GetMessage("QUICK_BUSINESSCARD_WORKS_NAME")?>" class="form-control req">
					<i class="fa fa-user form-control-feedback"></i>
				</div>
				<div class="form-group has-feedback">
					<input type="tel" name="CALLBACK[PHONE]" placeholder="<?=GetMessage("QUICK_BUSINESSCARD_WORKS_PHONE")?>" pattern="(([ ]*[\+]?[ ]*\d{1,5})[ ]*[\-]?[ ]*)?(\(?\d{1,5}\)?[ ]*[\-]?[ ]*)?[\d\- ]{5,13}" class="form-control req">
	...
	<div class="side product-item">
		<div class="tabs-style-2">
			<ul class="nav nav-tabs" role="tablist">
				<li><a href="#descriprion" role="tab" data-toggle="tab"><i class="fa fa-file-text-o"></i><?=GetMessage("QUICK_BUSINESSCARD_WORKS_TAB_DESCRIPTION")?></a></li>
				<li class="active"><a href="#specifications" role="tab" data-toggle="tab"><i class="fa fa-files-o"></i><?=GetMessage("QUICK_BUSINESSCARD_WORKS_TAB_SPECIFICATIONS")?></a></li>
				<li><a href="#documents" role="tab" data-toggle="tab"><i class="fa fa-file-word-o"></i><?=GetMessage("QUICK_BUSINESSCARD_WORKS_TAB_DOCUMENTS")?></a></li>
				<li><a href="#comments" role="tab" data-toggle="tab"><i class="fa fa-star"></i>(<?if(!empty($arResult["PROPERTIES"]["REVIEWS"]["ITEMS"])):?><?=count($arResult["PROPERTIES"]["REVIEWS"]["ITEMS"])?><?else:?>0<?endif?>) <?=GetMessage("QUICK_BUSINESSCARD_WORKS_TAB_REVIEWS")?></a></li>
			</ul>
...
<?foreach($arResult["PROPERTIES"]["MORE_PRODUCTS"]["ITEMS"] as $cell=>$arItem):?>
	<div class="listing-item">
		...
		<div class="col-lg-9 col-md-8 col-sm-8">
			<div class="overlay-container">
				<div class="tags">
					<?if(!empty($arItem["PROPERTIES"]["ACTION"]["VALUE"])):?><span class="badge default-bg"><?=GetMessage("QUICK_BUSINESSCARD_WORKS_ACTION")?> <?if(!empty($arItem["PROPERTIES"]["PERCENT"]["VALUE"])):?><?=$arItem["PROPERTIES"]["PERCENT"]["VALUE"]?><?endif?></span><?endif?>
					<?if(!empty($arItem["PROPERTIES"]["NEW"]["VALUE"])):?><span class="badge danger-bg"><?=GetMessage("QUICK_BUSINESSCARD_WORKS_NEW")?></span><?endif?>
					<?if(!empty($arItem["PROPERTIES"]["PRESENCE"]["VALUE"])):?><span class="badge success-bg"><?=GetMessage("QUICK_BUSINESSCARD_WORKS_PRESENCE")?></span>
					<?elseif(!empty($arItem["PROPERTIES"]["EXPECTED"]["VALUE"])):?><span class="badge warning-bg"><?=GetMessage("QUICK_BUSINESSCARD_WORKS_EXPECTED")?></span>
					<?elseif(!empty($arItem["PROPERTIES"]["UNDER"]["VALUE"])):?><span class="badge info-bg"><?=GetMessage("QUICK_BUSINESSCARD_WORKS_UNDER")?></span>
					<?elseif(!empty($arItem["PROPERTIES"]["UNAVAILABLE"]["VALUE"])):?><span class="badge gray-bg"><?=GetMessage("QUICK_BUSINESSCARD_WORKS_UNAVAILABLE")?></span><?endif?>
				</div>
				...
				<div class="clearfix">
					...
					<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="btn btn-default btn-sm hidden-xs pull-right"><?=GetMessage("QUICK_BUSINESSCARD_WORKS_DETAIL")?></a>
					<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="btn btn-white btn-sm hidden-xs pull-right"><i class="fa fa-shopping-cart pr-10"></i> <?=GetMessage("QUICK_BUSINESSCARD_WORKS_BUY")?></a>
