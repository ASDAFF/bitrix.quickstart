...
$colSize = 'col-md-' . round(12 / $arParams['COL_NUM']);
?>
<div class="main-map main-map-default row">
	<div class="<?=$colSize?>">
		<ul class="level-0">
			<?
			$previousLevel = -1;
			...
			foreach ($arResult['arMap'] as $index => $arItem) {
				if ($arItem['LEVEL'] < $previousLevel) {
					print str_repeat('</ul></li>', ($previousLevel - $arItem['LEVEL']));
				}
			...
			if ($previousLevel > 1) {
				print str_repeat('</ul></li>', ($previousLevel-1) );
			}
			?>
		</ul>
	</div>
