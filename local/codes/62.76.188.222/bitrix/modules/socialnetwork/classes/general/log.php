...
class CAllSocNetLog
{
	...
	{
		if (CSocNetLog::SendEvent($ID, $mailTemplate, $tmp_id))
			return "";
		else
	...
	function SendEvent($ID, $mailTemplate = "SONET_NEW_EVENT", $tmp_id = false)
	{
		...
		{
			CAgent::AddAgent("CSocNetLog::SendEventAgent(".$ID.", '".$mailTemplate."', ".($tmp_id ? $tmp_id : 'false').");", "socialnetwork", "N", 0, $arLog["LOG_DATE"], "Y", $arLog["LOG_DATE"]);
			return false;
		}
		...
		while ($arSubscriber = $dbSubscribers->Fetch())
		{
			...
			$bHasAccess = CSocNetLogRights::CheckForUser($arLog["ID"], $arSubscriber["USER_ID"], $arLogSites);
			if (!$bHasAccess)
				continue;
		...
		CSocNetLogCounter::Increment($arLog["ID"], $arLog["ENTITY_TYPE"], $arLog["ENTITY_ID"], $arLog["EVENT_ID"], $arLog["USER_ID"], $arOfEntities, false, false, "Y", "L");
