...
if(!empty($arUser))
{
	...
	// Get product name
	$elementIterator = CIblockElement::getList(array(), array("ID" => $arCatalogProductId), false, false, array("NAME", "DETAIL_PAGE_URL"));
	while($element = $elementIterator->GetNextElement())
	{
		$element = $element->getFields();
		foreach($arViewsData as &$arViews)
		{
			if($arViews['PRODUCT_ID'] == $element['ID'])
			{
				$arViews['DETAIL_PAGE_URL'] = $element['DETAIL_PAGE_URL'];
				break;
			}
		}
		unset($arViews);
