...
}else
{
	...
	else
	{
		...
		<script type="text/javascript">
			...
			{
				$('#PROFILE_ID_'+profileId).attr('checked','checked');
				$('.wrap_person_type label').removeClass('label-active');
				$('#PROFILE_ID_'+profileId).parent().addClass('label-active');
		...
		{
		?>
		<form style="<?=(!$arResult['TRUE_PT'])?'display:none':''?>" action="<?=$APPLICATION->GetCurPage();?>" method="POST" name="ORDER_FORM" id="ORDER_FORM" enctype="multipart/form-data">
		<?=bitrix_sessid_post()?>
...
if(!$arResult['TRUE_PT'])
{
	?>
	<script>
		$('[name="PERSON_TYPE"]').val('<?=reset($arParams['BUYER_PERSONAL_TYPE'])?>');
		submitForm();
	</script>
	<?php
}
