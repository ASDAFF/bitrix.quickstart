...
if ($arParams["ROOT_PATH"] == $arResult['COMPONENT_CURRENT_PAGE'] && empty($_REQUEST["q"])) {
...
if ($this->StartResultCache(false, $cacheParams, SITE_ID . "/novagr.shop/catalog.list/" . md5($USER->GetGroups()))) {
    ...
    if ($arResult['CATALOG_ROOT'] == 1) {
        ...
        if (method_exists($generalCatalogOffers, 'getLastResult')) {
            ...
            } elseif (isset($getLastResult['NAV_ARRAY'])) {
                $rs = new CDBResult;
                $rs->InitFromArray($getLastResult['NAV_ARRAY']);
                $arResult['NavPageNomer'] = $rs->NavPageNomer = (int)$_REQUEST['iNumPage'];
                $arResult['NavPageCount'] = $rs->NavPageCount = ceil(count($getLastResult['NAV_ARRAY'])/16);
                $arResult['NavRecordCount'] = $rs->NavRecordCount = count($getLastResult['NAV_ARRAY']);
                $arResult['NAV_STRING'] = $rs->GetPageNavStringEx($navComponentObject, "", "catalog");
