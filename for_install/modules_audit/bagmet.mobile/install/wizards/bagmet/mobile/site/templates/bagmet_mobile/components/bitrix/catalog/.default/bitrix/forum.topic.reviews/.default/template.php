...
// *************************/Input params***************************************************************
?>
<div class="tovar_review_wrapper">
	<div class="tovar_descr_title"><h2><?=GetMessage("REVIEW_TITLE")?></h2></div>
	<?
	if (!empty($arResult["MESSAGES"])):
		...
		$iCount = 0;
		?>
				<ul class="tovar_review">
			<?
			foreach ($arResult["MESSAGES"] as $res)
			{
				?>
					<li class="tovar_review_li">
						<ul class="tovar_review_title">
							<li class="tovar_review_title_name"><?=$res["AUTHOR_NAME"]?></li>
							<li class="tovar_review_title_date"><?=$res["POST_DATE"]?></li>
						</ul>
						<div id="message_text_<?=$res["ID"]?>"><?=$res["POST_MESSAGE_TEXT"]?></div>
					</li>
			<?
			}
			?>
	...
	?>
			<div class="reviews-reply-buttons">
				<input class="login_button" name="send_button" type="submit" value="<?=GetMessage("OPINIONS_SEND")?>" tabindex="<?=$tabIndex++;?>" <?
