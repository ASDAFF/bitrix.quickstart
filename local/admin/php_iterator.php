...
$MESS['php_cmd_title'] = "PHP-Итератор";
...
?>
<script>
function __FPHPSubmit(noConfirm)
{
	if(noConfirm || confirm('<?echo GetMessage("php_cmd_confirm")?>'))
	{
		window.scrollTo(0, 500);
		ShowWaitWindow();
		BX.ajax.post(
			'php_iterator.php?lang=' + phpVars.LANGUAGE_ID + '&sessid=' + phpVars.bitrix_sessid,
			{
			...
			function(result){
				...
				CloseWaitWindow();
				__FPHPParseResult(result);
			}
		);
	}
}

function __FPHPParseResult(result){
	if(result.match(/#ITERATOR_REPEAT#/g)){
		__FPHPSubmit(true);
	}
}
</script>
