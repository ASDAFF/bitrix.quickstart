...
class Prototype extends Entity\DataManager
{
	 ...
	 */
	public function getData($arFilter = array(), $arSelect = array('*'), $arOrder = array(), $offset = null, $limit = null, $runtime = array(), $bCountInfo = false)
	{
		...
		$arResult = array('NAV_OBJECT' => $rsData);
		while ($arRes = $rsData->Fetch()) {
			$arResult['ITEMS'][$arRes['ID']] = $arRes;
		}

		// Для формирования постранички нужно делать допзапрос
		if( $bCountInfo ){
			$countQuery = new Query($entity_data_class::getEntity());
			$countQuery->addSelect(new ExpressionField('CNT', 'COUNT(1)'));
			$countQuery->setFilter($arFilter);
			$totalCount = $countQuery->setLimit(null)->setOffset(null)->exec()->fetch();
			$arResult['COUNT_INFO']['TOTAL_COUNT'] = $totalCount['CNT'];
			$arResult['COUNT_INFO']['TOTAL_PAGES'] = ceil($totalCount['CNT'] / $limit);
		}
	...
	public function getElements($params = array())
	{
		...
		if ($cache->start()) {
			...
			$arResult['NAV_OBJECT'] = new \CDBResult($rsData, $this->id);;
