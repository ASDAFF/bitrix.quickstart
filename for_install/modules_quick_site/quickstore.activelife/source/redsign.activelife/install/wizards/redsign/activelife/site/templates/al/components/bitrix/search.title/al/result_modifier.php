...
if(CModule::IncludeModule('iblock') && CModule::IncludeModule('catalog')){
    ...
    if(!empty($arResult['CATEGORIES'])){
        foreach($arResult['CATEGORIES'] as $category_id => $arCategory){
            foreach($arCategory['ITEMS'] as $i => $arItem){
                if($arItem['MODULE_ID']=='iblock' && $arItem['PARAM2']==$arParams['IBLOCK_ID']){
                    $arCatalog['ITEMS'][] = $arItem['ITEM_ID'];
                }
                elseif($arItem['MODULE_ID']=='iblock'){
                    ...
                    if($arParams['ACTION_TO']=='catalog'){
                        $arItem['URL'] = str_replace($arResult['FORM_ACTION'],SITE_DIR.'catalog/',$arItem['URL']);
                    }
