...
class User
{
		...
		if($bOk) {
			...
			if($arParams['LOGIN'] <> '' || $arParams['EMAIL'] <> '') {
				...
				while($arUser = $res->Fetch()) {
					...
					if($arUser['ACTIVE'] == 'Y') {
						static::SendUserInfo($arUser['ID'], $arParams['SITE_ID'], Loc::getMessage('INFO_REQ'), true, 'API_AUTH_RESTORE');
						$f = true;
					}
					elseif($confirmation) {
	...
	public static function change($params)
	{
		...
		if($bOk) {
			...
			static::SendUserInfo($ID, $arParams['SITE_ID'], Loc::getMessage('CHANGE_PASS_SUCC'), true, 'API_AUTH_CHANGE');
		}
