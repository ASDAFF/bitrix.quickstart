...
<div class="b-news-detail">
	...
	if (is_array($arResult["PROPERTIES"]) && array_key_exists("contact", $arResult["PROPERTIES"]))
	{
		...
		{
			$contactPerson = CIBlockElement::GetList(array(), array("IBLOCK_ID" => \Citrus\Realty\Helper::getIblock("staff"), "XML_ID" => $contact["VALUE"]), false, array('nTopCount' => 1), array("ID", "IBLOCK_ID"))->GetNext();
			if ($contactPerson)
				$contactPersons[$contactPerson["IBLOCK_ID"]][] = $contactPerson["ID"];
		}
		else
		{
			$contactPerson = CIBlockElement::GetList(array(), array("IBLOCK_ID" => \Citrus\Realty\Helper::getIblock("staff"), "ID" => $contact["VALUE"]), false, array('nTopCount' => 1), array("ID", "IBLOCK_ID"))->GetNext();
			if ($contactPerson)
				$contactPersons[$contactPerson["IBLOCK_ID"]][] = $contactPerson["ID"];
		}
		if (!empty($contactPersons))
		{
			?><h2><?=GetMessage("CITRUS_TEMPLATE_CONTACT_PERSON")?></h2><?
