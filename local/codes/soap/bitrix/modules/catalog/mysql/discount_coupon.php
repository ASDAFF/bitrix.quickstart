...
class CCatalogDiscountCoupon extends CAllCatalogDiscountCoupon
{
	public function Add($arFields, $bAffectDataFile = true)
	{
		...
		$rsEvents = GetModuleEvents("catalog", "OnBeforeCouponAdd");
		...
		$rsEvents = GetModuleEvents("catalog", "OnCouponAdd");
		while ($arEvent = $rsEvents->Fetch())
		{
	...
	public function Update($ID, $arFields)
	{
		...
		$rsEvents = GetModuleEvents("catalog", "OnBeforeCouponUpdate");
		while ($arEvent = $rsEvents->Fetch())
		{
		...
		$rsEvents = GetModuleEvents("catalog", "OnCouponUpdate");
		while ($arEvent = $rsEvents->Fetch())
		{
			ExecuteModuleEventEx($arEvent, array($ID, $arFields));
		}
	...
	public function Delete($ID, $bAffectDataFile = true)
	{
		...
		$rsEvents = GetModuleEvents("catalog", "OnBeforeCouponDelete");
		while ($arEvent = $rsEvents->Fetch())
		{
		...
		$rsEvents = GetModuleEvents("catalog", "OnCouponDelete");
		while ($arEvent = $rsEvents->Fetch())
		{
	...
	public function __CouponOneOrderDisable($arCoupons)
	{
		...
		if (!empty($arCoupons))
		{
			...
			if (isset($intCouponID))
				unset($intCouponID);
			$strSql = "UPDATE b_catalog_discount_coupon SET ACTIVE='N' WHERE ID IN (".implode(', ', $arWhere).") AND ONE_TIME='O'";
			$DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
		}
	}
}
