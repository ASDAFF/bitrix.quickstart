var jsCIEESettings = {
	...
	{
		BX.loadCSS('/bitrix/components/citrus/iblock.element.form/templates/.default/style.css');
		BX.loadScript('/bitrix/components/citrus/iblock.element.form/settings/jquery.tablednd_0_5.js');
	
		this.arData = arFieldData;
		this.arDefaultFields = arDefaultFields;
		this.obForm = document.forms['bx_popup_form_ciee_element_form'];
		...
		for (var field in this.arData)
		{
			...
			$table.append(
				'<tr>' +
					...
					'<td><input type="text" size="20" value="' + BX.util.htmlspecialchars(arField.TITLE ? arField.TITLE : 'undefined') + '" class="title" /></td>' +
					...
					'<td align="center"><a href="javascript:void(0);" class="ciee-link-delete" onclick="jsCIEESettings.deleteField(this); return false;">' + BX.message('ciee_delete') + '</a></td>' +
				'</tr>'
			);
	...
	__updateAddLink: function()
	{
		...
		{
			this.$info.html(
				'<dl class="ciee-dropdown">' +
					'<dt><a href="javascript:void(0);" class="ciee-action" onclick="jsCIEESettings.showAddFields(); return false;">' + BX.message('ciee_add_field') + '</a></dt>' +
					'<dd><ul style="display: none;"></ul></dd>' + 
				'</dl>' 
			);
			this.$info.find(".ciee-dropdown dt a").click(function() {
				$(".ciee-dropdown dd ul").toggle();
			});
			
			// fill list
			$ul = $(".ciee-dropdown dd ul");
			for (field in _availableFields)
			{
				$ul.append('<li class="' + field + '"><a href="javascript:void(0);" onclick="jsCIEESettings.addField(\'' + field + '\')" class="ciee-action">' + this.arDefaultFields[field].ORIGINAL_TITLE + '</a></li>');
			}
		}			
		else
			this.$info.html('<p>' + BX.message('ciee_no_fields_to_add') + '</p>');
	},
	...
	__saveChanges: function()
	{
		...
		window.jsCIEESettingsOpener.saveData(this.__serialize(this.arData));
		return false;
	}
}
