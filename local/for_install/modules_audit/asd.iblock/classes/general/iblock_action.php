...
class CASDiblockAction {
	...
	public static function OnBeforePrologHandler() {
		...
		if ($bRightPage && check_bitrix_sessid() && !empty($_REQUEST['ID']) &&
			...
			isset($_REQUEST['asd_ib_dest']) && intval($_REQUEST['asd_ib_dest']) > 0 &&
				...
				foreach ($_REQUEST['ID'] as $eID) {
					...
					if ($boolCreateSection && $boolCopySect) {
						...
						if ($arSrcSect = $rsSections->Fetch())
						{
							...
							$arDestSect['PICTURE'] = intval($arDestSect['PICTURE']);
							...
							$arDestSect['DETAIL_PICTURE'] = intval($arDestSect['DETAIL_PICTURE']);
									...
									if ($arOneUserField['USER_TYPE']['BASE_TYPE'] == 'file') {
										if (!empty($arDestSect[$arOneUserField['FIELD_NAME']])) {
											if (is_array($arDestSect[$arOneUserField['FIELD_NAME']])) {
												...
												$arNewFile = false;
												$intFileID = intval($arDestSect[$arOneUserField['FIELD_NAME']]);
												if ($intFileID > 0) {
													$arNewFile = CFile::MakeFileArray($intFileID);
												}
		...
		if (isset($_REQUEST['action']) && $_REQUEST['action']=='asd_remove' && 0 < $IBLOCK_ID && isset($_REQUEST['find_section_section']) &&
			...
			if ($intSectionID > 0) {
				...
				if ($bElemPage || $bMixPage) {
					foreach ($_REQUEST['ID'] as $eID) {
						...
						if (CASDIblockRights::IsElementEdit($BID, $ID)) {
							$arSectionList = array();
							$rsSections = CIBlockElement::GetElementGroups($ID, true);
							while ($arSection = $rsSections->Fetch()) {
