...
function skuPropClickHandler(e)
{
	...
	if (!!target && target.hasAttribute('data-value-id'))
	{
		...
		BX.ajax({
			url: '/bitrix/components/bejetstore/sale.basket.basket/ajax.php',
			method: 'POST',
			data: postData,
			dataType: 'json',
	...
	if (basketItemId !== null)
	{
		...
		for (i = 0; i < arColumns.length; i++)
		{
			...
			else if (arColumns[i] == 'QUANTITY')
			{
				...
				{
					oCellQuantityHTML += '<td id="" class="custom">\
						<div class="">';
						oCellQuantityHTML += '<a href="javascript:void(0);" class="bj-quantity-minus minus" onclick="setQuantity(' + arItem['ID'] + ', ' + ratio + ', \'down\', ' + bUseFloatQuantity + ');"></a>';
						...
						oCellQuantityHTML += '<a href="javascript:void(0);" class="bj-quantity-plus plus" onclick="setQuantity(' + arItem['ID'] + ', ' + ratio + ', \'up\', ' + bUseFloatQuantity + ');"></a></div></td>';
				}
...
function getMobileQuantityControl(id, name, curVal, maxQuantity, ratio, measureText)
{
	...
	res = '<div class="some-class" style="display:none">';
