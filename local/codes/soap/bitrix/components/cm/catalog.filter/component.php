 ...
 *
 * Инфоблок торговых предложений прописан жестко SUDAR_TP 
 */ 
...
if($this->StartResultCache(false, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups())))
{
  ...
  // <-- Получение минимальной и максимальной цен ---
    if(!$arParams["SECTION_ID"]) {
      $arSection = CIBlockSection::GetList(array(), array("IBLOCK_ID" => $arParams["IBLOCK_ID"], "CODE" => $arParams["SECTION_CODE"]), false, array("ID"))->Fetch();
      if($arSection["ID"]) $arParams["SECTION_ID"] = $arSection["ID"];
    }
    $rsPriceMax = CIBlockElement::GetList(
        Array("CATALOG_PRICE_{$price_id}"=>"DESC"), 
        array("IBLOCK_ID" => $arParams["IBLOCK_ID"], "SECTION_ID" => $arParams["SECTION_ID"], "INCLUDE_SUBSECTIONS" => "Y", ">CATALOG_QUANTITY" => 0),
        false,
        Array("nTopCount"=>1),
        Array("ID","CATALOG_PRICE_{$price_id}")
    );
    if($arPriceMax = $rsPriceMax->GetNext())
    {
        $pMax = $arPriceMax["CATALOG_PRICE_{$price_id}"];
    } 
    else 
        $pMax = 20000;
      
    $arResult["MAX_PRICE"] = ceil($pMax);
  
    $rsPriceMin = CIBlockElement::GetList(
        Array("CATALOG_PRICE_{$price_id}"=>"ASC"), 
        array("IBLOCK_ID" => $arParams["IBLOCK_ID"], "SECTION_ID" => $arParams["SECTION_ID"], "INCLUDE_SUBSECTIONS" => "Y", ">CATALOG_PRICE_{$price_id}" => 0, ">CATALOG_QUANTITY" => 0),
        false,
        Array("nTopCount"=>1),
        Array("ID","CATALOG_PRICE_{$price_id}")
    );
    if($arPriceMin = $rsPriceMin->GetNext())
    {
      $pMin = $arPriceMin["CATALOG_PRICE_{$price_id}"];
    } 
    else 
        $pMin = 0;
      
    $arResult["MIN_PRICE"] = floor($pMin);
  // --- Получение минимальной и максимальной цен -->
  ...
  $arOffersIBlock = array('OFFERS_IBLOCK_ID' => SUDAR_TP, 'OFFERS_PROPERTY_ID' => '17');
//   p($arOffersIBlock, "export");
...
foreach($arResult["arrProp"] as $prop_id => $arProp)
{
    ...
    switch ($arProp["PROPERTY_TYPE"])
    {
        case "L":
            ...
            if(!is_array($val))
            {
                $res .= '<input type="text" name="'.$name.'" size="'.$arParams["TEXT_WIDTH"].'" value="'.htmlspecialchars($val).'" />';

                if (strlen($val) > 0)
                    ${$FILTER_NAME}["PROPERTY"]["?".$arProp["CODE"]][] = $val;
            }
            }
            break;
        case "N":
...
foreach($arResult["arrOfferProp"] as $prop_id => $arProp)
{
    ...
    if($res)
    {
        ...
        $arResult["ITEMS"][] = array(
            ...
            "INPUT" => $res,
            "INPUT_NAME" => $name,
            "INPUT_VALUE" => is_array($value) ? $value : htmlspecialchars($value),
            "~INPUT_VALUE" => $value,
         );
    }
}
