...
<form id="f_feedback" name="f_feedback_<?=$ALX?>" action="<?=$actionPage?>" method="post" enctype="multipart/form-data">
        ...
        <?if($arParams["USE_CAPTCHA"]):?>
                <div class="alx_feed_back_form_item_pole<?if(in_array('ALX_CP_WRONG_CAPTCHA', $errorField, true)):?> alx_feed_back_form_error_pole<?endif?>">
                        <div class="alx_feed_back_form_name"><?=GetMessage("ALX_TP_MESSAGE_INPUTF")?> <?=GetMessage("ALX_TP_MESSAGE_INPUTS")?> <span class="required_text">*</span></div>
...
<script type="text/javascript">
        $(document).ready(function()
        {
                file_w_<?=$ALX?> = parseInt($("#alx_feed_back_theme2_<?=$ALX?> .alx_feed_back_form_feedback_poles").width()/5);
                <?for($i=1;$i<=$k;$i++):?>
                $("#alx_feed_back_theme2_<?=$ALX?> #alx_feed_back_form_file_input_add<?=$i?>").attr('size', file_w_<?=$ALX?>);
                <?endfor?>
                ...
                <?for($i=1;$i<=$k;$i++):?>
                $("#alx_feed_back_theme2_<?=$ALX?> #alx_feed_back_form_file_input_add<?=$i?>").change(function() {
                        var newpath_<?=$ALX?> = str_replace_<?=$ALX?>("C:\\fakepath\\", "", $("#alx_feed_back_theme2_<?=$ALX?> #alx_feed_back_form_file_input_add<?=$i?>").val());
                        $("#alx_feed_back_theme2_<?=$ALX?> #alx_feed_back_form_filename<?=$i?>").text(newpath_<?=$ALX?>);
                });
                <?endfor?>
        });
</script>
