<?
    ...
    $aTabs = array(
        array(
		 ...
		 array(
		 	"DIV" => "edit2",
			"TAB" => GetMessage("NOVAGROUP_MEASURE_BLOCK_TAB"),
			"ICON"=>"main_user_edit",
			"TITLE"=>GetMessage("NOVAGROUP_SEARCH_CLEAR_TITLE")
		),
    );
    ...
    4 - увеличение при наведении + увеличении при всплытии
	*/
...
<form method="POST" action="<?echo $APPLICATION->GetCurPage()?>?lang=<?echo htmlspecialcharsbx(LANG)?>" name="fs1">
    <?
            ...
            $selectArr = array(
                ...
                '4' => GetMessage("NOVAGROUP_DETAIL_VALUE_4"),
            );
    ?>
    <tr>
...
?>
	<tr>
        <td width="60%"><?=GetMessage("NOVAGROUP_MEASURE_BLOCK_CHECKBOX")?></td>
        <td width="40%"><input type="checkbox" name="measure_block" value="1" <? if((int)$measure_block == 1) echo' checked';?> /></td>
    </tr> 
