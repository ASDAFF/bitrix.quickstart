	...
	if(!!result && !!result.ITEMS)
	{
			...
			else if(arItem.VALUES)
			{
				for(var i in arItem.VALUES)
				{
					...
					if(control)
					{
                        ...
                        control.disabled = ar.DISABLED ? 'disabled': '';
					}
				}
			}
		}
		...
		if(modef && modef_num)
		{
			...
			else
			{
				...
				var curProp = BX.findParent(window.curFilterinput, {'tag':'dd'});
                if (!curProp)
                    curProp = BX.findParent(window.curFilterinput, {'class':'bx_filter_block'}).parentNode;
