...
if ($REQUEST_METHOD == "POST" && ($save != "" || $apply != "") && $POST_RIGHT == "W" && check_bitrix_sessid()) {
    ...
    //die("SSS");
    if ($ID > 0) {
        $result = SotaParserSectionTable::Update($ID, $arFields);
        if (!$result->isSuccess()) {
        ...
        $result = SotaParserSectionTable::add($arFields);
...
if (isset($_REQUEST["ID"]) || $copy) {
    $ID = (int)$_REQUEST["ID"];
    if ($copy)
        $arDataTable = SotaParserSectionTable::GetByID($copy)->Fetch();
    else
        $arDataTable = SotaParserSectionTable::GetByID($ID)->Fetch();
...
$aTabs = array(
    array(
        "DIV" => "edit1",
        "TAB" => GetMessage("sota_parser_category_name"),
        "ICON" => "sota_parser_category_icon",
        "TITLE" => GetMessage("sota_parser_category_name")
    ),
);
...
?>
<form method="POST" id="sota-parser" Action="<? echo $APPLICATION->GetCurPage() ?>" ENCTYPE="multipart/form-data"
      name="post_form">
<?
$tabControl->Begin();
...
?>
    <tr>
        ...
        <td width="60%"><input type="checkbox" name="ACTIVE"
                               value="Y"<? if ($arDataTable["ACTIVE"] == "Y" || !$ID) echo " checked" ?>>
        </td>
    </tr>
    <tr>
    ...
    <tr>
        <td colspan="2" align="center">
            <textarea cols="60" rows="15" name="DESCRIPTION"
                      style="width:100%"><? echo $arDataTable["DESCRIPTION"] ?></textarea>
            <?= BeginNote(); ?>
...
$APPLICATION->SetTitle(($ID > 0 ? GetMessage("sota_parser_section_title_edit") : GetMessage("sota_parser_section_title_add")));
