		...
		<form>
			<div class="form-group">
				<select class="form-control">
					<option value=""><?=GetMessage("QUICK_BUSINESSCARD_WORKS_SORT")?></option>
					<option <?if(array_key_exists("date", $_REQUEST) && array_key_exists("desc", $_REQUEST)):?>selected<?endif?> value="?date&desc"><?=GetMessage("QUICK_BUSINESSCARD_WORKS_DATE_DESC")?></option>
					<option <?if(array_key_exists("date", $_REQUEST) && array_key_exists("asc",  $_REQUEST)):?>selected<?endif?> value="?date&asc"><?=GetMessage("QUICK_BUSINESSCARD_WORKS_DATE_ASC")?></option>
					<option <?if(array_key_exists("name", $_REQUEST) && array_key_exists("asc",  $_REQUEST)):?>selected<?endif?> value="?name&asc"><?=GetMessage("QUICK_BUSINESSCARD_WORKS_NAME_ASC")?></option>
					<option <?if(array_key_exists("name", $_REQUEST) && array_key_exists("desc", $_REQUEST)):?>selected<?endif?> value="?name&desc"><?=GetMessage("QUICK_BUSINESSCARD_WORKS_NAME_DESC")?></option>
				</select>
			</div>
		</form>
	</div>
