...
class CCustomCatalogViewedProductsComponent extends CBitrixComponent
{
	...
	protected $productFilter = ['IBLOCK_ID' => 2, 'CATALOG_AVAILABLE' => "Y"];
	...
	protected function getItems()
	{
		...
		foreach (array_keys($this->arParams['SHOW_PRODUCTS']) as $iblock)
		{
			...
			while ($element = $elementIterator->GetNext())
			{
				...
				$element['PREVIEW_TEXT'] = htmlspecialchars($element['PREVIEW_TEXT']);
                if (strlen($element['PREVIEW_TEXT']) > 100) {
                    $element['PREVIEW_TEXT'] = mb_substr($element['PREVIEW_TEXT'], 0, 100, 'utf-8') . "...";
                }
