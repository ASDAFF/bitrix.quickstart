...
<?endif?>
	<div class="col-md-<?=$span?>" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
		<div class="<?=$arParams["ICONS_VIEW"]?> object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="<?=($k*200)?>">
			<?if(!empty($arItem["PREVIEW_PICTURE"])):?>
			<a href="<?=$arItem["PROPERTIES"]["HREF"]["VALUE"]?>"><div class="icon-container default-bg image-block circle">
				<img src="<?=$arItem["PREVIEW_PICTURE"]["SRC"]?>" alt="<?=$arItem["NAME"]?>">
			</div></a>
			<?elseif(!empty($arItem["PROPERTIES"]["ICON"]["VALUE"])):?>
			<a href="<?=$arItem["PROPERTIES"]["HREF"]["VALUE"]?>"><div class="icon-container default-bg circle">
