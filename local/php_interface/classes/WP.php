	...
	class WP{
		...
		*/
		...
		private static function prepareGetListData($d){
			...
			/*
			elseif(isset($d['products'])){
				\CModule::IncludeModule('catalog');
				$info = \CCatalogSKU::GetInfoByOfferIBlock($d['products']['iblock']);
				if(!empty($info)){
					$d['f']['iblock'] = $info['PRODUCT_IBLOCK_ID'];
					$d['f']['ID'] = \CIBlockElement::SubQuery(
		...
		static function bit($data){
			...
			if(isset($settings['module'])){
				\CModule::IncludeModule($settings['module']);
			}
