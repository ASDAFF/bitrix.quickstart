...
if ($arParams["USE_TEMP_DIR"] === "Y" && strlen($_SESSION["BX_CML2_IMPORT"]["TEMP_DIR"]) > 0)
	...
	$DIR_NAME = $_SERVER["DOCUMENT_ROOT"]."/".COption::GetOptionString("main", "upload_dir", "upload")."/1c_catalog".$arParams["IB_ID"]."/";
...
{
		//*********************************************************************************Готовлю дополнительную врменную таблицу
		if ($_GET['FullExch']=='true'){
			global $DB;
			$QRes = $DB->Query("CREATE TABLE IF NOT EXISTS b_xml_tree_2_".$arParams["IB_ID"]." ( name TEXT, value TEXT)");
		}
		//************************************************************************************************************************************

	if ($arParams["USE_TEMP_DIR"] === "Y")
	{
		$DIR_NAME = CTempFile::GetDirectoryName(6, "1c_catalog".$arParams["IB_ID"]);
	}
	else
	{
	...
	if (isset($DATA) && $DATA !== false)
	{
		...
		if ($fp = fopen($ABS_FILE_NAME, "ab"))
		{
			...
			if ($result === $DATA_LEN)
			{
				...
				//arraytofile($_SESSION,  $_SERVER["DOCUMENT_ROOT"]."/upload/import_orders_log2.txt", "_SESSION");
			}
			else
			{
...
elseif (($_GET["mode"] == "import") && $_SESSION["BX_CML2_IMPORT"]["zip"])
{
	...
	$result = $obXMLF->UnZip($_SESSION["BX_CML2_IMPORT"]["zip"], $_SESSION["BX_CML2_IMPORT"]["last_zip_entry"]);
	if ($result===false)
	{
...
elseif (($_GET["mode"] == "import") && $ABS_FILE_NAME)
{
	...
	elseif ($NS["STEP"] == 1)
	{
		...
		if ($obXMLF ->CreateTemporaryTables())
		{
	...
	elseif ($NS["STEP"] == 2)
	{
		...
		if (($total > 0) && is_resource($fp))
		{
			...
			$obXMLFile = new CIBlockXMLFile("b_xml_tree".$arParams["IB_ID"]);
			if ($obXMLFile->ReadXMLToDatabase($fp, $NS, $arParams["INTERVAL"]))
			{
				$NS["STEP"] = 3;
				$strMessage = GetMessage("CC_BSC1_FILE_READ");
				
				//*********************************************************************************Добавляю в дополнительную временную таблицу все из обычной, если эта вторая существует
				if ($_GET['FullExch']=='true'){
					global $DB;
			
					$QRes = $DB->Query("INSERT INTO b_xml_tree_2_".$arParams["IB_ID"]." (name,value) SELECT name, value FROM b_xml_tree".$arParams["IB_ID"]." where name='Ид'");
				}
				//************************************************************************************************************************************
	...
	elseif ($NS["STEP"] == 7)
	{
		...
		{
			$obXMLF = new CIBlockXMLFile("b_xml_tree".$arParams["IB_ID"]);
			if ($obXMLF->IsExistTemporaryTable())
			{
				$NS["DONE"]["ALL"] = $obXMLF->GetCountItemsWithParent($NS["XML_ELEMENTS_PARENT"]);
			}
			else
			{
	...
	else
	{
		...
		//Собираем внешние коды тех товаров, к-е пришли в выгрузке
		/*global $DB;
		$res = $DB->Query("select name,value from b_xml_tree where name='Ид'", false, $err_mess.__LINE__);

		$arrNewXmlID=array();
		while ($arID = $res->GetNext()){
				$arrNewXmlID[]=$arID['value'];
		}*/
...
elseif ($_GET["mode"]=="deactivate")
{
	...
	//***************************************************************************************************Очищаю дополнительную временную таблицу
	
	//******************** ищу инфоблок, в котором это все происходит
	
	global $DB;
	$res_ibid = $DB->Query("select VALUE from b_xml_tree".$arParams["IB_ID"]." where NAME='ИдКаталога'", false, $err_mess.__LINE__);


	$IBLOCK_XML_ID="";
	if ($arIBLOCK_ID = $res_ibid->GetNext()){

		$IBLOCK_XML_ID=$arIBLOCK_ID[VALUE];

	}

	CModule::IncludeModule("iblock");

	$res_ibid2 = CIBlock::GetList(
    Array(), 
    Array(
	...
	$IBLOCK_ID=0;
	if($ar_res_ibid2 = $res_ibid2->Fetch())
	{
		$IBLOCK_ID=$ar_res_ibid2[ID];
		
		
		global $DB;
		$res = $DB->Query("select name,value from b_xml_tree_2_".$arParams["IB_ID"]." where name='Ид'", false, $err_mess.__LINE__);
		...
		$res2 = $DB->Query("select ID,XML_ID from b_iblock_element where IBLOCK_ID=".$IBLOCK_ID, false, $err_mess.__LINE__);


		$arrOldXmlID=array();
		while ($arID2 = $res2->GetNext()){

			$arrOldXmlID[$arID2['ID']]=$arID2['XML_ID'];

		}
		
		//arraytofile(array($arrOldXmlID),  $_SERVER["DOCUMENT_ROOT"]."/upload/deactive_log.txt", "	$arrOldXmlID");
		
		
		if (count($arrNewXmlID)>0){
		
		
		$arrIDForDeactive=array();
		foreach($arrOldXmlID as $key=>$val){
			if (!in_array($val, $arrNewXmlID)){
				$arrIDForDeactive[]=$key;
				$res3 = $DB->Query("update  b_iblock_element SET ACTIVE='N' where IBLOCK_ID=".$IBLOCK_ID." and ID =".$key, false, $err_mess.__LINE__);
		
			}
		}
		
		
		//arraytofile(array($arrIDForDeactive),  $_SERVER["DOCUMENT_ROOT"]."/upload/deactive_log.txt", "$arrIDForDeactive");
		
		
		

				//$strSQL="select ID, XML_ID, ACTIVE, IBLOCK_ID from b_iblock_element where IBLOCK_ID=".$IBLOCK_ID." and XML_ID not in (";
				
				/*$strSQL="update  b_iblock_element SET ACTIVE='N' where IBLOCK_ID=".$IBLOCK_ID." and XML_ID not in (";

				foreach ($arrNewXmlID as $val){

					$strSQL=$strSQL."'".$val."',";
				}

				$strSQL=$strSQL."'11111111')";*/
				
				//arraytofile(array($strSQL),  $_SERVER["DOCUMENT_ROOT"]."/upload/deactive_log.txt", "_GET");

				//$res2 = $DB->Query($strSQL, false, $err_mess.__LINE__);

				/*$arrNewXmlID2=array();
				while ($arID2 = $res2->GetNext()){

					$arrNewXmlID2[]=$arID2['ID'];

				}

				
				arraytofile($arrNewXmlID2,  $_SERVER["DOCUMENT_ROOT"]."/upload/deactive_log.txt", "_GET");*/
	...
	$QRes = $DB->Query("delete from b_xml_tree_2_".$arParams["IB_ID"]);
	$QRes = $DB->Query("drop table b_xml_tree_2_".$arParams["IB_ID"]);
