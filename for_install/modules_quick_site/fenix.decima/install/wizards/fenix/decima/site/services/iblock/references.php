...
$dbHblock = HL\HighloadBlockTable::getList(
	array(
		"filter" => array("TABLE_NAME" => "decima_color_reference")
	)
);
if (!$dbHblock->Fetch())
{
	$data = array(
		'NAME' => 'DecimaColorReference',
		'TABLE_NAME' => 'decima_color_reference',
	);

	$result = HL\HighloadBlockTable::add($data);
	$ID = $result->getId();

	$_SESSION["DECIMA_HBLOCK_COLOR_ID"] = $ID;
...
$dbHblock = HL\HighloadBlockTable::getList(
	array(
		"filter" => array("TABLE_NAME" => "decima_brand_reference")
	)
);
$br=$dbHblock->Fetch(); 
if (!$br)
{
	$data = array(
		'NAME' => 'DecimaBrandReference',
		'TABLE_NAME' => 'decima_brand_reference',
	);

	$result = HL\HighloadBlockTable::add($data);
	$ID = $result->getId();

	$_SESSION["DECIMA_HBLOCK_BRAND_ID"] = $ID;
    ...
    CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."_index.php", array("BRAND_IBLOCK_ID" => $ID));
    CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."catalog/index.php", array("BRAND_IBLOCK_ID" => $ID));
}
else{
    CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."_index.php", array("BRAND_IBLOCK_ID" => $br['ID']));
    CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."catalog/index.php", array("BRAND_IBLOCK_ID" => $br['ID']));
}
?>