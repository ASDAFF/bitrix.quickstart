		...
		if($arUserResult["CONFIRM_ORDER"] == "Y" && empty($arResult["ERROR"]))
		{
			if(!$USER->IsAuthorized() && $arParams["ALLOW_AUTO_REGISTER"] == "Y")
			{
				if(strlen($arUserResult["USER_EMAIL"]) > 0)
				{
					...
					$user = new CUser;	
					$dbUserLogin = $user->GetByLogin($NEW_LOGIN);
					...
					{
						$arResult["ERROR"][] = (strlen($user->LAST_ERROR) > 0) ? GetMessage("STOF_ERROR_REG").": ".$user->LAST_ERROR : GetMessage("STOF_ERROR_REG_2");
					}
					else
					{
			...
			if($USER->IsAuthorized() && empty($arResult["ERROR"]))
			{
				...
				{
					$arFields["ADDITIONAL_INFO"] = md5($arResult["ORDER_ID"].IntVal($USER->GetID()).$USER->GetLogin()."iloveyousergeland");					
				...
				if (empty($arResult["ERROR"]))
				{
					...
					$arFields = Array(
						...
						"PATH_TO_PERSONAL_AUTH" => $arParams["PATH_TO_PERSONAL"]."?hash=".$arOrder["ADDITIONAL_INFO"],
