...
if($arResult['ITEMS']){
	...
	if($arSectionIDs){
		...
		$dbRes = CIBlockSection::GetList(array("SORT"=>"ASC", "ID" => "DESC"), array('ID' => $arSectionIDs, 'ACTIVE' => 'Y', 'LEVEL_DEPTH' => 0), false, array('ID', 'NAME'));
		while($arSection = $dbRes->Fetch()){
			$arResult['SECTIONS'][$arSection['ID']] = $arSection['NAME'];
		}
	}
	
	$arResult['SECTIONS'][0] = GetMessage('STAFF_OTHER_SECTION');
}
?>