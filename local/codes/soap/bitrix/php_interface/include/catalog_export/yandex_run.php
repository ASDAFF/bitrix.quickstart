...
$av_res = $aviability->GetList(array(), array("!ITEM_ID"=>"0"));
while($av_ar = $av_res->Fetch()){
    $av_arr[$av_ar['ITEM_ID']]['ITEMS'][$av_ar['ID']] = $av_ar;
    $av_arr[$av_ar['ITEM_ID']]['COUNT'] += $av_ar['AVIABLE'];
}
...
$res = CIBlockElement::GetList(Array("SORT"=>"ASC"), $arFilter, Array("ID", "IBLOCK_ID", "PROPERTY_*"));
while($ar_f = $res->GetNextElement()){
  $el_ = $ar_f->GetFields();
  $el_['PROPERTIES'] = $ar_f->GetProperties();

  $tmp[$el_['ID']] = $el_;
}
 

$GLOBALS['av_arr'] = $av_arr;
$GLOBALS['sklady'] = $tmp;
	...
	function yandex_get_value($arOffer, $param, $PROPERTY)
	{
		...
		if (is_array($arProperties[$PROPERTY]))
		{
			...
			if ($bParam)
			{
				...
				{     
                                  $xm = yandex_text2xml($arProperties[$PROPERTY]['NAME'], true);
                             
                                 global $APPLICATION;
                                $noArr = array("Производитель","Модель", "Артикул","Спецификация", 'Тип'); 
                                        $newN = array();
                                foreach($noArr as $el_)
                                    $newN[] = $APPLICATION->ConvertCharset($el_, LANG_CHARSET, 'windows-1251');
                                
                                    if(!in_array($xm, $newN) )
                                     {
                                        if(strlen(yandex_text2xml($value, true))){


                                        $desc = '';
                                        if($description)
                                            $desc = ' unit="' . yandex_text2xml($description, true) .'"';  


                                           $strProperty .= '<param name="'.
                                               yandex_text2xml($arProperties[$PROPERTY]['NAME'], true). 
                                               '"' . $desc . '>'.yandex_text2xml($value, true).'</param>';
			...
			else
			{
				$param_h = yandex_text2xml($param, true);
                                $tm = yandex_text2xml($value, true);
                                if ($tm)
                                    $strProperty .= '<'.$param_h.'>'. $tm .'</'.$param_h.'>';
			}
		}
...
if (empty($arRunErrors))
{
	...
	@fwrite($fp, "<shop>\n"); 
	@fwrite($fp, "<name>COMP2YOU</name>\n"); 
	@fwrite($fp, "<company>COMP2YOU</company>\n");
	...
	$strTmp.= "<currency id=\"USD\" rate=\"CBRF\"/>\n";
        $strTmp.= "<currency id=\"EUR\" rate=\"CBRF\"/>\n";
        $strTmp.= "</currencies>\n";
	...
	if ('D' == $arCatalog['CATALOG_TYPE'] || 'O' == $arCatalog['CATALOG_TYPE'])
	{
		...
		while ($obElement = $res->GetNextElement())
		{
			$arAcc = $obElement->GetFields();
                        if(!in_array($arAcc['ID'], array_keys($GLOBALS['av_arr'])))
                                continue;
			...
			if (($str_QUANTITY <= 0) && ($str_QUANTITY_TRACE == "Y"))
				...
				$str_AVAILABLE = ' available="true"';
                        ...
                        $str_AVAILABLE = ' available="false"'; 
                        foreach($GLOBALS['av_arr'][$arAcc['ID']]['ITEMS'] as $skl){
                            $skladId = $skl['STORE_ID'];
                               if($GLOBALS['sklady'][$skladId]["PROPERTIES"]["DOSTUPNOST"]["VALUE"] == 0
                                       && 
                                      $skl['AVIABLE'] > 0
                                       ) 
                                  $str_AVAILABLE = ' available="true"';
...
//                                       ) {
//                                         prent($skladId);
//                                          prent($GLOBALS['sklady'][$skladId]["PROPERTIES"]["DOSTUPNOST"]["VALUE"]);
//                                             prent($skl['AVIABLE']);
//                                             prent('----');
//                                       }
//                                    
...
/*					$dbVAT = CCatalogProduct::GetVATInfo($arAcc['ID']);
			...
			$strTmpOff.= "<url>".str_replace('%2F', '/',  "http://".$ar_iblock['SERVER_NAME'].htmlspecialcharsbx($arAcc["~DETAIL_PAGE_URL"]).(strstr($arAcc['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;')."r1=<?echo \$strReferer1; ?>&amp;r2=<?echo \$strReferer2; ?>") . "</url>\n";
			...
			if (intval($arAcc["DETAIL_PICTURE"])>0 || intval($arAcc["PREVIEW_PICTURE"])>0)
			{
				...
				if ($ar_file = CFile::GetFileArray($pictNo))
				{
                    ...
                    //    $strTmpOff.= "<available>" .  $availableVal . "</available>\n"; 
                        
                        $samovivozVal = "false";
                        foreach($GLOBALS['av_arr'][$arAcc['ID']]['ITEMS'] as $skl){
                            $skladId = $skl['STORE_ID'];
                               if($GLOBALS['sklady'][$skladId]["PROPERTIES"]["SAMOVIVOZ"]["VALUE_XML_ID"] == 'Y') 
                                  $samovivozVal = 'true';
                        } 
                         
                        
                        $strTmpOff.= "<store>" . $samovivozVal . "</store>\n";
                        //$strTmpOff.= "<pickup>" . $samovivozVal . "</pickup>\n"; 
            ...
            foreach($GLOBALS['av_arr'][$arAcc['ID']]['ITEMS'] as $skl){
                        ...
                        $strTmpOff.= "<delivery>{$deliveryVal}</delivery>";
                        
                                                   
                   if ($arAcc['PROPERTIES']['type']['VALUE']) 
                           $strTmpOff.= "<typePrefix>" . yandex_text2xml($arAcc['PROPERTIES']['type']['VALUE'], true) . "</typePrefix>\n"; 
			...
			foreach ($arYandexFields as $key)
			{
						...
						if ('' != $strValue) 
							$strTmpOff .= $strValue."\n";
                                    ...
                                    case 'manufacturer_warranty': 
                                      ...
                                      $manufacturer_warranty = $arAcc['PROPERTIES']["manufacturer_warranty"]['VALUE_XML_ID'];
                                     if($manufacturer_warranty == 'b153783afe56f5327cdb82175e7ecaa0')
                                         $manufacturer_warranty = 'true';
                                     else 
                                         $manufacturer_warranty = 'false';
                                    
                                        $strTmpOff .= "<manufacturer_warranty>" . $manufacturer_warranty . "</manufacturer_warranty>\n";
                                        break; 
				case 'name':
				...
				case 'description':
                                        ...
                                        $strValue = '';
                                            $strValue =  "<model>" . 
                                                       $APPLICATION->ConvertCharset($arAcc['PROPERTIES']["model"]['VALUE'] , LANG_CHARSET, 'windows-1251');
                                    
                                             
                                            if ($arAcc['PROPERTIES']["specification"]['VALUE'])
						$strValue.= " (" . 
                                                      $APPLICATION->ConvertCharset($arAcc['PROPERTIES']["specification"]['VALUE'] , LANG_CHARSET, 'windows-1251')
                                                    . ")"; 
                                             
                                             
                                            if ($arAcc['PROPERTIES']["article"]['VALUE'])
						$strValue.= " [" . 
                                                      $APPLICATION->ConvertCharset($arAcc['PROPERTIES']["article"]['VALUE'] , LANG_CHARSET, 'windows-1251')
                                                     .']'; 
                   ...
                   if ($arAcc['PROPERTIES']['type']['VALUE']) 
                           $descTmp.=  yandex_text2xml($arAcc['PROPERTIES']['type']['VALUE'], true); 
                     
                   
                    $vendor= yandex_get_value($arAcc, 'vendor', $XML_DATA['XML_DATA']['vendor']);
                                              $vendor = trim ($vendor);
                                            $vendor = str_replace(array('<vendor>','</vendor>'), array('',''), $vendor);
                   if ( $vendor)
                           $descTmp.= ' ' .  $vendor; 
                     
                                    
                   if($APPLICATION->ConvertCharset($arAcc['PROPERTIES']["model"]['VALUE'] , LANG_CHARSET, 'windows-1251'))
                               $descTmp.= ' ' .  $APPLICATION->ConvertCharset($arAcc['PROPERTIES']["model"]['VALUE'] , LANG_CHARSET, 'windows-1251');        
				...
				if (!empty($arCurrentOffer) && !empty($arCurrentPrice))
				{
					...
					foreach ($arYandexFields as $key)
					{
						...
						case 'description':
							...
							{
								$descTmp .= $arItem['YANDEX_DESCR'];
							}
							else
							{
								$descTmp .= yandex_text2xml(TruncateText(
