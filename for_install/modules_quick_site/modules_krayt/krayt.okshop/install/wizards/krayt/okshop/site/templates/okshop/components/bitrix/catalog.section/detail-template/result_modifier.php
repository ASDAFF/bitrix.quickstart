...
if (!empty($arResult['ITEMS']))
{
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		//Set
		if($arItem['PROPERTIES']['EMARKET_ADDWORK']['VALUE'])
		{
			$arSelect = Array("ID", "NAME");
			$arFilter = Array(
				"IBLOCK_ID" => $arParams['IBLOCK_ID'], 
				"ID"=> $arItem['PROPERTIES']['EMARKET_ADDWORK']['VALUE'], 
				"ACTIVE_DATE"=>"Y", 
				"ACTIVE"=>"Y"
			);
			...
			if($ob = $res->GetNextElement())
			{
				...
				$arItem['ADDWORK'] = $arFields;
				$arItem['ADDWORK']['PROPERTIES'] = $arProps;
				$arSelectId[] = $arFields['ID'];
			}
		}		
...
//get discont
$arSelect = array("ID", "CATALOG_GROUP_ID", "PRICE", "CURRENCY", "PRODUCT_ID");
$arFilter = array("PRODUCT_ID" => $arSelectId);
...
while ($arPrice = $dbPrice->Fetch())
{
	...
	{
		if($arItem['ADDWORK']['ID'] == $arPrice['PRODUCT_ID'])
		{
			$arResult['ITEMS'][$id]['ADDWORK']['PRICE'] = $arPrice;
		}
	}
}
?>