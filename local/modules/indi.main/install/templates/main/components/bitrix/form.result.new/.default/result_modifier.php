...
$arFieldsWithError = array_keys($arResult['FORM_ERRORS']);
if(!empty($arFieldsWithError)) {
	$errorStr = Loc::getMessage('FORM_REQUIRED_FIELDS_ERROR');
	$arErrorsData = array();
	foreach ($arFieldsWithError as $questionId) {
		$arErrorsData[] = sprintf('<br />%s', $arResult['QUESTIONS'][$questionId]['CAPTION']);
	}
	$errorStr .= implode(', ', $arErrorsData);
	$arResult['FORM_ERRORS_TEXT_FIELDS'] = $errorStr;
}
