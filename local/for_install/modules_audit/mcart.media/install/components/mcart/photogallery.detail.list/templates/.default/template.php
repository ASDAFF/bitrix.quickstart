...
#print "<pre>"; print_r($arResult["ELEMENTS_LIST_JS"]); print "</pre>";
...
foreach ($arResult["ELEMENTS_LIST_JS"] as $key => $res){ ?>
		...
		<table class="photo-slider-thumb" cellpadding="0">
			<tr>
				...
				<?if ($res["id"] == $arParams["ELEMENT_ID"]):?>
					<div class="image">
						<img border="0" height="<?=$curr_width?>" alt="<?=$res["alt"]?>" <?
							?>src="<?=$res["src"]?>" title="<?=$res["title"]?>" />
					</div>
				<?else:?>
					<a href="<?=htmlspecialchars($res['DETAIL_PICTURE']['SRC'])?>" rel="prettyPhoto[mixed]">
						<img border="0" height="<?=$curr_width?>"  alt="<?=htmlspecialchars($res["alt"])?>" <?
							?>src="<?=$res["src"]?>" title="<?=$res["title"]?>" />
	...
	if (!$b_founded_active)
		$i_leftward += ($curr_width + $i_reserve);
}
if (!$b_founded_active)
	$i_leftward = 0; 
$str = ob_get_clean(); 
?>
<div class="photo-slider" id="lightBox">
