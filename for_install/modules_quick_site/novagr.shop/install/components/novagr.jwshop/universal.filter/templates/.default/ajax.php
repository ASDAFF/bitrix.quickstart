...
function deb($ar)
{
	...
	function callback($buffer)
	{
		...
		if((int)$_REQUEST['FILTER_RESET'] == 1)
		{
			...
			*/
			...
			//$_REQUEST['arElementsSearch'] = array(554);
			...
			if( empty($arParams['arFilterRequest']) && empty($arParams['arOfferRequest']) )
			{
				...
				if(!empty($arFilter['PROPERTY_CML2_LINK']))
				{
					...
					while($arElement = $rsElement -> Fetch())
					{
						...
						if((int)$arElement['CATALOG_PRICE_1'] > 0)
							$arPrice[ $arElement['CATALOG_PRICE_1'] ] = $arElement['CATALOG_PRICE_1'];
					}
				}
			}
			//deb($arResult);
			...
			if( !empty($arParams['arOfferRequest']) )
			{
				...
				foreach($arParams['arOfferRequest'] as $key => $val)
				{
					if($key == ">=CATALOG_PRICE_1")
						$arFilter['>=CATALOG_PRICE_1'] = $val;
					elseif($key == "<=CATALOG_PRICE_1")
						$arFilter['<=CATALOG_PRICE_1'] = $val;
					else
						$arFilter[0][$key] = $val;
				}
	...
	if((int)$_REQUEST['CATALOG_RESET'] == 1)
	{
		...
		{
			$APPLICATION->IncludeComponent("novagr.jwshop:fashion.list", ".default", array(
