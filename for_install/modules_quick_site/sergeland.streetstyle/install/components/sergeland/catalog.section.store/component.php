...
if(array_key_exists("desc", $_REQUEST))
{
	...
	{
		$arParams["ELEMENT_SORT_FIELD"] = $CATALOG_PRICE;
		$priseURL   = "asc&price";
	}		
...
if($this->StartResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation)))
{
	...
	if ($arParams["FLAG_PROPERTY_CODE"])
		$arFilter["!PROPERTY_".$arParams["FLAG_PROPERTY_CODE"]] = false;
...
?>
<div class="catalog-element-sort"><?=GetMessage("SERGELAND_STORE_SORT")?> <a href="<?=$APPLICATION->GetCurPageParam($priseURL, array("asc", "desc", "name", "price"));?>"><?=GetMessage("SERGELAND_STORE_SORT_PRICE")?></a> | <a href="<?=$APPLICATION->GetCurPageParam($nameURL, array("asc", "desc", "name", "price"));?>"><?=GetMessage("SERGELAND_STORE_SORT_NAME")?></a></div>