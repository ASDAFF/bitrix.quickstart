...
class RSLibFunc {
	public static function Init($arInit) {
		...
		if(in_array("jsfunc",$arInit)) {
			$APPLICATION->AddHeadString("<script>var RSLibFunc_BasketEndWord_end1 = \"".GetMessage("RSDF_END_1")."\";var RSLibFunc_BasketEndWord_end2 = \"".GetMessage("RSDF_END_2")."\";var RSLibFunc_BasketEndWord_end3 = \"".GetMessage("RSDF_END_3")."\";</script>");
			$APPLICATION->AddHeadScript("/bitrix/js/development.lib_func/script.js");
		}
	}
	...
	public static function GetDataForProductItem(&$arItems,$params=array()) {
		if(\Bitrix\Main\Loader::includeModule('iblock') && is_array($arItems) && count($arItems)>0) {
			...
			// add quickbuy
			if(\Bitrix\Main\Loader::includeModule('development.quickbuy')) {
				$arFilter = array(
					'DATE_FROM' => $iTime,
					'DATE_TO' => $iTime,
					'QUANTITY' => 0,
			...
			// /add quickbuy
			// add da2
			if(\Bitrix\Main\Loader::includeModule('development.daysarticle2')) {
				$arFilter = array(
					'DATE_FROM' => $iTime,
					'DATE_TO' => $iTime,
					'QUANTITY' => 0,
	...
	public static function GetNoPhoto($arSizes) {
		$return = false;
		$fileid = COption::GetOptionInt('development.lib_func', 'no_photo_fileid', 0);
		if($fileid>0) {
