...
// this is SUPER HACK!!! reed more #4230 task
// don't worry this is bitrix define user as search engine it's wrong
...
if ($_REQUEST["ADD2BASKET"] == 1 && !empty($_REQUEST["elemId"])) {
    ...
    $deleteFromShelves = false;
    // ищем в корзине товар с нужным ID
    while ($arItems = $dbBasketItems->Fetch())
    {
        // обрабатываем запрос на удаление из отложенных
        if ($_REQUEST["act"] == "delFromShelve" && $productID == $arItems["PRODUCT_ID"]) {
        ...
        if ($arItems["PRODUCT_ID"] == $productID and $arItems['SUBSCRIBE']!=="Y" and $arItems['DELAY']!=="Y" ) {
            ...
            if (is_array($mxResult)) {
                $action = new Novagroup_Classes_General_TimeToBuy(
                    $mxResult['ID'], $mxResult['IBLOCK_ID']
                );
