jQuery.fn.Deserialize = function (DataRaw) {
	...
	return this.each(function () {
		...
		for(var i in Data) {
			M = Data[i].match(/^(.*?)=(.*?)$/);
			if(M!=null) {
				var Field = $('input[name="'+M[1]+'"]:not([type=hidden]):not([type=image]):not([type=button]):not([type=submit]),textarea[name="'+M[1]+'"],select[name="'+M[1]+'"]',this);
...
function WDA_FilterRevision(IBlockID) {
	$('#wda_filters').find('.item').each(function(){
		...
		if (IBlockID==false || Code.length>0 && $('#wda_filter_param option[value='+Code+']').size()==0) {
			$(this).click();
		}
	});
}
...
function WDA_AddFilter(Data) {
	...
	var NewFilter =
		'<div class="item" title="'+WDA.Messages.FilterItemTitle+'">'+
			'<span class="param">'+Param1+'<input type="hidden" name="f_p1[]" class="f_p1" value="'+Param1+'" /><input type="hidden" name="f_p2[]" class="f_p2" value="'+Param2+'" /></span>'+' '+
			...
			'<span class="value">'+Value1.replace(/{html_([a-z]+)}/,'<span>&amp;</span><span>$1</span><span>;</span>')+'<input type="hidden" name="f_v1[]" class="f_v1" value="'+Value1+'" /><input type="hidden" name="f_v2[]"class="f_v2" value="'+Value2+'" /></span>'+
		'</div>';
...
function WDA_LoadProfile(ProfileID){
	...
	if (typeof Profile == 'object') {
		...
		WDA_OnChangeIBlockID($('#wda_select_iblock')[0],function(Select,IBlockID){
			...
			// ToDo: заполнение блока с фильтрами
...
$(document).ready(function(){
	...
	$('#wda_filters').delegate('.item','click',function(){
		$(this).fadeOut(200,function(){
			$(this).remove();
			if ($('#wda_filters').find('.item').size()==0) {
