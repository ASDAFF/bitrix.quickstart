...
class CMFavorite {
	...
	public static function AddFolder($arFields) {
		...
		if (!strlen($arFields['NAME']) || !$arFields['USER_ID'])
			...
			$ID = $GLOBALS['DB']->Insert('b_asdaff_favorite_folders', $arFields, self::mysqlError.__LINE__);
			return $ID;
		}
	}
	...
	public static function UpdateFolder($ID, $arFields, $checkRights = true) {
		...
		if (!strlen($arFields['NAME']))
			return false;
		$GLOBALS['DB']->Update('b_asdaff_favorite_folders', $arFields, 'WHERE ID=' . intval($ID) .
	...
	public static function DeleteFolder($ID, $checkRights = true) {
		...
		if ($checkRights && $GLOBALS['USER']->IsAdmin())
			$checkRights = false;
		$ID = intval($ID);
		$GLOBALS['DB']->Query("DELETE FROM b_asdaff_favorite_folders WHERE ID=$ID" . ($checkRights ? " AND USER_ID=" . intval($GLOBALS['USER']->GetID()) : ""));
		$GLOBALS['DB']->Query("DELETE FROM b_asdaff_favorite_likes WHERE FOLDER_ID=$ID;");
	}
	...
	public static function SetFolderDefault($UID, $CODE, $ID) {
		...
		$GLOBALS['DB']->Query("UPDATE b_asdaff_favorite_folders SET `DEFAULT`='N' WHERE USER_ID=$UID AND CODE='$CODE';");
		$GLOBALS['DB']->Query("UPDATE b_asdaff_favorite_folders SET `DEFAULT`='Y' WHERE ID=$ID AND CODE='$CODE';");
	}
	...
	public static function GetFolders($CODE, $UID = false) {
		...
		$rs = $GLOBALS['DB']->Query("SELECT * FROM b_asdaff_favorite_folders WHERE USER_ID=$UID AND CODE='$CODE' ORDER BY ID ASC;");
		while ($ar = $rs->GetNext())
			$arFolders[$ar['ID']] = $ar;
	...
	public static function AddType($arFields) {
		...
		if (!strlen($arFields['NAME']) || !strlen($arFields['CODE']))
			...
			$ID = $GLOBALS['DB']->Insert('b_asdaff_favorite_types', $arFields, self::mysqlError.__LINE__);
	...
	public static function UpdateType($CODE, $arFields) {
		...
		if (!strlen($CODE) || !strlen($arFields['NAME']))
			...
			$GLOBALS['DB']->Update('b_asdaff_favorite_types', $arFields, "WHERE CODE='" . $CODE . "'", self::mysqlError.__LINE__);
			self::MakeRefType($CODE, $arRef);
		}
	}
	...
	public static function MakeRefType($CODE, $arRef) {
		if (strlen(trim($CODE)) && is_array($arRef)) {
			$CODE = $GLOBALS['DB']->ForSQL(trim($CODE));
			$GLOBALS['DB']->Query("DELETE FROM b_asdaff_favorite_types_ref WHERE CODE='$CODE';");
			foreach ($arRef as $ref) {
				$GLOBALS['DB']->Insert('b_asdaff_favorite_types_ref', array(
																'CODE' => '"'.$CODE.'"',
	...
	public static function GetRefType($CODE) {
		if (strlen(trim($CODE))) {
			$CODE = $GLOBALS['DB']->ForSQL(trim($CODE));
			$arRefs = array();
			$rsRef = $GLOBALS['DB']->Query("SELECT * FROM b_asdaff_favorite_types_ref WHERE CODE='$CODE';");
	...
	public static function GetTypesByRef($REF) {
		if (strlen(trim($REF))) {
			return $GLOBALS['DB']->Query("SELECT * FROM b_asdaff_favorite_types_ref WHERE REF='".$GLOBALS['DB']->ForSQL(trim($REF))."';");
		}
	}

	public static function DeleteType($CODE) {
		$CODE = $GLOBALS['DB']->ForSQL(trim($CODE));
		$GLOBALS['DB']->Query("DELETE FROM b_asdaff_favorite_types WHERE CODE='$CODE';");
		$GLOBALS['DB']->Query("DELETE FROM b_asdaff_favorite_folders WHERE CODE='$CODE';");
		$GLOBALS['DB']->Query("DELETE FROM b_asdaff_favorite_likes WHERE CODE='$CODE';");
		$GLOBALS['DB']->Query("DELETE FROM b_asdaff_favorite_types_ref WHERE CODE='$CODE';");
	}
	...
	public static function GetTypes($arSort = array('CODE' => 'ASC'), $arFilter = array()) {
		...
		return $GLOBALS['DB']->Query("SELECT * FROM b_asdaff_favorite_types $strWhere ORDER BY $by $order;");
	}
	...
	public static function GetLikes($arFilter = array(), $groupBy = false) {
		...
		return $GLOBALS['DB']->Query("SELECT $strSelect FROM b_asdaff_favorite_likes $strWhere $strGroup ORDER BY ID DESC ;");
	}
	...
	public static function Like($ID, $CODE, $UID = false) {
		...
		$GLOBALS['DB']->Insert('b_asdaff_favorite_likes', $arFields, self::mysqlError.__LINE__);
	}
	...
	public static function UnLike($ID, $CODE, $UID = false) {
		...
		if ($ID <= 0 || !strlen($CODE) || $UID <= 0)
			return false;
		$GLOBALS['DB']->Query("DELETE FROM b_asdaff_favorite_likes WHERE ELEMENT_ID=$ID AND USER_ID=$UID AND CODE='" . $GLOBALS['DB']->ForSQL($CODE) . "';");
	}
	...
	public static function MoveLike($ID, $CODE, $NEW_FOLDER_ID) {
		...
		if ($ID <= 0 || !strlen($CODE) || $UID <= 0 || $NEW_FOLDER_ID <= 0)
			return false;
		$GLOBALS['DB']->Query("UPDATE b_asdaff_favorite_likes SET FOLDER_ID=$NEW_FOLDER_ID
							WHERE ELEMENT_ID=$ID AND USER_ID=$UID AND CODE='" . $GLOBALS['DB']->ForSQL($CODE) . "';");
	}

	public static function OnUserDeleteHandler($USER_ID) {
		$GLOBALS['DB']->Query("DELETE FROM b_asdaff_favorite_folders WHERE USER_ID=" . intval($USER_ID));
		$GLOBALS['DB']->Query("DELETE FROM b_asdaff_favorite_likes WHERE USER_ID=" . intval($USER_ID));
	}

	public static function OnBlogPostDeleteHandler($ID, &$result) {
		$GLOBALS['DB']->Query("DELETE FROM b_asdaff_favorite_likes
								WHERE CODE IN (SELECT CODE FROM b_asdaff_favorite_types WHERE MODULE='blog')
								AND ELEMENT_ID=" . intval($ID));
	}

	public static function OnIBlockElementDeleteHandler($ID) {
		$GLOBALS['DB']->Query("DELETE FROM b_asdaff_favorite_likes
								WHERE CODE IN (SELECT CODE FROM b_asdaff_favorite_types WHERE MODULE='iblock')
								AND ELEMENT_ID=" . intval($ID));
	}

	public static function OnAfterTopicDeleteHandler($ID, $arTopic) {
		$GLOBALS['DB']->Query("DELETE FROM b_asdaff_favorite_likes
								WHERE CODE IN (SELECT CODE FROM b_asdaff_favorite_types WHERE MODULE='forum')
								AND ELEMENT_ID=" . intval($ID));
	}
