...
if(CModule::IncludeModule("advertising") && !empty($arResult["BANNER_PROPERTIES"])){
	$arBannerList = array();
	if(strpos($arResult["BANNER"], '<a')===false){$arResult["BANNER"]='<span class="item active">'.$arResult["BANNER"].'</span>';}
	$arBannerList[ $arResult["BANNER_PROPERTIES"]["ID"] ] = str_replace("<a", "<a class='item active'", $arResult["BANNER"]);
	$arBannerProps = array();
	...
	for ($i=0; $i < 5; $i++) { 
		...
		if(!empty($arBanner)){
		 	...
		 	$arBannerList[ $arBanner["ID"] ] = str_replace("<a", "<a class='item'", $HTML);
		}
	}
