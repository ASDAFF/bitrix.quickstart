...
else
{
	...
	?><div id="sls-<?=$arResult['RANDOM_TAG']?>" class="bx-slst<?if($arResult['ADMIN_MODE']):?> bx-admin-mode<?endif?>"><?
		...
		?><script type="text/html" data-template-id="bx-ui-slst-selector-scope"><?
			...
			?><div class="grid-cell textinput40 back1 dropdown-block bx-ui-slst-input-block<?=($arParams['DISABLE_KEYBOARD_INPUT'] == 'Y' ? ' disabled-keyboard' : '')?>"><?

				?><span class="dropdown-icon"></span><?
				?><input type="text" name="" value="" autocomplete="off" class="dropdown-field form-control" /><?
				?><div class="bx-ui-combobox-container" style="margin: 0px; padding: 0px; border: none; position: relative;"><?
					if ($arParams['DISABLE_KEYBOARD_INPUT'] == 'Y')
					{
						?><div class="bx-ui-combobox-fake bx-combobox-fake-as-input"><?
							echo Loc::getMessage('SALE_SLS_SELECTOR_PROMPT');
						?></div><?
					}
					else
					{
						?><input type="text" value="" autocomplete="off" class="bx-ui-combobox-fake form-control" placeholder="<?=Loc::getMessage('SALE_SLS_SELECTOR_PROMPT')?>" /><?
					}
				?></div><?
				...
				?><div class="bx-ui-combobox-dropdown" data-bx-ui-id="combobox-dropdown"><?
					?><div data-bx-ui-id="pager-pane"></div><?
				?></div><?
			?></div><?
		?></script><?
