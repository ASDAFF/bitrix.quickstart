...
//Делаем выборку Блоков по коду
$rsIBlock = CIBlock::GetList(array(), array("CODE" => $iblockCode, "TYPE" => $iblockType));


$iblockID = false; 

//Если выборка есть то удаляем инфоблок с данным ID
...
//Если удалён или вообще не удалялся(вообще не существовал???) то
if($iblockID == false)
{
	$permissions = Array(
			"1" => "X",
    ...
    //$iblockID получается после импорта файла
	$iblockID = WizardServices::ImportIBlockFromXML(
		$iblockXMLFile,/*Cсылка на XML*/
		$iblockCode,/*Code*/
		$iblockType,/*Type*/
		WIZARD_SITE_ID,/**/
		$permissions/**/
