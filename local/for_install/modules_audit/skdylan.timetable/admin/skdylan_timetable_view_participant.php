...
if(isset($_REQUEST['SID']) && $_REQUEST['SID']!="")
{
    ...
    if($participant == null)
    {
        ...
        ShowError(GetMessage("TAMETABLE_PARTICIPANT_ERROR"));
        require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/epilog_admin.php");
        die();
    }

    $APPLICATION->SetTitle(GetMessage("TAMETABLE_VIEW_PARTICIPANT_TITLE")." - ".$participant["NAME"]);
}
...
<form method="POST" Action="<?=$APPLICATION->GetCurPageParam()?>" ENCTYPE="multipart/form-data" name="post_form">
    ...
    <tr>
        ...
        <td><input type="text" name="NAME" value="<?=$participant["NAME"];?>" size="80" maxlength="255" style="width:500px; min-width:500px; box-sizing:border-box;" disabled></td>
    </tr>
    <tr>
        <td><?=GetMessage("TAMETABLE_PHONE")?>:</td>
        <td><input type="text" name="PHONE" value="<?=$participant["PROPERTY"]["phone_number"]["VALUE"];?>" size="80" maxlength="255" style="width:500px; min-width:500px; box-sizing:border-box;" disabled></td>
    </tr>
    <tr>
        <td><?=GetMessage("TAMETABLE_EMAIL")?>:</td>
        <td><input type="text" name="EMAIL" value="<?=$participant["PROPERTY"]["email"]["VALUE"];?>" size="80" maxlength="255" style="width:500px; min-width:500px; box-sizing:border-box;" disabled></td>
    </tr>
    <tr>
        <td><?=GetMessage("TAMETABLE_COMMENT")?>:</td>
        <td><textarea disabled name="COMMENT" cols="80" rows="3" style="width:500px; min-width:500px; min-height:60px; box-sizing:border-box;"><?=$participant["PROPERTY"]["comment"]["VALUE"]?></textarea></td>
    </tr>
    ...
    <tr class="as-tt-shedule-id">
        ...
        <td>
            <input disabled name="EVENT_ID" id="IBLOCK_ELEMENT_ID" value="<?=$EID?>" size="5" type="text" data-cip-id="IBLOCK_ELEMENT_ID">
            <input disabled type="button" value="..." onclick="jsUtils.OpenWindow('/bitrix/admin/iblock_element_search.php?lang=ru&IBLOCK_ID=0&n=IBLOCK_ELEMENT_ID&k=iei', 900, 700);">&nbsp <?=MainTimeTable::GetElementNameByID($EID)?></td>
