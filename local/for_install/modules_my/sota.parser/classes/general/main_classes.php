...
class SotaContentParser
{
    ...
    public function __construct()
    {
        ...
        $this->index_element = ($sota_INDEX_ELEMENT == "Y") ? true : false;
        $this->code_element = $sota_CODE_ELEMENT;
        $this->resize_image = ($sota_RESIZE_IMAGE == "Y") ? true : false;
    ...
    public function startParser($agent = false)
    {
        ...
        if ($_GET["begin"]) {
            $this->auth(true);
            foreach (GetModuleEvents("sota.parser", "startPars", true) as $arEvent)
    ...
    protected function convetCyrillic(&$url)
    {
        ...
        $converter = new sota_idna_convert();
    ...
    protected function SaveParseSection($rss)
    {
        if (isset($this->section_array[$rss]) && !empty($this->section_array[$rss])) {
            file_put_contents($_SERVER["DOCUMENT_ROOT"] . BX_ROOT . "/modules/sota.parser/include/count_parser_section" . $this->id . ".txt", $this->section_array[$rss]);
        }
    }
    ...
    protected function parseCatalog()
    {
        ...
        if ($this->agent || $this->settings["catalog"]["mode"] == "debug") {
            foreach (GetModuleEvents("sota.parser", "EndPars", true) as $arEvent)
    ...
    protected function parseCatalogProducts()
    {
        ...
        foreach ($this->html[$element] as $el) {
            ...
            if ($this->typeN == "xml") $debug_item = SotaXmlParser::DEFAULT_DEBUG_ITEM;
    ...
    protected function parseCatalogProductElement(&$el)
    {
        ...
        $db_events = GetModuleEvents("sota.parser", "parserBeforeAddElementCatalog", true); //27.10.2015
        $error = false;
        ...
        if (!$error && !$error_isad) //27.10.2015
        {
            ...
            foreach (GetModuleEvents("sota.parser", "parserAfterAddElementCatalog", true) as $arEvent)
    ...
    protected function SetCatalogElementsResult($page = false)
    {
        ...
        file_put_contents($_SERVER["DOCUMENT_ROOT"] . BX_ROOT . "/modules/sota.parser/include/count_parser_catalog" . $this->id . ".txt", "|" . $countPage . "|" . $ciElement . "|" . $errorElement . "|" . $allError . "|" . $this->countSection);
    }
    ...
    {
        foreach (GetModuleEvents("sota.parser", "parserCatalogPreview", true) as $arEvent)
    ...
    protected function parserCatalogDetail()
    {
        ...
        if ($this->checkUniq() && !$this->isUpdate) return false;
        foreach (GetModuleEvents("sota.parser", "parserCatalogDetailBefore", true) as $arEvent)
            ExecuteModuleEventEx($arEvent, array($this->id, &$el, &$this->arFields));
        $el = $this->parserCatalogDetailPage();
        foreach (GetModuleEvents("sota.parser", "parserCatalogDetail", true) as $arEvent)
            ExecuteModuleEventEx($arEvent, array($this->id, &$el, &$this->arFields));
    ...
    {
        if (file_exists($_SERVER["DOCUMENT_ROOT"] . BX_ROOT . "/modules/sota.parser/include/count_parser_section" . $this->id . ".txt")) {
            $section_id = file_get_contents($_SERVER["DOCUMENT_ROOT"] . BX_ROOT . "/modules/sota.parser/include/count_parser_section" . $this->id . ".txt");
    ...
    protected function parserCatalogDetailPage()
    {
        ...
        foreach (GetModuleEvents("sota.parser", "parserCatalogDetailPageAfter", true) as $arEvent)
    ...
    protected function parseCatalogNamePreview($el)
    {
        ...
        if ($this->arFields["NAME"]) {
            $this->arFields["NAME"] = $this->actionFieldProps("sota_PARSER_NAME_E", $this->arFields["NAME"]);
            if (isset($this->settings["loc"]["f_name"]) && $this->settings["loc"]["f_name"] == "Y") {
        ...
        if ($this->settings["catalog"]["detail_picture"]) {
            ...
            foreach ($arSelPic as $sel) {
                ...
                */
                ...
                $src = $this->parseCaralogFilterSrc($src);
                $src = $this->getCatalogLink($src);
                foreach (GetModuleEvents("sota.parser", "ParserDetailPicture", true) as $arEvent) //27.10.2015
    ...
    public function parseCatalogPreviewPicturePreview(&$el) //27.10.2015
    {
        ...
        if ($this->settings["catalog"]["preview_picture"] && $this->settings["catalog"]["img_preview_from_detail"] != "Y") {
            ...
            $src = $this->parseCaralogFilterSrc($src);
            $src = $this->getCatalogLink($src);
            foreach (GetModuleEvents("sota.parser", "ParserPreviewPicture", true) as $arEvent) //27.10.2015
                ExecuteModuleEventEx($arEvent, array(&$this, $src));
            //$src = str_replace("cdn.", "", $src);
    ...
    protected function createAlbum()
    {
        ...
        CMedialib::Init();
        $arCollections = CMedialibCollection::GetList(array('arOrder' => Array('NAME' => 'ASC'), 'arFilter' => array('ACTIVE' => 'Y', "NAME" => "sota_PARSER")));
        if (!$arCollections) {
            $this->albumID = CMedialibCollection::Edit(array("arFields" => array("NAME" => "sota_PARSER")));
        } else
    ...
    protected function ClearAjaxFiles()
    {
        if (!$this->agent && $_GET["begin"]) {
            ...
            if (file_exists($_SERVER["DOCUMENT_ROOT"] . BX_ROOT . "/modules/sota.parser/include/loc" . $this->id . ".txt")) unlink($_SERVER["DOCUMENT_ROOT"] . BX_ROOT . "/modules/sota.parser/include/loc" . $this->id . ".txt");
            if (file_exists($_SERVER["DOCUMENT_ROOT"] . BX_ROOT . "/modules/sota.parser/include/count_parser_section" . $this->id . ".txt")) unlink($_SERVER["DOCUMENT_ROOT"] . BX_ROOT . "/modules/sota.parser/include/count_parser_section" . $this->id . ".txt");
    ...
    protected function checkActionBegin()
    {
        //if($this->updateActive && isset($this->settings["catalog"]["uniq"]["action"]) && $this->settings["catalog"]["uniq"]["action"]!="N")
        {
            if ((!$this->agent && $_GET["begin"]) || $this->agent) {
                ...
                if ($this->tmp == "b_sota_parser_tmp" || !$this->tmp) {
                    $rsData = SotaParserTmpTable::GetList($arr);

                    while ($arData = $rsData->Fetch()) {
                        SotaParserTmpTable::Delete($arData["ID"]);
                    }
                } elseif ($this->tmp == "b_sota_parser_tmp_old") {
                    $rsData = SotaParserTmpOldTable::GetList($arr);

                    while ($arData = $rsData->Fetch()) {
                        SotaParserTmpOldTable::Delete($arData["ID"]);
                    }
                }
    ...
    protected function checkActionAgent($agent = true)
    {
        if ((($this->agent && $agent) || !$agent) && $this->updateActive && isset($this->settings["catalog"]["uniq"]["action"]) && $this->settings["catalog"]["uniq"]["action"] != "N") {
            ...
            if ($this->tmp == "b_sota_parser_tmp" || !$this->tmp) {
                ...
                $rsDataOld = SotaParserTmpOldTable::GetList($arr);
                ...
                $parser = new SotaParserContent();
                $arFields['TMP'] = "b_sota_parser_tmp_old";
                ...
                $rsDataOld = SotaParserTmpTable::GetList($arr);
                ...
                $parser = new SotaParserContent();
                $arFields['TMP'] = "b_sota_parser_tmp";
                $parser->Update($this->id, $arFields);
                unset($parser);
            }
        }
    ...
    protected function addTmp($ID)
    {
        if (/*!$this->debug && */
            ...
            if ($this->tmp == "b_sota_parser_tmp" || !$this->tmp) {
                SotaParserTmpTable::add($arFields);
            } elseif ($this->tmp == "b_sota_parser_tmp_old") {
                SotaParserTmpOldTable::add($arFields);
            }
        }
    }
    ...
    protected function locText($text = "", $format = "plain", $test = false)
    {
        ...
        if (isset($this->settings["loc"]["type"]) && $this->settings["loc"]["type"] && $text) {
            if ($this->settings["loc"]["type"] == "yandex") {
                ...
                $arrorLoc[401] = GetMessage("sota_parser_loc_401");
                $arrorLoc[402] = GetMessage("sota_parser_loc_402");
                $arrorLoc[403] = GetMessage("sota_parser_loc_403");
                $arrorLoc[404] = GetMessage("sota_parser_loc_404");
                $arrorLoc[413] = GetMessage("sota_parser_loc_413");
                $arrorLoc[422] = GetMessage("sota_parser_loc_422");
                $arrorLoc[501] = GetMessage("sota_parser_loc_501");
                $data = curl_exec($ch);
                $xml = new CDataXML();
                $xml->LoadString($data);
                $arData = $xml->GetArray();
