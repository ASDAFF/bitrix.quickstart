...
class CCatalogProduct extends CAllCatalogProduct
{
	...
	function GetByID($ID)
	{
		...
		{
			$strSql =
				"SELECT ID, QUANTITY, IF (QUANTITY_TRACE = 'D', '".$DB->ForSql(COption::GetOptionString('catalog','default_quantity_trace','N'))."', QUANTITY_TRACE) as QUANTITY_TRACE, WEIGHT, PRICE_TYPE, RECUR_SCHEME_TYPE, RECUR_SCHEME_LENGTH, ".
				"	VAT_ID, VAT_INCLUDED, ".
				"   IF (CAN_BUY_ZERO = 'D', '".$DB->ForSql(COption::GetOptionString('catalog','default_can_buy_zero','N'))."', CAN_BUY_ZERO) as CAN_BUY_ZERO, ".
				"   QUANTITY_TRACE as QUANTITY_TRACE_ORIG, ".
				"   CAN_BUY_ZERO as CAN_BUY_ZERO_ORIG, ".
				"   NEGATIVE_AMOUNT_TRACE as NEGATIVE_AMOUNT_TRACE_ORIG, ".
	...
	function GetListEx($arOrder=Array("SORT"=>"ASC"), $arFilter=Array())
	{
		...
		if (!$USER->IsAdmin())
		{
			$strSql =
				...
				"	CAP.QUANTITY, IF (CAP.QUANTITY_TRACE = 'D', '".$DB->ForSql(COption::GetOptionString('catalog','default_quantity_trace','N'))."', CAP.QUANTITY_TRACE) as QUANTITY_TRACE, CAP.WEIGHT, ".
