...
<? if (!empty($arResult["ORDER"])): ?>
    ...
    //ONLINE PAY (SBERBANK)
    $sberBank = new Collected\Sberbank\PayHandler(array(
        'username' => 'USERNAME-api', //username of api
        'password' => 'PASSWORD', //password of api
        'orderId' => $arResult["ORDER"]['ID'], //created order id
        'returnUrl' => \Collected\Common\Helper::getFullUrl($APPLICATION->GetCurPageParam()), //return url (this page by default)
    ...
    //\Collected\Seo\ECommerce::get()->setIblockId(2)->load($arResult['ORDER']['ID'])->setViewContent();
