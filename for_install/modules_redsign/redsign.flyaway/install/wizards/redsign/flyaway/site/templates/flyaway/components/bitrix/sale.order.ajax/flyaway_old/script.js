...
function adjustFloatBasket() {
    var maxBasketHeight = $(".float-basket").parent().outerHeight() - 100,
        hiddenStartWith = -1;

    $(".float-basket__itemlist .js-fb-item").show().each(function(key, item) {
        var $item = $(item);
        if((maxBasketHeight - $item.outerHeight()) > 0) {
              maxBasketHeight -= $item.outerHeight();
        } else {
            hiddenStartWith = key;
            return false;
        }
    ...
    if(hiddenStartWith !== -1) {
        $(".float-basket__itemlist .js-fb-item").slice(hiddenStartWith).hide();
    }
}

function scrollFloatBasket() {
    var scrollTop = $(document).scrollTop(),
        $floatBasket = $(".float-basket"),
        maxOffset = $floatBasket.parent().height() - $floatBasket.outerHeight(),
        stickyHeaderHeight = $('.is-sticky .js-sticky-header').height();;

    var offset = scrollTop + 5 - $floatBasket.parent().offset().top;
    ...
    if(offset < 0) {
        $floatBasket.css("transform", "translateY(0px)");
        $(".float-basket-order__arrow").css("transform", "translateY(0px)");
        return;
    }
    ...
    $floatBasket.css("transform", "translateY(" + offset + "px)");
    $(".float-basket-order__arrow").css("transform", "translateY(" + offset + "px)");
}
$(document).ready(function() {
    adjustFloatBasket();

    $(document).scroll(scrollFloatBasket);

    $(window).resize(debounce(adjustFloatBasket, 250, false));

    $(document).on("shown.bs.collapse hidden.bs.collapse", "#order_form", function(e) {
        adjustFloatBasket();
        scrollFloatBasket();
    });

    $(document).on("show.bs.collapse hide.bs.collapse", "#order_form", function(e) {
        var currentTranslateY = parseInt($(".float-basket").css('transform').split(',')[5], 10),
            newTranslateY = 0;

        if(currentTranslateY > 0) {
            newTranslateY = currentTranslateY - $(e.target).outerHeight();
            if(newTranslateY > 0) {
                $(".float-basket").css('transform', "translateY(" + newTranslateY + "px)");
				$(".float-basket-order__arrow").css("transform", "translateY(" + newTranslateY + "px)");
            } else {
                $(".float-basket").css('transform', "translateY(0px)");
            }
        }
