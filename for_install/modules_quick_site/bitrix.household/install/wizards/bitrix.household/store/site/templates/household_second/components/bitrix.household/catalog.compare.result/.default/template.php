...
?>
		...
		<?if($itemsCnt > 1):?>
			<table class="compare" cellspacing="0" cellpadding="0" border="0" style="width:<?=($itemsCnt*25 + 25)?>%; table-layout: fixed;">
		<?else:?>
			<table class="compare" cellspacing="0" cellpadding="0" border="0">
				<col />
				<col span="<?=$itemsCnt?>"/>
		<?endif;?>
			<thead>
...
?>
			...
			<?if($itemsCnt > 1):?>
			<table class="r" cellspacing="0" cellpadding="0" border="0" style="width:<?=($itemsCnt*25 + 25)?>%; table-layout: fixed;">
			<?else:?>
			<table class="r" cellspacing="0" cellpadding="0" border="0">
			<?endif;?>
			<tbody>
			...
			?>
							<tr class="compare_price">
								<td class="pp"><?=$arResult["PRICES"][$code]["TITLE"]?></td>
			<?
					foreach($arResult["ITEMS"] as $arElement):
...
if(strlen($delUrlID) > 0)
{
	...
	?><br/><p><a href="<?=$delUrl?>"><?=GetMessage("CATALOG_DELETE_ALL")?></a></p><?
}
?>
