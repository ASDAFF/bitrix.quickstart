...
<b><?if($arResult["ALLOW_ANONYMOUS"]=="Y"):?>
...
<?if($arResult["ALLOW_ANONYMOUS"]=="Y" && $_REQUEST["authorize"]<>"YES" && $_REQUEST["register"]<>"YES"):?>
	...
	<tr valign="top">
		...
		<td width="50%" style="padding-left: 20px;"><?echo GetMessage("adm_reg_text")?></td>
	</tr>
	</table>
	<br />
		...
		<tr valign="top">
			<td width="50%" style="padding-right: 20px;">
				<form action="<?=$arResult["FORM_ACTION"]?>" method="post">
					...
					<b><?echo GetMessage("adm_auth_exist")?></b>
					...
					<input type="submit" name="Save" value="<?echo GetMessage("adm_auth_butt")?>" />
			...
			<td width="50%" style="padding-left: 20px;">
				<? if($arResult["ALLOW_REGISTER"]=="Y"): ?>
					<form action="<?=$arResult["FORM_ACTION"]?>" method="post">
						<?echo bitrix_sessid_post();?>
						<b><?echo GetMessage("adm_reg_new")?></b>
						...
						<? endif; ?>
						<input type="submit" name="Save" value="<?echo GetMessage("adm_reg_butt")?>" />
						<?foreach($arResult["RUBRICS"] as $itemID => $itemValue):?>
							<input type="hidden" name="RUB_ID[]" value="<?=$itemValue["ID"]?>">
						<?endforeach;?>
						<input type="hidden" name="PostAction" value="<?echo ($arResult["ID"]>0? "Update":"Add")?>" />
