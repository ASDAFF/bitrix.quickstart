	...
	function html($type, $data){
		...
		switch($type){
				...
				<div <?=$attrs?>>
					...
					<? if($zoomy){ ?>
						<div class="zoomy js-zoomy-pane">
							<img class="img js-big-img" src="<?=$image['src'];?>" alt="<?=$this->get('name')?>, артикул <?=$this->get('article')?>" data-imgs="<?=implode(";",$src)?>">
						</div>
					<? } ?>
						...
						<? } ?>
                                ...
                                <img class="product_image_original" src="<?=$this->get($size.'-image', 'src')?>" alt="анонс <?=$this->get('name')?>, артикул <?=$this->get('article')?>">
                                <img class="product_image_hover" src="<?=$this->get($size.'-image', 'src')?>" alt="анонс <?=$this->get('name')?>, артикул <?=$this->get('article')?>">
			...
			case 'rating':
				...
				?>
				<div class="product_assessment_block <?=isset($data['class']) ? $data['class'] : ''?> <?=\MHT::voted($this->get('id')) ? 'voted' : ''?>" data-id="<?=$this->get('id')?>"><div class="product_assessment" style="width:<?=$this->get('rating')?>%"></div></div>
				<?/*
				        	if($data['people']){
...
<!--
						...
						<? if($canBuy){ ?>
							...
							<div class="product_prices_block product_prices_block__mod">
								...
								?>
								<div class="product_price_block">
									<div class="product_price product_price__mod"><span class="product_price_value"><?=$price?></span><span class="rub"><span>рублей</span></span></div>
								</div>
							</div>
						<? } else { ?>
			...
			case 'favorites':
				?><div class="product" <?=$this->get('ids-attr')?>>
				<a href="#" class="remove" data-id="<?=$this->get('buy-id')?>">&times;</a>
				<div class="product_brand js-fit-1"><?=$this->get('name')?></div>
				<div class="product_description js-fit-2"><?=$this->get('brand')?></div>
				...
				<div class="product_prices_block">
					<div class="product_price_block">
						<div class="product_price"><span class="product_price_value"><?=$this->get('price')?></span><span class="rub"><span>рублей</span></span></div><a class="product_cart" href="<?=$this->get('buy')?>" onmousedown="try { rrApi.addToBasket(<?=$this->get('id')?>) } catch(e) {}"></a>
					</div>
				</div>
				</div><?
				break;
