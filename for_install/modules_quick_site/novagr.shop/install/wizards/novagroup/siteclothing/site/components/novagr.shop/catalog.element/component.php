...
if ($page404flag == true) {
    ...
    if ( $this -> StartResultCache(false, $USER->GetGroups()))
    {
        ...
        if(CModule::IncludeModule("novagr.shop"))
        {
            ...
            $content = unserialize(COption::GetOptionString("novagroup","comments"));
        ...
        if (is_array($arResult["CURRENT_ELEMENT"]["COLORS"])) {
            ...
            $rsElement = CIBlockElement::GetList(
                false,
                array("ID"=>$arResult["CURRENT_ELEMENT"]["COLORS"]),
                false, false, $arSelect
            );
            $CURRENT_ELEMENT_COLORS = array();
            while ($arElementColor = $rsElement -> Fetch())
            {
                $CURRENT_ELEMENT_COLORS[$arElementColor["ID"]] = $arElementColor['SORT'];
            }
        ...
        $arResult['CACHE_DATA'] = array(
            ...
            "arCurPrice" => $arResult["CUR_PRICE_ARR"],
    ...
    $arResult["CUR_PRICE_ARR"] = $arResult['CACHE_DATA']["arCurPrice"];
    $arResult["CUR_PRICE_CODE"] = $arResult['CACHE_DATA']["arCurPrice"]["NAME"];
    ...
    if(($_REQUEST['CAJAX'] != 1) && !empty($arElement['NAME'])){
        // keep information about fdfdwhat the product was viewed
        $arFields = array(
