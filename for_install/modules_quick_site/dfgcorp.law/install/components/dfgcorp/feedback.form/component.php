...
{
	$arParams["UCID"] = getImageUploaderId();

	$arResult["ELEMENT_ADD"] = false;
	...
	if ($bAllowAccess)
	{
		...
		if (check_bitrix_sessid() && 
			(!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])) &&
			$_REQUEST["UCID"]==$arParams["UCID"])
		{
            ...
            if(isset($_REQUEST["name"])){
                $arProperties["NAME"][0] = $_REQUEST["name"];
            }
			...
			if (count($arResult["ERRORS"]) == 0)
			{
				...
				else
				{
					...
					$arUpdateValues["PROPERTY_VALUES"][$arParams["HIDE_PROPERTY_ID"]] = $arParams["HIDE_PROPERTY_VALUE"];
			...
			if (count($arResult["ERRORS"]) == 0)
			{
				...
				$sRedirectUrl = $APPLICATION->GetCurPageParam("send_request=" . $arParams["UCID"], array("add", "send_request"));
                ...
                $arEventFields["TEXT"] = $arUpdateValues["PREVIEW_TEXT"];

                $arEventFields["PAGE"] = $APPLICATION->GetTitle();
                $arEventFields["ADDRESS_PAGE"] = 'http://'.$arSite["SERVER_NAME"] .  $APPLICATION->GetCurPage();//page_title_to_send
				
				$arEventFields["KEYWORD"] = iconv("UTF-8", "WINDOWS-1251", $_REQUEST["utm_term"]);
                $arEventFields["SOURCE"] = iconv("UTF-8", "WINDOWS-1251", $_REQUEST["utm_source"]);
                $arEventFields["COMPANY"] = iconv("UTF-8", "WINDOWS-1251", $_REQUEST["utm_campaign"]);
				
				foreach($arUpdatePropertyValues as $pid=>$prop){
				    if($arResult["PROPERTY_LIST_FULL"][$pid]["PROPERTY_TYPE"]=="F"){
                        $rsFileProp = CIBlockElement::GetProperty($arParams["IBLOCK_ID"], 
                                                $arParams["ID"], "sort", 
                                                "asc", 
                                                Array("CODE" => 
                                                    $arResult["PROPERTY_LIST_FULL"][$pid]["CODE"]));
                        if ($arFileProp = $rsFileProp->Fetch()){
                            if(intval($arFileProp["VALUE"]) > 0)
                                $arEventFields[$arResult["PROPERTY_LIST_FULL"][$pid]["CODE"]] = 'http://'.$arSite["SERVER_NAME"] . CFile::GetPath($arFileProp["VALUE"]);
                        }else{
                            $arEventFields[$arResult["PROPERTY_LIST_FULL"][$pid]["CODE"]] = '';
                        }
				    }else if($arResult["PROPERTY_LIST_FULL"][$pid]["PROPERTY_TYPE"]=="L"){
				        $prop = '';
						$rsFileProp = CIBlockElement::GetProperty($arParams["IBLOCK_ID"], 
															$arParams["ID"], "sort",  "asc", 
															Array("CODE" =>$arResult["PROPERTY_LIST_FULL"][$pid]["CODE"]));
						$prop_counter = 0;
						while ($arFileProp = $rsFileProp->Fetch()){
							if($prop_counter > 0) $prop .= ' / ';
							$prop .= $arFileProp["VALUE_ENUM"];
							$prop_counter++;
						}
						$arEventFields[$arResult["PROPERTY_LIST_FULL"][$pid]["CODE"]] = $prop;
				    }else{
						$arEventFields[$arResult["PROPERTY_LIST_FULL"][$pid]["CODE"]] = $prop;
					}
                }
                ...
                if(is_array($arParams["EVENT_MESSAGE_ID"]) && count($arParams["EVENT_MESSAGE_ID"])>0){
                    foreach($arParams["EVENT_MESSAGE_ID"] as $ev){
                        $rsEM = CEventMessage::GetByID(intval($ev));
                        if($arEM = $rsEM->Fetch()){
							$arEventFields["EMAIL_FROM"] = 
								(strlen($arEM["EMAIL_FROM"])>0? $arEM["EMAIL_FROM"] : 
									COption::GetOptionString("main", "email_from", ""));
							$arEventFields["EMAIL_TO"] = 
								(strlen($arEM["EMAIL_TO"])>0? $arEM["EMAIL_TO"] : 
									COption::GetOptionString("main", "email_from", ""));
                            CEvent::Send($arEM["EVENT_NAME"], $arSite, $arEventFields);
                        }
                    }
                }elseif(intval($arParams["EVENT_MESSAGE_ID"])>0){
					echo $arParams["EVENT_MESSAGE_ID"];
					$rsEM = CEventMessage::GetByID(intval($arParams["EVENT_MESSAGE_ID"]));
					if($arEM = $rsEM->Fetch()){
						$arEventFields["EMAIL_FROM"] = 
							(strlen($arEM["EMAIL_FROM"])>0? $arEM["EMAIL_FROM"] : 
								COption::GetOptionString("main", "email_from", ""));
		...
		if ($bShowForm)
		{
			...
			$arResult["ELEMENT_ADD_UCID"] = $_REQUEST["send_request"];
