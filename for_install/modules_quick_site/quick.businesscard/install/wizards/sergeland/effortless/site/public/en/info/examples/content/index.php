<?
require($_SERVER["DOCUMENT_ROOT"]."/bitrix/header.php");
$APPLICATION->SetTitle("Content");
?>
<h1 class="page-title">Examples of contents of pages on the site</h1>
<div class="text-muted">For your convenience, the examples are divided into categories</div>
<div class="space-bottom"></div>

<h2 class="mb-30">Horizontal Tabs - Version 1</h2>
<ul class="nav nav-pills white" role="tablist">
	<li class="active"><a href="#htab1" role="tab" data-toggle="tab"><i class="fa fa-home pr-5"></i>Very Flexible</a></li>
	<li><a href="#htab2" role="tab" data-toggle="tab"><i class="fa fa-user pr-5"></i>Top Notch 24/7 Support</a></li>
	<li><a href="#htab3" role="tab" data-toggle="tab"><i class="fa fa-envelope pr-5"></i>Well Documented</a></li>
</ul>
<div class="tab-content">
	<div class="tab-pane fade in active" id="htab1">
		<h1 class="text-center">Very Flexible</h1>
		<div class="space-bottom"></div>
		<div class="row">
			<div class="col-md-6">
				<ul class="list-icons">
					<li><i class="icon-check pr-10"></i> Lorem ipsum dolor sit amet maiores</li>
					<li><i class="icon-check pr-10"></i> Consectetur adipisicing elit</li>
					<li><i class="icon-check pr-10"></i> Aut earum numquam hic placeat</li>
					<li><i class="icon-check pr-10"></i> Fugiat rem consequatur neque</li>
					<li><i class="icon-check pr-10"></i> Expedita placeat omnis pariatur</li>
				</ul>
			</div>
			<div class="col-md-6">
				<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Facere hic qui non placeat ad explicabo dignissimos amet iusto veniam!</p>
				<p>Quo expedita tempore modi minima at adipisci saepe excepturi alias consequuntur sunt asperiores enim ut assumenda hic vitae, odio deleniti illo, veniam quas!</p>
			</div>
		</div>
		<a href="#" class="btn btn-default">Read more</a>
	</div>
	<div class="tab-pane fade" id="htab2">
		<h1 class="text-center">Top Notch 24/7 Support</h1>
		<div class="space-bottom"></div>
		<div class="row">
			<div class="col-md-6">
				<ul class="list-icons">
					<li><i class="icon-check pr-10"></i> Lorem ipsum dolor sit amet maiores</li>
					<li><i class="icon-check pr-10"></i> Consectetur adipisicing elit</li>
					<li><i class="icon-check pr-10"></i> Aut earum numquam hic placeat</li>
					<li><i class="icon-check pr-10"></i> Fugiat rem consequatur neque</li>
					<li><i class="icon-check pr-10"></i> Expedita placeat omnis pariatur</li>
				</ul>
			</div>
			<div class="col-md-6">
				<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Facere hic qui non placeat ad explicabo dignissimos amet iusto veniam!</p>
				<p>Quo expedita tempore modi minima at adipisci saepe excepturi alias consequuntur sunt asperiores enim ut assumenda hic vitae, odio deleniti illo, veniam quas!</p>
			</div>
		</div>
		<a href="#" class="btn btn-default">Read more</a>
	</div>
	<div class="tab-pane fade" id="htab3">
		<h1 class="text-center">Well Documented</h1>
		<div class="space-bottom"></div>
		<div class="row">
			<div class="col-md-6">
				<ul class="list-icons">
					<li><i class="icon-check pr-10"></i> Lorem ipsum dolor sit amet maiores</li>
					<li><i class="icon-check pr-10"></i> Consectetur adipisicing elit</li>
					<li><i class="icon-check pr-10"></i> Aut earum numquam hic placeat</li>
					<li><i class="icon-check pr-10"></i> Fugiat rem consequatur neque</li>
					<li><i class="icon-check pr-10"></i> Expedita placeat omnis pariatur</li>
				</ul>
			</div>
			<div class="col-md-6">
				<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Facere hic qui non placeat ad explicabo dignissimos amet iusto veniam!</p>
				<p>Quo expedita tempore modi minima at adipisci saepe excepturi alias consequuntur sunt asperiores enim ut assumenda hic vitae, odio deleniti illo, veniam quas!</p>
			</div>
		</div>
		<a href="#" class="btn btn-default">Read more</a>
	</div>
</div>
<div class="space"></div>

<h2 class="mb-40">Horizontal Tabs - Version 2</h2>
<div class="tabs-style-2">
	<ul class="nav nav-tabs" role="tablist">
		<li class="active"><a href="#h2tab1" role="tab" data-toggle="tab"><i class="fa fa-home pr-5"></i>Very Flexible</a></li>
		<li><a href="#h2tab2" role="tab" data-toggle="tab"><i class="fa fa-user pr-5"></i>Top Notch 24/7 Support</a></li>
		<li><a href="#h2tab3" role="tab" data-toggle="tab"><i class="fa fa-envelope pr-5"></i>Well Documented</a></li>
	</ul>
	<div class="tab-content">
		<div class="tab-pane fade in active" id="h2tab1">
			<h1 class="text-center title">Very Flexible</h1>
			<div class="space-bottom"></div>
			<div class="row">
				<div class="col-md-6">
					<ul class="list-icons">
						<li><i class="icon-check pr-10"></i> Lorem ipsum dolor sit amet maiores</li>
						<li><i class="icon-check pr-10"></i> Consectetur adipisicing elit</li>
						<li><i class="icon-check pr-10"></i> Aut earum numquam hic placeat</li>
						<li><i class="icon-check pr-10"></i> Fugiat rem consequatur neque</li>
						<li><i class="icon-check pr-10"></i> Expedita placeat omnis pariatur</li>
					</ul>
				</div>
				<div class="col-md-6">
					<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Facere hic qui non placeat ad explicabo dignissimos amet iusto veniam!</p>
					<p>Quo expedita tempore modi minima at adipisci saepe excepturi alias consequuntur sunt asperiores enim ut assumenda hic vitae, odio deleniti illo, veniam quas!</p>
				</div>
			</div>
			<a href="#" class="btn btn-default">Read more</a>
		</div>
		<div class="tab-pane fade" id="h2tab2">
			<h1 class="text-center title">Top Notch 24/7 Support</h1>
			<div class="space-bottom"></div>
			<div class="row">
				<div class="col-md-6">
					<ul class="list-icons">
						<li><i class="icon-check pr-10"></i> Lorem ipsum dolor sit amet maiores</li>
						<li><i class="icon-check pr-10"></i> Consectetur adipisicing elit</li>
						<li><i class="icon-check pr-10"></i> Aut earum numquam hic placeat</li>
						<li><i class="icon-check pr-10"></i> Fugiat rem consequatur neque</li>
						<li><i class="icon-check pr-10"></i> Expedita placeat omnis pariatur</li>
					</ul>
				</div>
				<div class="col-md-6">
					<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Facere hic qui non placeat ad explicabo dignissimos amet iusto veniam!</p>
					<p>Quo expedita tempore modi minima at adipisci saepe excepturi alias consequuntur sunt asperiores enim ut assumenda hic vitae, odio deleniti illo, veniam quas!</p>
				</div>
			</div>
			<a href="#" class="btn btn-default">Read more</a>
		</div>
		<div class="tab-pane fade" id="h2tab3">
			<h1 class="text-center title">Well Documented</h1>
			<div class="space-bottom"></div>
			<div class="row">
				<div class="col-md-6">
					<ul class="list-icons">
						<li><i class="icon-check pr-10"></i> Lorem ipsum dolor sit amet maiores</li>
						<li><i class="icon-check pr-10"></i> Consectetur adipisicing elit</li>
						<li><i class="icon-check pr-10"></i> Aut earum numquam hic placeat</li>
						<li><i class="icon-check pr-10"></i> Fugiat rem consequatur neque</li>
						<li><i class="icon-check pr-10"></i> Expedita placeat omnis pariatur</li>
					</ul>
				</div>
				<div class="col-md-6">
					<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Facere hic qui non placeat ad explicabo dignissimos amet iusto veniam!</p>
					<p>Quo expedita tempore modi minima at adipisci saepe excepturi alias consequuntur sunt asperiores enim ut assumenda hic vitae, odio deleniti illo, veniam quas!</p>
				</div>
			</div>
			<a href="#" class="btn btn-default">Read more</a>
		</div>
	</div>
</div>
<div class="space-bottom"></div>

<h2 class="mb-30">Vertical Tabs</h2>
<div class="vertical">
	<ul class="nav nav-tabs" role="tablist">
		<li class="active"><a href="#vtab1" role="tab" data-toggle="tab"><i class="fa fa-magic pr-10"></i> Very Flexible</a></li>
		<li><a href="#vtab2" role="tab" data-toggle="tab"><i class="fa fa-life-saver pr-10"></i> Top Notch 24/7 Support</a></li>
		<li><a href="#vtab3" role="tab" data-toggle="tab"><i class="fa fa-expand pr-10"></i> 100% Responsive</a></li>
		<li><a href="#vtab4" role="tab" data-toggle="tab"><i class="fa fa-file-o pr-10"></i> Well Documented</a></li>
	</ul>
	<div class="tab-content">
		<div class="tab-pane fade in active" id="vtab1">
			<h1 class="text-center title">Very Flexible</h1>
			<ul class="list-icons">
				<li><i class="icon-check pr-10"></i> Lorem ipsum dolor sit amet maiores</li>
				<li><i class="icon-check pr-10"></i> Consectetur adipisicing elit</li>
				<li><i class="icon-check pr-10"></i> Aut earum numquam hic placeat</li>
				<li><i class="icon-check pr-10"></i> Fugiat rem consequatur neque</li>
			</ul>
			<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Facere hic qui non placeat ad explicabo dignissimos amet iusto veniam!</p>
			<p>Quo expedita tempore modi minima at adipisci saepe excepturi alias consequuntur sunt asperiores enim ut assumenda hic vitae, odio deleniti illo, veniam quas!</p>
			<a href="#" class="btn btn-default">Read more</a>
		</div>
		<div class="tab-pane fade" id="vtab2">
			<h1 class="text-center title">Top Notch 24/7 Support</h1>
			<ul class="list-icons">
				<li><i class="icon-check pr-10"></i> Lorem ipsum dolor sit amet maiores</li>
				<li><i class="icon-check pr-10"></i> Consectetur adipisicing elit</li>
				<li><i class="icon-check pr-10"></i> Aut earum numquam hic placeat</li>
				<li><i class="icon-check pr-10"></i> Fugiat rem consequatur neque</li>
			</ul>
			<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Facere hic qui non placeat ad explicabo dignissimos amet iusto veniam!</p>
			<p>Quo expedita tempore modi minima at adipisci saepe excepturi alias consequuntur sunt asperiores enim ut assumenda hic vitae, odio deleniti illo, veniam quas!</p>
			<a href="#" class="btn btn-default">Read more</a>
		</div>
		<div class="tab-pane fade" id="vtab3">
			<h1 class="text-center title">100% Responsive</h1>
			<ul class="list-icons">
				<li><i class="icon-check pr-10"></i> Lorem ipsum dolor sit amet maiores</li>
				<li><i class="icon-check pr-10"></i> Consectetur adipisicing elit</li>
				<li><i class="icon-check pr-10"></i> Aut earum numquam hic placeat</li>
				<li><i class="icon-check pr-10"></i> Fugiat rem consequatur neque</li>
			</ul>
			<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Facere hic qui non placeat ad explicabo dignissimos amet iusto veniam!</p>
			<p>Quo expedita tempore modi minima at adipisci saepe excepturi alias consequuntur sunt asperiores enim ut assumenda hic vitae, odio deleniti illo, veniam quas!</p>
			<a href="#" class="btn btn-default">Read more</a>
		</div>
		<div class="tab-pane fade" id="vtab4">
			<h1 class="text-center title">Well Documented</h1>
			<ul class="list-icons">
				<li><i class="icon-check pr-10"></i> Lorem ipsum dolor sit amet maiores</li>
				<li><i class="icon-check pr-10"></i> Consectetur adipisicing elit</li>
				<li><i class="icon-check pr-10"></i> Aut earum numquam hic placeat</li>
				<li><i class="icon-check pr-10"></i> Fugiat rem consequatur neque</li>
			</ul>
			<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Facere hic qui non placeat ad explicabo dignissimos amet iusto veniam!</p>
			<p>Quo expedita tempore modi minima at adipisci saepe excepturi alias consequuntur sunt asperiores enim ut assumenda hic vitae, odio deleniti illo, veniam quas!</p>
			<a href="#" class="btn btn-default">Read more</a>
		</div>
	</div>
</div>
<div class="space-bottom"></div>

<h2>Vertical Tabs - Presentation</h2>
<div class="vertical hc-tabs">
	<div class="hc-tabs-top">
		<img data-tab="#hctab1" src="#SITE_DIR#images/content.jpg" alt="Effortless" data-tab-animation-effect="fadeInRightSmall">
		<img data-tab="#hctab2" src="#SITE_DIR#images/content.jpg" alt="Effortless" data-tab-animation-effect="fadeInRightSmall">
		<img data-tab="#hctab3" src="#SITE_DIR#images/content.jpg" alt="Effortless" data-tab-animation-effect="fadeInRightSmall">
		<img data-tab="#hctab4" src="#SITE_DIR#images/content.jpg" alt="Effortless" data-tab-animation-effect="fadeInRightSmall">
		<div class="space-bottom"></div>
	</div>	
	<div class="arrow hidden-sm hidden-xs"><i class="fa fa-caret-up"></i></div>	
	<ul class="nav nav-tabs" role="tablist">
		<li class="active"><a href="#hctab1" role="tab" data-toggle="tab"><i class="fa fa-magic pr-10"></i> Very Flexible</a></li>
		<li><a href="#hctab2" role="tab" data-toggle="tab"><i class="fa fa-life-saver pr-10"></i> Top Notch 24/7 Support</a></li>
		<li><a href="#hctab3" role="tab" data-toggle="tab"><i class="fa fa-expand pr-10"></i> 100% Responsive</a></li>
		<li><a href="#hctab4" role="tab" data-toggle="tab"><i class="fa fa-file-o pr-10"></i> Well Documented</a></li>
	</ul>
	<div class="tab-content">
		<div class="tab-pane fade in active" id="hctab1">
			<h1 class="text-center title">Very Flexible</h1>
			<ul class="list-icons">
				<li><i class="icon-check pr-10"></i> Lorem ipsum dolor sit amet maiores</li>
				<li><i class="icon-check pr-10"></i> Consectetur adipisicing elit</li>
				<li><i class="icon-check pr-10"></i> Aut earum numquam hic placeat</li>
				<li><i class="icon-check pr-10"></i> Fugiat rem consequatur neque</li>
			</ul>
			<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Facere hic qui non placeat ad explicabo dignissimos amet iusto veniam!</p>
			<p>Quo expedita tempore modi minima at adipisci saepe excepturi alias consequuntur sunt asperiores enim ut assumenda hic vitae, odio deleniti illo, veniam quas!</p>
			<a href="#" class="btn btn-default">Read more</a>
		</div>
		<div class="tab-pane fade" id="hctab2">
			<h1 class="text-center title">Top Notch 24/7 Support</h1>
			<ul class="list-icons">
				<li><i class="icon-check pr-10"></i> Lorem ipsum dolor sit amet maiores</li>
				<li><i class="icon-check pr-10"></i> Consectetur adipisicing elit</li>
				<li><i class="icon-check pr-10"></i> Aut earum numquam hic placeat</li>
				<li><i class="icon-check pr-10"></i> Fugiat rem consequatur neque</li>
			</ul>
			<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Facere hic qui non placeat ad explicabo dignissimos amet iusto veniam!</p>
			<p>Quo expedita tempore modi minima at adipisci saepe excepturi alias consequuntur sunt asperiores enim ut assumenda hic vitae, odio deleniti illo, veniam quas!</p>
			<a href="#" class="btn btn-default">Read more</a>
		</div>
		<div class="tab-pane fade" id="hctab3">
			<h1 class="text-center title">100% Responsive</h1>
			<ul class="list-icons">
				<li><i class="icon-check pr-10"></i> Lorem ipsum dolor sit amet maiores</li>
				<li><i class="icon-check pr-10"></i> Consectetur adipisicing elit</li>
				<li><i class="icon-check pr-10"></i> Aut earum numquam hic placeat</li>
				<li><i class="icon-check pr-10"></i> Fugiat rem consequatur neque</li>
			</ul>
			<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Facere hic qui non placeat ad explicabo dignissimos amet iusto veniam!</p>
			<p>Quo expedita tempore modi minima at adipisci saepe excepturi alias consequuntur sunt asperiores enim ut assumenda hic vitae, odio deleniti illo, veniam quas!</p>
			<a href="#" class="btn btn-default">Read more</a>
		</div>
		<div class="tab-pane fade" id="hctab4">
			<h1 class="text-center title">Well Documented</h1>
			<ul class="list-icons">
				<li><i class="icon-check pr-10"></i> Lorem ipsum dolor sit amet maiores</li>
				<li><i class="icon-check pr-10"></i> Consectetur adipisicing elit</li>
				<li><i class="icon-check pr-10"></i> Aut earum numquam hic placeat</li>
				<li><i class="icon-check pr-10"></i> Fugiat rem consequatur neque</li>
			</ul>
			<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Facere hic qui non placeat ad explicabo dignissimos amet iusto veniam!</p>
			<p>Quo expedita tempore modi minima at adipisci saepe excepturi alias consequuntur sunt asperiores enim ut assumenda hic vitae, odio deleniti illo, veniam quas!</p>
			<a href="#" class="btn btn-default">Read more</a>
		</div>
	</div>
</div>
<div class="space-bottom"></div>

<h2>Pills</h2>
<ul class="nav nav-pills white space-top" role="tablist">
	<li class="active"><a href="#pill-1" role="tab" data-toggle="tab" title="images"><i class="fa fa-camera pr-5"></i> Photo</a></li>
	<li><a href="#pill-2" role="tab" data-toggle="tab" title="video"><i class="fa fa-video-camera pr-5"></i> Video</a></li>
	<li><a href="#pill-3" role="tab" data-toggle="tab" title="text"><i class="fa fa-pencil pr-5"></i> Text</a></li>
</ul>
<div class="tab-content clear-style">
	<div class="tab-pane active" id="pill-1">
		<div class="owl-carousel content-slider-with-controls photo-block">
			<div class="overlay-container">
				<img src="#SITE_DIR#images/content.jpg" alt="">
				<a href="#SITE_DIR#images/content.jpg" class="popup-img overlay" title="image title"><i class="fa fa-search-plus"></i></a>
			</div>
			<div class="overlay-container">
				<img src="#SITE_DIR#images/content.jpg" alt="">
				<a href="#SITE_DIR#images/content.jpg" class="popup-img overlay" title="image title"><i class="fa fa-search-plus"></i></a>
			</div>
			<div class="overlay-container">
				<img src="#SITE_DIR#images/content.jpg" alt="">
				<a href="#SITE_DIR#images/content.jpg" class="popup-img overlay" title="image title"><i class="fa fa-search-plus"></i></a>
			</div>
		</div>
	</div>
	<div class="tab-pane" id="pill-2">
		<div class="embed-responsive embed-responsive-16by9">
			<iframe class="embed-responsive-item" src="https://www.youtube.com/embed/Fv8kevYUXjo?rel=0&showinfo=0&color=white&html5=1"></iframe>
		</div>
	</div>
	<div class="tab-pane" id="pill-3">
		<h3>Lorem ipsum dolor sit amet</h3>
		<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Sit, labore iste! Pariatur tempore, dicta voluptatibus quis blanditiis voluptates in. Molestiae asperiores sed, pariatur nesciunt saepe. Culpa ipsam ut enim reiciendis!</p>
		<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Debitis eaque nihil obcaecati minus perferendis officiis cumque nesciunt quae dolorem ad tenetur voluptate blanditiis labore libero id veritatis similique facere, fugiat soluta omnis minima? Iusto optio hic a reiciendis, inventore numquam impedit quaerat deserunt, voluptatum illum in magni, aspernatur corporis mollitia nam? Ipsum, cumque facilis recusandae commodi dolorum sequi quis possimus, aut, ab incidunt dolorem perferendis rerum tenetur. Labore exercitationem magni perspiciatis laborum, aliquam officia molestias obcaecati, voluptatum quidem eum iure aut, officiis dicta cupiditate ab, sit aliquid. Corporis ducimus ab commodi officia? Inventore suscipit saepe dolorem vitae aperiam autem, iusto id, enim assumenda modi a voluptate aliquam molestiae quidem sapiente, obcaecati recusandae. Magni repellat dignissimos sint iste dolorem maxime accusantium quas repellendus quam ipsa, sapiente, beatae enim ab ex. Optio eveniet necessitatibus, tempora dolor eaque saepe, ipsam vero at officiis cumque, maxime consequuntur in ea, voluptate quae nesciunt hic delectus.</p>
	</div>
</div>
<div class="space-bottom"></div>

<h2>Pills - Process</h2>
<div class="process">
	<ul class="nav nav-pills white space-top" role="tablist">
		<li class="active"><a href="#pill-pr-1" role="tab" data-toggle="tab" title="Step 1"><i class="fa fa-dot-circle-o pr-5"></i> Step 1</a></li>
		<li><a href="#pill-pr-2" role="tab" data-toggle="tab" title="Step 2"><i class="fa fa-dot-circle-o pr-5"></i> Step 2</a></li>
		<li><a href="#pill-pr-3" role="tab" data-toggle="tab" title="Step 3"><i class="fa fa-dot-circle-o pr-5"></i> Step 3</a></li>
	</ul>
	<div class="tab-content clear-style">
		<div class="tab-pane active" id="pill-pr-1">
			<h3>Lorem ipsum dolor sit amet</h3>
			<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Sit, labore iste! Pariatur tempore, dicta voluptatibus quis blanditiis voluptates in. Molestiae asperiores sed, pariatur nesciunt saepe. Culpa ipsam ut enim reiciendis!</p>
			<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Debitis eaque nihil obcaecati minus perferendis officiis cumque nesciunt quae dolorem ad tenetur voluptate blanditiis labore libero id veritatis similique facere, fugiat soluta omnis minima? Iusto optio hic a reiciendis, inventore numquam impedit quaerat deserunt, voluptatum illum in magni, aspernatur corporis mollitia nam? Ipsum, cumque facilis recusandae commodi dolorum sequi quis possimus, aut, ab incidunt dolorem perferendis rerum tenetur. Labore exercitationem magni perspiciatis laborum, aliquam officia molestias obcaecati, voluptatum quidem eum iure aut, officiis dicta cupiditate ab, sit aliquid. Corporis ducimus ab commodi officia? Inventore suscipit saepe dolorem vitae aperiam autem, iusto id, enim assumenda modi a voluptate aliquam molestiae quidem sapiente, obcaecati recusandae. Magni repellat dignissimos sint iste dolorem maxime accusantium quas repellendus quam ipsa, sapiente, beatae enim ab ex. Optio eveniet necessitatibus, tempora dolor eaque saepe, ipsam vero at officiis cumque, maxime consequuntur in ea, voluptate quae nesciunt hic delectus.</p>
		</div>
		<div class="tab-pane" id="pill-pr-2">
			<h3>Consectetur adipisicing elit</h3>
			<p>Ipsum dolor sit amet, consectetur adipisicing elit. Sit, labore iste! Pariatur tempore, dicta voluptatibus quis blanditiis voluptates in. Molestiae asperiores sed, pariatur nesciunt saepe. Culpa ipsam ut enim reiciendis!</p>
			<div class="embed-responsive embed-responsive-16by9">
				<iframe class="embed-responsive-item" src="https://www.youtube.com/embed/Fv8kevYUXjo?rel=0&showinfo=0&color=white&html5=1"></iframe>
			</div>
		</div>
		<div class="tab-pane" id="pill-pr-3">
			<h3>Dolor lorem ipsum sit amet</h3>
			<p>Consectetur adipisicing elit. Sit, labore iste! Pariatur tempore, dicta voluptatibus quis blanditiis voluptates in. Molestiae asperiores sed, pariatur nesciunt saepe. Culpa ipsam ut enim reiciendis!</p>
			<p>Ipsum dolor sit amet, consectetur adipisicing elit. Debitis eaque nihil obcaecati minus perferendis officiis cumque nesciunt quae dolorem ad tenetur voluptate blanditiis labore libero id veritatis similique facere, fugiat soluta omnis minima? Iusto optio hic a reiciendis, inventore numquam impedit quaerat deserunt, voluptatum illum in magni, aspernatur corporis mollitia nam? Ipsum, cumque facilis recusandae commodi dolorum sequi quis possimus, aut, ab incidunt dolorem perferendis rerum tenetur. Labore exercitationem magni perspiciatis laborum, aliquam officia molestias obcaecati, voluptatum quidem eum iure aut, officiis dicta cupiditate ab, sit aliquid. Corporis ducimus ab commodi officia? Inventore suscipit saepe dolorem vitae aperiam autem, iusto id, enim assumenda modi a voluptate aliquam molestiae quidem sapiente, obcaecati recusandae. Magni repellat dignissimos sint iste dolorem maxime accusantium quas repellendus quam ipsa, sapiente, beatae enim ab ex. Optio eveniet necessitatibus, tempora dolor eaque saepe, ipsam vero at officiis cumque, maxime consequuntur in ea, voluptate quae nesciunt hic delectus.</p>
		</div>
	</div>
</div>
<div class="space-bottom"></div>

<h2>Default Color</h2>
<div class="panel-group" id="accordion">
	<div class="panel panel-default">
		<div class="panel-heading">
			<h4 class="panel-title">
				<a data-toggle="collapse" data-parent="#accordion" href="#collapseOne">
					<i class="fa fa-bold"></i>Bootstrap Based
				</a>
			</h4>
		</div>
		<div id="collapseOne" class="panel-collapse collapse in">
			<div class="panel-body">
				Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 3 wolf moon officia aute, non cupidatat skateboard dolor brunch. Food truck quinoa nesciunt laborum eiusmod. Brunch 3 wolf moon tempor, sunt aliqua put a bird on it squid single-origin coffee nulla assumenda shoreditch et. Nihil anim keffiyeh helvetica, craft beer labore wes anderson cred nesciunt sapiente ea proident.
			</div>
		</div>
	</div>
	<div class="panel panel-default">
		<div class="panel-heading">
			<h4 class="panel-title">
				<a data-toggle="collapse" data-parent="#accordion" href="#collapseTwo" class="collapsed">
					<i class="fa fa-leaf"></i>Clean &amp; powerful code
				</a>
			</h4>
		</div>
		<div id="collapseTwo" class="panel-collapse collapse">
			<div class="panel-body">
				Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 3 wolf moon officia aute, non cupidatat skateboard dolor brunch. Food truck quinoa nesciunt laborum eiusmod. Brunch 3 wolf moon tempor, sunt aliqua put a bird on it squid single-origin coffee nulla assumenda shoreditch et. Nihil anim keffiyeh helvetica, craft beer labore wes anderson cred nesciunt sapiente ea proident.
			</div>
		</div>
	</div>
	<div class="panel panel-default">
		<div class="panel-heading">
			<h4 class="panel-title">
				<a data-toggle="collapse" data-parent="#accordion" href="#collapseThree" class="collapsed">
					<i class="fa fa-html5"></i>HTML5/CSS3
				</a>
			</h4>
		</div>
		<div id="collapseThree" class="panel-collapse collapse">
			<div class="panel-body">
				Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 3 wolf moon officia aute, non cupidatat skateboard dolor brunch. Food truck quinoa nesciunt laborum eiusmod. Brunch 3 wolf moon tempor, sunt aliqua put a bird on it squid single-origin coffee nulla assumenda shoreditch et. Nihil anim keffiyeh helvetica, craft beer labore wes anderson cred nesciunt sapiente ea proident.
			</div>
		</div>
	</div>
</div>
<div class="alert alert-info alert-dismissible" role="alert">
	<button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
	The <strong>Default</strong> color is defined by the current color scheme.
</div>
<div class="space-bottom"></div>

<h2>Dark Color</h2>
<div class="panel-group panel-dark" id="accordion-2">
	<div class="panel panel-default">
		<div class="panel-heading">
			<h4 class="panel-title">
				<a data-toggle="collapse" data-parent="#accordion-2" href="#collapseOne-2">
					<i class="fa fa-bold"></i>Bootstrap Based
				</a>
			</h4>
		</div>
		<div id="collapseOne-2" class="panel-collapse collapse in">
			<div class="panel-body">
				Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 3 wolf moon officia aute, non cupidatat skateboard dolor brunch. Food truck quinoa nesciunt laborum eiusmod. Brunch 3 wolf moon tempor, sunt aliqua put a bird on it squid single-origin coffee nulla assumenda shoreditch et. Nihil anim keffiyeh helvetica, craft beer labore wes anderson cred nesciunt sapiente ea proident.
			</div>
		</div>
	</div>
	<div class="panel panel-default">
		<div class="panel-heading">
			<h4 class="panel-title">
				<a data-toggle="collapse" data-parent="#accordion-2" href="#collapseTwo-2" class="collapsed">
					<i class="fa fa-leaf"></i>Clean &amp; powerful code
				</a>
			</h4>
		</div>
		<div id="collapseTwo-2" class="panel-collapse collapse">
			<div class="panel-body">
				Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 3 wolf moon officia aute, non cupidatat skateboard dolor brunch. Food truck quinoa nesciunt laborum eiusmod. Brunch 3 wolf moon tempor, sunt aliqua put a bird on it squid single-origin coffee nulla assumenda shoreditch et. Nihil anim keffiyeh helvetica, craft beer labore wes anderson cred nesciunt sapiente ea proident.
			</div>
		</div>
	</div>
	<div class="panel panel-default">
		<div class="panel-heading">
			<h4 class="panel-title">
				<a data-toggle="collapse" data-parent="#accordion-2" href="#collapseThree-2" class="collapsed">
					<i class="fa fa-html5"></i>HTML5/CSS3
				</a>
			</h4>
		</div>
		<div id="collapseThree-2" class="panel-collapse collapse">
			<div class="panel-body">
				Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 3 wolf moon officia aute, non cupidatat skateboard dolor brunch. Food truck quinoa nesciunt laborum eiusmod. Brunch 3 wolf moon tempor, sunt aliqua put a bird on it squid single-origin coffee nulla assumenda shoreditch et. Nihil anim keffiyeh helvetica, craft beer labore wes anderson cred nesciunt sapiente ea proident.
			</div>
		</div>
	</div>
</div>
<div class="space-bottom"></div>

<h2>Transparent</h2>
<div class="panel-group panel-transparent" id="accordion-3">
	<div class="panel panel-default">
		<div class="panel-heading">
			<h4 class="panel-title">
				<a data-toggle="collapse" data-parent="#accordion-3" href="#collapseOne-3">
					<i class="fa fa-bold"></i>Bootstrap Based
				</a>
			</h4>
		</div>
		<div id="collapseOne-3" class="panel-collapse collapse in">
			<div class="panel-body">
				Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 3 wolf moon officia aute, non cupidatat skateboard dolor brunch. Food truck quinoa nesciunt laborum eiusmod. Brunch 3 wolf moon tempor, sunt aliqua put a bird on it squid single-origin coffee nulla assumenda shoreditch et. Nihil anim keffiyeh helvetica, craft beer labore wes anderson cred nesciunt sapiente ea proident.
			</div>
		</div>
	</div>
	<div class="panel panel-default">
		<div class="panel-heading">
			<h4 class="panel-title">
				<a data-toggle="collapse" data-parent="#accordion-3" href="#collapseTwo-3" class="collapsed">
					<i class="fa fa-leaf"></i>Clean &amp; powerful code
				</a>
			</h4>
		</div>
		<div id="collapseTwo-3" class="panel-collapse collapse">
			<div class="panel-body">
				Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 3 wolf moon officia aute, non cupidatat skateboard dolor brunch. Food truck quinoa nesciunt laborum eiusmod. Brunch 3 wolf moon tempor, sunt aliqua put a bird on it squid single-origin coffee nulla assumenda shoreditch et. Nihil anim keffiyeh helvetica, craft beer labore wes anderson cred nesciunt sapiente ea proident.
			</div>
		</div>
	</div>
	<div class="panel panel-default">
		<div class="panel-heading">
			<h4 class="panel-title">
				<a data-toggle="collapse" data-parent="#accordion-3" href="#collapseThree-3" class="collapsed">
					<i class="fa fa-html5"></i>HTML5/CSS3
				</a>
			</h4>
		</div>
		<div id="collapseThree-3" class="panel-collapse collapse">
			<div class="panel-body">
				Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 3 wolf moon officia aute, non cupidatat skateboard dolor brunch. Food truck quinoa nesciunt laborum eiusmod. Brunch 3 wolf moon tempor, sunt aliqua put a bird on it squid single-origin coffee nulla assumenda shoreditch et. Nihil anim keffiyeh helvetica, craft beer labore wes anderson cred nesciunt sapiente ea proident.
			</div>
		</div>
	</div>
</div>
<div class="space-bottom"></div>

<h2>Social Icons</h2>
<ul class="social-links colored">
	<li class="facebook"><a target="_blank" href="http://www.facebook.com"><i class="fa fa-facebook"></i></a></li>
	<li class="twitter"><a target="_blank" href="http://www.twitter.com"><i class="fa fa-twitter"></i></a></li>
	<li class="googleplus"><a target="_blank" href="http://plus.google.com"><i class="fa fa-google-plus"></i></a></li>
	<li class="skype"><a target="_blank" href="http://www.skype.com"><i class="fa fa-skype"></i></a></li>
	<li class="instagram"><a target="_blank" href="http://www.instagram.com"><i class="fa fa-instagram"></i></a></li>
	<li class="linkedin"><a target="_blank" href="http://www.linkedin.com"><i class="fa fa-linkedin"></i></a></li>
	<li class="pinterest"><a target="_blank" href="http://www.pinterest.com"><i class="fa fa-pinterest"></i></a></li>
	<li class="flickr"><a target="_blank" href="http://www.flickr.com"><i class="fa fa-flickr"></i></a></li>
	<li class="youtube"><a target="_blank" href="https://www.youtube.com"><i class="fa fa-youtube"></i></a></li>
	<li class="foursquare"><a target="_blank" href="https://foursquare.com"><i class="fa fa-foursquare"></i></a></li>
	<li class="soundcloud"><a target="_blank" href="https://soundcloud.com"><i class="fa fa-soundcloud"></i></a></li>
	<li class="tumblr"><a target="_blank" href="https://www.tumblr.com"><i class="fa fa-tumblr"></i></a></li>
	<li class="vimeo"><a target="_blank" href="https://www.vimeo.com"><i class="fa fa-vimeo-square"></i></a></li>
	<li class="dribbble"><a target="_blank" href="https://dribbble.com/"><i class="fa fa-dribbble"></i></a></li>
	<li class="behance"><a target="_blank" href="https://www.behance.net/"><i class="fa fa-behance"></i></a></li>
	<li class="vine"><a target="_blank" href="https://vine.co"><i class="fa fa-vine"></i></a></li>
	<li class="stumbleupon"><a target="_blank" href="https://www.stumbleupon.com/"><i class="fa fa-stumbleupon"></i></a></li>
</ul>
<ul class="social-links large colored">
	<li class="facebook"><a target="_blank" href="http://www.facebook.com"><i class="fa fa-facebook"></i></a></li>
	<li class="twitter"><a target="_blank" href="http://www.twitter.com"><i class="fa fa-twitter"></i></a></li>
	<li class="googleplus"><a target="_blank" href="http://plus.google.com"><i class="fa fa-google-plus"></i></a></li>
	<li class="skype"><a target="_blank" href="http://www.skype.com"><i class="fa fa-skype"></i></a></li>
	<li class="instagram"><a target="_blank" href="http://www.instagram.com"><i class="fa fa-instagram"></i></a></li>
	<li class="linkedin"><a target="_blank" href="http://www.linkedin.com"><i class="fa fa-linkedin"></i></a></li>
	<li class="pinterest"><a target="_blank" href="http://www.pinterest.com"><i class="fa fa-pinterest"></i></a></li>
	<li class="flickr"><a target="_blank" href="http://www.flickr.com"><i class="fa fa-flickr"></i></a></li>
	<li class="youtube"><a target="_blank" href="https://www.youtube.com"><i class="fa fa-youtube"></i></a></li>
	<li class="foursquare"><a target="_blank" href="https://foursquare.com"><i class="fa fa-foursquare"></i></a></li>
	<li class="soundcloud"><a target="_blank" href="https://soundcloud.com"><i class="fa fa-soundcloud"></i></a></li>
	<li class="tumblr"><a target="_blank" href="https://www.tumblr.com"><i class="fa fa-tumblr"></i></a></li>
	<li class="vimeo"><a target="_blank" href="https://www.vimeo.com"><i class="fa fa-vimeo-square"></i></a></li>
</ul>
<div class="space-bottom"></div>

<h2>Social Icons Light</h2>
<ul class="social-links light">
	<li><a target="_blank" href="http://www.facebook.com"><i class="fa fa-facebook"></i></a></li>
	<li><a target="_blank" href="http://www.twitter.com"><i class="fa fa-twitter"></i></a></li>
	<li><a target="_blank" href="http://plus.google.com"><i class="fa fa-google-plus"></i></a></li>
	<li><a target="_blank" href="http://www.skype.com"><i class="fa fa-skype"></i></a></li>
	<li><a target="_blank" href="http://www.instagram.com"><i class="fa fa-instagram"></i></a></li>
	<li><a target="_blank" href="http://www.linkedin.com"><i class="fa fa-linkedin"></i></a></li>
	<li><a target="_blank" href="http://www.pinterest.com"><i class="fa fa-pinterest"></i></a></li>
	<li><a target="_blank" href="http://www.flickr.com"><i class="fa fa-flickr"></i></a></li>
	<li><a target="_blank" href="https://foursquare.com"><i class="fa fa-foursquare"></i></a></li>
	<li><a target="_blank" href="https://soundcloud.com"><i class="fa fa-soundcloud"></i></a></li>
	<li><a target="_blank" href="https://www.tumblr.com"><i class="fa fa-tumblr"></i></a></li>
	<li><a target="_blank" href="https://www.vimeo.com"><i class="fa fa-vimeo-square"></i></a></li>
	<li><a target="_blank" href="https://www.behance.net"><i class="fa fa-behance"></i></a></li>
	<li><a target="_blank" href="http://www.reddit.com"><i class="fa fa-reddit"></i></a></li>
	<li><a target="_blank" href="http://www.drupal.org"><i class="fa fa-drupal"></i></a></li>
	<li><a target="_blank" href="#"><i class="fa fa-share-alt"></i></a></li>
	<li><a target="_blank" href="https://vine.co"><i class="fa fa-vine"></i></a></li>
</ul>
<ul class="social-links light large">
	<li><a target="_blank" href="http://www.facebook.com"><i class="fa fa-facebook"></i></a></li>
	<li><a target="_blank" href="http://www.twitter.com"><i class="fa fa-twitter"></i></a></li>
	<li><a target="_blank" href="http://plus.google.com"><i class="fa fa-google-plus"></i></a></li>
	<li><a target="_blank" href="http://www.skype.com"><i class="fa fa-skype"></i></a></li>
	<li><a target="_blank" href="http://www.instagram.com"><i class="fa fa-instagram"></i></a></li>
	<li><a target="_blank" href="http://www.linkedin.com"><i class="fa fa-linkedin"></i></a></li>
	<li><a target="_blank" href="http://www.pinterest.com"><i class="fa fa-pinterest"></i></a></li>
	<li><a target="_blank" href="http://www.flickr.com"><i class="fa fa-flickr"></i></a></li>
	<li><a target="_blank" href="https://foursquare.com"><i class="fa fa-foursquare"></i></a></li>
	<li><a target="_blank" href="https://soundcloud.com"><i class="fa fa-soundcloud"></i></a></li>
	<li><a target="_blank" href="https://www.tumblr.com"><i class="fa fa-tumblr"></i></a></li>
	<li><a target="_blank" href="https://www.vimeo.com"><i class="fa fa-vimeo-square"></i></a></li>
	<li><a target="_blank" href="https://www.behance.net"><i class="fa fa-behance"></i></a></li>
</ul>
<div class="space-bottom"></div>

<h2>Social Icons Default</h2>
<ul class="social-links">
	<li><a target="_blank" href="http://www.facebook.com"><i class="fa fa-facebook"></i></a></li>
	<li><a target="_blank" href="http://www.twitter.com"><i class="fa fa-twitter"></i></a></li>
	<li><a target="_blank" href="http://plus.google.com"><i class="fa fa-google-plus"></i></a></li>
	<li><a target="_blank" href="http://www.skype.com"><i class="fa fa-skype"></i></a></li>
	<li><a target="_blank" href="http://www.instagram.com"><i class="fa fa-instagram"></i></a></li>
	<li><a target="_blank" href="http://www.linkedin.com"><i class="fa fa-linkedin"></i></a></li>
	<li><a target="_blank" href="http://www.pinterest.com"><i class="fa fa-pinterest"></i></a></li>
	<li><a target="_blank" href="http://www.flickr.com"><i class="fa fa-flickr"></i></a></li>
	<li><a target="_blank" href="https://foursquare.com"><i class="fa fa-foursquare"></i></a></li>
	<li><a target="_blank" href="https://soundcloud.com"><i class="fa fa-soundcloud"></i></a></li>
	<li><a target="_blank" href="https://www.tumblr.com"><i class="fa fa-tumblr"></i></a></li>
	<li><a target="_blank" href="https://www.vimeo.com"><i class="fa fa-vimeo-square"></i></a></li>
	<li><a target="_blank" href="https://www.behance.net"><i class="fa fa-behance"></i></a></li>
	<li><a target="_blank" href="http://www.reddit.com"><i class="fa fa-reddit"></i></a></li>
	<li><a target="_blank" href="http://www.drupal.org"><i class="fa fa-drupal"></i></a></li>
	<li><a target="_blank" href="#"><i class="fa fa-share-alt"></i></a></li>
	<li><a target="_blank" href="https://vine.co"><i class="fa fa-vine"></i></a></li>
</ul>
<ul class="social-links large">
	<li><a target="_blank" href="http://www.facebook.com"><i class="fa fa-facebook"></i></a></li>
	<li><a target="_blank" href="http://www.twitter.com"><i class="fa fa-twitter"></i></a></li>
	<li><a target="_blank" href="http://plus.google.com"><i class="fa fa-google-plus"></i></a></li>
	<li><a target="_blank" href="http://www.skype.com"><i class="fa fa-skype"></i></a></li>
	<li><a target="_blank" href="http://www.instagram.com"><i class="fa fa-instagram"></i></a></li>
	<li><a target="_blank" href="http://www.linkedin.com"><i class="fa fa-linkedin"></i></a></li>
	<li><a target="_blank" href="http://www.pinterest.com"><i class="fa fa-pinterest"></i></a></li>
	<li><a target="_blank" href="http://www.flickr.com"><i class="fa fa-flickr"></i></a></li>
	<li><a target="_blank" href="https://foursquare.com"><i class="fa fa-foursquare"></i></a></li>
	<li><a target="_blank" href="https://soundcloud.com"><i class="fa fa-soundcloud"></i></a></li>
	<li><a target="_blank" href="https://www.tumblr.com"><i class="fa fa-tumblr"></i></a></li>
	<li><a target="_blank" href="https://www.vimeo.com"><i class="fa fa-vimeo-square"></i></a></li>
	<li><a target="_blank" href="https://www.behance.net"><i class="fa fa-behance"></i></a></li>
</ul>
<div class="row">
	<div class="col-md-11">
		<div class="alert alert-info alert-dismissible" role="alert">
			<button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
			The <strong>Default</strong> color is defined by the current color scheme.
		</div>
	</div>
</div>
<div class="space-bottom"></div>

<h2>Social Icons Dark</h2>
<ul class="social-links dark">
	<li><a target="_blank" href="http://www.facebook.com"><i class="fa fa-facebook"></i></a></li>
	<li><a target="_blank" href="http://www.twitter.com"><i class="fa fa-twitter"></i></a></li>
	<li><a target="_blank" href="http://plus.google.com"><i class="fa fa-google-plus"></i></a></li>
	<li><a target="_blank" href="http://www.skype.com"><i class="fa fa-skype"></i></a></li>
	<li><a target="_blank" href="http://www.instagram.com"><i class="fa fa-instagram"></i></a></li>
	<li><a target="_blank" href="http://www.linkedin.com"><i class="fa fa-linkedin"></i></a></li>
	<li><a target="_blank" href="http://www.pinterest.com"><i class="fa fa-pinterest"></i></a></li>
	<li><a target="_blank" href="http://www.flickr.com"><i class="fa fa-flickr"></i></a></li>
	<li><a target="_blank" href="https://foursquare.com"><i class="fa fa-foursquare"></i></a></li>
	<li><a target="_blank" href="https://soundcloud.com"><i class="fa fa-soundcloud"></i></a></li>
	<li><a target="_blank" href="https://www.tumblr.com"><i class="fa fa-tumblr"></i></a></li>
	<li><a target="_blank" href="https://www.vimeo.com"><i class="fa fa-vimeo-square"></i></a></li>
	<li><a target="_blank" href="https://www.behance.net"><i class="fa fa-behance"></i></a></li>
	<li><a target="_blank" href="http://www.reddit.com"><i class="fa fa-reddit"></i></a></li>
	<li><a target="_blank" href="http://www.drupal.org"><i class="fa fa-drupal"></i></a></li>
	<li><a target="_blank" href="#"><i class="fa fa-share-alt"></i></a></li>
	<li><a target="_blank" href="https://vine.co"><i class="fa fa-vine"></i></a></li>
</ul>
<ul class="social-links dark large">
	<li><a target="_blank" href="http://www.facebook.com"><i class="fa fa-facebook"></i></a></li>
	<li><a target="_blank" href="http://www.twitter.com"><i class="fa fa-twitter"></i></a></li>
	<li><a target="_blank" href="http://plus.google.com"><i class="fa fa-google-plus"></i></a></li>
	<li><a target="_blank" href="http://www.skype.com"><i class="fa fa-skype"></i></a></li>
	<li><a target="_blank" href="http://www.instagram.com"><i class="fa fa-instagram"></i></a></li>
	<li><a target="_blank" href="http://www.linkedin.com"><i class="fa fa-linkedin"></i></a></li>
	<li><a target="_blank" href="http://www.pinterest.com"><i class="fa fa-pinterest"></i></a></li>
	<li><a target="_blank" href="http://www.flickr.com"><i class="fa fa-flickr"></i></a></li>
	<li><a target="_blank" href="https://foursquare.com"><i class="fa fa-foursquare"></i></a></li>
	<li><a target="_blank" href="https://soundcloud.com"><i class="fa fa-soundcloud"></i></a></li>
	<li><a target="_blank" href="https://www.tumblr.com"><i class="fa fa-tumblr"></i></a></li>
	<li><a target="_blank" href="https://www.vimeo.com"><i class="fa fa-vimeo-square"></i></a></li>
	<li><a target="_blank" href="https://www.behance.net"><i class="fa fa-behance"></i></a></li>
</ul>
<div class="space-bottom"></div>

<h2>Social Icons with border radius</h2>
<ul class="social-links colored radius-5px">
	<li class="facebook"><a target="_blank" href="http://www.facebook.com"><i class="fa fa-facebook"></i></a></li>
	<li class="twitter"><a target="_blank" href="http://www.twitter.com"><i class="fa fa-twitter"></i></a></li>
	<li class="googleplus"><a target="_blank" href="http://plus.google.com"><i class="fa fa-google-plus"></i></a></li>
	<li class="skype"><a target="_blank" href="http://www.skype.com"><i class="fa fa-skype"></i></a></li>
	<li class="instagram"><a target="_blank" href="http://www.instagram.com"><i class="fa fa-instagram"></i></a></li>
	<li class="linkedin"><a target="_blank" href="http://www.linkedin.com"><i class="fa fa-linkedin"></i></a></li>
	<li class="pinterest"><a target="_blank" href="http://www.pinterest.com"><i class="fa fa-pinterest"></i></a></li>
	<li class="flickr"><a target="_blank" href="http://www.flickr.com"><i class="fa fa-flickr"></i></a></li>
	<li class="youtube"><a target="_blank" href="https://www.youtube.com"><i class="fa fa-youtube"></i></a></li>
	<li class="foursquare"><a target="_blank" href="https://foursquare.com"><i class="fa fa-foursquare"></i></a></li>
	<li class="soundcloud"><a target="_blank" href="https://soundcloud.com"><i class="fa fa-soundcloud"></i></a></li>
	<li class="tumblr"><a target="_blank" href="https://www.tumblr.com"><i class="fa fa-tumblr"></i></a></li>
	<li class="vimeo"><a target="_blank" href="https://www.vimeo.com"><i class="fa fa-vimeo-square"></i></a></li>
	<li class="dribbble"><a target="_blank" href="https://dribbble.com/"><i class="fa fa-dribbble"></i></a></li>
	<li class="behance"><a target="_blank" href="https://www.behance.net/"><i class="fa fa-behance"></i></a></li>
	<li class="vine"><a target="_blank" href="https://vine.co"><i class="fa fa-vine"></i></a></li>
	<li class="stumbleupon"><a target="_blank" href="https://www.stumbleupon.com/"><i class="fa fa-stumbleupon"></i></a></li>
</ul>
<ul class="social-links light radius-5px">
	<li><a target="_blank" href="http://www.facebook.com"><i class="fa fa-facebook"></i></a></li>
	<li><a target="_blank" href="http://www.twitter.com"><i class="fa fa-twitter"></i></a></li>
	<li><a target="_blank" href="http://plus.google.com"><i class="fa fa-google-plus"></i></a></li>
	<li><a target="_blank" href="http://www.skype.com"><i class="fa fa-skype"></i></a></li>
	<li><a target="_blank" href="http://www.instagram.com"><i class="fa fa-instagram"></i></a></li>
	<li><a target="_blank" href="http://www.linkedin.com"><i class="fa fa-linkedin"></i></a></li>
	<li><a target="_blank" href="http://www.pinterest.com"><i class="fa fa-pinterest"></i></a></li>
	<li><a target="_blank" href="http://www.flickr.com"><i class="fa fa-flickr"></i></a></li>
	<li><a target="_blank" href="https://foursquare.com"><i class="fa fa-foursquare"></i></a></li>
	<li><a target="_blank" href="https://soundcloud.com"><i class="fa fa-soundcloud"></i></a></li>
	<li><a target="_blank" href="https://www.tumblr.com"><i class="fa fa-tumblr"></i></a></li>
	<li><a target="_blank" href="https://www.vimeo.com"><i class="fa fa-vimeo-square"></i></a></li>
	<li><a target="_blank" href="https://www.behance.net"><i class="fa fa-behance"></i></a></li>
	<li><a target="_blank" href="http://www.reddit.com"><i class="fa fa-reddit"></i></a></li>
	<li><a target="_blank" href="http://www.drupal.org"><i class="fa fa-drupal"></i></a></li>
	<li><a target="_blank" href="#"><i class="fa fa-share-alt"></i></a></li>
	<li><a target="_blank" href="https://vine.co"><i class="fa fa-vine"></i></a></li>
</ul>
<ul class="social-links radius-5px">
	<li><a target="_blank" href="http://www.facebook.com"><i class="fa fa-facebook"></i></a></li>
	<li><a target="_blank" href="http://www.twitter.com"><i class="fa fa-twitter"></i></a></li>
	<li><a target="_blank" href="http://plus.google.com"><i class="fa fa-google-plus"></i></a></li>
	<li><a target="_blank" href="http://www.skype.com"><i class="fa fa-skype"></i></a></li>
	<li><a target="_blank" href="http://www.instagram.com"><i class="fa fa-instagram"></i></a></li>
	<li><a target="_blank" href="http://www.linkedin.com"><i class="fa fa-linkedin"></i></a></li>
	<li><a target="_blank" href="http://www.pinterest.com"><i class="fa fa-pinterest"></i></a></li>
	<li><a target="_blank" href="http://www.flickr.com"><i class="fa fa-flickr"></i></a></li>
	<li><a target="_blank" href="https://foursquare.com"><i class="fa fa-foursquare"></i></a></li>
	<li><a target="_blank" href="https://soundcloud.com"><i class="fa fa-soundcloud"></i></a></li>
	<li><a target="_blank" href="https://www.tumblr.com"><i class="fa fa-tumblr"></i></a></li>
	<li><a target="_blank" href="https://www.vimeo.com"><i class="fa fa-vimeo-square"></i></a></li>
	<li><a target="_blank" href="https://www.behance.net"><i class="fa fa-behance"></i></a></li>
	<li><a target="_blank" href="http://www.reddit.com"><i class="fa fa-reddit"></i></a></li>
	<li><a target="_blank" href="http://www.drupal.org"><i class="fa fa-drupal"></i></a></li>
	<li><a target="_blank" href="#"><i class="fa fa-share-alt"></i></a></li>
	<li><a target="_blank" href="https://vine.co"><i class="fa fa-vine"></i></a></li>
</ul>
<ul class="social-links dark radius-5px">
	<li><a target="_blank" href="http://www.facebook.com"><i class="fa fa-facebook"></i></a></li>
	<li><a target="_blank" href="http://www.twitter.com"><i class="fa fa-twitter"></i></a></li>
	<li><a target="_blank" href="http://plus.google.com"><i class="fa fa-google-plus"></i></a></li>
	<li><a target="_blank" href="http://www.skype.com"><i class="fa fa-skype"></i></a></li>
	<li><a target="_blank" href="http://www.instagram.com"><i class="fa fa-instagram"></i></a></li>
	<li><a target="_blank" href="http://www.linkedin.com"><i class="fa fa-linkedin"></i></a></li>
	<li><a target="_blank" href="http://www.pinterest.com"><i class="fa fa-pinterest"></i></a></li>
	<li><a target="_blank" href="http://www.flickr.com"><i class="fa fa-flickr"></i></a></li>
	<li><a target="_blank" href="https://foursquare.com"><i class="fa fa-foursquare"></i></a></li>
	<li><a target="_blank" href="https://soundcloud.com"><i class="fa fa-soundcloud"></i></a></li>
	<li><a target="_blank" href="https://www.tumblr.com"><i class="fa fa-tumblr"></i></a></li>
	<li><a target="_blank" href="https://www.vimeo.com"><i class="fa fa-vimeo-square"></i></a></li>
	<li><a target="_blank" href="https://www.behance.net"><i class="fa fa-behance"></i></a></li>
	<li><a target="_blank" href="http://www.reddit.com"><i class="fa fa-reddit"></i></a></li>
	<li><a target="_blank" href="http://www.drupal.org"><i class="fa fa-drupal"></i></a></li>
	<li><a target="_blank" href="#"><i class="fa fa-share-alt"></i></a></li>
	<li><a target="_blank" href="https://vine.co"><i class="fa fa-vine"></i></a></li>
</ul>
<div class="space-bottom"></div>

<h2>Social Icons Circle</h2>
<ul class="social-links colored circle">
	<li class="facebook"><a target="_blank" href="http://www.facebook.com"><i class="fa fa-facebook"></i></a></li>
	<li class="twitter"><a target="_blank" href="http://www.twitter.com"><i class="fa fa-twitter"></i></a></li>
	<li class="googleplus"><a target="_blank" href="http://plus.google.com"><i class="fa fa-google-plus"></i></a></li>
	<li class="skype"><a target="_blank" href="http://www.skype.com"><i class="fa fa-skype"></i></a></li>
	<li class="instagram"><a target="_blank" href="http://www.instagram.com"><i class="fa fa-instagram"></i></a></li>
	<li class="linkedin"><a target="_blank" href="http://www.linkedin.com"><i class="fa fa-linkedin"></i></a></li>
	<li class="pinterest"><a target="_blank" href="http://www.pinterest.com"><i class="fa fa-pinterest"></i></a></li>
	<li class="flickr"><a target="_blank" href="http://www.flickr.com"><i class="fa fa-flickr"></i></a></li>
	<li class="youtube"><a target="_blank" href="https://www.youtube.com"><i class="fa fa-youtube"></i></a></li>
	<li class="foursquare"><a target="_blank" href="https://foursquare.com"><i class="fa fa-foursquare"></i></a></li>
	<li class="soundcloud"><a target="_blank" href="https://soundcloud.com"><i class="fa fa-soundcloud"></i></a></li>
	<li class="tumblr"><a target="_blank" href="https://www.tumblr.com"><i class="fa fa-tumblr"></i></a></li>
	<li class="vimeo"><a target="_blank" href="https://www.vimeo.com"><i class="fa fa-vimeo-square"></i></a></li>
	<li class="dribbble"><a target="_blank" href="https://dribbble.com/"><i class="fa fa-dribbble"></i></a></li>
	<li class="behance"><a target="_blank" href="https://www.behance.net/"><i class="fa fa-behance"></i></a></li>
	<li class="vine"><a target="_blank" href="https://vine.co"><i class="fa fa-vine"></i></a></li>
	<li class="stumbleupon"><a target="_blank" href="https://www.stumbleupon.com/"><i class="fa fa-stumbleupon"></i></a></li>
</ul>
<ul class="social-links light circle">
	<li><a target="_blank" href="http://www.facebook.com"><i class="fa fa-facebook"></i></a></li>
	<li><a target="_blank" href="http://www.twitter.com"><i class="fa fa-twitter"></i></a></li>
	<li><a target="_blank" href="http://plus.google.com"><i class="fa fa-google-plus"></i></a></li>
	<li><a target="_blank" href="http://www.skype.com"><i class="fa fa-skype"></i></a></li>
	<li><a target="_blank" href="http://www.instagram.com"><i class="fa fa-instagram"></i></a></li>
	<li><a target="_blank" href="http://www.linkedin.com"><i class="fa fa-linkedin"></i></a></li>
	<li><a target="_blank" href="http://www.pinterest.com"><i class="fa fa-pinterest"></i></a></li>
	<li><a target="_blank" href="http://www.flickr.com"><i class="fa fa-flickr"></i></a></li>
	<li><a target="_blank" href="https://foursquare.com"><i class="fa fa-foursquare"></i></a></li>
	<li><a target="_blank" href="https://soundcloud.com"><i class="fa fa-soundcloud"></i></a></li>
	<li><a target="_blank" href="https://www.tumblr.com"><i class="fa fa-tumblr"></i></a></li>
	<li><a target="_blank" href="https://www.vimeo.com"><i class="fa fa-vimeo-square"></i></a></li>
	<li><a target="_blank" href="https://www.behance.net"><i class="fa fa-behance"></i></a></li>
	<li><a target="_blank" href="http://www.reddit.com"><i class="fa fa-reddit"></i></a></li>
	<li><a target="_blank" href="http://www.drupal.org"><i class="fa fa-drupal"></i></a></li>
	<li><a target="_blank" href="#"><i class="fa fa-share-alt"></i></a></li>
	<li><a target="_blank" href="https://vine.co"><i class="fa fa-vine"></i></a></li>
</ul>
<ul class="social-links circle">
	<li><a target="_blank" href="http://www.facebook.com"><i class="fa fa-facebook"></i></a></li>
	<li><a target="_blank" href="http://www.twitter.com"><i class="fa fa-twitter"></i></a></li>
	<li><a target="_blank" href="http://plus.google.com"><i class="fa fa-google-plus"></i></a></li>
	<li><a target="_blank" href="http://www.skype.com"><i class="fa fa-skype"></i></a></li>
	<li><a target="_blank" href="http://www.instagram.com"><i class="fa fa-instagram"></i></a></li>
	<li><a target="_blank" href="http://www.linkedin.com"><i class="fa fa-linkedin"></i></a></li>
	<li><a target="_blank" href="http://www.pinterest.com"><i class="fa fa-pinterest"></i></a></li>
	<li><a target="_blank" href="http://www.flickr.com"><i class="fa fa-flickr"></i></a></li>
	<li><a target="_blank" href="https://foursquare.com"><i class="fa fa-foursquare"></i></a></li>
	<li><a target="_blank" href="https://soundcloud.com"><i class="fa fa-soundcloud"></i></a></li>
	<li><a target="_blank" href="https://www.tumblr.com"><i class="fa fa-tumblr"></i></a></li>
	<li><a target="_blank" href="https://www.vimeo.com"><i class="fa fa-vimeo-square"></i></a></li>
	<li><a target="_blank" href="https://www.behance.net"><i class="fa fa-behance"></i></a></li>
	<li><a target="_blank" href="http://www.reddit.com"><i class="fa fa-reddit"></i></a></li>
	<li><a target="_blank" href="http://www.drupal.org"><i class="fa fa-drupal"></i></a></li>
	<li><a target="_blank" href="#"><i class="fa fa-share-alt"></i></a></li>
	<li><a target="_blank" href="https://vine.co"><i class="fa fa-vine"></i></a></li>
</ul>
<ul class="social-links dark circle">
	<li><a target="_blank" href="http://www.facebook.com"><i class="fa fa-facebook"></i></a></li>
	<li><a target="_blank" href="http://www.twitter.com"><i class="fa fa-twitter"></i></a></li>
	<li><a target="_blank" href="http://plus.google.com"><i class="fa fa-google-plus"></i></a></li>
	<li><a target="_blank" href="http://www.skype.com"><i class="fa fa-skype"></i></a></li>
	<li><a target="_blank" href="http://www.instagram.com"><i class="fa fa-instagram"></i></a></li>
	<li><a target="_blank" href="http://www.linkedin.com"><i class="fa fa-linkedin"></i></a></li>
	<li><a target="_blank" href="http://www.pinterest.com"><i class="fa fa-pinterest"></i></a></li>
	<li><a target="_blank" href="http://www.flickr.com"><i class="fa fa-flickr"></i></a></li>
	<li><a target="_blank" href="https://foursquare.com"><i class="fa fa-foursquare"></i></a></li>
	<li><a target="_blank" href="https://soundcloud.com"><i class="fa fa-soundcloud"></i></a></li>
	<li><a target="_blank" href="https://www.tumblr.com"><i class="fa fa-tumblr"></i></a></li>
	<li><a target="_blank" href="https://www.vimeo.com"><i class="fa fa-vimeo-square"></i></a></li>
	<li><a target="_blank" href="https://www.behance.net"><i class="fa fa-behance"></i></a></li>
	<li><a target="_blank" href="http://www.reddit.com"><i class="fa fa-reddit"></i></a></li>
	<li><a target="_blank" href="http://www.drupal.org"><i class="fa fa-drupal"></i></a></li>
	<li><a target="_blank" href="#"><i class="fa fa-share-alt"></i></a></li>
	<li><a target="_blank" href="https://vine.co"><i class="fa fa-vine"></i></a></li>
</ul>
<div class="row">
	<div class="col-md-11">
		<div class="alert alert-info alert-dismissible" role="alert">
			<button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">?</span><span class="sr-only">Close</span></button>
			You can use any icon from: 
			<ul>
				<li><a target="_blank" href="http://fontawesome.io/icons/" class="alert-link">Font Awesome Icons</a></li>
				<li><a target="_blank" href="http://fontello.com/" class="alert-link">Fontello Icons</a></li>
				<li><a target="_blank" href="http://getbootstrap.com/components" class="alert-link">Glyphicons Icons</a></li>
			</ul>
		</div>
	</div>
</div>
<div class="space-bottom"></div>

<h2>Twitter Bootstrap Buttons</h2>
<div class="space-bottom"></div>

<h4>Small</h4>
<a href="#" class="btn btn-default btn-sm">Default</a>
<a href="#" class="btn btn-primary btn-sm">Primary</a>
<a href="#" class="btn btn-success btn-sm">Success</a>
<a href="#" class="btn btn-info btn-sm">Info</a>
<a href="#" class="btn btn-warning btn-sm">Warning</a>
<a href="#" class="btn btn-danger btn-sm">Danger</a>
<br>

<h4>Medium</h4>
<a href="#" class="btn btn-default">Default</a>
<a href="#" class="btn btn-primary">Primary</a>
<a href="#" class="btn btn-success">Success</a>
<br>
<a href="#" class="btn btn-info">Info</a>
<a href="#" class="btn btn-warning">Warning</a>
<a href="#" class="btn btn-danger">Danger</a>
<br>

<h4>Large</h4>
<a href="#" class="btn btn-default btn-lg">Default</a>
<a href="#" class="btn btn-primary btn-lg">Primary</a>
<a href="#" class="btn btn-success btn-lg">Success</a>
<a href="#" class="btn btn-info btn-lg">Info</a>
<a href="#" class="btn btn-warning btn-lg">Warning</a>
<a href="#" class="btn btn-danger btn-lg">Danger</a>
<div class="alert alert-info alert-dismissible" role="alert">
	<button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
	The <strong>Default</strong> color is defined by the current color scheme.
</div>
<div class="space"></div>

<h2>Effortless Buttons</h2>
<br>

<h4>Small</h4>
<a href="#" class="btn btn-white btn-sm">White</a>
<a href="#" class="btn btn-light-gray btn-sm">Light Gray</a>
<a href="#" class="btn btn-gray btn-sm">Gray</a>
<a href="#" class="btn btn-dark btn-sm">Dark</a>

<h4>Medium</h4>
<a href="#" class="btn btn-white">White</a>
<a href="#" class="btn btn-light-gray">Light Gray</a>
<br>
<a href="#" class="btn btn-gray">Gray</a>
<a href="#" class="btn btn-dark">Dark</a>

<h4>Large</h4>
<a href="#" class="btn btn-white btn-lg">White</a>
<a href="#" class="btn btn-light-gray btn-lg">Light Gray</a>
<br>
<a href="#" class="btn btn-gray btn-lg">Gray</a>
<a href="#" class="btn btn-dark btn-lg">Dark</a>
<div class="space"></div>		

<h2>Buttons With Border Radius</h2>
<br>

<h4>Small</h4>
<a href="#" class="btn radius btn-white btn-sm">White</a>
<a href="#" class="btn radius btn-light-gray btn-sm">Light Gray</a>
<a href="#" class="btn radius btn-gray btn-sm">Gray</a>
<a href="#" class="btn radius btn-dark btn-sm">Dark</a>
<a href="#" class="btn radius btn-default btn-sm">Default</a>
<a href="#" class="btn radius btn-primary btn-sm">Primary</a>
<a href="#" class="btn radius btn-success btn-sm">Success</a>
<a href="#" class="btn radius btn-info btn-sm">Info</a>
<a href="#" class="btn radius btn-warning btn-sm">Warning</a>
<a href="#" class="btn radius btn-danger btn-sm">Danger</a>

<h4>Medium</h4>
<a href="#" class="btn radius btn-white">White</a>
<a href="#" class="btn radius btn-light-gray">Light Gray</a>
<a href="#" class="btn radius btn-gray">Gray</a>
<a href="#" class="btn radius btn-dark">Dark</a>
<a href="#" class="btn radius btn-default">Default</a>
<a href="#" class="btn radius btn-primary">Primary</a>
<a href="#" class="btn radius btn-success">Success</a>
<a href="#" class="btn radius btn-info">Info</a>
<a href="#" class="btn radius btn-warning">Warning</a>
<a href="#" class="btn radius btn-danger">Danger</a>

<h4>Large</h4>
<a href="#" class="btn radius btn-white btn-lg">White</a>
<a href="#" class="btn radius btn-light-gray btn-lg">Light Gray</a>
<a href="#" class="btn radius btn-gray btn-lg">Gray</a>
<a href="#" class="btn radius btn-dark btn-lg">Dark</a>
<a href="#" class="btn radius btn-default btn-lg">Default</a>
<a href="#" class="btn radius btn-primary btn-lg">Primary</a>
<a href="#" class="btn radius btn-success btn-lg">Success</a>
<a href="#" class="btn radius btn-info btn-lg">Info</a>
<a href="#" class="btn radius btn-warning btn-lg">Warning</a>
<a href="#" class="btn radius btn-danger btn-lg">Danger</a>
<div class="space"></div>

<h2>Buttons With Icons</h2>
<br>

<h4>Small</h4>
<a href="#" class="btn btn-white btn-sm"><i class="fa fa-chevron-left pr-10"></i> Prev</a>
<a href="#" class="btn btn-light-gray btn-sm">Next <i class="fa fa-chevron-right pl-10"></i></a>
<a href="#" class="btn btn-gray btn-sm">Send <i class="fa fa-check pl-10"></i></a>
<a href="#" class="btn btn-dark btn-sm"><i class="fa fa-lock pr-10"></i> login</a>
<a href="#" class="btn btn-default btn-sm">Settings <i class="fa fa-cog pl-10"></i></a>
<a href="#" class="btn btn-primary btn-sm">Save <i class="fa fa-floppy-o pl-10"></i></a>
<a href="#" class="btn btn-success btn-sm"><i class="fa fa-users pr-10"></i> Our Team</a>
<a href="#" class="btn btn-info btn-sm">Print <i class="fa fa-print pl-10"></i></a>
<a href="#" class="btn btn-warning btn-sm">Open <i class="fa fa-folder-open-o pl-10"></i></a>
<a href="#" class="btn btn-danger btn-sm">Danger <i class="fa fa-warning pl-10"></i></a>

<h4>Medium</h4>
<a href="#" class="btn btn-white"><i class="fa fa-chevron-left pr-10"></i> Prev</a>
<a href="#" class="btn btn-light-gray">Next <i class="fa fa-chevron-right pl-10"></i></a>
<a href="#" class="btn btn-gray">Send <i class="fa fa-check pl-10"></i></a>
<a href="#" class="btn btn-dark"><i class="fa fa-lock pr-10"></i> login</a>
<a href="#" class="btn btn-default">Settings <i class="fa fa-cog pl-10"></i></a>
<a href="#" class="btn btn-primary">Save <i class="fa fa-floppy-o pl-10"></i></a>
<a href="#" class="btn btn-success"><i class="fa fa-users pr-10"></i> Our Team</a>
<a href="#" class="btn btn-info">Print <i class="fa fa-print pl-10"></i></a>
<a href="#" class="btn btn-warning">Open <i class="fa fa-folder-open-o pl-10"></i></a>
<a href="#" class="btn btn-danger">Danger <i class="fa fa-warning pl-10"></i></a>

<h4>Large</h4>
<a href="#" class="btn btn-white btn-lg"><i class="fa fa-chevron-left pr-10"></i> Prev</a>
<a href="#" class="btn btn-light-gray btn-lg">Next <i class="fa fa-chevron-right pl-10"></i></a>
<a href="#" class="btn btn-gray btn-lg">Send <i class="fa fa-check pl-10"></i></a>
<a href="#" class="btn btn-dark btn-lg"><i class="fa fa-lock pr-10"></i> login</a>
<a href="#" class="btn btn-default btn-lg">Settings <i class="fa fa-cog pl-10"></i></a>
<a href="#" class="btn btn-primary btn-lg">Save <i class="fa fa-floppy-o pl-10"></i></a>
<a href="#" class="btn btn-success btn-lg"><i class="fa fa-users pr-10"></i> Our Team</a>
<a href="#" class="btn btn-info btn-lg">Print <i class="fa fa-print pl-10"></i></a>
<a href="#" class="btn btn-warning btn-lg">Open <i class="fa fa-folder-open-o pl-10"></i></a>
<a href="#" class="btn btn-danger btn-lg">Danger <i class="fa fa-warning pl-10"></i></a>
<div class="alert alert-info alert-dismissible" role="alert">
	<button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">?</span><span class="sr-only">Close</span></button>
	Check all available Icons: 
	<ul>
		<li><a target="_blank" href="http://fontawesome.io/icons/" class="alert-link">Font Awesome Icons</a></li>
		<li><a target="_blank" href="http://fontello.com/" class="alert-link">Fontello Icons</a></li>
		<li><a target="_blank" href="http://getbootstrap.com/components" class="alert-link">Glyphicons Icons</a></li>
	</ul>
</div>
<div class="space-bottom"></div>

<h2>Twitter Bootstrap Progress Bars</h2>
<div class="space-bottom"></div>

<div class="progress">
	<div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width: 40%">
		<span class="sr-only">40% Complete - Success</span>
	</div>
</div>
<div class="progress">
	<div class="progress-bar progress-bar-info" role="progressbar" aria-valuenow="20" aria-valuemin="0" aria-valuemax="100" style="width: 20%">
		<span class="sr-only">20% Complete - Info</span>
	</div>
</div>
<div class="progress">
	<div class="progress-bar progress-bar-warning" role="progressbar" aria-valuenow="60" aria-valuemin="0" aria-valuemax="100" style="width: 60%">
		<span class="sr-only">60% Complete - Warning</span>
	</div>
</div>
<div class="progress">
	<div class="progress-bar progress-bar-danger" role="progressbar" aria-valuenow="80" aria-valuemin="0" aria-valuemax="100" style="width: 80%">
		<span class="sr-only">80% Complete - Danger</span>
	</div>
</div>
<div class="space-bottom"></div>
<h2>Effortless Progress Bars</h2>
<div class="space-bottom"></div>
<div class="progress">
	<div class="progress-bar progress-bar-default" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width: 40%">
		<span class="sr-only">40% Complete - Default</span>
	</div>
</div>
<div class="progress">
	<div class="progress-bar progress-bar-gray" role="progressbar" aria-valuenow="20" aria-valuemin="0" aria-valuemax="100" style="width: 20%">
		<span class="sr-only">20% Complete - Gray</span>
	</div>
</div>
<div class="progress">
	<div class="progress-bar progress-bar-dark" role="progressbar" aria-valuenow="60" aria-valuemin="0" aria-valuemax="100" style="width: 60%">
		<span class="sr-only">60% Complete - Dark</span>
	</div>
</div>
<div class="space-bottom"></div>

<h2>Progress Bars Stripped</h2>
<div class="space-bottom"></div>
<div class="progress">
	<div class="progress-bar progress-bar-success progress-bar-striped" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width: 40%">
		<span class="sr-only">40% Complete - Success</span>
	</div>
</div>
<div class="progress">
	<div class="progress-bar progress-bar-info progress-bar-striped" role="progressbar" aria-valuenow="20" aria-valuemin="0" aria-valuemax="100" style="width: 20%">
		<span class="sr-only">20% Complete - Info</span>
	</div>
</div>
<div class="progress">
	<div class="progress-bar progress-bar-warning progress-bar-striped" role="progressbar" aria-valuenow="60" aria-valuemin="0" aria-valuemax="100" style="width: 60%">
		<span class="sr-only">60% Complete - Warning</span>
	</div>
</div>
<div class="progress">
	<div class="progress-bar progress-bar-danger progress-bar-striped" role="progressbar" aria-valuenow="80" aria-valuemin="0" aria-valuemax="100" style="width: 80%">
		<span class="sr-only">80% Complete - Danger</span>
	</div>
</div>
<div class="progress">
	<div class="progress-bar progress-bar-default progress-bar-striped" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width: 40%">
		<span class="sr-only">40% Complete - Default</span>
	</div>
</div>
<div class="progress">
	<div class="progress-bar progress-bar-gray progress-bar-striped" role="progressbar" aria-valuenow="20" aria-valuemin="0" aria-valuemax="100" style="width: 20%">
		<span class="sr-only">20% Complete - Gray</span>
	</div>
</div>
<div class="progress">
	<div class="progress-bar progress-bar-dark progress-bar-striped" role="progressbar" aria-valuenow="60" aria-valuemin="0" aria-valuemax="100" style="width: 60%">
		<span class="sr-only">60% Complete - Dark</span>
	</div>
</div>
<div class="space-bottom"></div>

<h2>Progress Bars Stripped Animated</h2>
<div class="space-bottom"></div>
<div class="progress">
	<div class="progress-bar progress-bar-success progress-bar-striped active" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width: 40%">
		<span class="sr-only">40% Complete - Success</span>
	</div>
</div>
<div class="progress">
	<div class="progress-bar progress-bar-info progress-bar-striped active" role="progressbar" aria-valuenow="20" aria-valuemin="0" aria-valuemax="100" style="width: 20%">
		<span class="sr-only">20% Complete - Info</span>
	</div>
</div>
<div class="progress">
	<div class="progress-bar progress-bar-warning progress-bar-striped active" role="progressbar" aria-valuenow="60" aria-valuemin="0" aria-valuemax="100" style="width: 60%">
		<span class="sr-only">60% Complete - Warning</span>
	</div>
</div>
<div class="progress">
	<div class="progress-bar progress-bar-danger progress-bar-striped active" role="progressbar" aria-valuenow="80" aria-valuemin="0" aria-valuemax="100" style="width: 80%">
		<span class="sr-only">80% Complete - Danger</span>
	</div>
</div>
<div class="progress">
	<div class="progress-bar progress-bar-default progress-bar-striped active" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width: 40%">
		<span class="sr-only">40% Complete - Default</span>
	</div>
</div>
<div class="progress">
	<div class="progress-bar progress-bar-gray progress-bar-striped active" role="progressbar" aria-valuenow="20" aria-valuemin="0" aria-valuemax="100" style="width: 20%">
		<span class="sr-only">20% Complete - Gray</span>
	</div>
</div>
<div class="progress">
	<div class="progress-bar progress-bar-dark progress-bar-striped active" role="progressbar" aria-valuenow="60" aria-valuemin="0" aria-valuemax="100" style="width: 60%">
		<span class="sr-only">60% Complete - Dark</span>
	</div>
</div>
<div class="space-bottom"></div>

<h2>Animated Progress Bars</h2>
<div class="space-bottom"></div>
<div class="progress">
	<div class="progress-bar progress-bar-success" role="progressbar" data-animate-width="80%">
		<span class="object-non-visible" data-animation-effect="fadeInLeftBig" data-effect-delay="600">80% Complete - Success</span>
	</div>
</div>
<div class="progress">
	<div class="progress-bar progress-bar-info" role="progressbar" data-animate-width="90%">
		<span class="object-non-visible" data-animation-effect="fadeInLeftBig" data-effect-delay="600">90% Complete - Info</span>
	</div>
</div>
<div class="progress">
	<div class="progress-bar progress-bar-warning" role="progressbar" data-animate-width="70%">
		<span class="object-non-visible" data-animation-effect="fadeInLeftBig" data-effect-delay="600">70% Complete - Warning</span>
	</div>
</div>
<div class="progress">
	<div class="progress-bar progress-bar-danger" data-animate-width="85%">
		<span class="object-non-visible" data-animation-effect="fadeInLeftBig" data-effect-delay="600">85% Complete - Danger</span>
	</div>
</div>
<div class="progress">
	<div class="progress-bar progress-bar-default" data-animate-width="95%">
		<span class="object-non-visible" data-animation-effect="fadeInLeftBig" data-effect-delay="600">95% Complete - Default</span>
	</div>
</div>
<div class="progress">
	<div class="progress-bar progress-bar-gray" data-animate-width="92%">
		<span class="object-non-visible" data-animation-effect="fadeInLeftBig" data-effect-delay="600">92% Complete - Gray</span>
	</div>
</div>
<div class="progress">
	<div class="progress-bar progress-bar-dark" data-animate-width="100%">
		<span class="object-non-visible" data-animation-effect="fadeInLeftBig" data-effect-delay="600">100% Complete - Dark</span>
	</div>
</div>
<div class="space-bottom"></div>

<h2>Circular Progress Bars</h2>
<script src="<?=SITE_TEMPLATE_PATH?>/plugins/jquery.knob.min.js"></script>
<div class="space-bottom"></div>
<div class="row">
	<div class="col-md-3 col-sm-6">
		<div class="knob-container">
			<input class="knob" data-fgcolor="#5cb85c" data-thickness=".1" value="90%" data-displayInput="false" data-readOnly="true">
			<div class="knob-text">
				<label>Success</label>
				<span>90%</span>
			</div>
		</div>
	</div>
	<div class="col-md-3 col-sm-6">
		<div class="knob-container">
			<input class="knob" data-fgcolor="#5bc0de" data-thickness=".1" value="75%" data-displayInput="false" data-readOnly="true">
			<div class="knob-text">
				<label>Info</label>
				<span>75%</span>
			</div>
		</div>
	</div>
	<div class="col-md-3 col-sm-6">
		<div class="knob-container">
			<input class="knob" data-fgcolor="#f0ad4e" data-thickness=".1" value="80%" data-displayInput="false" data-readOnly="true">
			<div class="knob-text">
				<label>Warning</label>
				<span>80%</span>
			</div>
		</div>
	</div>
	<div class="col-md-3 col-sm-6">
		<div class="knob-container">
			<input class="knob" data-fgcolor="#d9534f" data-thickness=".1" value="85%" data-displayInput="false" data-readOnly="true">
			<div class="knob-text">
				<label>Danger</label>
				<span>85%</span>
			</div>
		</div>
	</div>
</div>
<div class="row">
	<div class="col-md-3 col-sm-6">
		<div class="knob-container">
			<input class="knob" data-fgcolor="#e84c3d" data-thickness=".1" value="100%" data-displayInput="false" data-readOnly="true">
			<div class="knob-text">
				<label>Effortless</label>
				<span>100%</span>
			</div>
		</div>
	</div>
	<div class="col-md-3 col-sm-6">
		<div class="knob-container">
			<input class="knob" data-fgcolor="#999999" data-thickness=".1" value="80%" data-displayInput="false" data-readOnly="true">
			<div class="knob-text">
				<label>Gray</label>
				<span>80%</span>
			</div>
		</div>
	</div>
	<div class="col-md-3 col-sm-6">
		<div class="knob-container">
			<input class="knob" data-fgcolor="#333333" data-thickness=".1" value="85%" data-displayInput="false" data-readOnly="true">
			<div class="knob-text">
				<label>Dark</label>
				<span>85%</span>
			</div>
		</div>
	</div>
</div>
<div class="space-bottom"></div>

<h2>Alerts</h2>
<div class="alert alert-success" role="alert">
	<strong>Well done!</strong> You successfully read this important alert message.
</div>
<div class="alert alert-info" role="alert">
	<strong>Heads up!</strong> This alert needs your attention, but it's not super important.
</div>
<div class="alert alert-warning" role="alert">
	<strong>Warning!</strong> Better check yourself, you're not looking too good.
</div>
<div class="alert alert-danger" role="alert">
	<strong>Oh snap!</strong> Change a few things up and try submitting again.
</div>
<div class="space-bottom"></div>

<h2>Dismissible Alerts</h2>
<div class="alert alert-warning alert-dismissible" role="alert">
	<button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
	<strong>Warning!</strong> Lorem ipsum dolor sit amet, consectetur adipisicing elit. Quos a voluptatem nisi. Iure velit eum placeat, doloribus veritatis dolorem veniam blanditiis optio animi praesentium. Laboriosam, illum sed asperiores perspiciatis recusandae!.
</div>
<div class="alert alert-danger fade in" role="alert">
	<button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">?</span><span class="sr-only">Close</span></button>
	<h4>Oh snap! You got an error!</h4>
	<p>Change this and that and try again. Duis mollis, est non commodo luctus, nisi erat porttitor ligula, eget lacinia odio sem nec elit. Cras mattis consectetur purus sit amet fermentum.</p>
	<p>
		<button type="button" class="btn btn-danger">Take this action</button>
		<button type="button" class="btn btn-danger">Or do this</button>
	</p>
</div>
<div class="space-bottom"></div>

<h2>Call to Action Blocks</h2>
<br />

<div class="section dark-bg clearfix">
	<div class="call-to-action">
		<div class="row">
			<div class="col-md-8">
				<h1 class="title text-center">Waste no more time</h1>
				<p class="text-center">Sed ut Perspiciatis Unde Omnis Iste Sed ut sit  voluptatem accusan tium </p>
			</div>
			<div class="col-md-4">
				<div class="text-center">
					<a href="#" class="btn btn-default btn-lg">Purchase</a>
				</div>
			</div>
		</div>
	</div>
</div>
<div class="space"></div>

<div class="section gray-bg clearfix">
	<div class="call-to-action">
		<div class="row">
			<div class="col-md-8">
				<h1 class="title text-center">We offer more possibilities to meet your every need.</h1>
			</div>
			<div class="col-md-4">
				<div class="text-center">
					<a href="#" class="btn btn-default btn-lg">Purchase</a>
				</div>
			</div>
		</div>
	</div>
</div>
<div class="space"></div>

<div class="section default-bg clearfix">
	<div class="call-to-action">
		<div class="row">
			<div class="col-md-8">
				<h1 class="title text-center">Waste no more time</h1>
				<p class="text-center">Sed ut Perspiciatis Unde Omnis Iste Sed ut sit  voluptatem accusan tium </p>
			</div>
			<div class="col-md-4">
				<div class="text-center">
					<a href="#" class="btn btn-white btn-lg">Purchase</a>
				</div>
			</div>
		</div>
	</div>
</div>
<div class="space-bottom"></div>
<br>
<h2>Content Slider with Controls (Autoplay off)</h2>
<div class="owl-carousel content-slider-with-controls  photo-block">
	<div class="overlay-container">
		<img src="#SITE_DIR#images/content.jpg" alt="">
		<div class="caption">
			<h3 class="title">Caption Title</h3>
			Lorem ipsum dolor sit amet, consectetur adipisicing elit. Laborum optio odio, recusandae qui minima eveniet cumque praesentium expedita sint accusamus itaque.
		</div>
		<a href="#SITE_DIR#images/content.jpg" class="popup-img-single overlay" title="image title"><i class="fa fa-search-plus"></i></a>
	</div>
	<div class="overlay-container">
		<img src="#SITE_DIR#images/content.jpg" alt="">
		<div class="caption">
			<h3 class="title">Caption Title</h3>
			Dolor sit amet, consectetur adipisicing elit. Laborum optio odio, recusandae qui minima eveniet cumque praesentium expedita sint accusamus itaque.
		</div>
		<a href="#SITE_DIR#images/content.jpg" class="popup-img-single overlay" title="image title"><i class="fa fa-search-plus"></i></a>
	</div>
	<div class="overlay-container">
		<img src="#SITE_DIR#images/content.jpg" alt="">
		<div class="caption">
			<h3 class="title">Caption Title</h3>
			Ipsum dolor sit amet, consectetur adipisicing elit. Laborum optio odio, recusandae qui minima eveniet cumque praesentium expedita sint accusamus itaque.
		</div>
		<a href="#SITE_DIR#images/content.jpg" class="popup-img-single overlay" title="image title"><i class="fa fa-search-plus"></i></a>
	</div>
</div>
<div class="space-bottom"></div>

<h2>Content Slider with Controls 2 (Autoplay off)</h2>
<div class="owl-carousel content-slider-with-controls-bottom  photo-block">
	<div class="overlay-container">
		<img src="#SITE_DIR#images/content.jpg" alt="">
		<div class="caption">
			<h3 class="title">Caption Title</h3>
			Lorem ipsum dolor sit amet, consectetur adipisicing elit. Laborum optio odio, recusandae qui minima eveniet cumque praesentium expedita sint accusamus itaque.
		</div>
		<a href="#SITE_DIR#images/content.jpg" class="popup-img overlay" title="image title"><i class="fa fa-search-plus"></i></a>
	</div>
	<div class="overlay-container">
		<img src="#SITE_DIR#images/content.jpg" alt="">
		<div class="caption">
			<h3 class="title">Caption Title</h3>
			Dolor sit amet, consectetur adipisicing elit. Laborum optio odio, recusandae qui minima eveniet cumque praesentium expedita sint accusamus itaque.
		</div>
		<a href="#SITE_DIR#images/content.jpg" class="popup-img overlay" title="image title"><i class="fa fa-search-plus"></i></a>
	</div>
	<div class="overlay-container">
		<img src="#SITE_DIR#images/content.jpg" alt="">
		<div class="caption">
			<h3 class="title">Caption Title</h3>
			Ipsum dolor sit amet, consectetur adipisicing elit. Laborum optio odio, recusandae qui minima eveniet cumque praesentium expedita sint accusamus itaque.
		</div>
		<a href="#SITE_DIR#images/content.jpg" class="popup-img overlay" title="image title"><i class="fa fa-search-plus"></i></a>
	</div>
</div>
<div class="space-bottom"></div>

<h2>Content Slider without controls (Autoplay on)</h2>
<div class="owl-carousel content-slider  photo-block">
	<div class="overlay-container">
		<img src="#SITE_DIR#images/content.jpg" alt="">
		<div class="caption">
			<h3 class="title">Caption Title</h3>
			Lorem ipsum dolor sit amet, consectetur adipisicing elit. Laborum optio odio, recusandae qui minima eveniet cumque praesentium expedita sint accusamus itaque.
		</div>
		<a href="#SITE_DIR#images/content.jpg" class="popup-img overlay" title="image title"><i class="fa fa-search-plus"></i></a>
	</div>
	<div class="overlay-container">
		<img src="#SITE_DIR#images/content.jpg" alt="">
		<div class="caption">
			<h3 class="title">Caption Title</h3>
			Dolor sit amet, consectetur adipisicing elit. Laborum optio odio, recusandae qui minima eveniet cumque praesentium expedita sint accusamus itaque.
		</div>
		<a href="#SITE_DIR#images/content.jpg" class="popup-img overlay" title="image title"><i class="fa fa-search-plus"></i></a>
	</div>
	<div class="overlay-container">
		<img src="#SITE_DIR#images/content.jpg" alt="">
		<div class="caption">
			<h3 class="title">Caption Title</h3>
			Ipsum dolor sit amet, consectetur adipisicing elit. Laborum optio odio, recusandae qui minima eveniet cumque praesentium expedita sint accusamus itaque.
		</div>
		<a href="#SITE_DIR#images/content.jpg" class="popup-img overlay" title="image title"><i class="fa fa-search-plus"></i></a>
	</div>
</div>
<div class="space-bottom"></div>

<h2>Content Slider with Controls (Autoplay on)</h2>
<div class="owl-carousel content-slider-with-controls-autoplay  photo-block">
	<div class="overlay-container">
		<img src="#SITE_DIR#images/content.jpg" alt="">
		<div class="caption">
			<h3 class="title">Caption Title</h3>
			Lorem ipsum dolor sit amet, consectetur adipisicing elit. Laborum optio odio, recusandae qui minima eveniet cumque praesentium expedita sint accusamus itaque.
		</div>
		<a href="#SITE_DIR#images/content.jpg" class="popup-img overlay" title="image title"><i class="fa fa-search-plus"></i></a>
	</div>
	<div class="overlay-container">
		<img src="#SITE_DIR#images/content.jpg" alt="">
		<div class="caption">
			<h3 class="title">Caption Title</h3>
			Dolor sit amet, consectetur adipisicing elit. Laborum optio odio, recusandae qui minima eveniet cumque praesentium expedita sint accusamus itaque.
		</div>
		<a href="#SITE_DIR#images/content.jpg" class="popup-img overlay" title="image title"><i class="fa fa-search-plus"></i></a>
	</div>
	<div class="overlay-container">
		<img src="#SITE_DIR#images/content.jpg" alt="">
		<div class="caption">
			<h3 class="title">Caption Title</h3>
			Ipsum dolor sit amet, consectetur adipisicing elit. Laborum optio odio, recusandae qui minima eveniet cumque praesentium expedita sint accusamus itaque.
		</div>
		<a href="#SITE_DIR#images/content.jpg" class="popup-img overlay" title="image title"><i class="fa fa-search-plus"></i></a>
	</div>
</div>
<div class="space-bottom"></div>

<h2>Version 1</h2>

<div class="row gray-bg">
	<div class="col-md-12">
		<div class="row">
			<div class="col-sm-4">
				<div class="box-style-1 white-bg object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="0">
					<i class="fa fa-leaf"></i>
					<h2>Clean Design and Code</h2>
					<p>Voluptatem ad provident non <a href="#">repudiandae</a> veritatis beatae cupiditate amet reiciendis.</p>
					<a href="#" class="btn-default btn">Read More</a>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-1 white-bg object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="200">
					<i class="fa fa-laptop"></i>
					<h2>Very Flexible</h2>
					<p>Iure sequi unde hic. Sapiente quaerat labore sequi inventore veritatis cumque.</p>
					<a href="#" class="btn-default btn">Read More</a>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-1 white-bg object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="400">
					<i class="fa fa-sitemap"></i>
					<h2>Latest Technologies</h2>
					<p>Inventore dolores aut laboriosam cum consequuntur delectus sequi! Eum est.</p>
					<a href="#" class="btn-default btn">Read More</a>
				</div>
			</div>
		</div>
	</div>
</div>

<div class="row">
	<div class="col-md-12">
		<div class="row">
			<div class="col-sm-4">
				<div class="box-style-1 gray-bg object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="0">
					<i class="fa fa-leaf"></i>
					<h2>Clean Design and Code</h2>
					<p>Voluptatem ad provident non <a href="#">repudiandae</a> veritatis beatae cupiditate amet reiciendis.</p>
					<a href="#" class="btn-default btn">Read More</a>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-1 gray-bg object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="200">
					<i class="fa fa-laptop"></i>
					<h2>Very Flexible</h2>
					<p>Iure sequi unde hic. Sapiente quaerat labore sequi inventore veritatis cumque.</p>
					<a href="#" class="btn-default btn">Read More</a>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-1 gray-bg object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="400">
					<i class="fa fa-sitemap"></i>
					<h2>Latest Technologies</h2>
					<p>Inventore dolores aut laboriosam cum consequuntur delectus sequi! Eum est.</p>
					<a href="#" class="btn-default btn">Read More</a>
				</div>
			</div>
		</div>
	</div>
</div>

<div class="row">
	<div class="col-md-12">
		<div class="row">
			<div class="col-sm-4">
				<div class="box-style-1 default-bg object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="0">
					<i class="fa fa-leaf"></i>
					<h2>Clean Design and Code</h2>
					<p>Voluptatem ad provident non <a href="#">repudiandae</a> veritatis beatae cupiditate amet reiciendis.</p>
					<a href="#" class="btn-default btn">Read More</a>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-1 default-bg object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="200">
					<i class="fa fa-laptop"></i>
					<h2>Very Flexible</h2>
					<p>Iure sequi unde hic. Sapiente quaerat labore sequi inventore veritatis cumque.</p>
					<a href="#" class="btn-default btn">Read More</a>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-1 default-bg object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="400">
					<i class="fa fa-sitemap"></i>
					<h2>Latest Technologies</h2>
					<p>Inventore dolores aut laboriosam cum consequuntur delectus sequi! Eum est.</p>
					<a href="#" class="btn-default btn">Read More</a>
				</div>
			</div>
		</div>
	</div>
</div>

<div class="row">
	<div class="col-md-12">
		<div class="row">
			<div class="col-sm-4">
				<div class="box-style-1 dark-bg object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="0">
					<i class="fa fa-leaf"></i>
					<h2>Clean Design and Code</h2>
					<p>Voluptatem ad provident non <a href="#">repudiandae</a> veritatis beatae cupiditate amet reiciendis.</p>
					<a href="#" class="btn-default btn">Read More</a>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-1 dark-bg object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="200">
					<i class="fa fa-laptop"></i>
					<h2>Very Flexible</h2>
					<p>Iure sequi unde hic. Sapiente quaerat labore sequi inventore veritatis cumque.</p>
					<a href="#" class="btn-default btn">Read More</a>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-1 dark-bg object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="400">
					<i class="fa fa-sitemap"></i>
					<h2>Latest Technologies</h2>
					<p>Inventore dolores aut laboriosam cum consequuntur delectus sequi! Eum est.</p>
					<a href="#" class="btn-default btn">Read More</a>
				</div>
			</div>
		</div>
	</div>
</div>

<div class="row gray-bg">
	<div class="col-md-12">
		<div class="row">
			<div class="col-sm-4">
				<div class="box-style-1 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="0">
					<i class="fa fa-leaf"></i>
					<h2>Clean Design and Code</h2>
					<p>Voluptatem ad provident non <a href="#">repudiandae</a> veritatis beatae cupiditate amet reiciendis.</p>
					<a href="#" class="btn-default btn">Read More</a>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-1 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="200">
					<i class="fa fa-laptop"></i>
					<h2>Very Flexible</h2>
					<p>Iure sequi unde hic. Sapiente quaerat labore sequi inventore veritatis cumque.</p>
					<a href="#" class="btn-default btn">Read More</a>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-1 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="400">
					<i class="fa fa-sitemap"></i>
					<h2>Latest Technologies</h2>
					<p>Inventore dolores aut laboriosam cum consequuntur delectus sequi! Eum est.</p>
					<a href="#" class="btn-default btn">Read More</a>
				</div>
			</div>
		</div>
	</div>
</div>
<div class="space-bottom"></div>

<h2>Version 2</h2>
<div class="row">
	<div class="col-md-12">
		<div class="row grid-space-10">
			<div class="col-sm-4">
				<div class="box-style-2 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="0">
					<div class="icon-container default-bg">
						<i class="fa fa-leaf"></i>
					</div>
					<div class="body">
						<h2>Clean Design and Code</h2>
						<p>Voluptatem ad provident non veritatis beatae cupiditate amet reiciendis.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-2 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="200">
					<div class="icon-container default-bg">
						<i class="fa fa-laptop"></i>
					</div>
					<div class="body">
						<h2>Very Flexible</h2>
						<p>Iure sequi unde hic. Sapiente quaerat labore inventore veritatis cumque.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-2 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="400">
					<div class="icon-container default-bg">
						<i class="fa fa-sitemap"></i>
					</div>
					<div class="body">
						<h2>Latest Technologies</h2>
						<p>Inventore dolores aut laboriosam cum consequuntur delectus sequi! Eum est.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
		</div>
		<div class="row grid-space-10">
			<div class="col-sm-4">
				<div class="box-style-2 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="600">
					<div class="icon-container default-bg">
						<i class="fa fa-bold"></i>
					</div>
					<div class="body">
						<h2>Bootstrap Based</h2>
						<p>Voluptatem ad provident non veritatis beatae cupiditate amet reiciendis.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-2 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="800">
					<div class="icon-container default-bg">
						<i class="fa fa-check"></i>
					</div>
					<div class="body">
						<h2>Unlimited Options</h2>
						<p>Iure sequi unde hic. Sapiente quaerat labore inventore veritatis cumque.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-2 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="1000">
					<div class="icon-container default-bg">
						<i class="fa fa-bicycle"></i>
					</div>
					<div class="body">
						<h2>Easy to Use</h2>
						<p>Inventore dolores aut laboriosam cum consequuntur delectus sequi! Eum est.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>

<div class="row">
	<div class="col-md-12">
		<div class="row">
			<div class="col-sm-4">
				<div class="box-style-2 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="0">
					<div class="icon-container dark-bg">
						<i class="fa fa-leaf"></i>
					</div>
					<div class="body">
						<h2>Clean Design and Code</h2>
						<p>Voluptatem ad provident non veritatis beatae cupiditate amet reiciendis.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-2 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="200">
					<div class="icon-container dark-bg">
						<i class="fa fa-laptop"></i>
					</div>
					<div class="body">
						<h2>Very Flexible</h2>
						<p>Iure sequi unde hic. Sapiente quaerat labore inventore veritatis cumque.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-2 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="400">
					<div class="icon-container dark-bg">
						<i class="fa fa-sitemap"></i>
					</div>
					<div class="body">
						<h2>Latest Technologies</h2>
						<p>Inventore dolores aut laboriosam cum consequuntur delectus sequi! Eum est.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
		</div>
		<div class="row">
			<div class="col-sm-4">
				<div class="box-style-2 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="600">
					<div class="icon-container dark-bg">
						<i class="fa fa-bold"></i>
					</div>
					<div class="body">
						<h2>Bootstrap Based</h2>
						<p>Voluptatem ad provident non veritatis beatae cupiditate amet reiciendis.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-2 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="800">
					<div class="icon-container dark-bg">
						<i class="fa fa-check"></i>
					</div>
					<div class="body">
						<h2>Unlimited Options</h2>
						<p>Iure sequi unde hic. Sapiente quaerat labore inventore veritatis cumque.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-2 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="1000">
					<div class="icon-container dark-bg">
						<i class="fa fa-bicycle"></i>
					</div>
					<div class="body">
						<h2>Easy to Use</h2>
						<p>Inventore dolores aut laboriosam cum consequuntur delectus sequi! Eum est.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>

<div class="row">
	<div class="col-md-12">
		<div class="row">
			<div class="col-sm-4">
				<div class="box-style-2 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="0">
					<div class="icon-container">
						<i class="fa fa-leaf"></i>
					</div>
					<div class="body">
						<h2>Clean Design and Code</h2>
						<p>Voluptatem ad provident non veritatis beatae cupiditate amet reiciendis.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-2 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="200">
					<div class="icon-container">
						<i class="fa fa-laptop"></i>
					</div>
					<div class="body">
						<h2>Very Flexible</h2>
						<p>Iure sequi unde hic. Sapiente quaerat labore inventore veritatis cumque.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-2 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="400">
					<div class="icon-container">
						<i class="fa fa-sitemap"></i>
					</div>
					<div class="body">
						<h2>Latest Technologies</h2>
						<p>Inventore dolores aut laboriosam cum consequuntur delectus sequi! Eum est.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
		</div>
		<div class="row">
			<div class="col-sm-4">
				<div class="box-style-2 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="600">
					<div class="icon-container">
						<i class="fa fa-bold"></i>
					</div>
					<div class="body">
						<h2>Bootstrap Based</h2>
						<p>Voluptatem ad provident non veritatis beatae cupiditate amet reiciendis.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-2 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="800">
					<div class="icon-container">
						<i class="fa fa-check"></i>
					</div>
					<div class="body">
						<h2>Unlimited Options</h2>
						<p>Iure sequi unde hic. Sapiente quaerat labore inventore veritatis cumque.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-2 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="1000">
					<div class="icon-container">
						<i class="fa fa-bicycle"></i>
					</div>
					<div class="body">
						<h2>Easy to Use</h2>
						<p>Inventore dolores aut laboriosam cum consequuntur delectus sequi! Eum est.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>
<div class="space-bottom"></div>
					
<div class="row gray-bg">
	<div class="col-md-12">
		<div class="row">
			<div class="col-sm-4">
				<div class="box-style-2 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="0">
					<div class="icon-container white-bg">
						<i class="fa fa-leaf"></i>
					</div>
					<div class="body">
						<h2>Clean Design and Code</h2>
						<p>Voluptatem ad provident non veritatis beatae cupiditate amet reiciendis.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-2 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="200">
					<div class="icon-container white-bg">
						<i class="fa fa-laptop"></i>
					</div>
					<div class="body">
						<h2>Very Flexible</h2>
						<p>Iure sequi unde hic. Sapiente quaerat labore inventore veritatis cumque.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-2 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="400">
					<div class="icon-container white-bg">
						<i class="fa fa-sitemap"></i>
					</div>
					<div class="body">
						<h2>Latest Technologies</h2>
						<p>Inventore dolores aut laboriosam cum consequuntur delectus sequi! Eum est.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
		</div>
		<div class="row">
			<div class="col-sm-4">
				<div class="box-style-2 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="600">
					<div class="icon-container white-bg">
						<i class="fa fa-bold"></i>
					</div>
					<div class="body">
						<h2>Bootstrap Based</h2>
						<p>Voluptatem ad provident non veritatis beatae cupiditate amet reiciendis.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-2 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="800">
					<div class="icon-container white-bg">
						<i class="fa fa-check"></i>
					</div>
					<div class="body">
						<h2>Unlimited Options</h2>
						<p>Iure sequi unde hic. Sapiente quaerat labore inventore veritatis cumque.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="box-style-2 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="1000">
					<div class="icon-container white-bg">
						<i class="fa fa-bicycle"></i>
					</div>
					<div class="body">
						<h2>Easy to Use</h2>
						<p>Inventore dolores aut laboriosam cum consequuntur delectus sequi! Eum est.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>
<div class="space-bottom"></div>

<h2>Version 3</h2>
<div class="row">
	<div class="col-md-12">
		<div class="row">
			<div class="col-sm-6">
				<div class="box-style-3 right object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="0">
					<div class="icon-container default-bg">
						<i class="fa fa-bold"></i>
					</div>
					<div class="body">
						<h2>Bootstrap Based</h2>
						<p>Voluptatem ad provident non veritatis beatae cupiditate amet reiciendis.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
				<div class="box-style-3 right object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="200">
					<div class="icon-container default-bg">
						<i class="fa fa-check"></i>
					</div>
					<div class="body">
						<h2>Unlimited Options</h2>
						<p>Iure sequi unde hic. Sapiente quaerat labore inventore veritatis cumque.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
				<div class="box-style-3 right object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="400">
					<div class="icon-container default-bg">
						<i class="fa fa-bicycle"></i>
					</div>
					<div class="body">
						<h2>Easy to Use</h2>
						<p>Inventore dolores aut laboriosam cum consequuntur delectus sequi! Eum est.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
			<div class="col-sm-6">
				<div class="box-style-3 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="0">
					<div class="icon-container default-bg">
						<i class="fa fa-leaf"></i>
					</div>
					<div class="body">
						<h2>Clean Design and Code</h2>
						<p>Voluptatem ad provident non veritatis beatae cupiditate amet reiciendis.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
				<div class="box-style-3 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="200">
					<div class="icon-container default-bg">
						<i class="fa fa-laptop"></i>
					</div>
					<div class="body">
						<h2>Very Flexible</h2>
						<p>Iure sequi unde hic. Sapiente quaerat labore inventore veritatis cumque.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
				<div class="box-style-3 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="400">
					<div class="icon-container default-bg">
						<i class="fa fa-sitemap"></i>
					</div>
					<div class="body">
						<h2>Latest Technologies</h2>
						<p>Inventore dolores aut laboriosam cum consequuntur delectus sequi! Eum est.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
		</div>
		<div class="row">
			<div class="col-sm-6">
				<div class="box-style-3 right object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="0">
					<div class="icon-container dark-bg">
						<i class="fa fa-bold"></i>
					</div>
					<div class="body">
						<h2>Bootstrap Based</h2>
						<p>Voluptatem ad provident non veritatis beatae cupiditate amet reiciendis.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
				<div class="box-style-3 right object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="200">
					<div class="icon-container dark-bg">
						<i class="fa fa-check"></i>
					</div>
					<div class="body">
						<h2>Unlimited Options</h2>
						<p>Iure sequi unde hic. Sapiente quaerat labore inventore veritatis cumque.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
				<div class="box-style-3 right object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="400">
					<div class="icon-container dark-bg">
						<i class="fa fa-bicycle"></i>
					</div>
					<div class="body">
						<h2>Easy to Use</h2>
						<p>Inventore dolores aut laboriosam cum consequuntur delectus sequi! Eum est.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
			<div class="col-sm-6">
				<div class="box-style-3 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="0">
					<div class="icon-container dark-bg">
						<i class="fa fa-leaf"></i>
					</div>
					<div class="body">
						<h2>Clean Design and Code</h2>
						<p>Voluptatem ad provident non veritatis beatae cupiditate amet reiciendis.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
				<div class="box-style-3 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="200">
					<div class="icon-container dark-bg">
						<i class="fa fa-laptop"></i>
					</div>
					<div class="body">
						<h2>Very Flexible</h2>
						<p>Iure sequi unde hic. Sapiente quaerat labore inventore veritatis cumque.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
				<div class="box-style-3 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="400">
					<div class="icon-container dark-bg">
						<i class="fa fa-sitemap"></i>
					</div>
					<div class="body">
						<h2>Latest Technologies</h2>
						<p>Inventore dolores aut laboriosam cum consequuntur delectus sequi! Eum est.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
		</div>
		<div class="row">
			<div class="col-sm-6">
				<div class="box-style-3 right object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="0">
					<div class="icon-container">
						<i class="fa fa-bold"></i>
					</div>
					<div class="body">
						<h2>Bootstrap Based</h2>
						<p>Voluptatem ad provident non veritatis beatae cupiditate amet reiciendis.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
				<div class="box-style-3 right object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="200">
					<div class="icon-container">
						<i class="fa fa-check"></i>
					</div>
					<div class="body">
						<h2>Unlimited Options</h2>
						<p>Iure sequi unde hic. Sapiente quaerat labore inventore veritatis cumque.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
				<div class="box-style-3 right object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="400">
					<div class="icon-container">
						<i class="fa fa-bicycle"></i>
					</div>
					<div class="body">
						<h2>Easy to Use</h2>
						<p>Inventore dolores aut laboriosam cum consequuntur delectus sequi! Eum est.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
			<div class="col-sm-6">
				<div class="box-style-3 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="0">
					<div class="icon-container">
						<i class="fa fa-leaf"></i>
					</div>
					<div class="body">
						<h2>Clean Design and Code</h2>
						<p>Voluptatem ad provident non veritatis beatae cupiditate amet reiciendis.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
				<div class="box-style-3 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="200">
					<div class="icon-container">
						<i class="fa fa-laptop"></i>
					</div>
					<div class="body">
						<h2>Very Flexible</h2>
						<p>Iure sequi unde hic. Sapiente quaerat labore inventore veritatis cumque.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
				<div class="box-style-3 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="400">
					<div class="icon-container">
						<i class="fa fa-sitemap"></i>
					</div>
					<div class="body">
						<h2>Latest Technologies</h2>
						<p>Inventore dolores aut laboriosam cum consequuntur delectus sequi! Eum est.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>
<div class="space-bottom"></div>

<div class="row gray-bg">
	<div class="col-md-12">
		<div class="row">
			<div class="col-sm-6">
				<div class="box-style-3 right object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="0">
					<div class="icon-container white-bg">
						<i class="fa fa-bold"></i>
					</div>
					<div class="body">
						<h2>Bootstrap Based</h2>
						<p>Voluptatem ad provident non veritatis beatae cupiditate amet reiciendis.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
				<div class="box-style-3 right object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="200">
					<div class="icon-container white-bg">
						<i class="fa fa-check"></i>
					</div>
					<div class="body">
						<h2>Unlimited Options</h2>
						<p>Iure sequi unde hic. Sapiente quaerat labore inventore veritatis cumque.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
				<div class="box-style-3 right object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="400">
					<div class="icon-container white-bg">
						<i class="fa fa-bicycle"></i>
					</div>
					<div class="body">
						<h2>Easy to Use</h2>
						<p>Inventore dolores aut laboriosam cum consequuntur delectus sequi! Eum est.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
			<div class="col-sm-6">
				<div class="box-style-3 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="0">
					<div class="icon-container white-bg">
						<i class="fa fa-leaf"></i>
					</div>
					<div class="body">
						<h2>Clean Design and Code</h2>
						<p>Voluptatem ad provident non veritatis beatae cupiditate amet reiciendis.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
				<div class="box-style-3 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="200">
					<div class="icon-container white-bg">
						<i class="fa fa-laptop"></i>
					</div>
					<div class="body">
						<h2>Very Flexible</h2>
						<p>Iure sequi unde hic. Sapiente quaerat labore inventore veritatis cumque.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
				<div class="box-style-3 object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="400">
					<div class="icon-container white-bg">
						<i class="fa fa-sitemap"></i>
					</div>
					<div class="body">
						<h2>Latest Technologies</h2>
						<p>Inventore dolores aut laboriosam cum consequuntur delectus sequi! Eum est.</p>
						<a href="#" class="link"><span>Read More</span></a>
					</div>
				</div>
			</div>
		</div>
		<div class="space"></div>
	</div>
</div>
<div class="space-bottom"></div>

<h2>Icon Box Single</h2>
<div class="space-bottom"></div>
<div class="clearfix">
	<div class="box gray-bg">
		<i class="fa fa-leaf"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-birthday-cake"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-android"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-bullhorn"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-camera-retro"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-briefcase"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-bank"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-cc-paypal"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-cc-visa"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-coffee"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-child"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-certificate"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-cutlery"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-desktop"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-comments"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-database"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-envelope-o"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-home"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-life-bouy"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-male"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-music"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-map-marker"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-location-arrow"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-heart"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-graduation-cap"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-instagram"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-puzzle-piece"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-shield"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-space-shuttle"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-thumbs-o-up"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-support"></i>
	</div>
	<div class="box gray-bg">
		<i class="fa fa-video-camera"></i>
	</div>
</div>

<div class="clearfix">
	<div class="space"></div>
	<div class="box small gray-bg">
		<i class="fa fa-leaf"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-birthday-cake"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-android"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-bullhorn"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-camera-retro"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-briefcase"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-bank"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-cc-paypal"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-cc-visa"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-coffee"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-child"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-certificate"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-cutlery"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-desktop"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-comments"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-database"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-envelope-o"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-home"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-life-bouy"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-male"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-music"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-map-marker"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-location-arrow"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-heart"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-graduation-cap"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-instagram"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-puzzle-piece"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-shield"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-space-shuttle"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-thumbs-o-up"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-support"></i>
	</div>
	<div class="box small gray-bg">
		<i class="fa fa-video-camera"></i>
	</div>
</div>

<div class="clearfix">
	<div class="space"></div>
	<div class="box default-bg">
		<i class="fa fa-leaf"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-birthday-cake"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-android"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-bullhorn"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-camera-retro"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-briefcase"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-bank"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-cc-paypal"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-cc-visa"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-coffee"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-child"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-certificate"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-cutlery"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-desktop"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-comments"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-database"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-envelope-o"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-home"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-life-bouy"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-male"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-music"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-map-marker"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-location-arrow"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-heart"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-graduation-cap"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-instagram"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-puzzle-piece"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-shield"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-space-shuttle"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-thumbs-o-up"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-support"></i>
	</div>
	<div class="box default-bg">
		<i class="fa fa-video-camera"></i>
	</div>
</div>

<div class="clearfix">
	<div class="space"></div>
	<div class="box small default-bg">
		<i class="fa fa-leaf"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-birthday-cake"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-android"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-bullhorn"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-camera-retro"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-briefcase"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-bank"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-cc-paypal"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-cc-visa"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-coffee"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-child"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-certificate"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-cutlery"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-desktop"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-comments"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-database"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-envelope-o"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-home"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-life-bouy"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-male"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-music"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-map-marker"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-location-arrow"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-heart"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-graduation-cap"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-instagram"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-puzzle-piece"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-shield"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-space-shuttle"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-thumbs-o-up"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-support"></i>
	</div>
	<div class="box small default-bg">
		<i class="fa fa-video-camera"></i>
	</div>
</div>

<div class="clearfix">
	<div class="space"></div>
	<div class="box dark-bg">
		<i class="fa fa-leaf"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-birthday-cake"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-android"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-bullhorn"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-camera-retro"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-briefcase"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-bank"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-cc-paypal"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-cc-visa"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-coffee"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-child"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-certificate"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-cutlery"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-desktop"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-comments"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-database"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-envelope-o"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-home"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-life-bouy"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-male"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-music"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-map-marker"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-location-arrow"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-heart"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-graduation-cap"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-instagram"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-puzzle-piece"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-shield"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-space-shuttle"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-thumbs-o-up"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-support"></i>
	</div>
	<div class="box dark-bg">
		<i class="fa fa-video-camera"></i>
	</div>
</div>

<div class="clearfix">
	<div class="space"></div>
	<div class="box small dark-bg">
		<i class="fa fa-leaf"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-birthday-cake"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-android"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-bullhorn"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-camera-retro"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-briefcase"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-bank"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-cc-paypal"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-cc-visa"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-coffee"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-child"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-certificate"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-cutlery"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-desktop"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-comments"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-database"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-envelope-o"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-home"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-life-bouy"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-male"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-music"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-map-marker"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-location-arrow"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-heart"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-graduation-cap"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-instagram"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-puzzle-piece"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-shield"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-space-shuttle"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-thumbs-o-up"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-support"></i>
	</div>
	<div class="box small dark-bg">
		<i class="fa fa-video-camera"></i>
	</div>
</div>
<div class="space-bottom"></div>

<div class="alert alert-info alert-dismissible" role="alert">
	<button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">?</span><span class="sr-only">Close</span></button>
	Check all available Icons: 
	<ul>
		<li><a target="_blank" href="http://fontawesome.io/icons/" class="alert-link">Font Awesome Icons</a></li>
		<li><a target="_blank" href="http://fontello.com/" class="alert-link">Fontello Icons</a></li>
		<li><a target="_blank" href="http://getbootstrap.com/components" class="alert-link">Glyphicons Icons</a></li>
	</ul>
</div>
<div class="space-bottom"></div>

<h2>Animations</h2>
<div class="row">
	<div class="col-md-3">
		<h4 class="text-center">Fade In Left</h4>
		<div class="box-style-1 gray-bg object-non-visible" data-animation-effect="fadeInLeft" data-effect-delay="0">
			<i class="fa fa-arrows-alt"></i>
		</div>
	</div>
	<div class="col-md-3">
		<h4 class="text-center">Fade In Right</h4>
		<div class="box-style-1 gray-bg object-non-visible" data-animation-effect="fadeInRight" data-effect-delay="200">
			<i class="fa fa-home"></i>
		</div>
	</div>
	<div class="col-md-3">
		<h4 class="text-center">Fade In Up</h4>
		<div class="box-style-1 gray-bg object-non-visible" data-animation-effect="fadeInUp" data-effect-delay="400">
			<i class="fa fa-camera"></i>
		</div>
	</div>
	<div class="col-md-3">
		<h4 class="text-center">Fade In Down</h4>
		<div class="box-style-1 gray-bg object-non-visible" data-animation-effect="fadeInDown" data-effect-delay="600">
			<i class="fa fa-leaf"></i>
		</div>
	</div>
</div>
<div class="row">
	<div class="col-md-3">
		<h4 class="text-center">Swing</h4>
		<div class="box-style-1 gray-bg object-non-visible" data-animation-effect="swing" data-effect-delay="800">
			<i class="fa fa-laptop"></i>
		</div>
	</div>
	<div class="col-md-3">
		<h4 class="text-center">Pulse</h4>
		<div class="box-style-1 gray-bg object-non-visible" data-animation-effect="pulse" data-effect-delay="1000">
			<i class="fa fa-sitemap"></i>
		</div>
	</div>
	<div class="col-md-3">
		<h4 class="text-center">Fade In</h4>
		<div class="box-style-1 gray-bg object-non-visible" data-animation-effect="fadeIn" data-effect-delay="1200">
			<i class="fa fa-rocket"></i>
		</div>
	</div>
	<div class="col-md-3">
		<h4 class="text-center">Zoom In</h4>
		<div class="box-style-1 gray-bg object-non-visible" data-animation-effect="zoomIn" data-effect-delay="1400">
			<i class="fa fa-file"></i>
		</div>
	</div>
</div>
<div class="row">
	<div class="col-md-3">
		<h4 class="text-center">Bounce</h4>
		<div class="box-style-1 gray-bg object-non-visible" data-animation-effect="bounce" data-effect-delay="400">
			<i class="fa fa-heart"></i>
		</div>
	</div>
	<div class="col-md-3">
		<h4 class="text-center">Fade In Dowm</h4>
		<div class="box-style-1 gray-bg object-non-visible" data-animation-effect="fadeInDown" data-effect-delay="600">
			<i class="fa fa-graduation-cap"></i>
		</div>
	</div>
	<div class="col-md-3">
		<h4 class="text-center">Fade In Left</h4>
		<div class="box-style-1 gray-bg object-non-visible" data-animation-effect="fadeInLeft" data-effect-delay="800">
			<i class="fa fa-instagram"></i>
		</div>
	</div>
	<div class="col-md-3">
		<h4 class="text-center">Fade In Right</h4>
		<div class="box-style-1 gray-bg object-non-visible" data-animation-effect="fadeInRight" data-effect-delay="1000">
			<i class="fa fa-puzzle-piece"></i>
		</div>
	</div>
</div>
<div class="row">
	<div class="col-md-3">
		<h4 class="text-center">Fade In Up</h4>
		<div class="box-style-1 gray-bg object-non-visible" data-animation-effect="fadeInUp" data-effect-delay="400">
			<i class="fa fa-shield"></i>
		</div>
	</div>
	<div class="col-md-3">
		<h4 class="text-center">Rotate In</h4>
		<div class="box-style-1 gray-bg object-non-visible" data-animation-effect="rotateIn" data-effect-delay="600">
			<i class="fa fa-space-shuttle"></i>
		</div>
	</div>
	<div class="col-md-3">
		<h4 class="text-center">Zoom In Down</h4>
		<div class="box-style-1 gray-bg object-non-visible" data-animation-effect="zoomInDown" data-effect-delay="800">
			<i class="fa fa-thumbs-o-up"></i>
		</div>
	</div>
	<div class="col-md-3">
		<h4 class="text-center">Wobble</h4>
		<div class="box-style-1 gray-bg object-non-visible" data-animation-effect="wobble" data-effect-delay="1000">
			<i class="fa fa-support"></i>
		</div>
	</div>
</div>
<div class="alert alert-info alert-dismissible" role="alert">
	<button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
	Check all available Animations at <a  target="_blank" href="http://daneden.github.io/animate.css/" class="alert-link">http://daneden.github.io/animate.css/</a>
</div>
<div class="space-bottom"></div>

<h2>Counters</h2>
<script src="<?=SITE_TEMPLATE_PATH?>/plugins/jquery.countTo.js"></script>
<div class="stats row">
	<div class="col-md-3 col-sm-6">
		<div class="box-style-1">
			<h2 class="title">Projects</h2>
			<i class="fa fa-briefcase"></i>
			<span class="stat-num" data-to="1525" data-speed="3000">0</span>
		</div>
	</div>
	<div class="col-md-3 col-sm-6">
		<div class="box-style-1">
			<h2 class="title">Clients</h2>
			<i class="fa fa-users"></i>
			<span class="stat-num" data-to="1125" data-speed="3000">0</span>
		</div>
	</div>
	<div class="col-md-3 col-sm-6">
		<div class="box-style-1">
			<h2 class="title">Downloads</h2>
			<i class="fa fa-cloud-download"></i>
			<span class="stat-num" data-to="1836" data-speed="3000">0</span>
		</div>
	</div>
	<div class="col-md-3 col-sm-6">
		<div class="box-style-1">
			<h2 class="title">Likes</h2>
			<i class="fa fa-thumbs-o-up"></i>
			<span class="stat-num" data-to="2687" data-speed="3000">0</span>
		</div>
	</div>
</div>
<div class="stats row">
	<div class="col-md-3 col-sm-6">
		<div class="box-style-1 gray-bg">
			<h2 class="title">Projects</h2>
			<i class="fa fa-briefcase"></i>
			<span class="stat-num" data-to="1525" data-speed="3000">0</span>
		</div>
	</div>
	<div class="col-md-3 col-sm-6">
		<div class="box-style-1 gray-bg">
			<h2 class="title">Clients</h2>
			<i class="fa fa-users"></i>
			<span class="stat-num" data-to="1125" data-speed="3000">0</span>
		</div>
	</div>
	<div class="col-md-3 col-sm-6">
		<div class="box-style-1 gray-bg">
			<h2 class="title">Downloads</h2>
			<i class="fa fa-cloud-download"></i>
			<span class="stat-num" data-to="1836" data-speed="3000">0</span>
		</div>
	</div>
	<div class="col-md-3 col-sm-6">
		<div class="box-style-1 gray-bg">
			<h2 class="title">Likes</h2>
			<i class="fa fa-thumbs-o-up"></i>
			<span class="stat-num" data-to="2687" data-speed="3000">0</span>
		</div>
	</div>
</div>
<div class="stats row">
	<div class="col-md-3 col-sm-6">
		<div class="box-style-1 default-bg">
			<h2 class="title">Projects</h2>
			<i class="fa fa-briefcase"></i>
			<span class="stat-num" data-to="1525" data-speed="3000">0</span>
		</div>
	</div>
	<div class="col-md-3 col-sm-6">
		<div class="box-style-1 default-bg">
			<h2 class="title">Clients</h2>
			<i class="fa fa-users"></i>
			<span class="stat-num" data-to="1125" data-speed="3000">0</span>
		</div>
	</div>
	<div class="col-md-3 col-sm-6">
		<div class="box-style-1 default-bg">
			<h2 class="title">Downloads</h2>
			<i class="fa fa-cloud-download"></i>
			<span class="stat-num" data-to="1836" data-speed="3000">0</span>
		</div>
	</div>
	<div class="col-md-3 col-sm-6">
		<div class="box-style-1 default-bg">
			<h2 class="title">Likes</h2>
			<i class="fa fa-thumbs-o-up"></i>
			<span class="stat-num" data-to="2687" data-speed="3000">0</span>
		</div>
	</div>
</div>
<div class="stats row">
	<div class="col-md-3 col-sm-6">
		<div class="box-style-1 dark-bg">
			<h2 class="title">Projects</h2>
			<i class="fa fa-briefcase"></i>
			<span class="stat-num" data-to="1525" data-speed="3000">0</span>
		</div>
	</div>
	<div class="col-md-3 col-sm-6">
		<div class="box-style-1 dark-bg">
			<h2 class="title">Clients</h2>
			<i class="fa fa-users"></i>
			<span class="stat-num" data-to="1125" data-speed="3000">0</span>
		</div>
	</div>
	<div class="col-md-3 col-sm-6">
		<div class="box-style-1 dark-bg">
			<h2 class="title">Downloads</h2>
			<i class="fa fa-cloud-download"></i>
			<span class="stat-num" data-to="1836" data-speed="3000">0</span>
		</div>
	</div>
	<div class="col-md-3 col-sm-6">
		<div class="box-style-1 dark-bg">
			<h2 class="title">Likes</h2>
			<i class="fa fa-thumbs-o-up"></i>
			<span class="stat-num" data-to="2687" data-speed="3000">0</span>
		</div>
	</div>
</div>
<div class="space-bottom"></div>

<h2>Basic table</h2>
<br>
<table class="table">
	<thead>
		<tr>
			<th>#</th>
			<th>First Name</th>
			<th>Last Name</th>
			<th>Username</th>
		</tr>
	</thead>
	<tbody>
		<tr>
			<td>1</td>
			<td>Mark</td>
			<td>Otto</td>
			<td>@mdo</td>
		</tr>
		<tr>
			<td>2</td>
			<td>Jacob</td>
			<td>Thornton</td>
			<td>@fat</td>
		</tr>
		<tr>
			<td>3</td>
			<td>Larry</td>
			<td>the Bird</td>
			<td>@twitter</td>
		</tr>
	</tbody>
</table>
<pre>&lt;table class="table"&gt;...&lt;/table&gt;</pre>
<br>
<br>
<h2>Striped Rows</h2>
<br>
<table class="table table-striped">
	<thead>
		<tr>
			<th>#</th>
			<th>First Name</th>
			<th>Last Name</th>
			<th>Username</th>
		</tr>
	</thead>
	<tbody>
		<tr>
			<td>1</td>
			<td>Mark</td>
			<td>Otto</td>
			<td>@mdo</td>
		</tr>
		<tr>
			<td>2</td>
			<td>Jacob</td>
			<td>Thornton</td>
			<td>@fat</td>
		</tr>
		<tr>
			<td>3</td>
			<td>Larry</td>
			<td>the Bird</td>
			<td>@twitter</td>
		</tr>
	</tbody>
</table>
<pre>&lt;table class="table table-striped"&gt;....&lt;/table&gt;</pre>
<br>
<br>
<h2>Bordered table</h2>
<br>
<table class="table table-bordered">
	<thead>
		<tr>
			<th>#</th>
			<th>First Name</th>
			<th>Last Name</th>
			<th>Username</th>
		</tr>
	</thead>
	<tbody>
		<tr>
			<td rowspan="2">1</td>
			<td>Mark</td>
			<td>Otto</td>
			<td>@mdo</td>
		</tr>
		<tr>
			<td>Mark</td>
			<td>Otto</td>
			<td>@TwBootstrap</td>
		</tr>
		<tr>
			<td>2</td>
			<td>Jacob</td>
			<td>Thornton</td>
			<td>@fat</td>
		</tr>
		<tr>
			<td>3</td>
			<td colspan="2">Larry the Bird</td>
			<td>@twitter</td>
		</tr>
	</tbody>
</table>
<pre>&lt;table class="table table-bordered"&gt;....&lt;/table&gt;</pre>
<br>
<br>
<h2>Hover rows</h2>
<br>
<table class="table table-hover">
	<thead>
		<tr>
			<th>#</th>
			<th>First Name</th>
			<th>Last Name</th>
			<th>Username</th>
		</tr>
	</thead>
	<tbody>
		<tr>
			<td>1</td>
			<td>Mark</td>
			<td>Otto</td>
			<td>@mdo</td>
		</tr>
		<tr>
			<td>2</td>
			<td>Jacob</td>
			<td>Thornton</td>
			<td>@fat</td>
		</tr>
		<tr>
			<td>3</td>
			<td colspan="2">Larry the Bird</td>
			<td>@twitter</td>
		</tr>
	</tbody>
</table>
<pre>&lt;table class="table table-hover"&gt;....&lt;/table&gt;</pre>
<div class="space-bottom"></div>

<h2>Charts</h2>
<script src="<?=SITE_TEMPLATE_PATH?>/plugins/charts/Chart.js"></script>
<div class="alert alert-info alert-dismissible" role="alert">
	<button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">?</span><span class="sr-only">Close</span></button>
	For more info about charts please visit <a target="_blank" class="alert-link" href="http://www.chartjs.org/docs/">http://www.chartjs.org/docs/</a>.
</div>
<br>
<div class="row">
	<div class="col-sm-6">
		<h2>Line Chart</h2>
		<canvas class="graph line" id="lines-graph" height="210" width="270"></canvas>
		<br>
		<br>
	</div>
	<div class="col-sm-6">
		<h2>Bar Chart</h2>
		<canvas class="graph bar" id="bars-graph" height="210" width="270"></canvas>
		<br>
		<br>
	</div>
</div>

<h2>Pie &amp; Doughnut Charts</h2>
<div class="row">
	<div class="col-sm-6">
		<canvas class="graph pie" id="pie-graph" height="300"></canvas>
	</div>
	<div class="col-sm-6">
		<canvas class="graph doughnut" id="doughnut-graph" height="230"></canvas>
	</div>
</div>
<div class="space-bottom"></div>
<br />

<h2>Pricing Tables Stripped</h2>
<div class="pricing-tables stripped object-non-visible" data-animation-effect="fadeInUpSmall">
	<div class="row grid-space-0">
		<div class="col-sm-4 plan stripped">
			<div class="header">
				<h3>Basic</h3>
				<div class="price"><span>Free</span></div>
			</div>
			<ul>
				<li>1 User</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="15GB Storage" data-trigger="hover">1GB Storage</a>
				</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Vivamus sagittis lacus vel augue laoreet rutrum faucibus. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="15 Email Acounts">1 Email Acounts</a>
				</li>
				<li>Subdomains</li>
				<li>Security</li>
				<li>Bandwidth</li>
				<li>Databases</li>
				<li><a class="btn btn-default">Subscribe</a></li>
			</ul>
		</div>
		<div class="col-sm-4 plan stripped best-value">
			<div class="header">
				<h3>Premium</h3>
				<div class="price"><span>$19.99</span>/m.</div>
			</div>
			<ul>
				<li>80 Users</li>
				<li>Unlimited Disk Space</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="Unlimited Subdomains">Unlimited Email Acounts</a>
				</li>        
				<li>Subdomains</li>
				<li>Security</li>
				<li>500 Visitors per month</li>
				<li>1 Database</li>
				<li><a class="btn btn-default"><i class="fa fa-shopping-cart pr-10"></i>Add to cart</a></li>
			</ul>
		</div>
		<div class="col-sm-4 plan stripped">
			<div class="header">
				<h3>Pro</h3>
				<div class="price"><span>$24.99</span>/m.</div>
			</div>
			<ul>
				<li>Unlimited Users</li>
				<li>Unlimited Disk Space</li>
				<li>Unlimited Email Acounts</li>
				<li>Unlimited Subdomains</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="Enchaned Security">Enchaned Security</a>
				</li>
				<li>Unlimited Bandwidth</li>
				<li>Unlimited Databases</li>
				<li><a class="btn btn-default"><i class="fa fa-shopping-cart pr-10"></i>Add to cart</a></li>
			</ul>
		</div>
	</div>
</div>
<div class="pricing-tables stripped object-non-visible" data-animation-effect="fadeInUpSmall">
	<div class="row grid-space-0">
		<div class="col-sm-3 plan stripped">
			<div class="header">
				<h3>Basic</h3>
				<div class="price"><span>Free</span></div>
			</div>
			<ul>
				<li>1 User</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="15GB Storage" data-trigger="hover">1GB Storage</a>
				</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Vivamus sagittis lacus vel augue laoreet rutrum faucibus. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="15 Email Acounts">1 Email Acounts</a>
				</li>
				<li>Subdomains</li>
				<li>Security</li>
				<li>Bandwidth</li>
				<li>Databases</li>
				<li><a class="btn btn-default">Subscribe</a></li>
			</ul>
		</div>
		<div class="col-sm-3 plan stripped">
			<div class="header">
				<h3>Premium</h3>
				<div class="price"><span>$19.99</span>/m.</div>
			</div>
			<ul>
				<li>80 Users</li>
				<li>Unlimited Disk Space</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="Unlimited Subdomains">Unlimited Email Acounts</a>
				</li>        
				<li>Subdomains</li>
				<li>Security</li>
				<li>500 Visitors per month</li>
				<li>1 Database</li>
				<li><a class="btn btn-default"><i class="fa fa-shopping-cart pr-10"></i>Add to cart</a></li>
			</ul>
		</div>
		<div class="col-sm-3 plan stripped best-value">
			<div class="header">
				<h3>Pro</h3>
				<div class="price"><span>$24.99</span>/m.</div>
			</div>
			<ul>
				<li>Unlimited Users</li>
				<li>Unlimited Disk Space</li>
				<li>Unlimited Email Acounts</li>
				<li>Unlimited Subdomains</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="Enchaned Security">Enchaned Security</a>
				</li>
				<li>Unlimited Bandwidth</li>
				<li>Unlimited Databases</li>
				<li><a class="btn btn-default"><i class="fa fa-shopping-cart pr-10"></i>Add to cart</a></li>
			</ul>
		</div>
		<div class="col-sm-3 plan">
			<div class="header">
				<h3>Ultimate</h3>
				<div class="price"><span>$45.99</span>/m.</div>
			</div>
			<ul>
				<li>Unlimited Users</li>
				<li>Unlimited Disk Space</li>
				<li>Unlimited Email Acounts</li>
				<li>Unlimited Subdomains</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="Enchaned Security">Enchaned Security</a>
				</li>
				<li>Unlimited Bandwidth</li>
				<li>Unlimited Databases</li>
				<li><a class="btn btn-default"><i class="fa fa-shopping-cart pr-10"></i>Add to cart</a></li>
			</ul>
		</div>
	</div>
</div>
<div class="space-bottom"></div>

<h2>Pricing Tables Solid Gray</h2>
<div class="pricing-tables gray object-non-visible" data-animation-effect="fadeInUpSmall">
	<div class="row grid-space-0">
		<div class="col-sm-4 plan">
			<div class="header">
				<h3>Basic</h3>
				<div class="price"><span>Free</span></div>
			</div>
			<ul>
				<li>1 User</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="15GB Storage" data-trigger="hover">1GB Storage</a>
				</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Vivamus sagittis lacus vel augue laoreet rutrum faucibus. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="15 Email Acounts">1 Email Acounts</a>
				</li>
				<li>Subdomains</li>
				<li>Security</li>
				<li>Bandwidth</li>
				<li>Databases</li>
				<li><a class="btn btn-gray">Subscribe</a></li>
			</ul>
		</div>
		<div class="col-sm-4 plan best-value">
			<div class="header">
				<h3>Premium</h3>
				<div class="price"><span>$19.99</span>/m.</div>
			</div>
			<ul>
				<li>80 Users</li>
				<li>Unlimited Disk Space</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="Unlimited Subdomains">Unlimited Email Acounts</a>
				</li>        
				<li>Subdomains</li>
				<li>Security</li>
				<li>500 Visitors per month</li>
				<li>1 Database</li>
				<li><a class="btn btn-gray"><i class="fa fa-shopping-cart pr-10"></i>Add to cart</a></li>
			</ul>
		</div>
		<div class="col-sm-4 plan">
			<div class="header">
				<h3>Pro</h3>
				<div class="price"><span>$24.99</span>/m.</div>
			</div>
			<ul>
				<li>Unlimited Users</li>
				<li>Unlimited Disk Space</li>
				<li>Unlimited Email Acounts</li>
				<li>Unlimited Subdomains</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="Enchaned Security">Enchaned Security</a>
				</li>
				<li>Unlimited Bandwidth</li>
				<li>Unlimited Databases</li>
				<li><a class="btn btn-gray"><i class="fa fa-shopping-cart pr-10"></i>Add to cart</a></li>
			</ul>
		</div>
	</div>
</div>
<div class="pricing-tables gray object-non-visible" data-animation-effect="fadeInUpSmall">
	<div class="row grid-space-0">
		<!-- pricing table start -->
		<div class="col-sm-3 plan">
			<div class="header">
				<h3>Basic</h3>
				<div class="price"><span>Free</span></div>
			</div>
			<ul>
				<li>1 User</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="15GB Storage" data-trigger="hover">1GB Storage</a>
				</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Vivamus sagittis lacus vel augue laoreet rutrum faucibus. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="15 Email Acounts">1 Email Acounts</a>
				</li>
				<li>Subdomains</li>
				<li>Security</li>
				<li>Bandwidth</li>
				<li>Databases</li>
				<li><a class="btn btn-gray">Subscribe</a></li>
			</ul>
		</div>
		<div class="col-sm-3 plan">
			<div class="header">
				<h3>Premium</h3>
				<div class="price"><span>$19.99</span>/m.</div>
			</div>
			<ul>
				<li>80 Users</li>
				<li>Unlimited Disk Space</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="Unlimited Subdomains">Unlimited Email Acounts</a>
				</li>        
				<li>Subdomains</li>
				<li>Security</li>
				<li>500 Visitors per month</li>
				<li>1 Database</li>
				<li><a class="btn btn-gray"><i class="fa fa-shopping-cart pr-10"></i>Add to cart</a></li>
			</ul>
		</div>
		<div class="col-sm-3 plan best-value">
			<div class="header">
				<h3>Pro</h3>
				<div class="price"><span>$24.99</span>/m.</div>
			</div>
			<ul>
				<li>Unlimited Users</li>
				<li>Unlimited Disk Space</li>
				<li>Unlimited Email Acounts</li>
				<li>Unlimited Subdomains</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="Enchaned Security">Enchaned Security</a>
				</li>
				<li>Unlimited Bandwidth</li>
				<li>Unlimited Databases</li>
				<li><a class="btn btn-gray"><i class="fa fa-shopping-cart pr-10"></i>Add to cart</a></li>
			</ul>
		</div>
		<div class="col-sm-3 plan">
			<div class="header">
				<h3>Ultimate</h3>
				<div class="price"><span>$45.99</span>/m.</div>
			</div>
			<ul>
				<li>Unlimited Users</li>
				<li>Unlimited Disk Space</li>
				<li>Unlimited Email Acounts</li>
				<li>Unlimited Subdomains</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="Enchaned Security">Enchaned Security</a>
				</li>
				<li>Unlimited Bandwidth</li>
				<li>Unlimited Databases</li>
				<li><a class="btn btn-gray"><i class="fa fa-shopping-cart pr-10"></i>Add to cart</a></li>
			</ul>
		</div>
	</div>
</div>
<div class="space-bottom"></div>

<h2>Pricing Tables White</h2>
<div class="pricing-tables white object-non-visible" data-animation-effect="fadeInUpSmall">
	<div class="row grid-space-0">
		<div class="col-sm-4 plan">
			<div class="header">
				<h3>Basic</h3>
				<div class="price"><span>Free</span></div>
			</div>
			<ul>
				<li>1 User</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="15GB Storage" data-trigger="hover">1GB Storage</a>
				</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Vivamus sagittis lacus vel augue laoreet rutrum faucibus. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="15 Email Acounts">1 Email Acounts</a>
				</li>
				<li>Subdomains</li>
				<li>Security</li>
				<li>Bandwidth</li>
				<li>Databases</li>
				<li><a class="btn btn-default">Subscribe</a></li>
			</ul>
		</div>
		<div class="col-sm-4 plan best-value">
			<div class="header">
				<h3>Premium</h3>
				<div class="price"><span>$19.99</span>/m.</div>
			</div>
			<ul>
				<li>80 Users</li>
				<li>Unlimited Disk Space</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="Unlimited Subdomains">Unlimited Email Acounts</a>
				</li>        
				<li>Subdomains</li>
				<li>Security</li>
				<li>500 Visitors per month</li>
				<li>1 Database</li>
				<li><a class="btn btn-default"><i class="fa fa-shopping-cart pr-10"></i>Add to cart</a></li>
			</ul>
		</div>
		<div class="col-sm-4 plan">
			<div class="header">
				<h3>Pro</h3>
				<div class="price"><span>$24.99</span>/m.</div>
			</div>
			<ul>
				<li>Unlimited Users</li>
				<li>Unlimited Disk Space</li>
				<li>Unlimited Email Acounts</li>
				<li>Unlimited Subdomains</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="Enchaned Security">Enchaned Security</a>
				</li>
				<li>Unlimited Bandwidth</li>
				<li>Unlimited Databases</li>
				<li><a class="btn btn-default"><i class="fa fa-shopping-cart pr-10"></i>Add to cart</a></li>
			</ul>
		</div>
	</div>
</div>
<div class="pricing-tables white object-non-visible" data-animation-effect="fadeInUpSmall">
	<div class="row grid-space-0">
		<div class="col-sm-3 plan">
			<div class="header">
				<h3>Basic</h3>
				<div class="price"><span>Free</span></div>
			</div>
			<ul>
				<li>1 User</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="15GB Storage" data-trigger="hover">1GB Storage</a>
				</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Vivamus sagittis lacus vel augue laoreet rutrum faucibus. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="15 Email Acounts">1 Email Acounts</a>
				</li>
				<li>Subdomains</li>
				<li>Security</li>
				<li>Bandwidth</li>
				<li>Databases</li>
				<li><a class="btn btn-default">Subscribe</a></li>
			</ul>
		</div>
		<div class="col-sm-3 plan">
			<div class="header">
				<h3>Premium</h3>
				<div class="price"><span>$19.99</span>/m.</div>
			</div>
			<ul>
				<li>80 Users</li>
				<li>Unlimited Disk Space</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="Unlimited Subdomains">Unlimited Email Acounts</a>
				</li>        
				<li>Subdomains</li>
				<li>Security</li>
				<li>500 Visitors per month</li>
				<li>1 Database</li>
				<li><a class="btn btn-default"><i class="fa fa-shopping-cart pr-10"></i>Add to cart</a></li>
			</ul>
		</div>
		<div class="col-sm-3 plan best-value">
			<div class="header">
				<h3>Pro</h3>
				<div class="price"><span>$24.99</span>/m.</div>
			</div>
			<ul>
				<li>Unlimited Users</li>
				<li>Unlimited Disk Space</li>
				<li>Unlimited Email Acounts</li>
				<li>Unlimited Subdomains</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="Enchaned Security">Enchaned Security</a>
				</li>
				<li>Unlimited Bandwidth</li>
				<li>Unlimited Databases</li>
				<li><a class="btn btn-default"><i class="fa fa-shopping-cart pr-10"></i>Add to cart</a></li>
			</ul>
		</div>
		<div class="col-sm-3 plan">
			<div class="header">
				<h3>Ultimate</h3>
				<div class="price"><span>$45.99</span>/m.</div>
			</div>
			<ul>
				<li>Unlimited Users</li>
				<li>Unlimited Disk Space</li>
				<li>Unlimited Email Acounts</li>
				<li>Unlimited Subdomains</li>
				<li>
					<a href="#" class="pt-popover" data-toggle="popover" data-placement="right" data-content="Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum." title="" data-original-title="Enchaned Security">Enchaned Security</a>
				</li>
				<li>Unlimited Bandwidth</li>
				<li>Unlimited Databases</li>
				<li><a class="btn btn-default"><i class="fa fa-shopping-cart pr-10"></i>Add to cart</a></li>
			</ul>
		</div>
	</div>
</div>
<div class="space-bottom"></div>

<h2>Grid System</h2>
<br>
<div class="alert alert-info alert-dismissible" role="alert">
	<button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
	<strong>iDea</strong> is based on Bootstrap 3.3.x. Bootstrap includes a responsive, mobile first fluid grid system that appropriately scales up to 12 columns as the device or viewport size increases. It includes predefined classes for easy layout options. For more info about Grid System please visit <a  target="_blank" href="http://getbootstrap.com/css/#grid" class="alert-link">http://getbootstrap.com/css/#grid</a>		
</div>
<br>
<div class="grid">
	<div class="row">
		<div class="col-md-1"><span>1/12</span></div>
		<div class="col-md-1"><span>1/12</span></div>
		<div class="col-md-1"><span>1/12</span></div>
		<div class="col-md-1"><span>1/12</span></div>
		<div class="col-md-1"><span>1/12</span></div>
		<div class="col-md-1"><span>1/12</span></div>
		<div class="col-md-1"><span>1/12</span></div>
		<div class="col-md-1"><span>1/12</span></div>
		<div class="col-md-1"><span>1/12</span></div>
		<div class="col-md-1"><span>1/12</span></div>
		<div class="col-md-1"><span>1/12</span></div>
		<div class="col-md-1"><span>1/12</span></div>
	</div>
	<br>
	<div class="row">
		<div class="col-md-2"><span>1/6</span></div>
		<div class="col-md-2"><span>1/6</span></div>
		<div class="col-md-2"><span>1/6</span></div>
		<div class="col-md-2"><span>1/6</span></div>
		<div class="col-md-2"><span>1/6</span></div>
		<div class="col-md-2"><span>1/6</span></div>
	</div>
	<br>
	<div class="row">
		<div class="col-md-3"><span>1/4</span></div>
		<div class="col-md-3"><span>1/4</span></div>
		<div class="col-md-3"><span>1/4</span></div>
		<div class="col-md-3"><span>1/4</span></div>
	</div>
	<br>
	<div class="row">
		<div class="col-md-4"><span>1/3</span></div>
		<div class="col-md-4"><span>1/3</span></div>
		<div class="col-md-4"><span>1/3</span></div>
	</div>
	<br>
	<div class="row">
		<div class="col-md-6"><span>1/2</span></div>
		<div class="col-md-6"><span>1/2</span></div>
	</div>
	<br>
	<div class="row">
		<div class="col-md-12"><span>1</span></div>
	</div>
</div>
<?require($_SERVER["DOCUMENT_ROOT"]."/bitrix/footer.php");?>