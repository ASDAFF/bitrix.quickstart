...
<?
$arRes = array();
$ID = is_set($_REQUEST, 'ID') ? IntVal($_REQUEST['ID']) : false;
if (false === $ID)
	$ID = is_set($_REQUEST, 'PROPERTY') && is_set($_REQUEST['PROPERTY'], 'PROPERTY_ITEMS') ? IntVal($_REQUEST['PROPERTY']['PROPERTY_ITEMS']) : false;
...
?>
<form id="<?=$arResult["FORM_ID"]?>_order" name="<?=$arResult["FORM_ID"]?>_order" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data" class="ciee-form">
...
foreach($arResult["ITEMS"] as $code => $fieldInfo):
	...
	for($i = 0;$i < $inputNum;$i++)
	{
		...
		switch($fieldInfo['PROPERTY_TYPE']):
			...
			case "S":
			case "N":
				?><input type="text" name="<?=$name?>" size="25" value="<?=$value?>" id="<?=$code?>" <?if ($code =="PROPERTY_QUANTITY") echo 'class="input-quantity"';?>/><?
			...
			case 'CAPTCHA':
				...
				<img src="/bitrix/tools/captcha.php?captcha_sid=<?=$arResult["CAPTCHA_CODE"]?>" width="180" height="40" alt="CAPTCHA" class="ciee-captcha-image" align="left"/>
...
?>
	...
	<input type="submit" name="iblock_submit" value="<?=$arParams['SUBMIT_TEXT']?>" />
</form>