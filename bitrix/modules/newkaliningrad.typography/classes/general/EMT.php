...
class newkaliningrad_EMT_Lib
{
	 ...
	 * <code>
	 *  // Remove UTF-8 chars:
	 * 	$str = newkaliningrad_EMT_Lib::clear_special_chars('your text', 'utf8');
	 *  // ... or HTML codes only:
	 * 	$str = newkaliningrad_EMT_Lib::clear_special_chars('your text', 'html');
	 * 	// ... or combo:
	 *  $str = newkaliningrad_EMT_Lib::clear_special_chars('your text');
	 * </code>
	 *
	 * @param 	string $text
	 * @param   mixed $mode
    ...
    {
    	if ($way) 
        	$text = preg_replace_callback('/(\<\/?)(.+?)(\>)/s', create_function('$m','return $m[1].( substr(trim($m[2]), 0, 1) === "a" ? "%%___"  : ""  ) . newkaliningrad_EMT_Lib::encrypt_tag(trim($m[2]))  . $m[3];'), $text);
        else
        	$text = preg_replace_callback('/(\<\/?)(.+?)(\>)/s', create_function('$m','return $m[1].( substr(trim($m[2]), 0, 3) === "%%___" ? newkaliningrad_EMT_Lib::decrypt_tag(substr(trim($m[2]), 4)) : newkaliningrad_EMT_Lib::decrypt_tag(trim($m[2])) ) . $m[3];'), $text);	
        return $text;
    }
     ...
     * @return 	string
     */
    public static function build_safe_tag($content, $tag = 'span', $attribute = array(), $layout = newkaliningrad_EMT_Lib::LAYOUT_STYLE )
    {
    	$htmlTag = $tag;
    	...
    	if (count($attribute)) 
		{
			...
			if($layout & newkaliningrad_EMT_Lib::LAYOUT_STYLE)
			{
		...
		if( ($layout & newkaliningrad_EMT_Lib::LAYOUT_CLASS ) && $classname) {
	...
	public static function convert_html_entities_to_unicode(&$text)
	{
		$text = preg_replace_callback("/\&#([0-9]+)\;/", 
				create_function('$m', 'return newkaliningrad_EMT_Lib::_getUnicodeChar(intval($m[1]));')
				, $text);
		$text = preg_replace_callback("/\&#x([0-9A-F]+)\;/", 
				create_function('$m', 'return newkaliningrad_EMT_Lib::_getUnicodeChar(hexdec($m[1]));')
				, $text);
		$text = preg_replace_callback("/\&([a-zA-Z0-9]+)\;/", 
				create_function('$m', '$r = newkaliningrad_EMT_Lib::html_char_entity_to_unicode($m[1]); return $r ? $r : $m[0];')
				, $text);
	}
}
...
class EMT_Tret {
	 ...
	 *   newkaliningrad_EMT_Lib::LAYOUT_STYLE - с помощью стилей
	 *   newkaliningrad_EMT_Lib::LAYOUT_CLASS - с помощью классов
	 *
	 * @param int $kind
	 */
	 ...
	 *   newkaliningrad_EMT_Lib::LAYOUT_STYLE - с помощью стилей
	 *   newkaliningrad_EMT_Lib::LAYOUT_CLASS - с помощью классов
	 *   newkaliningrad_EMT_Lib::LAYOUT_STYLE|newkaliningrad_EMT_Lib::LAYOUT_CLASS - оба метода
	 *
	 * @param int $kind
	 */
	public function set_tag_layout($layout = newkaliningrad_EMT_Lib::LAYOUT_STYLE)
	{
		$this->use_layout = $layout;
	 ...
	 *
	 * @see 	newkaliningrad_EMT_Lib::build_safe_tag
	...
	protected function tag($content, $tag = 'span', $attribute = array())
	{
		...
		return newkaliningrad_EMT_Lib::build_safe_tag($content, $tag, $attribute, 
				$this->use_layout === false? newkaliningrad_EMT_Lib::LAYOUT_STYLE  : $this->use_layout );
	}
...
class EMT_Tret_Quote extends EMT_Tret
{
	...
	protected function build_sub_quotations()
	{
		...
		while(true)
		{
			$p = newkaliningrad_EMT_Lib::strpos_ex($this->_text, array("&laquo;", "&raquo;"), $off);
			if($p===false) break;
			if($p['str'] == "&laquo;")
			{
...
class EMT_Tret_Space extends EMT_Tret
{
	...
	public $rules = array(
		...
		'autospace_after_dot' => array(
				...
				'replacement' 	=> array(
						'\1\2. \4',
						'$m[1].$m[2]."." .(in_array(newkaliningrad_EMT_Lib::strtolower($m[3]), $this->domain_zones)? "":" "). $m[3]'
						),
			),	
...
class EMT_Tret_Text extends EMT_Tret
{
	...
	public $rules = array(	
		...
		'no_repeat_words' => array(
				...
				'replacement' 	=> array(
					'\1',
					'$m[1].($m[7] === newkaliningrad_EMT_Lib::strtolower($m[3]) ? $m[2] : $m[2].$m[5].$m[6] )',
					)
			),			
		'paragraphs' => array(
...
class newkaliningrad_EMT_Base 
{
    ...
    public function safe_blocks($text, $way, $show = true)
    {
    	...
    	{
    		$safeType = true === $way ? "newkaliningrad_EMT_Lib::encrypt_tag(\$m[2])" : "stripslashes(newkaliningrad_EMT_Lib::decrypt_tag(\$m[2]))";
	...
	public function apply($trets = null)
	{
		...
		$this->_text = newkaliningrad_EMT_Lib::safe_tag_chars($this->_text, true);
		$this->debug($this, 'safe_tag_chars', $this->_text);
		
		$this->_text = newkaliningrad_EMT_Lib::clear_special_chars($this->_text);
		...
		foreach ($atrets as $tret) 		
		{
			...
			if($this->debug_enabled)
				...
				{
					$unsafetext = $di['text'];
					$unsafetext = newkaliningrad_EMT_Lib::safe_tag_chars($unsafetext, false);
		...
		{
			newkaliningrad_EMT_Lib::convert_html_entities_to_unicode($this->_text);
		}
		
		$this->_text = newkaliningrad_EMT_Lib::safe_tag_chars($this->_text, false);
	...
	protected function doset($selector, $key, $value)
	{
		...
		newkaliningrad_EMT_Lib::_process_selector_pattern($tret_pattern);
		newkaliningrad_EMT_Lib::_process_selector_pattern($rule_pattern);
		if($selector == "*") $this->settings[$key] = $value;
		
		foreach ($this->trets as $tret) 		
		{
			$t1 = $this->get_short_tret($tret);
			if(!newkaliningrad_EMT_Lib::_test_pattern($tret_pattern, $t1))	if(!newkaliningrad_EMT_Lib::_test_pattern($tret_pattern, $tret)) continue;
			$tret_obj = $this->get_tret($tret);
			if($key == "active")
			{
				foreach($tret_obj->rules as $rulename => $v)
				{
					if(!newkaliningrad_EMT_Lib::_test_pattern($rule_pattern, $rulename)) continue;
...
class newkaliningrad_EMTypograph extends newkaliningrad_EMT_Base 
{
	...
	public function do_setup($name, $value)
	{
		...
		{
			if($value == "style") $this->set_tag_layout(newkaliningrad_EMT_Lib::LAYOUT_STYLE);
			if($value == "class") $this->set_tag_layout(newkaliningrad_EMT_Lib::LAYOUT_CLASS);
		}
