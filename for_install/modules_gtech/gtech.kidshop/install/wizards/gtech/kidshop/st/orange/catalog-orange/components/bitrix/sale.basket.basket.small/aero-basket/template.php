...
<div class="aero-basket-inner-shadow">&nbsp;</div>
	<?$quantity = GetMessage("QUANTITY_NONE");?>
	<?if($arResult["ITEMS"]){$quantity=0;?>
	<table cellpadding="0" cellspacing="0" border="0" width="290px" align="center">
		<?foreach($arResult["ITEMS"] as $arItem){if($arItem["DELAY"]=="N"){$quantity++; $total = $total+$arItem["PRICE"]*$arItem["QUANTITY"];?>
			<tr>
				<td class="aero-basket-item" colspan="2"><b><?=$arItem["NAME"]?></b></br><small><?=GetMessage("TSBS_PRICE")?> <span class="rouble">c</span><?=$arItem["PRICE"]?>&nbsp;&nbsp;&nbsp;<?=GetMessage("TSBS_QUANTITY")?> <?=$arItem["QUANTITY"]?></small></td>
			</tr>
		<?}}?>
			<tr><td class="aero-basket-item" style="padding-top:10px;" colspan="2"><?=GetMessage("TSBS_TOTAL")?> <span class="rouble">c</span><b><?=$total?></b></td></tr>
			<tr>
...
<div class="aero-basket-bookmark">
	<table cellpadding="0" cellspacing="0" border="0" width="290px" align="center"><tr>
		<td align="left" height="30px" valign="middle" class="aero-basket-button-img"><a href="<?=$arParams["PATH_TO_BASKET"]?>" class="aero-basket-link" title="<?=GetMessage("TSBS_TOBASKET")?>"><?=GetMessage("TSBS_INBASKET")?> <?=$quantity?></a></td>
