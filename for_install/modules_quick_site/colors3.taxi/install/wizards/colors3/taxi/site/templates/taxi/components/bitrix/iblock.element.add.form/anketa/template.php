    ...
    <div class="modal-body"> 
        <form class="form-horizontal" name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
        	...
        	<?if ($arParams["MAX_FILE_SIZE"] > 0):?><input type="hidden" name="MAX_FILE_SIZE" value="<?=$arParams["MAX_FILE_SIZE"]?>" /><?endif?>
        		<?if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])):?>
        			...
        			<?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
                        <?
                            if((strpos($arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"], "person") !== false) || $propertyID == "NAME")
                            {
                                $title = GetMessage("personal_information");
                                $m++;
                            }
                            elseif(strpos($arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"], "car") !== false)
                            {
                        ...
                        <?if($m == 1 || $k == 1):?>
                            <h3><?=$title?></h3>
                        <?endif?>
        				<div class="control-group" <?if($propertyID == 81):?> id="other"<?endif?>>
                            <?if($propertyID != 82):?>
            ...
            <div class="modal-footer">
        		<input class="btn call_me btn-large" type="submit" name="iblock_submit" value="<?=GetMessage("IBLOCK_FORM_SUBMIT")?>" id="submit_anketa" />
