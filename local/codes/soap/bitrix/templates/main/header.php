...
<head>
    ...
    <!-- google webfonts -->
    <link rel="stylesheet" href="http://fonts.googleapis.com/css?family=Open+Sans:300,600,700&subset=latin,cyrillic">
 ...
 $(function() {
  ...
  $(".b-cart-mini__link").hover(onCartOver, onCartOut);
 });
 </script>    
 ...
 $(document).ready(function() {
   ...
   function(){
    ...
    $.ajax({
      ...
      success: function(out){
       var result = getBasketHTML(out) + "\<script\>$(function() {$(\'.b-cart-mini__link\').hover(onCartOver, onCartOut);});\<\/script\>";
  ...
  $('a[rel*="ADD_TO_COMPARE_LIST"]').click(function() {
   var imgid = $(this).attr("id"),
    $_this = $(this);
   $.ajax({
     type: "GET",
     ...
     success: function(out){
      ...
      var result = getConfHTML(out) + '\<script\>$(function() {$(\"#js-compare__btn\").popup();});\<\/script\>';
...
<aside class="b-sidebar">
    <? if (preg_match('/user/', $_SERVER['SCRIPT_URL'])) {?>
        ...
        <?}elseif(preg_match('/wishlist/', $_SERVER['SCRIPT_URL'])){?>
        <?$APPLICATION->IncludeComponent("cm:wishlist.categories", "categories", Array(
                    "IBLOCK_TYPE" => "catalog",	// IBLOCK_TYPE
                    "IBLOCK_ID" => "2",	// IBLOCK_ID
                    "ITEMS_LIMIT" => "10",	// ITEMS_LIMIT
                    "CACHE_TYPE" => "A",	// Тип кеширования
