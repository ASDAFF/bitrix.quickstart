...
class Group
{
	...
	public static function GetPropertyDescription()
	{
		return array(
			'PROPERTY_TYPE' => 'N',
			'USER_TYPE' => 'ASKARON_PROP_Group',
			'DESCRIPTION' => GetMessage('ASKARON_PROP_GROUP_DESCRIPTION'),
	...
	public static function GetUIEntityEditorProperty($settings, $value)
	{
		...
		$arAllItems = static::GetAllItems( );
		//dd($arAllItems);
		foreach ($arAllItems as $key => $arItem)
		{
			$items[] = [
				'NAME' => "[" . $arItem["ID"] . "] " . htmlspecialcharsbx($arItem["NAME"]),
				'VALUE' => $arItem["ID"],
				'ID' => $arItem["ID"],
			];
		}
	...
	public static function GetUIFilterProperty($arProperty, $strHTMLControlName, &$fields)
	{
		...
		$arItems = static::GetAllItems();
		foreach ($arItems as $key=>$arItem)
		{
			$fields["items"][ $arItem["ID"] ] = "[".$arItem["ID"]."] ".htmlspecialcharsbx( $arItem["NAME"] );
		}
	}
	...
	protected static function GetGroupElements()
	{
		$arResult = array();
		$arResult = GroupTable::getList(array(
