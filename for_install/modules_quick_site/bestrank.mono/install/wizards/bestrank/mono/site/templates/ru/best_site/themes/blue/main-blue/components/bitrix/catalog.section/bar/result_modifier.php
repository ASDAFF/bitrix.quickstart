...
/*SKU -- */
			...
			$arSkuTmp["ADD_URL"] = htmlspecialcharsback($arElement["DETAIL_PAGE_URL"]."?id=".$arOffer["ID"]."&action=ADD2BASKET");
...
/* -- SKU*/
	...
	else
	{
		...
		//$arResult["ITEMS"][$cell]["ADD_URL"] = $arElement["DETAIL_PAGE_URL"]."?id=".$arElement["ID"]."&action=ADD2BASKET";
		//$arResult["ITEMS"][$cell]["COMPARE_URL"] = $arElement["DETAIL_PAGE_URL"]."?id=".$arElement["ID"]."&action=ADD_TO_COMPARE_LIST";
...
foreach ($arResult["ITEMS"] as $key => $arElement) 
{
	if($arElement["DISPLAY_PROPERTIES"]["ARTNUMBER"]){
		$arResult["ITEMS"][$key]["ARTNUMBER"]=$arElement["DISPLAY_PROPERTIES"]["ARTNUMBER"];
		unset($arResult["ITEMS"][$key]["DISPLAY_PROPERTIES"]["ARTNUMBER"]);
	}
	...
	if(is_array($arElement["DETAIL_PICTURE"]))
	{
		...
		$arFileTmp = CFile::ResizeImageGet(
			...
			array("width" => $arParams["DISPLAY_IMG_WIDTH"]-40, "height" => $arParams["DISPLAY_IMG_HEIGHT"]-40),
