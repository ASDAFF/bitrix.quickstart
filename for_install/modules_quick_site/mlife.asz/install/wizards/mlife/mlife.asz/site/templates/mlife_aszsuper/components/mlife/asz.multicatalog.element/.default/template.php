...
<div class="catalogElement prod<?=$arResult["ID"]?>">
	...
	<div class="descrBlock">
		...
		<?}?>
		<?if(isset($arResult["DAY"])){
		$dateOb = new \Bitrix\Main\Type\DateTime($arResult["DISCOUNT_INFO"][$arResult["DAY"]]["DATE_END"]);
		?>
			<script>
			$(document).ready(function(){
				var unix_timestamp = <?=$dateOb->getTimestamp()?>;
				$('#share_<?=$arResult["ID"]?>').countdown({
			...
			<div class="titleDay"><?=GetMessage("MLIFE_ASZ_CATALOG_DAY_TITLE_MAIN")?>:</div><div class="share" id="share_<?=$arResult["ID"]?>"></div>
			</div>
		<?}?>
