    ...
    static public function getBasketSizesColors($userID, $iblockID) {
        if (CModule::IncludeModule("sale"))
        {
            ...
            if (count($basketIDS) > 0) {
                ...
                $arSelect = array( 'ID', 'NAME', 'PROPERTY_STD_SIZE', 'PROPERTY_COLOR' );
                ...
                while ($data = $rsElement -> Fetch())
                {
                    if (in_array($data["ID"], $basketProductsIDS)) {
                        ...
                        if (!empty($data["PROPERTY_COLOR_VALUE"])) {
                            $arResult["COLORS"]["BASKET"][$data["PROPERTY_COLOR_VALUE"]] = $arResult["COLORS"]["BASKET"][$data["PROPERTY_COLOR_VALUE"]]+1;
                        }
                        ...
                        if (!empty($data["PROPERTY_COLOR_VALUE"])) {
                            $arResult["COLORS"]["ORDERS"][$data["PROPERTY_COLOR_VALUE"]] = $arResult["COLORS"]["ORDERS"][$data["PROPERTY_COLOR_VALUE"]]+1;
                        }
                    }
                }
