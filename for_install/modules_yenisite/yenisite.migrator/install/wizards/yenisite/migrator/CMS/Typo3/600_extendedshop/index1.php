...
/* количество записей */
$query = "SELECT COUNT(*) AS CNT FROM {$arResult['prefix']}tx_extendedshop_products";
...
{
	$query = "SELECT * FROM {$arResult['prefix']}tx_extendedshop_products LIMIT {$left}, 10";
	...
	$ib = CIBlock::GetList(array(), array("CODE"=>"typo3_extendedshop"))->GetNext();
	...
	if($arItem['image'])
	{
		$path = $arResult["site"].'/uploads/tx_extendedshop/'.$arItem['image'];
	...
	$sec = array();
	$r = CIBlockSection::GetList(array(), array('IBLOCK_ID' =>$ib['ID'], 'XML_ID' => $arItem['pid']))->GetNext();
	$sec[] = $r['ID'];
	...
	if($arItem['offertprice'])
	{
		$arFields = Array(
			"PRODUCT_ID" => $PRODUCT_ID,
			"CATALOG_GROUP_ID" => 2,
			"PRICE" => $arItem['offertprice'],
			"CURRENCY" => "RUB"
		);
...
/*	
	...
	*/
	...
	CIBlockElement::SetPropertyValueCode($PRODUCT_ID, 'COLOR', $arItem['colors']);
	CIBlockElement::SetPropertyValueCode($PRODUCT_ID, 'SIZE', $arItem['sizes']);
	CIBlockElement::SetPropertyValueCode($PRODUCT_ID, 'VOLUME', $arItem['volume']);
