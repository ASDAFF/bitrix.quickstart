...
if(CModule::includeModule("iblock")){
	if($arResult['DISPLAY_PROPERTIES']["BRAND"]["VALUE"]){
		$rsBrand = CIBlockElement::GetByID($arResult['DISPLAY_PROPERTIES']["BRAND"]["VALUE"]);
		if($arBrand = $rsBrand->Fetch()){
			if($arBrand["PREVIEW_PICTURE"]){
				$rsFile = CFile::GetByID($arBrand["PREVIEW_PICTURE"]);
				$arFile = $rsFile->Fetch();
				$arResult['DISPLAY_PROPERTIES']["BRAND"]["PICTURE_SRC"] = "/upload/".$arFile["SUBDIR"]."/".$arFile["FILE_NAME"];
			}
		}
	}
}
