...
<div id="altasib_geobase_window">
	<div id="altasib_geobase_window_block">
		<a href="#" onclick="altasib_geobase.yc_x_clc(); return false" title="<?=GetMessage("ALTASIB_GEOBASE_CLOSE");?>">
			<div id="altasib_geobase_close_kr"></div>
		</a>
		<div id="altasib_geobase_page">
			<div class="altasib_geobase_yc_ttl"><?=GetMessage("ALTASIB_GEOBASE_YOUR_CITY");?></div>
			<?if($arResult["CODE"] == 00000000000 && empty($arResult["auto"]["CITY_NAME"])){?>
				<div class="altasib_geobase_your_city_block">
					<div class="altasib_geobase_your_city"><?=GetMessage("ALTASIB_GEOBASE_NOT_CITY");?></div>
				</div>
			<a class="altasib_geobase_yc_btn altasib_geobase_yc_disabled" style="margin: 10px 34%; width: 109px;" onclick="altasib_geobase.yc_no_click(); return false;" href="#"><?=GetMessage("ALTASIB_GEOBASE_SELECT");?></a><?
			}
			else {?>
			<div class="altasib_geobase_your_city_block">
				<?if(isset($arResult["CITY"]["NAME"])){?>
					<span class="altasib_geobase_your_city"><?=$arResult["CITY"]["NAME"];?></span>
					<?if($arResult["REGION_DISABLE"] != "Y"){?>
						<span class="altasib_geobase_your_city_2"><?=' ('.$arResult["REGION"]["FULL_NAME"]
				...
				<span class="altasib_geobase_your_city"><?=$arResult["auto"]["CITY_NAME"];?></span>
					<?if($arResult["REGION_DISABLE"] != "Y" && !empty($arResult["auto"]["REGION_NAME"])){?>
						<span class="altasib_geobase_your_city_2"><?=' ('.$arResult["auto"]["REGION_NAME"].')';?></span>
					<?}?>
					<?if(!empty($arResult["auto"]["COUNTRY_NAME"])){?>
						<span class="altasib_geobase_your_city_2"><?=', '.$arResult["auto"]["COUNTRY_NAME"];?></span>
					<?}?>
				<?}?>
			</div>
			<a class="altasib_geobase_yc_btn altasib_geobase_yc_disabled" onclick="altasib_geobase.yc_yes_click('<?=$arResult["CODE"]?>'); return false;" href="#"><?=GetMessage("ALTASIB_GEOBASE_YES");?></a>
			<a class="altasib_geobase_yc_btn altasib_geobase_yc_disabled" onclick="altasib_geobase.yc_no_click(); return false;" href="#"><?=GetMessage("ALTASIB_GEOBASE_NO");?></a><?
			}
		?></div>
	</div>
</div>
