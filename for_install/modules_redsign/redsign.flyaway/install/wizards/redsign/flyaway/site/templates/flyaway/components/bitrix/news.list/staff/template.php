...
if(is_array($arResult['ITEMS']) && count($arResult['ITEMS'])>0) {
	...
	?><div class="<?if($arParams['RSFLYAWAY_USE_OWL']=='Y'):?>owl<?else:?>row<?endif;?> staff<?if($arParams['RSFLYAWAY_SHOW_BUTTON']=='Y'):?> withbuttons<?endif;?>" <?
		...
		foreach($arResult["ITEMS"] as $arItem) {
			...
			?><div class="item col col-sm-6 col-md-3" id="<?=$this->GetEditAreaId($arItem['ID']);?>"><?
				?><div class="row"><?
					?><div class="col col-md-12"><?
						...
						?><div class="row staff-data"><?
							?><div class="col col-md-12 staff-name"><?=$arItem['NAME']?></div><?
							?><div class="col col-md-12 staff-description"><?=$arItem['PREVIEW_TEXT']?></div><?
						?></div><?
						if( $arParams['RSFLYAWAY_SHOW_BUTTON']=='Y' ) {
							?><div class="row button"><?
								?><div class="col col-md-12"><?
									?><a class="JS-Popup-Ajax btn btn-default btn-button" <?
										?>href="<?=SITE_DIR?>forms/emploeye/" <?
										?>title="<?=$arParams['RSFLYAWAY_BUTTON_CAPTION']?>" <?
										?>data-insertdata='{"RS_EXT_FIELD_0":"<?=CUtil::JSEscape($arItem['NAME'])?>"}' <?
										?>><?=$arParams['RSFLYAWAY_BUTTON_CAPTION']?></a><?
								?></div><?
							?></div><?
						}
					?></div><?
