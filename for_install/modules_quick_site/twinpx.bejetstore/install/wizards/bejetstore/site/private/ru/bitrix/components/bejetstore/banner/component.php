...
if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter, $bSub)))
{
	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		if(strlen($arParams["GROUP_XML_ID"])){
			if($arParams["GROUP_XML_ID"] == "SQUARE_WIDE"){
				$XML_ID = array("SQUARE","WIDE");
			}else{
				$XML_ID = $arParams["GROUP_XML_ID"];
			}
			$property_enums = CIBlockPropertyEnum::GetList(Array("SORT"=>"ASC"), Array("IBLOCK_ID"=>$arParams["IBLOCK_ID"], "XML_ID"=>$XML_ID) );
			while($enum_fields = $property_enums->GetNext()){
				$arFilter["PROPERTY_GROUP"][] = $enum_fields["ID"];
			}
		}
		...
		while($obElement = $rsElement->GetNextElement())
		{
			...
			$arItem["PROPERTIES"]["LINK"]["VALUE"] = str_replace("#SITE_DIR#", SITE_DIR, $arItem["PROPERTIES"]["LINK"]["VALUE"]);
