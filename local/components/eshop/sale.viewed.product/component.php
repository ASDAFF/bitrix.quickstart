...
if (!empty($arViewedId))
{
	...
	foreach ($arElementSort as $arElements)
	{
		...
		if ($arOffersIblock["OFFERS_IBLOCK_ID"] > 0)
		{
			...
			$arIblockOfferPropsFilterCode = array();
			foreach ($arIblockOfferPropsFilter as $val)
			{
				$arIblockOfferProps[] = array("CODE" => $val["CODE"], "NAME" => $val["NAME"]);
				$arIblockOfferPropsFilterCode[] = $val["CODE"];
			}
			$arIblockOfferPropsFilter = $arIblockOfferPropsFilterCode;
			if (isset($arIblockOfferPropsFilterCode))
				unset($arIblockOfferPropsFilterCode);
