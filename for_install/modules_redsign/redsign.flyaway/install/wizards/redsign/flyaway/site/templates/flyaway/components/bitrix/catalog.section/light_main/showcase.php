...
<div class="products-new">
	<h2><?=$arParams["RSFLYAWAY_TITLE_SECTION"]?></h2>
...
if (is_array($arResult['ITEMS']) && count($arResult['ITEMS']) > 0) {
	?><div class="owlslider products-owl-new products-owl-slider row products <?=$arResult['TEMPLATE_DEFAULT']['CSS']?>"
	<?php if($arParams['SIDEBAR'] == "Y"):?> data-sidebar="Y"<? endif; ?>><?
        ...
        $PRODUCT = &$arItem['OFFERS'][0];
			...
			?><div class="products__item item
				<? if ($IS_PREVIEW_TEXT == 'Y') {
					echo ' products__item_wide';
				} ?>
				<? if(isset($PRODUCT['DAYSARTICLE2']) && is_array($PRODUCT['DAYSARTICLE2']) ||
							isset($arItem['DAYSARTICLE2']) && is_array($arItem['DAYSARTICLE2']) ) {
					echo 'da2';
				} elseif(isset($PRODUCT['QUICKBUY']) && is_array($PRODUCT['QUICKBUY']) ||
								isset($arItem['QUICKBUY']) && is_array($arItem['QUICKBUY'])) {
					echo 'qb';
				}	?>
				js-element
							...
							?><div class="products__pic"><?
								?><a class="JS-Compare-Label js-detail_page_url" href="<?=$arItem['DETAIL_PAGE_URL']?>"><?
									if (isset($arItem['FIRST_PIC']['RESIZE']['src']) && trim($arItem['FIRST_PIC']['RESIZE']['src']) != '') {
										?><img class="products__img" src="<?=$arItem['FIRST_PIC']['RESIZE']['src']?>" alt="<?=$arItem['FIRST_PIC']['ALT']?>" title="<?=$arItem['FIRST_PIC']['TITLE']?>" /><?
									} else {
										?><img class="products__img" src="<?=$arResult['NO_PHOTO']['src']?>" title="<?=$arItem['NAME']?>" alt="<?=$arItem['NAME']?>" /><?
									}
								?></a><?
								?><div class="stickers"><?
                    ?><div class="da2_icon hidden-xs"><?=GetMessage('DA2_ICON_TITLE')?></div><?
                    ?><div class="qb_icon hidden-xs"><?=GetMessage('QB_ICON_TITLE')?></div><?
                    if ($PRODUCT['MIN_PRICE']['DISCOUNT_DIFF_PERCENT'] > 0) {
