...
$arResult["FORM_ACTION"] = isset($_SERVER['REQUEST_URI'])? htmlspecialcharsbx($_SERVER['REQUEST_URI']): "";//($APPLICATION->GetCurPageParam("", array_keys($checkbox_names))
...
foreach($arResult["arrProp"] as $prop_id => $arProp)
{
	...
	switch ($arProp["PROPERTY_TYPE"])
	{
		case "E":
			...
			{
				//EXECUTE ELEMENT
				$rsElement = CIBlockElement::GetList(array('NAME'=>'ASC','SORT'=>'ASC'), array('IBLOCK_CODE' => $arProp['CODE']), false, false, array('ID','NAME','IBLOCK_CODE', 'IBLOCK_ID'));
				while( $arElement = $rsElement->GetNext() )
				{
					$arrEnum[$arElement['ID']] = $arElement['NAME'];
					$arResult["LINKED"][$arProp['CODE']][$arElement['ID']] = $arElement['NAME'];
				}
			}
			...
			$cnt_s++;
			$res .= '<div class="select-cont select-cont-'.$cnt_s.'"><select name="'.$name.'" class="selectable">';
			$res .= '<option value="">'.GetMessage("CC_BCF_ALL_".$arProp["CODE"]).'</option>';
...
$custom_checkbox_names = array('new_only'=>true, 'digital_only'=>true, 'paper_only'=>true);
foreach(array_merge($_GET, $_POST) as $key => $value)
{
	
	if(
		!array_key_exists($key, $arResult["arrInputNames"])
		&& !array_key_exists($key, $arSkip) && !array_key_exists($key, $custom_checkbox_names) 
	)
	{
...
//array("LOGIC" => "OR", array('PROPERTY_archive_VALUE'=>false), array('PROPERTY_archive_VALUE'=>"нет"))
if(isset($_REQUEST['new_only'])
		|| isset($_REQUEST['digital_only'])
		|| isset($_REQUEST['paper_only'])
)
{
	$arFilterTemp["LOGIC"] = "OR";
}
...
if(isset($_REQUEST['digital_only'])){ 
	$arFilterTemp[] = array('PROPERTY' => array('BOOK_TYPE_VALUE' => 'Электронная')); 
}
if(isset($_REQUEST['paper_only'])){ 
	$arFilterTemp[] = array('PROPERTY' => array('BOOK_TYPE_VALUE' => 'Бумажная')); 
}
$arrFilter[] = $arFilterTemp;
$this->IncludeComponentTemplate();
return $arResult['LINKED'];
?>
