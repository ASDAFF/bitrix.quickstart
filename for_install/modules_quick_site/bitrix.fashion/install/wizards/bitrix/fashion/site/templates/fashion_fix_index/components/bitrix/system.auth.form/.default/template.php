...
<ul class="profile">
    <li class="auth"><span><span><?=GetMessage("AUTH_TITLE")?></span></span></li>
</ul>
<div id="auth">
<?
...
<script>$('#header').prepend($('#auth'));</script>
<?else:?>
<ul class="profile">
    <li><a href="<?=$arResult["PROFILE_URL"]?>" title="<?=GetMessage("AUTH_PROFILE")?>"><?=$arResult["USER_NAME"]?></a></li>
    <li><strong><a href="<?=$APPLICATION->GetCurPageParam('logout=yes', array('logout'))?>"><?=GetMessage("AUTH_LOGOUT_BUTTON")?></a></strong></li>
</ul>
<?endif?>