					...
					<div class="col-md-12 mt-20">
						<div id="results-vacancies-<?=$arItem['ID']?>">
							<div class="alert alert-danger" id="beforesend-vacancies-<?=$arItem['ID']?>">
								<?=GetMessage("QUICK_BUSINESSCARD_VACANCIES_BEFORESEND")?>
							</div>
							<div class="alert alert-danger" id="error-vacancies-<?=$arItem['ID']?>">
								<?=GetMessage("QUICK_BUSINESSCARD_VACANCIES_ERROR")?>
							</div>
							<div class="alert alert-success" id="success-vacancies-<?=$arItem['ID']?>">
								<?=GetMessage("QUICK_BUSINESSCARD_VACANCIES_SUCCESS")?>
							</div>
						</div>
						...
						<form name="VACANCIES" action="<?=SITE_DIR?>include/" method="POST" enctype="multipart/form-data" role="form">
							...
							<div class="form-group has-feedback">
								<input type="text" name="VACANCIES[NAME]" class="form-control req" placeholder="<?=GetMessage("QUICK_BUSINESSCARD_VACANCIES_NAME")?>">
								<i class="fa fa-user form-control-feedback"></i>
							</div>
							<div class="form-group has-feedback">
								<input type="tel" name="VACANCIES[PHONE]" class="form-control req" placeholder="<?=GetMessage("QUICK_BUSINESSCARD_VACANCIES_PHONE")?>" pattern="(([ ]*[\+]?[ ]*\d{1,5})[ ]*[\-]?[ ]*)?(\(?\d{1,5}\)?[ ]*[\-]?[ ]*)?[\d\- ]{5,13}">
								<i class="fa fa-phone form-control-feedback"></i>
							</div>
							<div class="form-group has-feedback">
								<input type="email" name="VACANCIES[EMAIL]" class="form-control" placeholder="<?=GetMessage("QUICK_BUSINESSCARD_VACANCIES_EMAIL")?>">
							...
							<div class="form-group has-feedback">
								<textarea name="VACANCIES[COMMENT]" class="form-control" rows="4" placeholder="<?=GetMessage("QUICK_BUSINESSCARD_VACANCIES_COMMENT")?>"></textarea>
								<i class="fa fa-pencil form-control-feedback"></i>
							</div>
							<button type="submit" class="btn btn-sm btn-default pull-right"><i class="fa fa-check pr-5"></i><?=GetMessage("QUICK_BUSINESSCARD_VACANCIES_SEND")?></button>
							<div class="clearfix"></div>
						</form>
					</div>
				</div>
