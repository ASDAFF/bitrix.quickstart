...
function qminus(id){
	var itemq = document.getElementById("QUANTITY_"+id);
	if(itemq.value > 0){
		itemq.value--;
	}
}
		...
		<tr>
			...
			<?if (in_array("QUANTITY", $arParams["COLUMNS_LIST"])):?>
				<td class="cart-item-quantity">
					...
					<td width="20px" style="padding:0px; padding-right:2px; border:none;"><img src="<?=$templateFolder?>/images/minus.png" border="0" style="cursor:pointer;" onClick="qminus(<?=$arBasketItems[ID]?>);"></td>
					<td align="center" style="padding:0px; border:none;"><input maxlength="18" type="text" name="QUANTITY_<?=$arBasketItems["ID"] ?>" id="QUANTITY_<?=$arBasketItems["ID"] ?>" value="<?=$arBasketItems["QUANTITY"]?>" size="3"></td>
					<td width="20px" style="padding:0px; padding-left:2px; border:none;"><img src="<?=$templateFolder?>/images/plus.png" border="0" style="cursor:pointer;" onClick="qplus(<?=$arBasketItems[ID]?>);"></td>
					</tr></table>
				</td>
			<?endif;?>
			<td class="cart-item-actions">
