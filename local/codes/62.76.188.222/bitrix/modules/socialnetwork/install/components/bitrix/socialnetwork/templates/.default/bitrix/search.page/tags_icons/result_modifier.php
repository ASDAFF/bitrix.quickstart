...
foreach($GLOBALS["arSocNetFeaturesSettings"] as $feature_id => $arFeatureTmp)
{
	if (
	...
	$arFeaturesTitles[$feature_id] = (
		...
		: (is_array($arMenuTmp) && array_key_exists("Title", $arMenuTmp) && is_array($arMenuTmp["Title"]) && array_key_exists($feature_id, $arMenuTmp["Title"]) && strlen($arMenuTmp["Title"][$feature_id]) > 0
