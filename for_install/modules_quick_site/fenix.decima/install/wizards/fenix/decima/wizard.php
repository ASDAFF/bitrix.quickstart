...
class SelectTemplateStep extends CSelectTemplateWizardStep
{
	...
	function OnPostForm()
	{
		...
		{
			$arTemplates = array("decima_swcolor0", "decima_swcolor1", "decima_swcolor2", "decima_swcolor3", "decima_swcolor4", "decima_swcolor5", "decima_swcolor6", "decima_swcolor7", "decima_swcolor8");
			...
			if (in_array($templateID,  $arTemplates))
			{
				...
				$wizard->SetVar("templateID", "decima");
			}
			//else
			//	$wizard->SetVar("templateID", "decima");
		}
	...
	function ShowStep()
	{
        ...
        $arT= array("decima_swcolor0", "decima_swcolor1", "decima_swcolor2", "decima_swcolor3", "decima_swcolor4", "decima_swcolor5", "decima_swcolor6", "decima_swcolor7", "decima_swcolor8");
		if (in_array("decima", array_keys($arTemplates)))
		{
			$arTemplateOrder = $arT;
        }


		$defaultTemplateID = COption::GetOptionString("main", "wizard_template_id", "decima_swcolor0", $wizard->GetVar("siteID"));
		if (!in_array($defaultTemplateID, $arT)) $defaultTemplateID = "decima_swcolor0";
		$wizard->SetDefaultVar("wizTemplateID", $defaultTemplateID);
        $arTemplateInfo=Array();
        $arThemes=array("swcolor0", "swcolor1", "swcolor2", "swcolor3", "swcolor4", "swcolor5", "swcolor6", "swcolor7", "swcolor8");
        
        foreach($arThemes as $templ){
          $arTemplateInfo['decima_'.$templ]=array(
                "NAME" => GetMessage("WIZ_TEMPLATE_decima_".$templ),
                "DESCRIPTION" => "",
	...
	//	$this->content .= "<input type='hidden' value='decima' name='templateID' id='templateID'>";//$this->ShowInputField('hidden', 'templateID', array("id" => "templateID", "value" => "decima"));
...
class SiteSettingsStep extends CSiteSettingsWizardStep
{
	function InitStep()
	{
		...
		if(COption::GetOptionString("decima", "wizard_installed", "N", $siteID) == "Y" && !WIZARD_INSTALL_DEMO_DATA)
	...
	/*	$arSocNetsValues = array();
		if ($wizard->GetVar("templateID") == "decima")
		{
			$arSocNetsValues["facebook"] = COption::GetOptionString("decima", "shopFacebook", GetMessage("WIZ_SHOP_FACEBOOK_DEF"), $siteID);
			$arSocNetsValues["twitter"] = COption::GetOptionString("decima", "shopTwitter", GetMessage("WIZ_SHOP_TWITTER_DEF"), $siteID);
			$arSocNetsValues["vk"] = COption::GetOptionString("decima", "shopVk", GetMessage("WIZ_SHOP_VK_DEF"), $siteID);
			$arSocNetsValues["google"] = COption::GetOptionString("decima", "shopGooglePlus", GetMessage("WIZ_SHOP_GOOGLE_PLUS_DEF"), $siteID);
		}
		else
		{
        ...
        */
		$wizard->SetDefaultVars(
			Array(
				...
				"shopEmail" => COption::GetOptionString("decima", "shopEmail", "sale@".$_SERVER["SERVER_NAME"], $siteID),
				...
				//"installdecimaApp" => COption::GetOptionString("decima", "installdecimaApp", "Y", $siteID),	
			)
		);
...
/*		if (file_exists($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/decimaapp") && LANGUAGE_ID == "ru")
			$this->content .= '
			<div class="wizard-input-form-block">
				'.$this->ShowCheckboxField("installdecimaApp", "Y", array("id"=>"installdecimaApp")).
				' <label for="installdecimaApp">'.GetMessage("wiz_install_decimaapp").'</label>
			</div>';
*/
		if(LANGUAGE_ID != "ru")
		{
...
class CatalogSettings extends CWizardStep
{
	function InitStep()
	{
		...
		$wizard->SetDefaultVars(
			Array(
				"catalogSubscribe" => (isset($arSubscribe[$siteID])) ? ($arSubscribe[$siteID]['use'] == "Y" ? "Y" : false) : "Y",//COption::GetOptionString("decima", "catalogSubscribe", "Y", $siteID),
				//"catalogView" => COption::GetOptionString("decima", "catalogView", "list", $siteID),
...
class ShopSettings extends CWizardStep
{
	function InitStep()
	{
		...
		$wizard->SetDefaultVars(
			Array(
				"shopLocalization" => COption::GetOptionString("decima", "shopLocalization", "ru", $siteID),
				"shopEmail" => COption::GetOptionString("decima", "shopEmail", "sale@".$_SERVER["SERVER_NAME"], $siteID),
				"shopOfName" => COption::GetOptionString("decima", "shopOfName", GetMessage("WIZ_SHOP_OF_NAME_DEF"), $siteID),
				"shopLocation" => COption::GetOptionString("decima", "shopLocation", GetMessage("WIZ_SHOP_LOCATION_DEF"), $siteID),
				//"shopZip" => 101000,
				"shopAdr" => COption::GetOptionString("decima", "shopAdr", GetMessage("WIZ_SHOP_ADR_DEF"), $siteID),
				"shopINN" => COption::GetOptionString("decima", "shopINN", "1234567890", $siteID),
				"shopKPP" => COption::GetOptionString("decima", "shopKPP", "123456789", $siteID),
				"shopNS" => COption::GetOptionString("decima", "shopNS", "0000 0000 0000 0000 0000", $siteID),
				"shopBANK" => COption::GetOptionString("decima", "shopBANK", GetMessage("WIZ_SHOP_BANK_DEF"), $siteID),
				"shopBANKREKV" => COption::GetOptionString("decima", "shopBANKREKV", GetMessage("WIZ_SHOP_BANKREKV_DEF"), $siteID),
				"shopKS" => COption::GetOptionString("decima", "shopKS", "30101 810 4 0000 0000225", $siteID),
				"siteStamp" => COption::GetOptionString("decima", "siteStamp", $siteStamp, $siteID),

				//"shopCompany_ua" => COption::GetOptionString("decima", "shopCompany_ua", "", $siteID),
				"shopOfName_ua" => COption::GetOptionString("decima", "shopOfName_ua", GetMessage("WIZ_SHOP_OF_NAME_DEF_UA"), $siteID),
				"shopLocation_ua" => COption::GetOptionString("decima", "shopLocation_ua", GetMessage("WIZ_SHOP_LOCATION_DEF_UA"), $siteID),
				"shopAdr_ua" => COption::GetOptionString("decima", "shopAdr_ua", GetMessage("WIZ_SHOP_ADR_DEF_UA"), $siteID),
				"shopEGRPU_ua" =>  COption::GetOptionString("decima", "shopEGRPU_ua", "", $siteID),
				"shopINN_ua" =>  COption::GetOptionString("decima", "shopINN_ua", "", $siteID),
				"shopNDS_ua" =>  COption::GetOptionString("decima", "shopNDS_ua", "", $siteID),
				"shopNS_ua" =>  COption::GetOptionString("decima", "shopNS_ua", "", $siteID),
				"shopBank_ua" =>  COption::GetOptionString("decima", "shopBank_ua", "", $siteID),
				"shopMFO_ua" =>  COption::GetOptionString("decima", "shopMFO_ua", "", $siteID),
				"shopPlace_ua" =>  COption::GetOptionString("decima", "shopPlace_ua", "", $siteID),
				"shopFIO_ua" =>  COption::GetOptionString("decima", "shopFIO_ua", "", $siteID),
				"shopTax_ua" =>  COption::GetOptionString("decima", "shopTax_ua", "", $siteID),

				"installPriceBASE" => COption::GetOptionString("decima", "installPriceBASE", "Y", $siteID),
			)
		);
	}
	...
	function InitStep()
	{
		...
		if ($shopLocalization == "ua")
			...
			$wizard->SetDefaultVars(
				Array(
					"personType" => Array(
						"fiz" =>  COption::GetOptionString("decima", "personTypeFiz", "Y", $siteID),
						"ur" => COption::GetOptionString("decima", "personTypeUr", "Y", $siteID),
					)
				)
			);
	}
...
===*/
	...
	function ShowStep()
	{
		...
		if (
			...
			LANGUAGE_ID == "ru" &&
			(
				COption::GetOptionString("decima", "wizard_installed", "N", $siteID) != "Y"
			...
			<div class="wizard-input-form-block">
				...
				<div>
					<div class="wizard-input-form-field wizard-input-form-field-checkbox">';
						...
						if(COption::GetOptionString("decima", "wizard_installed", "N", $siteID) != "Y")
						{
...
class DataInstallStep extends CDataInstallWizardStep
{
	...
	{
		if($_SESSION["BX_decima_LOCATION"] == "Y")
...
class FinishStep extends CFinishWizardStep
{
	...
	//	$this->content .= "<br clear=\"all\"><a href=\"/bitrix/admin/wizard_install.php?lang=".LANGUAGE_ID."&site_id=".$siteID."&wizardName=bitrix:decima.mobile&".bitrix_sessid_get()."\" class=\"button-next\"><span id=\"next-button-caption\">".GetMessage("wizard_store_mobile")."</span></a><br>";
