...
<div class="zag"><?=GetMessage("ZAG_ON_TABLE")?></div>

<table class="data-table rabots" cellspacing="0" cellpadding="0" border="0" width="100%">
	<thead>
	<tr>
		<td width="50%" class="trtd"><?=GetMessage("CATALOG_TITLE")?></td>
		<?$kol=count($arResult["ITEMS"]);
			if($kol > 0):	
				
			foreach($arResult["ITEMS"][0]["DISPLAY_PROPERTIES"] as $arProperty):?>
				<td width="100" class="trtd"><?=$arProperty["NAME"]?></td>
			<?endforeach;
		endif;?>
		...
		<?if(count($arResult["PRICES"]) > 0):?>
			<td>&nbsp;</td>
		<?endif?>
		<td class="trtd"><?=GetMessage("PODROBNEE")?></td>
	</tr>
	</thead>
	...
	<tr id="<?=$this->GetEditAreaId($arElement['ID']);?>">
		...
		<?endif;?>
		<td><a href="<?=$arElement["DETAIL_PAGE_URL"]?>"><?=GetMessage("PEREITI")?></a></td>
	</tr>
	<?endforeach;?>
</table>
