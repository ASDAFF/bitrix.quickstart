<?
if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true)die();

$arServices = Array(
	"main" => Array(
		"NAME" => GetMessage("SERVICE_MAIN_SETTINGS"),
		"STAGES" => Array(
			"files.php", // Copy bitrix files
			"search.php", // Indexing files
			"template.php", // Install template
			"theme.php", // Install theme
			"menu.php", // Install menu
			"settings.php",
		),
	),
	"iblock" => Array(
		"NAME" => GetMessage("SERVICE_IBLOCK_DEMO_DATA"),
		"STAGES" => Array(
			"types.php", // iblock types
			"sw_catalog_1.php", // category iblock
			"sw_catalog_2.php", // software iblock
			"sw_catalog_3.php", // other catalog iblock's
			"sw_content.php", // content iblock's
			"sw_services.php", // services iblock's
		),
	),
	"sale" => Array(
		"NAME" => GetMessage("SERVICE_SALE_DEMO_DATA"),
		"STAGES" => Array(
			"step1.php", "step2.php", "step3.php"
		),
	),
	"catalog" => Array(
		"NAME" => GetMessage("SERVICE_CATALOG_SETTINGS")
	),
	"blog" => Array(
		"NAME" => GetMessage("SERVICE_BLOG_DEMO_DATA"),
		"STAGES" => Array(
			"blog.php", //Blog
		),
	),
	/*"forum" => Array(
		"NAME" => GetMessage("SERVICE_FORUM")
	),*/
);
?>