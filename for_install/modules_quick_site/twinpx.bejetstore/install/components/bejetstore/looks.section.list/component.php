...
/*************************************************************************
			Work with cache
*************************************************************************/
if($this->StartResultCache(false, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()).$_REQUEST['PAGEN_1']))
{
	...
	//EXECUTE
	$rsSections = CIBlockSection::GetList($arSort, $arFilter, $arParams["COUNT_ELEMENTS"], $arSelect, Array("nPageSize"=>$arParams["PAGE_ELEMENTS_COUNT"], 'iNumPage'=>$_REQUEST['PAGEN_1']));
	$rsSections->SetUrlTemplates("", $arParams["SECTION_URL"]);
	//$arResult["NAV_STRING"] = $res->GetPageNavStringEx(, "", "nPageSize"=>$arParams["PAGE_ELEMENTS_COUNT"]);
	$rsElements->nPageWindow = 3;
	$arResult["NAV_STRING"] = $rsSections->GetPageNavStringEx($navComponentObject, 'Заголовок', 'bejetstore', 'Y');
