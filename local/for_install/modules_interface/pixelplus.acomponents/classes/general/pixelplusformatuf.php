...
class CPixelPlusFormatUF extends CPixelPlusFormat {
			...
			foreach ($this->arFormatted as $pid) {
				...
				if ($arUFProps['USER_TYPE_ID'] == "enumeration") {
					...
					while ($arFieldEnum = $rsfieldsenum->GetNext()) {
						$this->arEntityMeta[$pid]['PX_VALUES'][$arFieldEnum['ID']] = $arFieldEnum['VALUE'];
					}
				}		
			}			
			return true;
	...
	private function __GetDispayProperties($arValue,$arUFProps,$pid) {
		...
		$events = GetModuleEvents("pixelplus.acomponents", "OnFormatUF");
		...
		if ($arUFProps['USER_TYPE_ID'] == "enumeration") {
			if ($arUFProps['MULTIPLE'] == "Y") {
				foreach ($arValue['VALUE'] as $key=>$value) {
					$arReturn['FORMATTED_VALUE'][$key] = $arUFProps['PX_VALUES'][$value];
				}
				$arReturn['DISPLAY_VALUE'] = implode($this->mseparator,$arReturn['FORMATTED_VALUE']);
			} else {
				$arReturn['FORMATTED_VALUE'] = $arUFProps['PX_VALUES'][$arValue['VALUE']];
			...
			if ($arUFProps['MULTIPLE'] == "Y") {
				foreach ($arValue['VALUE'] as $key=>$value) {
					$value = intval($value);
					if ($value > 0) {
						if(!array_key_exists($value,  CPixelPlusFormatUF::$CACHE["G"])) {
							$arFilter["=ID"] = $value;								
							$rsSection = CIBlockSection::GetList(Array(), $arFilter, false, array("ID","NAME","IBLOCK_ID","SECTION_PAGE_URL"));
							 CPixelPlusFormatUF::$CACHE["G"][$value] = $rsSection->GetNext();
						}
				
						if(is_array(CPixelPlusFormatUF::$CACHE["G"][$value])) {
							$arReturn['FORMATTED_VALUE'][$key] =  CPixelPlusFormatUF::$CACHE["G"][$value]["NAME"];
							$arReturn['VALUE_LINK'][$key]='<a href="'. CPixelPlusFormatUF::$CACHE["G"][$value]["SECTION_PAGE_URL"].'">'. CPixelPlusFormatUF::$CACHE["G"][$value]["NAME"].'</a>';
						}
					}
				}
				...
				if ($value > 0) {
					...
					if(is_array( CPixelPlusFormatUF::$CACHE["G"][$value])) {
						$arReturn['FORMATTED_VALUE'] =  CPixelPlusFormatUF::$CACHE["G"][$value]["NAME"];
						$arReturn['VALUE_LINK']='<a href="'. CPixelPlusFormatUF::$CACHE["G"][$value]["SECTION_PAGE_URL"].'">'. CPixelPlusFormatUF::$CACHE["G"][$value]["NAME"].'</a>';
			...
			if ($arUFProps['MULTIPLE'] == "Y") {
				foreach ($arValue['VALUE'] as $key=>$value) {
					$value = intval($value);
					if ($value > 0) {
						if(!array_key_exists($value,  CPixelPlusFormatUF::$CACHE["E"])) {
							$arFilter["=ID"] = $value;						
							$rsElement = CIBlockElement::GetList(Array(), $arFilter, false, false, array("ID","NAME","IBLOCK_ID","DETAIL_PAGE_URL"));
							 CPixelPlusFormatUF::$CACHE["E"][$value] = $rsElement->GetNext();
						}
					
						if(is_array( CPixelPlusFormatUF::$CACHE["E"][$value])) {
							$arReturn['FORMATTED_VALUE'][$key] =  CPixelPlusFormatUF::$CACHE["E"][$value]["NAME"];
							$arReturn['VALUE_LINK'][$key]='<a href="'. CPixelPlusFormatUF::$CACHE["E"][$value]["DETAIL_PAGE_URL"].'">'. CPixelPlusFormatUF::$CACHE["E"][$value]["NAME"].'</a>';
						}
					}
				}
				$arReturn['DISPLAY_VALUE'] = implode($this->mseparator,$arReturn['VALUE_LINK']);	
				...
				if ($value > 0) {
					...
					if(is_array( CPixelPlusFormatUF::$CACHE["E"][$value])) {
						$arReturn['FORMATTED_VALUE'] =  CPixelPlusFormatUF::$CACHE["E"][$value]["NAME"];
						$arReturn['VALUE_LINK']='<a href="'. CPixelPlusFormatUF::$CACHE["E"][$value]["DETAIL_PAGE_URL"].'">'. CPixelPlusFormatUF::$CACHE["E"][$value]["NAME"].'</a>';
						$arReturn['DISPLAY_VALUE'] = $arReturn['VALUE_LINK'];
					}
				}
			}
			...
			if ($arUFProps['MULTIPLE'] == "Y") {
				foreach ($arValue['VALUE'] as $key=>$value) {
					$value = intval($value);
					if ($value === 1) {
						$arReturn['FORMATTED_VALUE'][$key] = getMessage('PIXELPLUS_UF_MOD_YES');
					} elseif ($value === 0) {
						$arReturn['FORMATTED_VALUE'][$key] =  getMessage('PIXELPLUS_UF_MOD_NO');
					}
				}
				$arReturn['DISPLAY_VALUE'] = implode($this->mseparator,$arReturn['FORMATTED_VALUE']);
			} else {
				$value = intval($arValue['VALUE']);
				if ($value === 1) {
					$arReturn['FORMATTED_VALUE'] = getMessage('PIXELPLUS_UF_MOD_YES');
				} elseif ($value === 0) {
					$arReturn['FORMATTED_VALUE'] = getMessage('PIXELPLUS_UF_MOD_NO');
				}
				$arReturn['DISPLAY_VALUE'] = $arReturn['FORMATTED_VALUE'];
			}
