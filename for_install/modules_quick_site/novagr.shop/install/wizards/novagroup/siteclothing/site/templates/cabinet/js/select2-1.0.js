    ...
    Select2 = createClass({
        attrs: {
            ...
            formatInputTooShort: {init: function () {
                return function (input, min) {
                	return "Please enter " + (min - input.length) + "  more character";
        ...
        methods: {
            init: function () {
				...
				// add html for search and add container

                if (this.el.data("search-container") != undefined && this.el.data("add-mess") != undefined) {
                    var addElemID = this.el.data("search-container");

                    var searchHTML = "<div class='form-request'><a id='"+addElemID+"' class='editable editable-click' href='#' data-type='text' data-pk='1' data-original-title='"+this.el.data("add-mess")+"'>"+this.el.data("add-label")+"</a></div>"; 
                ...
                this.el.after(self.container);
                 ...
                 // add html for search and add container
				//alert(' addElemID ' + addElemID);
                if (this.el.data("search-container") != undefined && this.el.data("add-mess") != undefined) {  
                    $('#'+addElemID).editable({
                    	buttonValue: this.el.data("send-btn"),
                    	ajaxOptions: { dataType: 'json' }, 
                    	url: this.el.data("ajax-handler"),
                    	type: 'text',
                    	/*pk: 1,*/
                    	autotext: 'never',
                    	display: false,
                    	name: this.el.data("search-container"),
                    	title: this.el.data("title-mess"),
                    	success: function(data, config) {
                    		...
                    		/*if (data && data.id != '') {
								$(this).editable('option', 'pk', data.id);
								var placeholder = {title: data.title, id: data.id};
								
								var parentDiv = $(data.selectID).parent();
								var elemHTML = $(data.selectID).wrap("<div>").parent().html();
								parentDiv.html(elemHTML);
								dcProduct.initSelect( data.selectID, placeholder, data.actionStr);
                    		} */
