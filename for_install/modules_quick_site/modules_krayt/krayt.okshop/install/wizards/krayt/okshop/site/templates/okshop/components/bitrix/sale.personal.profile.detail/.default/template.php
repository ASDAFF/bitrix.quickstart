...
<?if(strlen($arResult["ID"])>0):?>
	...
	<br />
	<div class="btn-wrp" style="text-align: right;">
		<input type="submit" class="btn_all btn_green" style="display: inline-block;" name="save" value="<?echo GetMessage("SALE_SAVE") ?>">
		&nbsp;
		<input type="submit" class="btn_all btn_green" style="display: inline-block;" name="apply" value="<?=GetMessage("SALE_APPLY")?>">
		&nbsp;
		<input type="submit" class="btn_all btn_green" style="display: inline-block;" name="reset" value="<?echo GetMessage("SALE_RESET")?>">
	</div>
	</form>
<?else:?>
	<?=ShowError($arResult["ERROR_MESSAGE"]);?>
