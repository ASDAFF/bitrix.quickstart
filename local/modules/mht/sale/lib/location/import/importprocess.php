...
final class ImportProcess extends Process
{
	...
	public function onBeforePerformIteration()
	{
		...
		if(!$this->data['inited'])
		{
			...
			if($opts['SOURCE'] == self::SOURCE_REMOTE)
			{
				$this->data['settings']['additional'] = is_array($_REQUEST['ADDITIONAL']) ? array_flip(array_values($_REQUEST['ADDITIONAL'])) : array();
			}
	...
	protected function readBlockFromCurrentFile(&$buffer, &$bufferSize)
	{
		...
		//('READ FROM File: '.$fName.' seek to '.$this->data['current']['bytesRead'].' stepsize: '.$this->getCurrStageStepSize());
		//('Block size '.$this->getCurrStageStepSize());
		...
		//('Bytes read: '.$this->data['current']['bytesRead']);
	}
	*/
