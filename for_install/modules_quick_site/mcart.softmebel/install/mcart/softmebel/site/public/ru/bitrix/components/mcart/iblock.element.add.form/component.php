	...
	if ($bAllowAccess)
	{
		...
		if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
		{
			...
			if (count($arResult["ERRORS"]) == 0){
				...
				if(!empty($arParams["USER_MESSAGE_COUPON_CODE"])){				
					$obElements = CIBlockElement::GetList(Array(), Array("ID" => $arParams["ID"]), false, false, Array("ID", "PROPERTY_CODE"));
					if($arFieldsElement = $obElements->GetNext()){			
						if(!empty($arFieldsElement['PROPERTY_CODE_VALUE'])){	
							$sRedirectUrl .= urlencode("\n".$arParams["USER_MESSAGE_COUPON_CODE"]." ".$arFieldsElement['PROPERTY_CODE_VALUE']);		
						}	
					}
				}
