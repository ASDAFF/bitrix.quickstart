			...
			foreach ($arParams["USER_USER_F_FIELDS"] as $fid) {
				if ($arUser[$fid]) {
					if ($fid == "PERSONAL_PHOTO") {
						...
						$arUser["DISPLAY_FIELDS"][$fid] = getMessage('CP_PXUL_USER_SEX_'.$arUser[$fid]);
					} elseif ($fid == "ACTIVE") {
						if ($arUser[$fid] == "Y") {
							$arUser["DISPLAY_FIELDS"][$fid] = getMessage('CP_PXUL_USER_YES');
						} else {
							$arUser["DISPLAY_FIELDS"][$fid] = getMessage('CP_PXUL_USER_NO');
						}
					} else {
