...
while ($arResult = $dbResultList->NavNext(true, "f_")) {
	$row = & $lAdmin->AddRow($f_ID, $arResult, "asdaff_redirects_edit.php?ID=" . UrlEncode($arResult["ID"]) . "&lang=" . LANG, GetMessage("MURL_EDIT"));
	...
	//CONTEXT MENU
	$arActions = Array();
	$arActions[] = array("ICON" => "edit", "TEXT" => GetMessage("MURL_EDIT"), "ACTION" => $lAdmin->ActionRedirect("asdaff_redirects_edit.php?ID=" . UrlEncode($arResult["ID"]) . "&lang=" . LANG), "DEFAULT" => true);
	if ($isAdmin)
...
while (($arRes = $dbRes->Fetch())) {
	$arDDMenu[] = array(
			...
			"ACTION" => "window.location = 'asdaff_redirects_edit.php?ADD=Y&lang=" . urlencode(LANG) . "&site_id=" . $arRes["LID"] . "';"
	);
}
