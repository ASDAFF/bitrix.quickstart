...
else
{
	...
	if (StrLen($arResult["FatalError"]) <= 0)
	{
		...
		$arResult["Urls"]["Entity"] = CComponentEngine::MakePathFromTemplate(
				$arParams[$GLOBALS["arSocNetAllowedSubscribeEntityTypesDesc"][$arParams["ENTITY_TYPE"]]["URL_PARAM_KEY"]], 
				array(
					$GLOBALS["arSocNetAllowedSubscribeEntityTypesDesc"][$arParams["ENTITY_TYPE"]]["URL_PATTERN"] => $arParams["ENTITY_ID"]
				)
			);
