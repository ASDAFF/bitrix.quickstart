...
//			return phpQuery::selectDocument($id);
    ...
    {
        $loaded = false;
        if (!file_exists($_SERVER["DOCUMENT_ROOT"] . "/bitrix/modules/sota.parser/classes/general/main_classes.php")) return;
        if ($this->contentType) {
...
//		$this->document->encoding = $charset;
    ...
    {
        global $sota_preview, $sota_ENCODING, $sota_first, $sota_DOC_ENCODING;
        if (phpQuery::$debug)
            phpQuery::debug('Full markup load (HTML): ' . substr($markup, 0, 250)); //if(!$sota_preview) print $markup;
        ...
        $addDocumentCharset = false;  //if(!$sota_preview) print "CHARS".$documentCharset;
        if ($documentCharset) {      //print "DOC";
            $charset = $documentCharset;//if(!$sota_preview) print "CHARS".$documentCharset;
            $markup = $this->charsetFixHTML($markup);
            //print "ENC".$charset;
        } elseif (!$sota_preview) {
            $charset = $sota_ENCODING;
            $addDocumentCharset = true;
        } else {
            $charset = $requestedCharset;  //if(!$sota_preview)print "TEXT".$markup;
            //$addDocumentCharset = true;
        }
        //if(!$sota_preview) print_r(array($requestedCharset, $documentCharset)); //return;
        $documentCharset = strtoupper($charset);
        $docEncoding = strtoupper($charset);
        $sota_DOC_ENCODING = $docEncoding;
        //if(!$sota_preview) print_r(array($requestedCharset, $documentCharset));
       ...
       if (! $documentCharset) {
        ...
        //if(!$sota_preview) print_r(array($requestedCharset, $documentCharset));
        phpQuery::debug("DOC: $documentCharset REQ: $requestedCharset");//if(!$sota_preview) print_r(array($requestedCharset, $documentCharset));
        if ($requestedCharset && $documentCharset && $requestedCharset !== $documentCharset) {
            phpQuery::debug("CHARSET CONVERT");
            // Document Encoding Conversion
            // http://code.google.com/p/phpquery/issues/detail?id=86
            //if(!$sota_preview) print_r(array($markup, $requestedCharset, $docEncoding));
            //return;
...
//		return strpos($markup, '<?xml') !== false && stripos($markup, 'xhtml') === false;
    ...
    {
        global $sota_preview, $sota_first, $sota_ENCODING;
        $matches = array();
        //print "LANG".LANG_CHARSET;
        //if(!$sota_preview)print_r(array($matches, $markup));
        //if(!$sota_first && !$sota_preview) return LANG_CHARSET;
        // find meta tag
