...
$query = "SELECT COUNT(*) AS CNT FROM {$arResult['prefix']}tx_dam_cat";	
...
$arFields = array(
	...
	"NAME" => "typo3-Dam-gallery",
	"CODE" => "typo3_dam_gallery",		
	"SORT" => "100",
);
		
$iblock = new CIBlock;
$res = CIBlock::GetList(array(), array("CODE" => "typo3_dam_gallery"))->GetNext();
if($res)
	$id = $res["ID"];
else
...
{	
	...
	$query = "SELECT * FROM {$arResult['prefix']}tx_dam_cat LIMIT {$left}, 10";
	//echo $query;
	//die($query);
	...
	while($arItem = mysql_fetch_assoc($result))
	{
		...
		$res = CIBlockSection::GetList(array(), array('IBLOCK_ID' => $id, 'XML_ID' => $arItem['parent_id']))->GetNext();
