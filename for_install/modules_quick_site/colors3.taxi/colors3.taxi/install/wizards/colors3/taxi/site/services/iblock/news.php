...
if(COption::GetOptionString("taxi", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		...
		if ($arIBlock = $rsIBlock->Fetch())
		{
			...
			CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/news/index.php", array("SITE_ID" => WIZARD_SITE_ID));		
			CWizardUtil::ReplaceMacros($_SERVER['DOCUMENT_ROOT'].'/bitrix/templates/'.WIZARD_TEMPLATE_ID.'_'.WIZARD_THEME_ID.'/footer.php', array("NEWS_IBLOCK_ID" => $iblockID));
			CWizardUtil::ReplaceMacros($_SERVER['DOCUMENT_ROOT'].'/bitrix/templates/'.WIZARD_TEMPLATE_ID.'_'.WIZARD_THEME_ID.'/header.php', array("NEWS_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}
...
CUserOptions::SetOption("form", "form_element_".$iblockID, array ( 'tabs' => 'edit1--#--'.GetMessage("WZD_OPTION_NEWS_1")		
		.'--,--ACTIVE--#--'.GetMessage("WZD_OPTION_NEWS_2")
		.'--,--ACTIVE_FROM--#--'.GetMessage("WZD_OPTION_NEWS_3")
		.'--,--ACTIVE_TO--#--'.GetMessage("WZD_OPTION_NEWS_4")
		.'--,--NAME--#--'.GetMessage("WZD_OPTION_NEWS_5")
		.'--,--CODE--#--'.GetMessage("WZD_OPTION_NEWS_11")
		.'--,--PREVIEW_TEXT--#--'.GetMessage("WZD_OPTION_NEWS_8")	
		.'--,--SORT--#--'.GetMessage("WZD_OPTION_NEWS_SORT")
		.'--,--DETAIL_PICTURE--#--'.GetMessage("WZD_OPTION_NEWS_9")
		.'--,--DETAIL_TEXT--#--'.GetMessage("WZD_OPTION_NEWS_10").'--;--', ));		
