...
<p>&nbsp;<br /><a href="../../"><?=GetMessage("BACK_PATH_TO_ORDERS")?></a></p>

<div class="c-table">


<table class="equipment ordersmore">


	<tbody>

		<tr>
			<td colspan="2" class="ct-header">
				<h2><?=GetMessage("SPOD_ORDER_NO")?>&nbsp;<?=$arResult["ID"]?>&nbsp;<?=GetMessage("SPOD_FROM")?> <?=$arResult["DATE_INSERT"] ?></h2>
			</td>
		</tr>
	...
	<?if (IntVal($arResult["USER_ID"])>0):?>
		<tr>
			<td colspan="2" class="ct-header">
				<h2><?=GetMessage("SPOD_ACCOUNT_DATA")?></h2>
			</td>
		</tr>
	...
	<?endif;?>
		...
		<tr>
			<td colspan="2" class="ct-header">
				<h2><?=GetMessage("P_ORDER_PAYMENT")?></h2>
			</td>
		</tr>
...
<div id="cart-table">
	...
	<table class="equipment" rules="rows">
		<thead>
			<tr>
				<th class="cart-item-name"><?= GetMessage("SPOD_NAME") ?></td>
				<th class="cart-item-price"><?= GetMessage("SPOD_PRICE") ?></td>
				<th class="cart-item-price"><?= GetMessage("SPOD_VAT") ?></td>
				<th class="cart-item-weight"><?= GetMessage("SPOD_WEIGHT") ?></td>
				<th class="cart-item-quantity"><?= GetMessage("SPOD_QUANTITY") ?></td>
			</tr>
		</thead>
		<tbody>
		<?
		foreach($arResult["BASKET"] as $val)
		{
			...
			<tr>
				...
				<td ><b><?=$val["PRICE_FORMATED"]?></b></td>
				<td ><?=$val["VAT_RATE"]*100?>%</td>
	...
	<table class="myorders_itog">
			...
			<tr>
				...
				<td><strong class="red"><?=$arResult["PRICE_FORMATED"]?></strong></td>
			</tr>
