...
<?foreach($arResult["ITEMS"] as $arItem):?>
	...
	?>
	<p class="article-item" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
		...
		<?if($arParams["DISPLAY_DATE"]!="N" && $arItem["DISPLAY_ACTIVE_FROM"]):?>
			<span class="article-date-time"><?echo $arItem["DISPLAY_ACTIVE_FROM"]?></span>
		<?endif?>
		<?if($arParams["DISPLAY_NAME"]!="N" && $arItem["NAME"]):?>
			<?if(!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])):?>
				<a href="<?echo $arItem["DETAIL_PAGE_URL"]?>"><b class="article-title"><?echo $arItem["NAME"]?></b></a><br />
			<?else:?>
				<b class="article-title"><?echo $arItem["NAME"]?></b><br />
			<?endif;?>
		<?endif;?>
