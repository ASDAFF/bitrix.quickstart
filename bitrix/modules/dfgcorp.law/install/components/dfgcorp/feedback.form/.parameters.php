...
$arHideProperties = array();

if($arCurrentValues["IBLOCK_ID"] > 0){
	$obProperties = CIBlockProperty::GetList(
		array("sort"=>"asc", "name"=>"asc"), 
		array("PROPERTY_TYPE"=>"L", "ACTIVE"=>"Y", "IBLOCK_ID"=>$arCurrentValues["IBLOCK_ID"]));
	while($arProperties = $obProperties->GetNext()){
		$arHideProperties[$arProperties["ID"]] = $arProperties["NAME"];
	}
}

if($arCurrentValues["HIDE_PROPERTY_ID"]>0){
	$obPValues = CIBlockPropertyEnum::GetList(
		array("DEF"=>"DESC", 
		 	  "SORT"=>"ASC"), 
		array("IBLOCK_ID"=>$IBLOCK_ID, 
				"PROPERTY_ID"=>$arCurrentValues["HIDE_PROPERTY_ID"]));
	
	while($arPValues = $obPValues->GetNext())
	{
		$arPropertyValues[$arPValues["ID"]] = $arPValues["VALUE"];
	}
}

$arEventList = array();

$rsEvent = CEventMessage::GetList($by=LANGUAGE_ID, $order="desc", array());
while ($arEvent = $rsEvent->GetNext()){
    $arEventList[$arEvent["ID"]] = $arEvent["EVENT_TYPE"];
}
...
$arComponentParameters = array(
	...
	"PARAMETERS" => array(
		...
		"HIDE_PROPERTY_ID" => array(
			"PARENT" => "FIELDS",
			"NAME" => GetMessage("HIDE_PROP_ID"),
			"TYPE" => "LIST",
			"MULTIPLE" => "N",
			"ADDITIONAL_VALUES" => "N",
			"REFRESH" => "Y",
			"VALUES" => $arHideProperties,
		),
		
		"HIDE_PROPERTY_VALUE" => array(
			"PARENT" => "FIELDS",
			"NAME" => GetMessage("HIDE_PROP_VALUE"),
			"TYPE" => "LIST",
			"MULTIPLE" => "N",
			"ADDITIONAL_VALUES" => "N",
...
$arComponentParameters["PARAMETERS"]["BUTTON_TITLE"] = array(
	"PARENT" => "TITLES",
	"NAME" => GetMessage("CP_BUTTON_TITLE"),
	"TYPE" => "TEXT",
	"DEFAULT" => GetMessage("CP_BUTTON_TITLE_DEF_VALUE"),
);
?>