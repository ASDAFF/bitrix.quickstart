...
if(0 && $_GET["mode"] == "checkauth" && $USER->IsAuthorized())
{
	if(
...
elseif(0 && !$USER->IsAuthorized())
{
	echo "failure\n",GetMessage("CC_BSC1_ERROR_AUTHORIZE");
}
elseif(0 && !$bUSER_HAVE_ACCESS)
{
	...
	elseif(($_GET["mode"] == "import") && $ABS_FILE_NAME)
	{
		...
		{ 
                    ...
                    CModule::IncludeModule('tc');
                        ...
                        $rs = $DB->Query("select * from b_xml_tree where PARENT_ID = 2");
                       
                        $arr = array();
                        
                        $needFiels = array('НомерКарты', 
                                           'ТипСкидки', 
                                           'ВладелецКарты', 
                                           'ОстатокБонусов', 
                                           'СуммаНакоплений', 
        ...
        $arr[$ar["NAME"]] = $ar['VALUE'];
        
        
                                if($ar["NAME"] == 'ПроцентСкидки'){
                                       
                                        tcCards::Add( array(
                                                        'VLADELEC'=> $arr['ВладелецКарты'],
                                                        'TIP_SKIDKI'=> $arr['ТипСкидки'],
                                                        'OSTATOK'=> $arr['ОстатокБонусов'],
                                                        'SUMMA'=> $arr['СуммаНакоплений'],
                                                        'PROCENT'=> $arr['ПроцентСкидки'],
                                                        'NOMER'=> $arr['НомерКарты']
                                                       )
                                                );
