$(document).ready(function(){
	...
	$('#asd_save').live('click', function(){
		...
		$('.asd_fav_input input:visible').each(function(i){
			...
			if (val.length>0)
			{
				...
				{
					$('#asd_i0 input').val('');
					$.get('/bitrix/tools/asdaff_favorite.php', {name : val, action : 'add', charset : sCharset, sessid : sSessId, maxchars : iMaxChars, type : sType, key : sKey}, function(data){
						var newId = data;
						if (parseInt(newId) > 0)
						{
		...
		if (aFolders.length > 0)
		{
			...
			$.get('/bitrix/tools/asdaff_favorite.php', obGet);
		}
		$('.asd_fav_input').hide();
		$('.asd_fav_buttons').hide();
	});
	$('.asd_fav_del').live('click',function(){
		...
		if ($('#asd_f'+id).size() && (id==0 || confirm(sMessDelConfirm)))
		{
			...
			if (id != 0)
			{
				$.get('/bitrix/tools/asdaff_favorite.php', {id : id, action : 'del', sessid : sSessId});
				$('#asd_f'+id).remove();
			}
	...
	$('.asd_fav_star').live('click',function(){
		...
		$.get('/bitrix/tools/asdaff_favorite.php', {id : id, action : 'default', sessid : sSessId, maxchars : iMaxChars, type : sType, key : sKey});
	});
