...
if (!CModule::IncludeModule('pixelplus.acomponents')) return;
...
$arParams["CP_PXSD_SHOW_ONLY_ACTIVE"] = $arParams["CP_PXSD_SHOW_ONLY_ACTIVE"]==="Y";
...
if($this->StartResultCache(false, array($arNavigation)))
{
	...
	if ($arParams["CP_PXSD_SHOW_ONLY_ACTIVE"] === true) {
		$arFilter['ACTIVE'] = "Y";
	...
	if ($bneedfound === true) {
		...
		//format fields
		$obpxformatsf = new CPixelPlusFormatSF;
		if (count($arParams["SECTION_F_FIELDS"])) {
			...
			foreach (Array("PICTURE","DETAIL_PICTURE") as $fkey) {
				if ($arParams['PXR_'.$fkey.'_C'] = "Y") {
					$bInitSizes = $arParams['PXR_'.$fkey.'_RT'] === "Y";
					
					$obpxformatsf->paramformatclass->AddParam($fkey,Array(
						"RESIZE" => Array(
							"arSize" => Array("width"=>$arParams['PXR_'.$fkey.'_W'],"height"=>$arParams['PXR_'.$fkey.'_H']),
							"resizeType" => constant($arParams['PXR_'.$fkey.'_RT']),
