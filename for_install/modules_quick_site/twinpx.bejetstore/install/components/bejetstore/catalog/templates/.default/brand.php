...
}elseif('' != $arResult["VARIABLES"]["BRAND_CODE"]){
	$arAddCacheParams = array(
		"CODE" => $arResult["VARIABLES"]["BRAND_CODE"]
	);
}
?>
<?$obCache = new CPHPCache();
if ($obCache->InitCache(36000, serialize(array_merge($arFilter,$arAddCacheParams)), "/iblock/brand"))
{
	$arElementIDList = $obCache->GetVars();
}
elseif ($obCache->StartDataCache())
{
	$arElementIDList = array();
	if (\Bitrix\Main\Loader::includeModule("iblock"))
	{
		if('' != $arResult["VARIABLES"]["BRAND_CODE"]){
			$rsBrand = CIBlockElement::GetList(array(), array("=CODE" => $arResult["VARIABLES"]["BRAND_CODE"], "ACTIVE" => "Y"));
			if($arBrand = $rsBrand->Fetch()){
				$arFilter["PROPERTY_BRAND"] = $arBrand["ID"];
			}
		}

		$dbRes = CIBlockElement::GetList(array(), $arFilter, array("ID"), false);

		if(defined("BX_COMP_MANAGED_CACHE"))
		{
			global $CACHE_MANAGER;
			$CACHE_MANAGER->StartTagCache("/iblock/brand");

			while ($arGoup = $dbRes->Fetch())
			{
				$arElementIDList[] = $arGoup["ID"];
			}
			$CACHE_MANAGER->RegisterTag("brand_id_".$arFilter["PROPERTY_BRAND"]);
	...
	$obCache->EndDataCache($arElementIDList);
}?>
