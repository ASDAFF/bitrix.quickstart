...
class CFavoritesComponent extends CCatalogViewedProductsComponent
{
	/**
	 * @param $params
	 * @override
	...
	protected function getProductIds()
	{
		...
		if ($USER->IsAuthorized()) {
            ...
            $arUser = $res->Fetch();
            $elementsId = $arUser['UF_FAVORITES'];
        } else {
            $cookie = Application::getInstance()->getContext()->getRequest()->getCookie("Favorites");

            $elementsId = unserialize($cookie);
        }
		if (count($elementsId) > 0 && is_array($elementsId)) {

			$productIds = array();
			$res = CIBlockElement::GetList(Array(), ['IBLOCK_ID' => 2, 'ID' => $elementsId], false, false, ['ID']);
