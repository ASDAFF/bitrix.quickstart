	...
	<table>
		<tr>
			...
			<?foreach($arResult["ITEMS"] as $arElement):
				...
				$sticker="";
				if($arElement["NEWPRODUCT"]) $sticker .= "<div class=\"newproduct\"></div>";
				if($arElement["SPECIALOFFER"]) $sticker .= "<div class=\"specialoffer\"></div>";
			?>
			<td valign="top"><div style="position:relative">
				
				<a class="close button deleteitem_compare"  onclick="return deleteFromCompareTable(this)" href="<?=htmlspecialcharsbx($APPLICATION->GetCurPageParam("action=DELETE_FROM_COMPARE_RESULT&IBLOCK_ID=".$arParams['IBLOCK_ID']."&ID[]=".$arElement['ID'],array("action", "IBLOCK_ID", "ID")))?>" title="<?=GetMessage("CATALOG_REMOVE_PRODUCT")?>"></a>
				...
				<?endif;?>
				<?if (strlen($sticker)>0):?>
					<div class="badge" style="top:25px;"><?=$sticker?></div>
				<?endif?>
			</div></td>
			<?endforeach;?>
		</tr>
