...
<?if ($arResult['INTIS_CONFIRM']=="N" && !$_POST['CONFIRM_ID'] && !$_POST['CONFIRM_CODE'] && !$_REQUEST['wait_confirm']):?>
...
<?if ($_GET['wait_confirm']=="Y" && !$USER->IsAuthorized()):?>
    <form method="post" action="<?=$arParams['CONFIRM_URL']?>" name="regform" enctype="multipart/form-data">
        <table>
            <thead>
                <tr>
                    <td colspan="2"><b><?=GetMessage("AUTH_CONFIRM_TITLE")?></b></td>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td>
                        <input type="text" <?if ($_GET['wrong']=="Y"):?>style='background-color: #fda2a2;'<?endif;?> name="CONFIRM_ID" maxlength="100" value="" placeholder="<?=GetMessage("AUTH_CONFIRM_ID_MESS")?>"<?=$err?> />
                    </td>
                </tr>
                <tr>
                    <td>
                        <input type="text" <?if ($_GET['wrong']=="Y"):?>style='background-color: #fda2a2;'<?endif;?> name="CONFIRM_CODE" maxlength="10" value="" placeholder="<?=GetMessage("AUTH_CONFIRM_CODE_MESS")?>"<?=$err?> />
                    </td>
                </tr>
            </tbody>
            ...
            <tfoot>
            <tr>
                <td><input type="submit" name="register_submit_button" value="<?=GetMessage("AUTH_CONFIRM_BUTTON")?>" /></td>
            </tr>
            </tfoot>
        </table>
    </form>
