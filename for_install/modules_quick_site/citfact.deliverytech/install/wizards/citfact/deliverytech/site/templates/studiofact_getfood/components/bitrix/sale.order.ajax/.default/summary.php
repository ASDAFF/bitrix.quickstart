			...
			<thead>
				<tr>
					...
					foreach ($arResult["GRID"]["HEADERS"] as $id => $arColumn):
						...
						?>
							<td class="custom<? if ($arColumn["id"] == "DISCOUNT_PRICE_PERCENT_FORMATED") { echo ' order_hide_mobile_1'; } else if ($arColumn["id"] == "PRICE_FORMATED") { echo ' order_hide_mobile_2'; } ?>">
						<?
							echo $arColumn["name"];
						endif;
						?>
			...
			<tbody>
				...
				<tr>
					...
					foreach ($arResult["GRID"]["HEADERS"] as $id => $arColumn):
						...
						?>
							<td class="price right order_hide_mobile_2">
								...
								<? if ($arItem["DISCOUNT_PRICE"] > 0) { ?><div class="basket_old_price"><?=str_replace(GetMessage("STUDIOFACT_RUB"), "<span class=\"rub black\">".GetMessage("STUDIOFACT_R")."</span>", SaleFormatCurrency($arItem["PRICE"] + $arItem["DISCOUNT_PRICE"], $arItem["CURRENCY"]));?></div><? } ?>
						...
						?>
							<td class="custom right order_hide_mobile_1">
						...
						?>
							<td class="custom right<? if ($arColumn["id"] == "DISCOUNT_PRICE_PERCENT_FORMATED") { echo ' order_hide_mobile_1'; } else if ($arColumn["id"] == "SUM") { echo ' order_hide_mobile_3'; }?>">
								<span><?=getColumnName($arColumn)?>:</span>
								<? if ($arColumn["id"] == "SUM") { ?>
									<div class="basket_price"><?=str_replace(GetMessage("STUDIOFACT_RUB"), "<span class=\"rub black\">".GetMessage("STUDIOFACT_R")."</span>", $arItem[$arColumn["id"]]);?></div>
								<? } else { ?>
									<?=$arItem[$arColumn["id"]]?>
								<? } ?>
							</td>
			...
			<table class="bx_ordercart_order_sum">
				<tbody>
					...
					<tr>
						...
						<td class="custom_t2" class="price"><?=str_replace(GetMessage("STUDIOFACT_RUB"), "<span class=\"rub black\">".GetMessage("STUDIOFACT_R")."</span>", $arResult["ORDER_PRICE_FORMATED"]);?></td>
					</tr>
					<?
					if (doubleval($arResult["DISCOUNT_PRICE"]) > 0)
					{
						...
						<tr>
							...
							<td class="custom_t2" class="price"><?=str_replace(GetMessage("STUDIOFACT_RUB"), "<span class=\"rub black\">".GetMessage("STUDIOFACT_R")."</span>", $arResult["DISCOUNT_PRICE_FORMATED"]);?></td>
						</tr>
						<?
					}
					if(!empty($arResult["TAX_LIST"]))
					{
						foreach($arResult["TAX_LIST"] as $val)
						{
							...
							<tr>
								...
								<td class="custom_t2" class="price"><?=str_replace(GetMessage("STUDIOFACT_RUB"), "<span class=\"rub black\">".GetMessage("STUDIOFACT_R")."</span>", $val["VALUE_MONEY_FORMATED"]);?></td>
							</tr>
							<?
						}
					}
					if (doubleval($arResult["DELIVERY_PRICE"]) > 0)
					{
						...
						<tr>
							...
							<td class="custom_t2" class="price"><?=str_replace(GetMessage("STUDIOFACT_RUB"), "<span class=\"rub black\">".GetMessage("STUDIOFACT_R")."</span>", $arResult["DELIVERY_PRICE_FORMATED"]);?></td>
						</tr>
						<?
					}
					if (strlen($arResult["PAYED_FROM_ACCOUNT_FORMATED"]) > 0)
					{
						...
						<tr>
							...
							<td class="custom_t2" class="price"><?=str_replace(GetMessage("STUDIOFACT_RUB"), "<span class=\"rub black\">".GetMessage("STUDIOFACT_R")."</span>", $arResult["PAYED_FROM_ACCOUNT_FORMATED"]);?></td>
						</tr>
						<?
					}
					...
					?>
						<tr>
							...
							<td class="custom_t2 fwb" class="price"><?=str_replace(GetMessage("STUDIOFACT_RUB"), "<span class=\"rub black\">".GetMessage("STUDIOFACT_R")."</span>", $arResult["ORDER_TOTAL_PRICE_FORMATED"]);?></td>
						</tr>
						<tr>
							<td class="custom_t1" colspan="<?=$colspan?>"></td>
							<td class="custom_t2" style="text-decoration:line-through; color:#828282;"><?=str_replace(GetMessage("STUDIOFACT_RUB"), "<span class=\"rub black\">".GetMessage("STUDIOFACT_R")."</span>", $arResult["PRICE_WITHOUT_DISCOUNT"]);?></td>
						</tr>
					<?
					else:
					?>
