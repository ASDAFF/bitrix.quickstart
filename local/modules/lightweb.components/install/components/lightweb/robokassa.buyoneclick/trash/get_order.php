...
function send_mail($options) {
	...
	$email_fields = Array(
		"EMAIL_TO" => $request_fields["EMAIL"],//$parameters["EMAIL_TO"],
		...
		"EMAIL_ADMINISTRATOR" => $adminEmail,//"v.korotovskikh@lightweb.pro",//$parameters["EMAIL_ADMINISTRATOR"],
...
function get_product_properties($product_id) {
	...
	$obElement = CIBlockElement::GetByID($product_id);
	if($element = $obElement->GetNext())
	$name = $element["NAME"];
	
	if ($name != null && $product_result['PRICE_CURRENT']['VALUE'] != null) {
		$answer = array(
			'name' => $name,
			'cost' => $product_result['PRICE_CURRENT']['VALUE'],
		);
	}
	return $answer;
}
