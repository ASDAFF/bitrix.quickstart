...
if(COption::GetOptionString("softeffect.storesoftware", "wizard_installed", "N", WIZARD_SITE_ID) != "Y" || WIZARD_INSTALL_DEMO_DATA) {
	...
	COption::SetOptionString("softeffect.storesoftware", "shopLocalization", $shopLocalization, "ru", WIZARD_SITE_ID);
	$shopLocation = $wizard->GetVar("shopLocation");
	COption::SetOptionString("softeffect.storesoftware", "shopLocation", $shopLocation, false, WIZARD_SITE_ID);
	$shopOfName = $wizard->GetVar("shopOfName");
	COption::SetOptionString("softeffect.storesoftware", "shopOfName", $shopOfName, false, WIZARD_SITE_ID);
	$personType = $wizard->GetVar("personType");
	$paysystem = $wizard->GetVar("paysystem");

	$shopINN = $wizard->GetVar("shopINN");
	COption::SetOptionString("softeffect.storesoftware", "shopINN", $shopINN, false, WIZARD_SITE_ID);
	$shopKPP = $wizard->GetVar("shopKPP");
	COption::SetOptionString("softeffect.storesoftware", "shopKPP", $shopKPP, false, WIZARD_SITE_ID);
	$shopNS = $wizard->GetVar("shopNS");
	COption::SetOptionString("softeffect.storesoftware", "shopNS", $shopNS, false, WIZARD_SITE_ID);
	$shopBANK = $wizard->GetVar("shopBANK");
	COption::SetOptionString("softeffect.storesoftware", "shopBANK", $shopBANK, false, WIZARD_SITE_ID);
	$shopBANKREKV = $wizard->GetVar("shopBANKREKV");
	COption::SetOptionString("softeffect.storesoftware", "shopBANKREKV", $shopBANKREKV, false, WIZARD_SITE_ID);
	$shopKS = $wizard->GetVar("shopKS");
	COption::SetOptionString("softeffect.storesoftware", "shopKS", $shopKS, false, WIZARD_SITE_ID);
	/*$siteStamp = $wizard->GetVar("siteStamp");
	COption::SetOptionString("softeffect.storesoftware", "siteStamp", $siteStamp, false, WIZARD_SITE_ID);*/

	$shopAdr = $wizard->GetVar("shopAdr");
	COption::SetOptionString("softeffect.storesoftware", "shopAdr", $shopAdr, false, WIZARD_SITE_ID);
	$siteTelephone = $wizard->GetVar("siteTelephone");
	COption::SetOptionString("softeffect.storesoftware", "siteTelephone", $siteTelephone, false, WIZARD_SITE_ID);
	$shopEmail = $wizard->GetVar("shopEmail");
	COption::SetOptionString("softeffect.storesoftware", "shopEmail", $shopEmail, false, WIZARD_SITE_ID);
	$siteName = $wizard->GetVar("siteName");
	COption::SetOptionString("softeffect.storesoftware", "siteName", $siteName, false, WIZARD_SITE_ID);
	//SocNets
	$shopFacebook = $wizard->GetVar("shopFacebook");
	COption::SetOptionString("softeffect.storesoftware", "shopFacebook", $shopFacebook, false, WIZARD_SITE_ID);
	$shopTwitter = $wizard->GetVar("shopTwitter");
	COption::SetOptionString("softeffect.storesoftware", "shopTwitter", $shopTwitter, false, WIZARD_SITE_ID);
	$shopVk = $wizard->GetVar("shopVK");	
	COption::SetOptionString("softeffect.storesoftware", "shopVK", $shopVk, false, WIZARD_SITE_ID);
	...
	if(!$dbPerson->Fetch()) { //if there are no data in module
		...
		foreach($arProps as $prop) {
			...
			if ($prop["PERSON_TYPE_ID"]==$arGeneralInfo["personType"]["fiz"] && $prop['NAME']==GetMessage("SALE_WIZARD_PROP_4")) { // index
				$propReplace .= '"PROP_'.$arGeneralInfo["personType"]["fiz"].'" => array(0 => "'.$id.'"), ';
			} elseif ($prop["PERSON_TYPE_ID"]==$arGeneralInfo["personType"]["fiz"] && $prop['NAME']==GetMessage("SALE_WIZARD_PROP_21")) { // gorod
				CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/personal/order/index.php", array("fiz_town" => $id));
			} elseif ($prop["PERSON_TYPE_ID"]==$arGeneralInfo["personType"]["ur"] && $prop['NAME']==GetMessage("SALE_WIZARD_PROP_2")) { // mestopologenie
				CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/personal/order/index.php", array("ur_location" => $id));
			} elseif ($prop["PERSON_TYPE_ID"]==$arGeneralInfo["personType"]["ur"]) {
				if ($prop['NAME']==GetMessage("SALE_WIZARD_PROP_11")) {
					$propReplace .= '
	"PROP_'.$arGeneralInfo["personType"]["ur"].'" => array(0 => "'.$id.'", ';
				} elseif ($prop['NAME']==GetMessage("SALE_WIZARD_PROP_4")) {
					$propReplace .= '1 => "'.$id.'", ';
				} elseif ($prop['NAME']==GetMessage("SALE_WIZARD_PROP_21")) {
					$propReplace .= '2 => "'.$id.'"),';
				}
			}
		...
		WizardServices::ReplaceMacrosRecursive(WIZARD_SITE_PATH."basket/", Array("PROPS" => $propReplace));
		...
		if($personType["fiz"] == "Y") {
			...
			if ($paysystem["sber"] == "Y") {
				$arPaySystems[] = Array(
					...
					"ACTION" => Array(Array(
						"PERSON_TYPE_ID" => $arGeneralInfo["personType"]["fiz"],
						"NAME" => GetMessage("SALE_WIZARD_PS_SB"),
						"ACTION_FILE" => "/bitrix/php_interface/include/sale_payment/sberbank_new_softeffect",
						"RESULT_FILE" => "",
						"NEW_WINDOW" => "Y",
			...
			if ($paysystem["robo"] == "Y") {
				$arPaySystems[] = Array(
					"NAME" => GetMessage("SALE_WIZARD_PS_ROBO"),
					"SORT" => 10,
					"DESCRIPTION" => GetMessage("SALE_WIZARD_PS_ROBO_DESCR"),
					"CODE_TEMP" => "robokassa",
					"ACTION" => Array(Array(
						"PERSON_TYPE_ID" => $arGeneralInfo["personType"]["fiz"],
						"NAME" => GetMessage("SALE_WIZARD_PS_ROBO"),
						"ACTION_FILE" => "/bitrix/php_interface/include/sale_payment/robokassa",
						"RESULT_FILE" => "",
						"NEW_WINDOW" => "N",
						"PARAMS" => serialize(Array(
							"ShopLogin" => Array("TYPE" => "", "VALUE" => ""),
							"ShopPassword" => Array("TYPE" => "", "VALUE" => ""),
							"ShopPassword2" => Array("TYPE" => "", "VALUE" => ""),
							"OrderDescr" => Array("TYPE" => "ORDER", "VALUE" => "ID"),
							"SHOULD_PAY" => Array("TYPE" => "ORDER", "VALUE" => "PRICE"),
							"CURRENCY" => Array("TYPE" => "ORDER", "VALUE" => "CURRENCY"),
							"DATE_INSERT" => Array("TYPE" => "ORDER", "VALUE" => "DATE_INSERT"),							
						)),
						"HAVE_PAYMENT" => "Y",
						"HAVE_ACTION" => "N",
						"HAVE_RESULT" => "N",
		...
		if($personType["ur"] == "Y" && $paysystem["bill"] == "Y")
		{
			$arPaySystems[] = Array(
				...
				"ACTION" => Array(Array(
					"PERSON_TYPE_ID" => $arGeneralInfo["personType"]["ur"],
					"NAME" => GetMessage("SALE_WIZARD_PS_BILL"),
					"ACTION_FILE" => "/bitrix/php_interface/include/sale_payment/invoice_legal_entities_softeffect",
					"RESULT_FILE" => "",
					"NEW_WINDOW" => "Y",
					"PARAMS" => serialize(Array(
						"DATE_INSERT" => Array("TYPE" => "ORDER", "VALUE" => "DATE_INSERT"),
	...
	foreach($arPaySystems as $val) {
		...
		if ($val['CODE_TEMP']=='robokassa') {
			CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/robokassa/res.php", array("robokassa" => $id, 'personal_type'=>$arGeneralInfo["personType"]["fiz"]));
		}
	...
	if(!$bStatusA) {
		$arFields = Array("ID" => "A", "SORT" => 50);
		foreach($arLanguages as $langID) {
			WizardServices::IncludeServiceLang("step1.php", $langID);
			$arFields["LANG"][] = Array("LID" => $langID, "NAME" => GetMessage("WIZ_SALE_STATUS_A"), "DESCRIPTION" => GetMessage("WIZ_SALE_STATUS_DESCRIPTION_A"));
		}
	...
	function __MakeOrder($prdCnt=1, $arData = Array())
	{
		...
		$dbBlock = CIBlock::GetList(array(), array('ACTIVE'=>'Y', 'XML_ID'=>'sw_software', 'TYPE'=>'sw_software'));
		if ($arBlock = $dbBlock->GetNext()) {
			$dbItem = CIBlockElement::GetList(Array("PROPERTY_MORE_PHOTO" => "DESC", "ID" => "ASC"), Array("IBLOCK_ID" => $arBlock['ID'], "IBLOCK_SITE_ID" => WIZARD_SITE_ID), false, Array("nTopCount" => 10), Array("ID", "IBLOCK_ID", "XML_ID", "NAME", "DETAIL_PAGE_URL", "IBLOCK_XML_ID"));
	...
	elseif(IntVal($arGeneralInfo["paySystem"]["sber"][$personType]) > 0 )
		$paySystem = $arGeneralInfo["paySystem"]["sber"][$personType];
	elseif(IntVal($arGeneralInfo["paySystem"]["robo"][$personType]) > 0 )
		$paySystem = $arGeneralInfo["paySystem"]["robo"][$personType];
	else
	{
