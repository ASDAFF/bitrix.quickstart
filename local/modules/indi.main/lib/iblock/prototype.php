...
class Prototype
{
    ...
    /**
     * Определяет константы вида Indi\Main\Iblock\ID_{CODE} и Indi\Main\Iblock\CODE_{ID} для всех инфоблоков
     *
     ...
     * @return array
     */
    public function getSectionsList($keyName = '', $arSort = array(), $arFilter = array(), $arSelect = array("*"), $arNavStartParams = array(),  $cacheTime = 3600)
    {
        if ($this->sections === null) {
            $cache = $this->getCache(array(__METHOD__), $cacheTime);
            if ($cache->start()) {
     ...
     * $aMenuLinks = array_merge($aMenuLinks, \Indi\Main\Iblock\Prototype::getInstance('Content-News')->getElementsForMenu());
     *
     ...
     * @param array $arParams ($arFields события)
     */
    public function updateLinkedElements($arParams)
    {
        ...
        $rsProperties = \CIBlockProperty::GetList(
            array(),
            array("ACTIVE" => "Y", "IBLOCK_ID" => $this->id, "CODE" => "%_LINK")
        );
        ...
        while ($arProp = $rsProperties->GetNext()) {
            $rsLinkElements = \CIBlockElement::GetList(
                ...
                array(
                    ...
                    "ACTIVE" => "Y",
                    "CHECK_PERMISSIONS" => "Y",
                    "PROPERTY_" . $arProp['CODE'] => $arParams["ID"],
                ),
                false,
                false,
                Array("ID")
