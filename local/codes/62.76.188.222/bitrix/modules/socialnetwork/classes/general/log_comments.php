...
class CAllSocNetLogComments
{
	...
	function SendEvent($ID, $mailTemplate = "SONET_NEW_EVENT")
	{
		...
		while ($arSubscriber = $dbSubscribers->Fetch())
		{
			...
			$bHasAccess = CSocNetLogRights::CheckForUser(($arLog["ID"] ? $arLog["ID"] : $arLogComment["LOG_ID"]), $arSubscriber["USER_ID"], $arLogSites);
			if (!$bHasAccess)
				continue;
		...
		CSocNetLogCounter::Increment($arLog["ID"], $arLog["ENTITY_TYPE"], $arLog["ENTITY_ID"], $arLog["EVENT_ID"], $arLogComment["USER_ID"], $arOfEntities, false, false, "Y", "LC");

		return true;
	}

	function UpdateLogData($tmp_id, $bSetDate = true, $bSetDateByLastComment = false)
	{
		$dbResult = CSocNetLogComments::GetList(array(), array("LOG_ID" => $tmp_id), array());
		$comments_count = $dbResult;

		$dbResult = CSocNetLog::GetList(array(), array("ID" => $tmp_id), false, false, array("ID", "COMMENTS_COUNT", "LOG_DATE"));
		while ($arResult = $dbResult->Fetch())
		{
			$arFields = array("COMMENTS_COUNT" => $comments_count);
			if ($bSetDateByLastComment)
			{
				$dbComment = CSocNetLogComments::GetList(array("LOG_DATE" => "DESC"), array("LOG_ID" => $tmp_id), false, false, array("ID", "LOG_DATE"));
