...
if($arParams["SET_TITLE"] == 'Y')
	$APPLICATION->SetTitle(GetMessage("WS_SPPL_DEFAULT_TITLE"));
...
if ($del_id > 0 && check_bitrix_sessid())
{
	...
	if ($arUserProps = $dbUserProps->Fetch())
	{
		...
		{
			$errorMessage = GetMessage("WS_SALE_DEL_PROFILE");
		}
	}
	else
	{
		$errorMessage = GetMessage("WS_SALE_NO_PROFILE");
	}
...
if(IntVal($_REQUEST["del_id"]) > 0)
	$errorMessage = GetMessage("WS_SALE_DEL_PROFILE", array("#ID#" => IntVal($_REQUEST["del_id"])));
elseif(IntVal($_REQUEST["success_del_id"]) > 0)
	$errorMessage = GetMessage("WS_SALE_DEL_PROFILE_SUC", array("#ID#" => IntVal($_REQUEST["success_del_id"])));
...
$arResult["NAV_STRING"] = $dbUserProps->GetPageNavString(GetMessage("WS_SPPL_PAGES"));
