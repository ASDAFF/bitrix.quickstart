...
JCSmartFilter.prototype.reload = function(input)
{
	...
	if (this.position.top < 1) {
		this.position.top = $("#"+input.id).offset().top - 150;
	}
		...
		for(var PID in result.ITEMS)
		{
			...
			else if(arItem.VALUES)
			{
				for(var i in arItem.VALUES)
				{
					...
					if(control)
					{
						control.parentNode.className = ar.DISABLED ? 'disabled NiceCheck': 'NiceCheck';
					}
				}
			}
		}
...
$(document).on("click", ".smart_filter_props .bx_filter_container_title", function() {
...
$(document).on("click", ".control_button_show a", function () {
	if ($(".bx_catalog_section_box .control_button_show").hasClass("active")) {
		$(".smart_filter").slideUp("fast");
		$(".bx_catalog_section_box .control_button_show").removeClass("active");
	} else {
		$(".bx_catalog_section_box .control_button_show").addClass("active");
		$(".smart_filter").slideDown("fast");
	}
