<?
	...
	if(file_exists('ajax_modifier.php'))
		include 'ajax_modifier.php';
	
    $ajaxParamsFile = getenv('DOCUMENT_ROOT').dirname($_SERVER["SCRIPT_NAME"]).'/ajax_modifier.php';
    if(file_exists($ajaxParamsFile))
        include_once $ajaxParamsFile;
	...
	function callback($buffer)
	{
		...
		if((int)$_REQUEST['FILTER_RESET'] == 1)
		{
			...
			*/
			...
			{
				if($arParams['arFilterRequest']['min'.$arParams['CATALOG_PRICE']][0] > 0)
			...
			{
				if($arParams['arFilterRequest']['max'.$arParams['CATALOG_PRICE']][0])
			...
			while($arVAT = $rsVAT -> Fetch())
				$arResult['VAT'][] = $arVAT['RATE'];
			$minVAT = $arResult['VAT'][0];
			rsort($arResult['VAT']);
			$maxVAT = $arResult['VAT'][0];
			
			$minPrice = $arParams['arOfferRequest']['>='.$arParams['CATALOG_PRICE']];
			$maxPrice = $arParams['arOfferRequest']['<='.$arParams['CATALOG_PRICE']];
			$minPrice = floor( ($minPrice * 100) / ($minVAT + 100) );
			$maxPrice = ceil( ($maxPrice * 100) / ($maxVAT + 100) );
			$arParams['arOfferRequest']['>='.$arParams['CATALOG_PRICE']] = $minPrice;
			$arParams['arOfferRequest']['<='.$arParams['CATALOG_PRICE']] = $maxPrice;
			...
			*/
			...
			if( !empty($arParams['arFilterRequest']['max'.$arParams['CATALOG_PRICE']]) )
			{
				if( $arParams['arFilterRequest']['max'.$arParams['CATALOG_PRICE']][0] > 0 )
					$arParams['arOfferRequest']['<='.$arParams['CATALOG_PRICE']] = $arParams['arFilterRequest']['max'.$arParams['CATALOG_PRICE']][0];
				unset( $arParams['arFilterRequest']['max'.$arParams['CATALOG_PRICE']] );
			}
		...
		*/
		...
		// VAT
		$minPrice = round($minPrice * $minVAT / 100) + $minPrice;
		$maxPrice = round($maxPrice * $maxVAT / 100) + $maxPrice;
		/*
