...
{
//echo'<pre>';print_r(array($arResult,$arTuple,$arTemp));echo'</pre>';die();
	if(count($arTuple) == 0)
		$arResult[] = $arTemp;
	else
	{
		...
		if (strlen($strError) <= 0)
		{
			...
			if($first_line_names == "Y")
			{
				...
				foreach($arNeedFields as $field_name){
					if($field_name=="IP_PROP".$asz_system_id){
						$arResFields[] = "ASZ_KOL";
						$arResFields[] = "ASZ_ZAK";
						foreach($arPrice as $pricer){
							$arResFields[] = "ASZ_PRICE_".$pricer["CODE"]."_VAL";
							$arResFields[] = "ASZ_PRICE_".$pricer["CODE"]."_CUR";
						}
			...
			while ($obElement = $res->GetNextElement())
			{
				...
				$arPropsValues = array();
				$arPropsValuesAsz = array();
				foreach($arProperties as $prop_id => $arProperty)
				{
					if($arProperty["ID"]==$asz_system_id){
						$strPropAsz = $arProperty["VALUE"];
						$resArAsz = \Mlife\Asz\Functions::getPriceValue($strPropAsz);
						$arPropsValuesAsz["ASZ_KOL"] = $resArAsz["KOL"]["KOL"];
						$arPropsValuesAsz["ASZ_ZAK"] = $resArAsz["KOL"]["ZAK"];
						foreach($arPrice as $pricer){
							$arPropsValuesAsz["ASZ_PRICE_".$pricer["CODE"]."_VAL"] = $resArAsz["PRICE"][$pricer["CODE"]]["VAL"];
							$arPropsValuesAsz["ASZ_PRICE_".$pricer["CODE"]."_CUR"] = $resArAsz["PRICE"][$pricer["CODE"]]["CUR"];
						}
					}else{
				...
				foreach($arResSections as $arPath)
				{
					...
					foreach($arNeedFields as $field_name)
					{
						/*if(strncmp($field_name, "ASZ_", 4) == 0){
							$arTuple[] = $arPropsValuesAsz[$field_name];
						}else
						*/if(strncmp($field_name, "IE_", 3) == 0){
							$arTuple[] = $arElement["~".substr($field_name, 3)];
						}elseif(strncmp($field_name, "IP_PROP", 7) == 0){
							//echo '-'.substr($field_name, 7).'--'.$asz_system_id;
							if(substr($field_name, 7) == $asz_system_id){
								//print_r($arPropsValuesAsz);echo'test';
								foreach($arPropsValuesAsz as $val){
									$arTuple[] = $val;
								}
							}else{
					...
					//echo'<pre>';print_r($arNeedFields);echo'</pre>';die();
					ArrayMultiply($arResFields, $arTuple);
				}
				//echo'<pre>';print_r($arResFields);echo'</pre>';die();
