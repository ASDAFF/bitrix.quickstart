...
<div class="span6">
	...
	<?if ($_SESSION['order']->descr == 'OK'):?>
		<h4 class="title_order_info"><?=GetMessage("colors3_info_to_order")?><?=$_SESSION['order']->data->order_id; ?></h4>
		<div id="result_order"></div>
		<div id="loading_info"><font class="notetext"><?=GetMessage("colors3_loading")?></font></div>
		<script>
			jQuery(document).ready(function($){
				function GetInfoCar(){		
					var status = {new_order: '<?=GetMessage("colors3_new_order")?>', driver_assigned: '<?=GetMessage("colors3_driver_assigned")?>', car_at_place: '<?=GetMessage("colors3_car_at_place")?>',client_inside: '<?=GetMessage("colors3_client_inside")?>', finished: '<?=GetMessage("colors3_finished")?>', aborted: '<?=GetMessage("colors3_aborted")?>'};
					
					$.ajax({
					  url: '/include/ajax_result_order.php',
					  type: 'GET',	
					  success: function(data){                        
						if (parseInt(data.car_id, 10) > 0){ 
							$.get('/include/ajax_result_order.php',{car_id: data.car_id}, function(data_car){
								var object = $.parseJSON(data_car);
                                                                $('#loading_info').css('display','none');
								$('#result_order').html('<div><font class="notetext"><?=GetMessage("colors3_status_order")?> '+ data.state_kind +'</font></div><div><font class="notetext"><?=GetMessage("colors3_status_order1")?> '+ data.source +'</font></div><div><font class="notetext"><?=GetMessage("colors3_status_order2")?> '+ data.destination +'</font></div><div><font class="notetext"><?=GetMessage("colors3_status_order3")?> '+ object.name_car +' '+ object.color_car +' <?=GetMessage("colors3_status_order4")?> '+ object.gos_number +'</font></div>');	
							});
						} else {
							$('#loading_info').css('display','none');
                                                        $('#result_order').html('<div><font class="notetext"><?=GetMessage("colors3_status_order")?> '+ status[data.state_kind] +'</font></div><div><font class="notetext"><?=GetMessage("colors3_status_order1")?> '+ data.source +'</font></div><div><font class="notetext"><?=GetMessage("colors3_status_order2")?> '+ data.destination +'</font></div><div><font class="notetext"><?=GetMessage("colors3_status_order5")?></font></div>');
						}
				...
				setInterval(function(){GetInfoCar()}, 5000);
			});
		</script>	


		<br />
		<a href="#" id="new_order" onclick="return false;"><font class="notetext"><?=GetMessage("colors3_create_new_order")?></font></a>
								...
								case "N":
									...
									<?if ($arResult["PROPERTY_LIST_FULL"][$propertyID]['CODE'] == 'TIP'):
										$db_city = CIBlockElement::GetList(false, array('IBLOCK_CODE'=>'tarify', 'ACTIVE'=>'Y'), false, false, array('ID', 'IBLOCK', 'NAME', 'PROPERTY_KM_VKL_POSAD', 'PROPERTY_PRICE_KM_CITY', 'PROPERTY_PRICE_POSADKA_CITY', 'PROPERTY_MIN_PRICE_CITY'));
											...
											?></select>
									<?else:?>
										<input id="FIELD_<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]['CODE']?>" class="<?echo $first ? 'find-me-input ui-autocomplete-input span9' : 'zak_input_ span12';?>  <?=($small)?' span8 ':''?>" type="text" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" value="<?=($propertyID == 'NAME' && $i==0)?'Заказ с сайта':$value?>" />
									<?endif;?>
