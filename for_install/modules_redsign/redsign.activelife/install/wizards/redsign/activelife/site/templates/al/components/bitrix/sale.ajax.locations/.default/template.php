...
if(count($arResult['COUNTRY_LIST']) > 0):
	...
	?><div class="grid-cell"<?if($cDisabled){?> style="display:none"<?}?>><?
	?><select <?if($disabled || $cDisabled) echo "disabled";?> id="<?=$arParams['COUNTRY_INPUT_NAME'].$arParams['CITY_INPUT_NAME']?>" name="<?=$arParams['COUNTRY_INPUT_NAME'].$arParams['CITY_INPUT_NAME']?>" onChange="<?=$change?>" type="location" class="textinput40 back1"><?
...
if(count($arResult['REGION_LIST']) > 0):
	...
	?><div class="grid-cell"<?if($cDisabled){?> style="display:none"<?}?>><?
	?> <select <?=$id?> <?if($disabled || $rDisabled) echo "disabled";?> name="<?=$arParams['REGION_INPUT_NAME'].$arParams['CITY_INPUT_NAME']?>" onChange="<?=$change?>" type="location" class="textinput40 back1"><?
...
if(count($arResult['CITY_LIST']) > 0):
	...
	?><div class="grid-cell"<?if($cDisabled){?> style="display:none"<?}?>><?
	?> <select <?=$id?> <?if($disabled) echo "disabled";?> name="<?=$arParams['CITY_INPUT_NAME']?>"<?if(strlen($arParams['ONCITYCHANGE']) > 0):?> onchange="<?=$arParams['ONCITYCHANGE']?>"<?endif;?> type="location" class="textinput40 back1"><?
