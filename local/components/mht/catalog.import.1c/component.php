...
elseif ($_GET["mode"]=="deactivate")
{
    ...
    AddMessage2Log($_GET, "my_module_id");
	...
	if ($_GET["timestamp"] > 0)
	{
		...
		AddMessage2Log(array($timeStamp,$_GET["timestamp"]), "my_module_id");

		while ($arField = $rsImportedIBlocks->fetch())
		{
			//arraytofile($arField,  $_SERVER["DOCUMENT_ROOT"]."/upload/deactive_log.txt", "arField");
			...
			while ($arElement = $rsElements->Fetch())
			{
				...
				AddMessage2Log($arElement["ID"], "my_module_id");
			}
			...
			while ($arSection = $rsSections->Fetch())
			{
				...
				//arraytofile($arSection,  $_SERVER["DOCUMENT_ROOT"]."/upload/deactive_log.txt", "arSection");
