		...
		function GetPropertyFieldHtml($arProperty, $value, $strHTMLControlName)
		{
			...
			if($bFileman)
			{
				...
				else
				{
					...
					//  ”—Ћќ¬»≈ ¬џ¬ќƒј јѕѕЋ≈“ј - он должен быть только один на странице
					if(
						isset($_REQUEST['ID'])
						&& $_REQUEST['ID'] > 0
...
//						&& empty($GLOBALS['APPLET_NOTIFY_IS_ON_PAGE' . $arProperty['ID']])
						...
						$GLOBALS['APPLET_IS_ON_PAGE'] = 'Y'; // ”становка флага, что один апплет уже выведен
						$GLOBALS['APPLET_IS_ON_PAGE'. $arProperty['ID']] = 'Y'; // ”становка флага, что один апплет уже выведен
