<?php 
/**
 * ќбработчик ajax дл€ формы обр. св€зи
 * 
 */ 
...
if (strtolower($arSite["CHARSET"]) == "windows-1251") {
	$siteUTF8 = false;	
	// конвертим реквест чтоб в письме не было кракоз€бр
...
if (CModule::IncludeModule("form") && $_SERVER["REQUEST_METHOD"] == "POST" && !empty($_REQUEST["web_form_submit"])) {
	...
	// если все ок то заполн€ем форму
	// check user session
	if (check_bitrix_sessid())
	{
		...
		if($RESULT_ID = CFormResult::Add($_REQUEST["WEB_FORM_ID"], $arResult["arrVALUES"]))
		{
			...
			$result['result'] = 'OK';
			$result['message'] = "—ообщение отправлено успешно";
