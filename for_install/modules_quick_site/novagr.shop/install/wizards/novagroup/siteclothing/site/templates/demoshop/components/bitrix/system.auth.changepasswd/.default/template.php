...
$(document).ready(function(){
	$('#changePassForm').ajaxForm({
			...
			success: function(json) {
				...
				if (json.message) $( "#errorPassContainer" ).html(json.message);	
	        }
		});	
});
</script>
<div id="errorPassContainer"> </div>
<form method="post" id="changePassForm" action="<?=SITE_DIR?>auth/ajax/forms.php" name="bform">
	<?if (strlen($arResult["BACKURL"]) > 0): ?>
	<input type="hidden" name="backurl" value="<?=$arResult["BACKURL"]?>" />
	<? endif ?>
	<input type="hidden" name="AUTH_FORM" value="Y">
	<input type="hidden" name="TYPE" value="CHANGE_PWD_AJAX">
...
<p>
<a onClick="$( '#authForm' ).modal('show');return false;" href="#"><b><?=GetMessage("AUTH_AUTH")?></b></a>
</p>
