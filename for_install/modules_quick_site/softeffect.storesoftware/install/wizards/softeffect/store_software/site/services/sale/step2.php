...
if(COption::GetOptionString("softeffect.storesoftware", "wizard_installed", "N", WIZARD_SITE_ID) != "Y" || WIZARD_INSTALL_DEMO_DATA) {
	...
	if(!$dbDelivery->Fetch()) {
		...
		$arFields = Array(
				"NAME" => GetMessage("SALE_WIZARD_COUR2"),
				"LID" => WIZARD_SITE_ID,
				"PERIOD_FROM" => 0,
				"PERIOD_TO" => 0,
				"PERIOD_TYPE" => "D",
				...
				"CURRENCY" => $defCurrency,
				"SORT" => 200,
				"DESCRIPTION" => GetMessage("SALE_WIZARD_COUR2_DESCR"),
				"LOCATIONS" => $arLocation4Delivery,
			);
		if($delivery["email"] != "Y")
			$arFields["ACTIVE"] = "N";
...
COption::SetOptionString("softeffect.storesoftware", "wizard_installed", "Y", false, WIZARD_SITE_ID);
?>